...
# Updated configurable conditions - Now 14 conditions (8 from second code + 6 from first code)
CONFIG = {
    "conditions": {
        # Original conditions from first code
        "ML_Forecasted_Price_over_Current_Close": True,  # ML Forecast Price over Current Close
        "current_close_below_average_threshold_5m": True,  # Current Close Below Average Threshold (5m)
        ...
        # New conditions from second code
...
def analyze_thresholds_with_argmin_argmax(candles, timeframe):
    """
    ...
    try:
        ...
        df['timestamp'] = pd.to_datetime(df['time'], unit='s').dt.tz_localize('UTC').dt.tz_convert(LOCAL_TIMEZONE)
...
def analyze_fft_cycle(candles, timeframe):
    """
    Analyze FFT cycle between argmin and argmax for the specified timeframe.
...
while True:
    ...
    print("\n--- Condition 4: Volume Bullish Dominance (1m) ---")
    volume_1m_dominance = calculate_volume_dominance(candle_map['1m'])
    if 'error' not in volume_1m_dominance:
        conditions_status["volume_bullish_dominance_1m"] = volume_1m_dominance['bullish_dominance']
        print(f"Bullish Volume %: {volume_1m_dominance['bullish_pct']:.2f}%")
        print(f"Bearish Volume %: {volume_1m_dominance['bearish_pct']:.2f}%")
        print(f"Dominance: {volume_1m_dominance['dominance']}")
        print(f"Bullish Dominance: {volume_1m_dominance['bullish_dominance']}")
        print(f"Condition Met: {conditions_status['volume_bullish_dominance_1m']}")
    else:
        print(f"Error analyzing volume dominance: {volume_1m_dominance['error']}")
        print(f"Condition Met: {conditions_status['volume_bullish_dominance_1m']}")
    ...
    print("\n--- Condition 8: Argmin More Recent (5m) ---")
    thresholds_5m_result = analyze_thresholds_with_argmin_argmax(candle_map['5m'], '5m')
    if 'error' not in thresholds_5m_result:
        conditions_status["argmin_more_recent_5m"] = thresholds_5m_result['argmin_more_recent']
        print(f"Min Close Index: {thresholds_5m_result['min_close_idx']}")
        print(f"Max Close Index: {thresholds_5m_result['max_close_idx']}")
        print(f"Argmin More Recent: {thresholds_5m_result['argmin_more_recent']}")
        print(f"Condition Met: {conditions_status['argmin_more_recent_5m']}")
    else:
        print(f"Error analyzing thresholds: {thresholds_5m_result['error']}")
        print(f"Condition Met: {conditions_status['argmin_more_recent_5m']}")
    ...
    for timeframe in ['1m', '3m', '5m']:
        if timeframe in candle_map and candle_map[timeframe]:
            ...
            # Check current_close_below_average_threshold_5m condition
            if timeframe == '5m':
