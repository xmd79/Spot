...
PROFIT_TARGET_PERCENT = 0.35  # 0.35% net profit goal (Clean Profit)
TOTAL_FEE_PERCENT = 0.22      # 0.22% total fee percentage
MIN_TRADE_AMOUNT = 10

# CONFIGURATION UPDATED
# Removed: thresholds_5m, sine_wave_oscillator_5m
# Added/Replaced: sine_wave_oscillator_15sec
CONFIG = {
    "conditions": {
        ...
        "sine_wave_oscillator_15sec": True,     # REPLACED 5m
        "stoch_15s_oversold": True,
        "stoch_1m_oversold": True,
        "thresholds_15sec": True,
        "volume_bullish_1min": True,
        "volume_bullish_15sec": True,
    },
    "min_conditions_met": 10  # Updated from 11 to 10
}
...
def check_exit_condition(initial_investment, asset_balance, entry_price):
    """
    Calculates exit condition based on Net Profit Target + Fees.
    """
...
# =========================================================
# UPDATED FUNCTION: SEPARATED INDICATOR CONDITIONS (STOCH ONLY)
# =========================================================
...
# =========================================================
# NEW FUNCTION: SINE WAVE OSCILLATOR WITH FFT (15SEC)
# =========================================================
...
try:
    while True:
        ...
        conditions_status = {
            ...
            "thresholds_5m": False, # Kept in status init but removed from config/active
            "sine_wave_oscillator_5m": False, # Kept in status init but removed from config/active
            "sine_wave_oscillator_15sec": False, # NEW
            "stoch_15s_oversold": False,
            "stoch_1m_oversold": False,
        ...
        # Condition 5: Thresholds Analysis (5m) -> REMOVED FROM ACTIVE
        
        # Condition 6: Sine Wave Oscillator (15s) -> REPLACED 5m
        print("\n--- Condition 6: Sine Wave Oscillator (15s) ---")
        sine_wave_result = analyze_sine_wave_oscillator_15sec(candles_15s)
        if 'error' not in sine_wave_result:
            conditions_status["sine_wave_oscillator_15sec"] = sine_wave_result['condition_met']
            ...
            print(f"Condition Met: {conditions_status['sine_wave_oscillator_15sec']}")
        else:
            print(f"Error analyzing sine wave oscillator: {sine_wave_result['error']}")
            print(f"Condition Met: {conditions_status['sine_wave_oscillator_15sec']}")

        # Conditions 7 & 8: Separated Stochastic States (Stoch Only, RSI Removed)
        print("\n--- Condition 7: Stochastic (15s) Oversold ---")
        stoch_results = analyze_stoch_conditions(candles_1m, candles_15s)
        
        if "stoch_15s" in stoch_results:
            conditions_status["stoch_15s_oversold"] = stoch_results["stoch_15s"]["condition_met"]
            print(f"State: {stoch_results['stoch_15s']['state']} (Val: {stoch_results['stoch_15s']['current_val']:.2f})")
            print(f"Condition Met (Oversold Found): {conditions_status['stoch_15s_oversold']}")
        else:
            print("Error analyzing Stoch 15s.")

        print("\n--- Condition 8: Stochastic (1m) Oversold ---")
        if "stoch_1m" in stoch_results:
            conditions_status["stoch_1m_oversold"] = stoch_results["stoch_1m"]["condition_met"]
            print(f"State: {stoch_results['stoch_1m']['state']} (Val: {stoch_results['stoch_1m']['current_val']:.2f})")
            print(f"Condition Met (Oversold Found): {conditions_status['stoch_1m_oversold']}")
        else:
            print("Error analyzing Stoch 1m.")

        # Condition 9: Thresholds 15sec
        print("\n--- Condition 9: Thresholds Analysis (15s) ---")
