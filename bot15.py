...
def detect_golden_ratio_patterns(prices, forecasted_max_price):
    ...
    try:
        ...
        if forecasted_swing_high <= swing_low:
            print(f"Warning: Forecasted high ({forecasted_swing_high}) is not above dip low ({swing_low}). Cannot calculate levels.")
            return None
...
def perform_final_analysis(client, symbol):
    ...
    golden_levels = detect_golden_ratio_patterns(df['close'].values, consensus_target) # <--- UPDATED CALL
