...
# ==========================================
# NEW: REJECTION PATTERN STACK
# ==========================================
# Combines TA-Lib's most reliable bullish-reversal candlestick recognizers
# with two hand-rolled patterns (pin bar, tweezer bottom) that TA-Lib doesn't
# cover well. This is a CONFLUENCE COUNTER, not a guarantee generator -- more
# hits raises the odds price is done falling at this bar, it does not prove it.
...
def detect_rejection_patterns(raw_klines: list, lookback: int = 15) -> Dict:
    ...
    pattern_fns = {
        'HAMMER': ta.CDLHAMMER,
        'INV_HAMMER': ta.CDLINVERTEDHAMMER,
        'BULL_ENGULFING': ta.CDLENGULFING,
        ...
        'TAKURI': ta.CDLTAKURI,          # long lower shadow dragonfly variant
    ...
    for name, fn in pattern_fns.items():
        ...
        recent = arr[-lookback:]
        nz = recent[recent != 0]
        if len(nz) > 0 and nz[-1] > 0:   # only count BULLISH signals (positive value)
            hits[name] = int(nz[-1])

    # Custom pin bar: long lower wick, small body, bullish or neutral close
    body = abs(c[-1] - o[-1])
    lower_wick = min(o[-1], c[-1]) - l[-1]
    upper_wick = h[-1] - max(o[-1], c[-1])
    rng = h[-1] - l[-1]
    pin_bar = False
    if rng > 0 and (lower_wick / rng) > 0.6 and (body / rng) < 0.30 and c[-1] >= o[-1]:
        pin_bar = True

    # Custom tweezer bottom: two consecutive bars sharing (near-)identical lows,
    # first bearish, second bullish
    tweezer = False
    if len(l) >= 2 and l[-2] > 0:
        low_diff = abs(l[-1] - l[-2]) / l[-2]
        if low_diff < 0.0015 and c[-2] < o[-2] and c[-1] > o[-1]:
            tweezer = True

    # Wyckoff spring: quick undercut of recent swing low followed by immediate
    # bullish close back above it, on above-average volume
    spring = False
    if len(l) >= 20:
        ...
        if l[-1] < recent_low and c[-1] > recent_low and last_vol > avg_vol * 1.2:
            spring = True
...
# ==========================================
# NEW: ENTRY-ZONE EXHAUSTION
# ==========================================
# Different from detect_extreme_exhaustion() below -- that one checks exhaustion
# at the STRUCTURAL high/low of a lookback window. This checks exhaustion at
# the CURRENT PRICE ITSELF, i.e. "is selling pressure right here, right now,
# already spent, such that price is more likely to coil/sit than keep dumping."

def detect_entry_exhaustion(raw_klines: list, current_price: float,
                             zone_pct: float = 0.015, min_bars: int = 8) -> Dict:
    if not raw_klines:
        return {'exhausted': False, 'confidence': 0.0, 'detail': 'no data'}

    z_lo = current_price * (1 - zone_pct)
    z_hi = current_price * (1 + zone_pct)
    ...
    if len(zv) < min_bars:
        return {'exhausted': False, 'confidence': 0.0,
                'detail': f'only {len(zv)} bars touched zone, need {min_bars}'}

    split = max(1, int(len(zv) * 0.5))
    early_vol = float(np.mean(zv[:split]))
    late_vol = float(np.mean(zv[split:]))
    vol_ratio = late_vol / early_vol if early_vol > 0 else 1.0

    early_std = float(np.std(zc[:split]))
    late_std = float(np.std(zc[split:]))
    ...
    confidence = max(0.0, min(1.0,
        (1 - min(vol_ratio, 1.0)) * 0.5 +
        (0.3 if coiling else 0.0) +
        bull_ratio_in_zone * 0.2
    ))

    return {'exhausted': exhausted, 'confidence': confidence, 'vol_ratio': vol_ratio,
            'coiling': coiling, 'bull_ratio_in_zone': bull_ratio_in_zone,
            'bars_in_zone': len(zv),
            'detail': (f"Vol {vol_ratio:.0%} of earlier pace, coiling={coiling}, "
                       f"bull closes in zone={bull_ratio_in_zone:.0%} "
                       f"({len(zv)} touches)")}


# ==========================================
# NEW: 360-DEGREE HARMONIC TIME/PRICE/VOLUME CIRCUIT
# ==========================================
# Maps bar-index position within a fixed cycle length onto a stationary
# 0-360 degree wheel (Gann-style squaring of time). Produces a sine and a
# cosine oscillator from that phase angle, then requires the phase to be
# turning upward AT THE SAME TIME price rate-of-change and volume
# rate-of-change are both accelerating. This is a timing/confluence filter,
# not a price predictor -- the wheel doesn't know anything about the market,
# it only tells you when in the cycle you currently sit.
#
# cycle_length choices matter: 144 = Fibonacci/Gann favorite, 90 = quarter-
# circle timing, 360 = full annual/daily analog scaled to your bar interval.

def harmonic_circuit_confirmation(close: List[float], volumes: List[float],
                                   cycle_length: int = 144) -> Dict:
    n = len(close)
    if n < cycle_length + 5:
        return {'confirmed': False, 'votes': 0, 'detail': 'insufficient bars for cycle'}

    close_arr = np.asarray(close, dtype='float64')
    vol_arr = np.asarray(volumes, dtype='float64')

    idx = np.arange(n)
    theta = (idx % cycle_length) / cycle_length * 2 * np.pi
    sine_wave = np.sin(theta)
    cosine_wave = np.cos(theta)

    price_roc = ta.ROC(close_arr, timeperiod=5)
    vol_roc = ta.ROC(vol_arr, timeperiod=5)

    s_now, s_prev, s_prev2 = sine_wave[-1], sine_wave[-2], sine_wave[-3]
    c_now, c_prev, c_prev2 = cosine_wave[-1], cosine_wave[-2], cosine_wave[-3]

    sine_turning_up = (s_now > s_prev) and (s_prev <= s_prev2)
    cosine_turning_up = (c_now > c_prev) and (c_prev <= c_prev2)

    price_accel = (not np.isnan(price_roc[-1]) and not np.isnan(price_roc[-2])
                   and price_roc[-1] > 0 and price_roc[-1] > price_roc[-2])
    vol_accel = (not np.isnan(vol_roc[-1]) and vol_roc[-1] > 0)

    phase_deg = float(np.degrees(theta[-1]) % 360)

    votes = sum([sine_turning_up, cosine_turning_up, price_accel, vol_accel])
    confirmed = votes >= 3   # require 3 of 4 independent signals to align

    return {
        'confirmed': confirmed, 'votes': votes, 'phase_deg': phase_deg,
        'sine_turning_up': sine_turning_up, 'cosine_turning_up': cosine_turning_up,
        'price_accel': price_accel, 'vol_accel': vol_accel,
        'cycle_length': cycle_length,
        'detail': f"Phase {phase_deg:.1f}°/360°, votes {votes}/4 aligned"
    }
...
def check_1m_final(trader, symbol):
    ...
    # ---- NEW confluence layers evaluated per-candidate on the 1m closed bar ----
    ...
    circuit = harmonic_circuit_confirmation(close, volumes, cycle_length=144)

    return (symbol, cmo_val, vratio, is_strong, bull_ratio, prob,
            rejection, entry_exh, circuit)
...
def format_sr_output(symbol, sr, current_price, cmo_val, vratio,
    ...
    print("-" * W)
    print("  🔎  ENTRY-ZONE EXHAUSTION (is selling done RIGHT HERE)")
    tag = "🟢 EXHAUSTED" if entry_exh.get('exhausted') else "⚪ NOT CONFIRMED"
    print(f"  {tag}  (confidence {entry_exh.get('confidence',0)*100:.0f}%)")
    print(f"    {entry_exh.get('detail','')}")

    print("-" * W)
    print("  🕯  REJECTION PATTERN STACK (last closed bar)")
    print(f"  Score: {rejection.get('rejection_score',0)}  |  {rejection.get('detail','')}")
    if rejection.get('talib_hits'):
        print(f"    TA-Lib: {', '.join(rejection['talib_hits'].keys())}")
    extras = []
    ...
    if extras:
        print(f"    Custom: {', '.join(extras)}")

    print("-" * W)
    print("  🌀  360° HARMONIC TIME/PRICE/VOLUME CIRCUIT")
    ctag = "🟢 CONFIRMED" if circuit.get('confirmed') else "⚪ NOT ALIGNED"
    print(f"  {ctag}  ({circuit.get('votes',0)}/4 votes)  |  {circuit.get('detail','')}")
    ...
    if entry_exh.get('exhausted'):
        all_signals.append(f"Entry-zone exhaustion confirmed ({entry_exh['confidence']*100:.0f}% conf.)")
    if rejection.get('rejection_score', 0) >= 2:
        all_signals.append(f"Rejection pattern stack ({rejection['rejection_score']} hits)")
    if circuit.get('confirmed'):
        all_signals.append(f"Harmonic circuit aligned ({circuit['votes']}/4)")
    ...
    print()
    # Verdict tiers now require the 3 new confluence layers to push a candidate
    # from "probable" into "strong" -- but this is still a probability ranking,
    # not a promise. Position sizing / stop placement is on you.
    if ns >= 6: v = "✅  STRONG LONG  —  Full-stack confluence (entry exh + rejection + circuit + structure)"
    elif ns >= 4: v = "✅  LONG  —  Good structural + timing alignment"
    elif ns >= 3: v = "⏳  PROBABLE LONG  —  Awaiting final confirmation"
...
def print_scan_header(scan_count: int):
    ...
    print("║" + "  + entry-zone exhaustion · rejection stack · 360° harmonic circuit".ljust(W) + "║")
    print("║" + " " * W + "║")
    print("╚" + "═" * W + "╝\n")
...
def main():
    ...
    W = 78
    print("=" * W)
    print("  🚀  MTF DIP SCANNER  —  CONTINUOUS MODE  (v2: confluence stack)")
    ...
    print(f"    New Layers         : Entry-zone exhaustion, rejection pattern stack,")
    print(f"                         360° harmonic time/price/volume circuit")
    print(f"    Exit Logic         : Instant exit on MTF dip (No Warnings checked)")
    print(f"\n  NOTE: these layers raise confluence, they do not guarantee")
    print(f"  price won't dip below entry or that a target will be hit.")
    ...
    while True:
        ...
        try:
            ...
            # ---- NEW: require confluence for "strong" bucket -------------
            # A candidate counts as strong only if the linear-regression dip
            # AND the new confluence layers (rejection stack / entry exhaustion
            # / harmonic circuit) agree. Falls back to old logic if none qualify.
            confluent = [r for r in results_1m
                         if r[3] is True
                         and (r[6].get('rejection_score', 0) >= 1
                              or r[7].get('exhausted', False)
                              or r[8].get('confirmed', False))]

            if confluent:
                final = max(confluent, key=lambda x: (x[5], x[6].get('rejection_score', 0), -x[1]))
                mode = "STRONG + CONFLUENCE (rejection/exhaustion/circuit)"
            else:
                strong = [r for r in results_1m if r[3] is True]
                if strong:
                    final = max(strong, key=lambda x: (x[5], -x[1]))
                    mode = "STRONG + ML ENERGY (no confluence layer fired)"
                else:
                    final = min(results_1m, key=lambda x: x[1])
                    mode = "FALLBACK (Best CMO)"

            (sym, cmo_val, vratio, is_strong_1m, live_bull_ratio, ml_prob,
             rejection, entry_exh, circuit) = final
            ...
            print(f"  Rejection Score : {rejection.get('rejection_score',0)}")
            print(f"  Entry Exhausted : {'YES' if entry_exh.get('exhausted') else 'NO'} "
                  f"({entry_exh.get('confidence',0)*100:.0f}% conf.)")
            print(f"  Circuit Aligned : {'YES' if circuit.get('confirmed') else 'NO'} "
                  f"({circuit.get('votes',0)}/4)")
            print("-" * W)
