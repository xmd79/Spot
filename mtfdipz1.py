...
# ==========================================
# NEW: ML FORECAST ENGINE (ZERO RAM/HDD USAGE)
# ==========================================
...
# ==========================================
def harmonic_circuit_confirmation(close: List[float], volumes: List[float], cycle_length: int = 144) -> Dict:
    n = len(close)
    if n < cycle_length + 5: return {'confirmed': False, 'votes': 0, 'detail': 'insufficient bars'}
    close_arr, vol_arr = np.asarray(close, dtype='float64'), np.asarray(volumes, dtype='float64')
    idx = np.arange(n)
    theta = (idx % cycle_length) / cycle_length * 2 * np.pi
    sine_wave, cosine_wave = np.sin(theta), np.cos(theta)
    price_roc, vol_roc = ta.ROC(close_arr, timeperiod=5), ta.ROC(vol_arr, timeperiod=5)
    ...
    price_accel = (not np.isnan(price_roc[-1]) and not np.isnan(price_roc[-2]) and price_roc[-1] > 0 and price_roc[-1] > price_roc[-2])
    ...
    return {'confirmed': votes >= 3, 'votes': votes, 'phase_deg': phase_deg, 'sine_turning_up': sine_turning_up, 'cosine_turning_up': cosine_turning_up, 'price_accel': price_accel, 'vol_accel': vol_accel, 'cycle_length': cycle_length, 'detail': f"Phase {phase_deg:.1f}°/360°, votes {votes}/4 aligned"}


# ==========================================
# STRUCTURAL RANGE ENGINE (DYNAMIC MAX LOOKBACKS)
# ==========================================
...
def get_sr_targets(raw_klines: list, current_price: float) -> Dict:
    ...
    # DYNAMIC LOOKBACKS: Uses 100% of maximum allowed data fetched (25%, 50%, 100%)
    n = len(raw_klines)
...
def format_sr_output(symbol, sr, current_price, cmo_val, vratio, bull_ratio, ml_prob, tf_volumes, rejection, entry_exh, circuit, forecast_price, new_feats, live_bt_stats):
    ...
    print(f"  🎯 ML FORECAST  : {forecast_price:.10f}")
    ...
    print("-" * W)
    print("  🌀  360° HARMONIC CIRCUIT"); ctag = "🟢 CONFIRMED" if circuit.get('confirmed') else "⚪ NOT ALIGNED"
    ...
    if circuit.get('confirmed'): all_signals.append(f"Harmonic circuit ({circuit['votes']}/4)")
...
def main():
    ...
    while True:
        ...
        try:
            ...
            confluent = [r for r in results_1m if r[3] is True and (r[6].get('rejection_score', 0) >= 1 or r[7].get('exhausted', False) or r[8].get('confirmed', False))]
            if confluent: 
            ...
            if ml_prob > 0.6 and forecast_atr_mult > 0:
                live_predictions.append({'sym': sym, 'target': forecast_price, 'entry': current_price, 'time': time.time(), 'status': 'PENDING'})
            ...
            # ==========================================
            # STOP BOT, CLEAN DATA, AND CLOSE LOGIC
            # ==========================================
            print("\n" + "╔" + "═" * 78 + "╗")
            print("║" + " " * 78 + "║")
            ...
            # Clear RAM cleanly before OS takes over
            del trading_pairs, filtered1, filtered2, filtered3, results_1m, tf_volumes, klines_1m, sr, final
            del live_predictions
            gc.collect()
            
            # Safely close and exit OS process (bypasses finally block safely)
            trader = None
            sys.exit(0)

        except SystemExit:
            raise  # Re-raise to cleanly push the exit to the OS
        except Exception as e:
