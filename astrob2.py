...
class CelestialBody:
    ...
    def compute(self, observer):
        ...
        self.lon = float(self.body.hlong)
        self.lat = float(self.body.hlat)
        self.alt = math.degrees(self.body.alt)
        
        # SYNTAX FIX HERE: Corrected the list
        signs = ["Ari", "Tau", "Gem", "Can", "Leo", "Vir", "Lib", "Sco", "Sag", "Cap", "Aqu", "Pis"]
        self.zodiac_sign = signs[int(self.lon / 30)]
...
class MarketEngine:
    def __init__(self):
        ...
        # Increased max_iter to ensure convergence on small datasets
...
def get_observer_data():
    try:
        ...
        print(f"Geo Error: {e}. Using Default.")
        lat, lon, city, tz_str = 51.5074, -0.1278, "London", "Europe/London"

    # FIX: Check if lat/lon are None before converting to string
    if not lat or not lon:
        print("[WARN] Invalid Geo Data, defaulting to Greenwich.")
        lat, lon, city = 51.4778, -0.0014, "Greenwich"
    ...
    local_time = datetime.now(pytz.timezone(tz_str)) if tz_str else datetime.utcnow()
...
def calculate_gann_squares(price):
    ...
    # Square of 9: Price moves in spiral. 
    # Important angles: 0, 90, 180, 270 degrees from center
    root = math.sqrt(price)
    gann_level = int(root)
    remainder = root - gann_level
    
    # Check if near resistance (Cardinal Cross)
    is_resistance = False
...
def calculate_golden_harmonics(angle):
    ...
    # Golden angle approx 137.5 or 222.5 degrees
    phi_deg = 137.507764
...
def main():
    ...
    # Create a synthetic astro vector for training (dummy here, in prod use historical ephemeris)
    ...
    # Check for hard aspects (Square/Opp) involving Mars/Uranus/Saturn
    ...
    golden_hits = calculate_golden_harmonics(sun.deg_in_sign)
    if golden_hits:
        print(f"GOLDEN RATIO:     {', '.join(golden_hits)} aligns with Sun. Potential Cycle Turn.")
