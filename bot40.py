...
→ Implements Local Dip/Top detection on 1-minute timeframe using argmin/argmax
...
→ NEW: Three separate oscillators (1m 1200 values, 1m 500 values, 15s momentum)
...
VOLUME_BULLISH_THRESHOLD = 55.0  # Minimum bullish volume percentage to consider bullish predominant
...
def analyze_linear_regression_channel_condition(client, symbol, lookback=500, timeframe='1m'):
    ...
    try:
        ...
        if timeframe == '1m':
            ...
            klines_1m = client.get_klines(symbol=symbol, interval='1m', limit=lookback // 4)
...
def analyze_local_dip_condition(client, symbol, lookback=500):
    """
    Analyze local dip condition on 1-minute timeframe using a two-step confirmation process.
    ...
    Step 3: Confirm the dip from the first check (1200 values) with the dip from the second check (500 values)
    Step 4: Only consider it a valid dip if both checks confirm the same dip
    
    All timestamps are displayed in GMT+2 timezone.
    """
    try:
        # Step 1: Get 1-minute data for initial check (1200 candles)
        klines_1200 = client.get_klines(symbol=symbol, interval='1m', limit=LOCAL_DIP_CONFIRM_LOOKBACK)
        if not klines_1200 or len(klines_1200) < 1:
            print("No 1-minute data available for local dip analysis")
            return False, {"error": "No data available"}
            
        df_1200 = pd.DataFrame(klines_1200, columns=[
            'timestamp','open','high','low','close','volume','close_time',
            'quote_asset_volume','number_of_trades','taker_buy_base_asset_volume',
            'taker_buy_quote_asset_volume','ignore'])
        
        # Convert timestamp to datetime in GMT+2 timezone
        df_1200['timestamp'] = pd.to_datetime(df_1200['timestamp'], unit='ms').dt.tz_localize('UTC').dt.tz_convert(LOCAL_TIMEZONE)
        
        # Clean OHLC data
        df_1200 = clean_ohlc_data(df_1200)
        
        # Find the index of the lowest low and highest high using argmin/argmax in 1200 candles
        lowest_low_idx_1200 = df_1200['low'].idxmin()
        highest_high_idx_1200 = df_1200['high'].idxmax()
        
        # Get the values and timestamps
        lowest_low_price_1200 = df_1200.loc[lowest_low_idx_1200, 'low']
        lowest_low_time_1200 = df_1200.loc[lowest_low_idx_1200, 'timestamp']
        
        highest_high_price_1200 = df_1200.loc[highest_high_idx_1200, 'high']
        highest_high_time_1200 = df_1200.loc[highest_high_idx_1200, 'timestamp']
        
        # Determine which occurred more recently in 1200 candles
        dip_more_recent_1200 = lowest_low_idx_1200 > highest_high_idx_1200
        
        # Step 2: Get 1-minute data for confirmation check (500 candles)
        klines_500 = client.get_klines(symbol=symbol, interval='1m', limit=lookback)
        if not klines_500 or len(klines_500) < 1:
            print("No 1-minute data available for local dip analysis")
            return False, {"error": "No data available"}
            
        df_500 = pd.DataFrame(klines_500, columns=[
            'timestamp','open','high','low','close','volume','close_time',
            'quote_asset_volume','number_of_trades','taker_buy_base_asset_volume',
            'taker_buy_quote_asset_volume','ignore'])
        
        # Convert timestamp to datetime in GMT+2 timezone
        df_500['timestamp'] = pd.to_datetime(df_500['timestamp'], unit='ms').dt.tz_localize('UTC').dt.tz_convert(LOCAL_TIMEZONE)
        
        # Clean OHLC data
        df_500 = clean_ohlc_data(df_500)
        
        # Find the index of the lowest low and highest high using argmin/argmax in 500 candles
        lowest_low_idx_500 = df_500['low'].idxmin()
        highest_high_idx_500 = df_500['high'].idxmax()
        
        # Get the values and timestamps
        lowest_low_price_500 = df_500.loc[lowest_low_idx_500, 'low']
        lowest_low_time_500 = df_500.loc[lowest_low_idx_500, 'timestamp']
        
        highest_high_price_500 = df_500.loc[highest_high_idx_500, 'high']
        highest_high_time_500 = df_500.loc[highest_high_idx_500, 'timestamp']
        
        # Determine which occurred more recently in 500 candles
        dip_more_recent_500 = lowest_low_idx_500 > highest_high_idx_500
        
        # Step 3: Confirm the dip from the first check with the dip from the second check
        # Only consider it a valid dip if both checks confirm the same dip
        dip_confirmed = dip_more_recent_1200 and dip_more_recent_500
        
        # Print the analysis
        print("\n" + "="*80)
        print(f"LOCAL DIP/TOP ANALYSIS - 1-MINUTE TIMEFRAME")
        print(f"TIMEZONE: GMT+2")
        print("="*80)
        print(f"--- STEP 1: LAST {LOCAL_DIP_CONFIRM_LOOKBACK} CANDLES ---")
        ...
        print(f"--- STEP 2: LAST {lookback} CANDLES ---")
...
def format_quantity(quantity, step_size):
    ...
    # For very small quantities, use regular rounding instead of floor
...
def perform_single_iteration_analysis(client):
    ...
    # Oscillator 1: 1-minute timeframe (1200 values)
    print("\n--- Oscillator 1: 1-minute Timeframe (1200 values) ---")
    osc_1m_1200 = analyze_harmonic_oscillator(client, SYMBOL, lookback=1200, timeframe='1m', oscillator_name='1m 1200')
    
    if 'error' not in osc_1m_1200:
        print(f"Cycle Direction: {osc_1m_1200['cycle_direction']}")
        print(f"Current Price: {osc_1m_1200['current_price']:.25f}")
        print(f"Forecast Target: {osc_1m_1200['forecast_target']:.25f}")
        print(f"Forecast Difference: {osc_1m_1200['forecast_diff_pct']:.2f}%")
        print(f"Lowest Low: {osc_1m_1200['lowest_low_price']:.25f} at {osc_1m_1200['lowest_low_time'].strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"Highest High: {osc_1m_1200['highest_high_price']:.25f} at {osc_1m_1200['highest_high_time'].strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"Most Recent: {'Lowest Low (Dip)' if osc_1m_1200['dip_more_recent'] else 'Highest High (Top)'}")
        
        # Display frequency analysis
        freq_analysis = osc_1m_1200.get('frequency_analysis', {})
        print(f"\nFrequency Analysis:")
        ...
        vol_analysis = osc_1m_1200.get('volume_analysis', {})
        print(f"\nVolume Analysis:")
        ...
        print(f"Error analyzing 1m 1200 oscillator: {osc_1m_1200['error']}")
    
    # Oscillator 2: 1-minute timeframe (500 values)
    print("\n--- Oscillator 2: 1-minute Timeframe (500 values) ---")
    osc_1m_500 = analyze_harmonic_oscillator(client, SYMBOL, lookback=500, timeframe='1m', oscillator_name='1m 500')
    
    if 'error' not in osc_1m_500:
        print(f"Cycle Direction: {osc_1m_500['cycle_direction']}")
        print(f"Current Price: {osc_1m_500['current_price']:.25f}")
        print(f"Forecast Target: {osc_1m_500['forecast_target']:.25f}")
        print(f"Forecast Difference: {osc_1m_500['forecast_diff_pct']:.2f}%")
        print(f"Lowest Low: {osc_1m_500['lowest_low_price']:.25f} at {osc_1m_500['lowest_low_time'].strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"Highest High: {osc_1m_500['highest_high_price']:.25f} at {osc_1m_500['highest_high_time'].strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"Most Recent: {'Lowest Low (Dip)' if osc_1m_500['dip_more_recent'] else 'Highest High (Top)'}")
        
        # Display frequency analysis
        freq_analysis = osc_1m_500.get('frequency_analysis', {})
        print(f"\nFrequency Analysis:")
        print(f"  Most Predominant Frequency: {freq_analysis.get('dominant_frequency', 'unknown')}")
        print(f"  Dominant Frequency Value: {freq_analysis.get('dominant_frequency_value', 0):.25f}")
        print(f"  Positive Dominance: {freq_analysis.get('positive_dominance_pct', 0):.2f}%")
        ...
        # Display volume analysis
        vol_analysis = osc_1m_500.get('volume_analysis', {})
        print(f"\nVolume Analysis:")
        print(f"  Bullish Volume: {vol_analysis.get('bullish_pct', 0):.2f}%")
        print(f"  Bearish Volume: {vol_analysis.get('bearish_pct', 0):.2f}%")
        print(f"  Volume Sentiment: {vol_analysis.get('sentiment', 'neutral')}")
    else:
        print(f"Error analyzing 1m 500 oscillator: {osc_1m_500['error']}")
    
    # Oscillator 3: 15-second timeframe (momentum)
    print("\n--- Oscillator 3: 15-second Timeframe (Momentum) ---")
    osc_15s = analyze_harmonic_oscillator(client, SYMBOL, lookback=1200, timeframe='15s', oscillator_name='15s Momentum')
    
    if 'error' not in osc_15s:
        print(f"Cycle Direction: {osc_15s['cycle_direction']}")
        print(f"Current Price: {osc_15s['current_price']:.25f}")
        print(f"Forecast Target: {osc_15s['forecast_target']:.25f}")
        print(f"Forecast Difference: {osc_15s['forecast_diff_pct']:.2f}%")
        print(f"Lowest Low: {osc_15s['lowest_low_price']:.25f} at {osc_15s['lowest_low_time'].strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"Highest High: {osc_15s['highest_high_price']:.25f} at {osc_15s['highest_high_time'].strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"Most Recent: {'Lowest Low (Dip)' if osc_15s['dip_more_recent'] else 'Highest High (Top)'}")
        
        # Display frequency analysis
        freq_analysis = osc_15s.get('frequency_analysis', {})
        print(f"\nFrequency Analysis:")
        print(f"  Most Predominant Frequency: {freq_analysis.get('dominant_frequency', 'unknown')}")
        print(f"  Dominant Frequency Value: {freq_analysis.get('dominant_frequency_value', 0):.25f}")
        print(f"  Positive Dominance: {freq_analysis.get('positive_dominance_pct', 0):.2f}%")
        ...
        print(f"Error analyzing 15s oscillator: {osc_15s['error']}")
    ...
    conditions_met = 0
    total_conditions = 10  # 4 original + 3 harmonic oscillator + 2 linear regression + 1 volume
    condition_details = {}
    condition_results = {}  # Store individual condition results
    
    # Condition 1: Local Dip Detection on 1-minute timeframe using argmin/argmax
    print("\n--- Condition 1: Local Dip Detection (argmin/argmax) ---")
    ...
    # Condition 5: Harmonic Oscillator 1 (1m 1200)
    print("\n--- Condition 5: Harmonic Oscillator (1m 1200) ---")
    osc_1m_1200_condition = False
    if 'error' not in osc_1m_1200:
        # Condition: cycle is up and forecast target is higher than current price
        osc_1m_1200_condition = osc_1m_1200['cycle_direction'] == 'up' and osc_1m_1200['forecast_target'] > osc_1m_1200['current_price']
        condition_results['1m 1200 Oscillator Up Cycle'] = osc_1m_1200_condition
        
        print(f"\nCycle Direction: {osc_1m_1200['cycle_direction']}")
        print(f"Current Price: {osc_1m_1200['current_price']:.25f}")
        print(f"Forecast Target: {osc_1m_1200['forecast_target']:.25f}")
        print(f"Condition Met: {osc_1m_1200_condition}")
        
        if osc_1m_1200_condition:
            conditions_met += 1
            print("\nTRUE - 1m 1200 Oscillator condition MET")
        else:
            print("\nFALSE - 1m 1200 Oscillator condition NOT met")
    else:
        condition_results['1m 1200 Oscillator Up Cycle'] = False
        print(f"\nError analyzing 1m 1200 oscillator: {osc_1m_1200['error']}")
    
    # Condition 6: Harmonic Oscillator 2 (1m 500)
    print("\n--- Condition 6: Harmonic Oscillator (1m 500) ---")
    osc_1m_500_condition = False
    if 'error' not in osc_1m_500:
        # Condition: cycle is up and forecast target is higher than current price
        osc_1m_500_condition = osc_1m_500['cycle_direction'] == 'up' and osc_1m_500['forecast_target'] > osc_1m_500['current_price']
        condition_results['1m 500 Oscillator Up Cycle'] = osc_1m_500_condition
        
        print(f"\nCycle Direction: {osc_1m_500['cycle_direction']}")
        print(f"Current Price: {osc_1m_500['current_price']:.25f}")
        print(f"Forecast Target: {osc_1m_500['forecast_target']:.25f}")
        print(f"Condition Met: {osc_1m_500_condition}")
        
        if osc_1m_500_condition:
            conditions_met += 1
            print("\nTRUE - 1m 500 Oscillator condition MET")
        else:
            print("\nFALSE - 1m 500 Oscillator condition NOT met")
    else:
        condition_results['1m 500 Oscillator Up Cycle'] = False
        print(f"\nError analyzing 1m 500 oscillator: {osc_1m_500['error']}")
    
    # Condition 7: Harmonic Oscillator 3 (15s Momentum)
    print("\n--- Condition 7: Harmonic Oscillator (15s Momentum) ---")
    osc_15s_condition = False
    if 'error' not in osc_15s:
        # Condition: cycle is up and forecast target is higher than current price
        osc_15s_condition = osc_15s['cycle_direction'] == 'up' and osc_15s['forecast_target'] > osc_15s['current_price']
        condition_results['15s Oscillator Up Cycle'] = osc_15s_condition
        
        print(f"\nCycle Direction: {osc_15s['cycle_direction']}")
        print(f"Current Price: {osc_15s['current_price']:.25f}")
        print(f"Forecast Target: {osc_15s['forecast_target']:.25f}")
        print(f"Condition Met: {osc_15s_condition}")
        
        if osc_15s_condition:
            conditions_met += 1
            print("\nTRUE - 15s Oscillator condition MET")
        else:
            print("\nFALSE - 15s Oscillator condition NOT met")
    else:
        condition_results['15s Oscillator Up Cycle'] = False
        print(f"\nError analyzing 15s oscillator: {osc_15s['error']}")
...
def main():
    ...
    print("3. Analyze harmonic oscillators (1m 1200, 1m 500, 15s momentum)")
    print("4. Analyze linear regression channels (1m, 15s)")
    print("5. Analyze volume condition (NEW)")
    print("6. Analyze all 10 trading conditions (4 original + 3 harmonic + 2 LRC + 1 volume)")
    print("7. Execute trade if ALL conditions met")
    print("8. Use 100% of USDC balance for entry")
    print("9. Convert any remaining USDC to BTC after entering trade")
    print("10. Monitor for profit target every 5 seconds")
    print("11. Convert any remaining BTC to USDC after exiting trade")
    print("12. Clean up for next iteration")
    print("\nEnhanced Features:")
    print("- Sinusoidal harmonic wave oscillators for cycle analysis")
    print("- FFT-based forecasting with 144 harmonics (-72 to +72)")
    print("- Three separate oscillators (1m 1200, 1m 500, 15s momentum)")
    ...
    print("- Dynamic support/resistance based on extremas from 1200 and 500 values")
    ...
    print("- Volume analysis for bullish/bearish sentiment (NEW)")
    print("\nFixed Issues:")
