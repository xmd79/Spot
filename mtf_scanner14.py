...
def _rich_tf_dip_check(pair, interval, tf_label, limit=600):
    ...
    try:
        ...
        #   25 pts  channel pass
        #   15 pts  argmin > argmax
        #   15 pts  HT_SINE dip score (proportional)
        #   15 pts  HTF volume sustained (strict backing)
        #   10 pts  FFT at trough
        #    8 pts  RSI oversold more recent
        #    7 pts  impulse accumulation current TF
        #    3 pts  POC above current price (market profile)
        #    2 pts  bid wall > ask wall (order book)
        _sc = 0.0
        if result['channel_pass']:         _sc += 25.0
        if result['argmin_gt_argmax']:     _sc += 15.0
        _sc += min(15.0, result['ht_sine_dip_score'] * 0.15)
        if result['htf_sustained']:        _sc += 15.0
        if result['fft_at_trough']:        _sc += 10.0
        if result['rsi_dip_ok']:           _sc += 8.0
        if result['impulse_ok']:           _sc += 7.0
        if result['poc_above']:            _sc += 3.0
        if result.get('book_bullish'):     _sc += 2.0
        result['composite_score'] = round(min(_sc, 100.0), 1)

        # ── 12. Pass decision ──────────────────────────────────────────
        #   MANDATORY : channel_pass  AND  htf_sustained (strict rule)
        #   SOFT (≥2) : argmin>argmax | ht_dip≥30 | fft_trough | bull≥48%
...
def filter1(pair, out, lock):
    ...
    # 2h gate: channel_pass + HTF (4h) volume must be bullish (strict rule)
    if rich.get('channel_pass') and rich.get('htf_sustained', True):
        with lock:
            out.append(pair)
...
# ─────────────────────────────────────────────
#  MTF SPIKE SCORE  (new selection path)
# ─────────────────────────────────────────────
#
#  ★ COMPOSITE RANKING FACTORS:
#
#   MTF_SPIKE_SCORE = (
#     20 %  CMO depth           — deeper oversold = more compressed = bigger bounce
#     18 %  HT_SINE consensus   — avg cycle trough score across ALL filter TFs + 1m
#     15 %  TF argmin agree     — fraction of TFs where deepest wick > highest wick
#     15 %  HTF volume sustain  — strict: HTF bull vol > bear vol on each TF's higher TF
#     10 %  current-TF vol/abs  — bull% + absorption (fuel in current TF)
#     10 %  RSI dip quality     — oversold most recent across TFs
#      7 %  impulse accum       — enough vol, price coiled, bull dominant
#      5 %  φ·e·π geometry      — structural quality of the dip (curvature etc.)
#   ) × normalisation → 0..100+
#
#   Bonuses : +8 LeadSine crossed above Sine on any TF  (Ehlers trigger)
#              +5 sine < −0.9 (extreme trough) on ≥2 TFs
#              +6 HTF impulse ok on majority of TFs (volume already building)
#              +4 POC above + book bullish (market profile + order book aligned)
#   Penalty : −15 fake dip detected (negative curvature)
#             −10 RSI oversold NOT more recent than overbought on 1m
#
#  Principle: volume precedes price. HTF confirmation is a HARD FACTOR
#  (not a bonus) because if the higher TF is bearish in volume, the
#  current-TF dip is unlikely to produce a sustained spike pump.
# ─────────────────────────────────────────────

def _compute_mtf_spike_score(pair, tf_cache, sel_detail):
    """
    Compute composite MTF spike score 0-100+ for ranking.
    Uses _tf_cache data (populated by filters) + sel_detail (1m gate).
    """
    _score = 0.0
    _d     = sel_detail.get(pair, {})
    _cache = tf_cache.get(pair, {})

    # ── 20 pts: CMO depth ─────────────────────────────────────
    _cmo = _d.get('raw_cmo')
    if _cmo is not None:
        _score += min(20.0, max(0.0, -_cmo * 0.20))

    # ── 18 pts: HT_SINE dip consensus across all TFs ──────────
    # Weighted by TF proximity: closer TF = stronger weight.
    # HTF total volume used as an additional weight multiplier:
    #   higher absolute HTF volume behind the TF = more meaningful sine signal.
    _ht_w  = {'2h': 0.50, '30m': 0.65, '15m': 0.75, '5m': 0.85, '3m': 0.90, '1m': 1.00}
    _wsum  = 0.0; _wval = 0.0
    for _tk, _rd in _cache.items():
        _w   = _ht_w.get(_tk, 0.60)
        # scale weight by HTF total volume (log-normalized, max 2×)
        ...
        _wsum += _w * _hvw
    _ds1m = _d.get('ht_sine_dip_score', 0.0)
    _w1   = _ht_w.get('1m', 1.0)
    _wval += _ds1m * _w1; _wsum += _w1
    if _wsum > 0:
        _score += min(18.0, (_wval / _wsum) * 0.18)

    # ── 15 pts: TF argmin>argmax agreement ratio ───────────────
    _n_am  = sum(1 for _rd in _cache.values() if _rd.get('argmin_gt_argmax'))
    _n_am += (1 if _d.get('cond_ext') else 0)
    _tot   = len(_cache) + 1
    _score += (_n_am / max(_tot, 1)) * 15.0

    # ── 15 pts: HTF volume sustained (strict: bull > bear per TF) ─
    # Each TF in cache has its own higher TF backing.
    # Count how many TFs are backed by bullish HTF volume.
    _n_htf_ok = sum(1 for _rd in _cache.values() if _rd.get('htf_sustained', True))
    # 1m gate has its own HTF check via cond_vol
    _n_htf_ok += (1 if _d.get('cond_vol') else 0)
    _htf_ratio  = _n_htf_ok / max(_tot, 1)
    _score += _htf_ratio * 15.0

    # ── 10 pts: current-TF volume / absorption ────────────────
    _vol_sc = []
    for _rd in _cache.values():
        _bp  = _rd.get('vol_bull_pct', 50.0)
        _ab  = _rd.get('vol_absorption', 0.0)
        _vs  = (min(1.0, max(0.0, (_bp - 45.0) / 10.0)) * 0.60
               + min(1.0, _ab / 8.0) * 0.40) * 10.0
        _vol_sc.append(max(0.0, _vs))
    _dr  = _d.get('delta_ratio',      0.0)
    _ab1 = _d.get('absorption_score', 0.0)
    _vol_sc.append(max(0.0,
        (min(1.0, max(0.0, _dr)) * 0.50 + min(1.0, _ab1 / 8.0) * 0.50) * 10.0))
    if _vol_sc:
        _score += float(np.mean(_vol_sc))

    # ── 10 pts: RSI dip quality ────────────────────────────────
    # Each TF gets a score. oversold_more_recent = good, rsi_dip_ok = better.
    _rsi_sc = []
    for _rd in _cache.values():
        _r = _rd.get('rsi_data') or {}
        _omr = bool(_r.get('oversold_more_recent', False))
        _rok = bool(_r.get('rsi_dip_ok',           False))
        _rsi_sc.append(10.0 if _rok else (6.0 if _omr else 0.0))
    # 1m RSI from sel_detail
    _rsi_1m = _d.get('rsi_1m') or {}
    _omr_1m = bool(_rsi_1m.get('oversold_more_recent', False))
    _rok_1m = bool(_rsi_1m.get('rsi_dip_ok',           False))
    _rsi_sc.append(10.0 if _rok_1m else (6.0 if _omr_1m else 0.0))
    if _rsi_sc:
        _score += float(np.mean(_rsi_sc))

    # ── 7 pts: impulse accumulation ───────────────────────────
    _n_imp = sum(1 for _rd in _cache.values() if _rd.get('impulse_ok'))
    _n_imp += (1 if _d.get('impulse_ok_1m') else 0)
    _score += (_n_imp / max(_tot, 1)) * 7.0

    # ── 5 pts: φ·e·π geometry score ──────────────────────────
    _score += _d.get('geometry_score', 0.0) * 0.05

    # ── Bonuses ───────────────────────────────────────────────
    # +8: LeadSine crossed above Sine (Ehlers entry trigger)
    _n_cross  = sum(1 for _rd in _cache.values() if _rd.get('ht_sine_cross'))
    _n_cross += (1 if _d.get('ht_sine_cross') else 0)
    if _n_cross >= 1:
        _score += 8.0

    # +5: sine < −0.9 on ≥2 TFs (extreme trough multi-TF)
    _n_str  = sum(1 for _rd in _cache.values() if _rd.get('ht_sine_strong'))
    _n_str += (1 if _d.get('ht_sine_strong') else 0)
    if _n_str >= 2:
        _score += 5.0

    # +6: HTF impulse ok on majority of TFs (volume building on HTF)
    _n_htfimp = sum(1 for _rd in _cache.values() if _rd.get('htf_impulse_ok'))
    if _n_htfimp >= max(1, len(_cache) // 2):
        _score += 6.0

    # +4: POC above + book bullish aligned (market profile + order book)
    ...
    # ── Penalties ─────────────────────────────────────────────
    # −15: fake dip (negative curvature on 1m)
    if _d.get('geometry_detail', {}).get('is_fake_dip', False):
        _score -= 15.0

    # −10: RSI not confirming on 1m (OB more recent than OS)
...
def print_mtf_signals_summary(fp4, label_map, tf_cache, sel_detail, spike_scores):
    ...
    if not fp4:
        return
    _TFS = ['2h', '30m', '15m', '5m', '1m']
    _WL  = 80
    print(f'\n  {"═" * _WL}')
    print(f'  ◈  MTF SIGNAL MATRIX  ({len(fp4)} candidates)')
    print(f'  {"═" * _WL}')
    print(f'  ★ SELECTION FACTORS:')
    print(f'    CMO(20%) + HT_SINE(18%) + ArgMin(15%) + HTFvol(15%)')
    print(f'    + CurVol(10%) + RSI(10%) + Impulse(7%) + Geo(5%)')
    print(f'    Bonus: +8 LeadSine cross | +5 sine<-0.9 ≥2TFs | +6 HTFimp | +4 POC+Book')
    print(f'    Penalty: -15 fake dip | -10 RSI not confirming')
    print()

    # column layout: Ticker | Score | per-TF[Sine AM Ft HTF RSI] | CMO | Geo | RSI1m | Imp1m
    _tf_hdr = '  '.join(f'[{_t}]Sn AM Ft HV RS' for _t in _TFS)
    print(f'  {"Ticker":<10}  {"MTFSpk":>6}  {_tf_hdr}  CMO  Geo  RSI1m Imp1m')
    print('  ' + '─' * (_WL + 55))
    ...
    for _p in _sorted_fp4:
        ...
        for _tf in _TFS:
            if _tf == '1m':
                ...
                _ftt = '✔' if (_d.get('ht_sine_1m') or {}).get('in_dip_zone') else '—'
                _hv  = '✔' if _d.get('cond_vol')    else '✗'
                _ros = '✔' if (_r1m.get('oversold_more_recent')) else '✗'
            else:
                _rd  = (tf_cache.get(_p, {}).get(_tf)) or {}
                _sn  = _rd.get('sine_now')
                _am  = '✔' if _rd.get('argmin_gt_argmax')  else '✗'
                _ftt = '✔' if _rd.get('fft_at_trough')     else '✗'
                _hv  = '✔' if _rd.get('htf_sustained', True) else '✗'
                _ros = '✔' if _rd.get('oversold_more_recent') else '✗'
            _sn_s = f'{_sn:+.1f}' if _sn is not None else '  — '
            _parts.append(f'{_sn_s} {_am} {_ftt} {_hv} {_ros}')
        _row  = '  '.join(_parts)
        _mark = '★' if _p == _sorted_fp4[0] else ' '
        print(f'  {_mark}{_lbl:<9}  {_msc:>6.1f}  {_row}  {_cmo_s:>4}  {_geo:>4.0f}  {_rsi1_s:>5} {_imp1}')
