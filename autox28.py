...
# CHANGE 1: Removed volume condition and added sine wave oscillator condition
CONFIG = {
    "conditions": {
    ...
    "min_conditions_met": 8  # Updated to 8 active conditions
}
...
def analyze_sine_wave_oscillator_5m(candles_5m=None):
    ...
    try:
        ...
        # Print the argmin and argmax information
        print(f"Last argmin found at index: {last_min_idx} with value: {close_prices[last_min_idx]:.2f}")
        print(f"Last argmax found at index: {last_max_idx} with value: {close_prices[last_max_idx]:.2f}")
        
        # Determine which is more recent
        current_idx = len(close_prices) - 1
        current_price = close_prices[-1]
        
        # Print which extremum is most recent
        if last_min_idx > last_max_idx:
            print(f"Most recent extremum: ARGMIN (minimum) at index {last_min_idx}")
        else:
        ...
        if last_min_idx > last_max_idx:
            ...
            # Get the last minimum value
            last_min_value = close_prices[last_min_idx]
            
            # Calculate the amplitude using Fibonacci scaling
            # Use the previous maximum to determine amplitude
            ...
            # Ensure forecast price is above current price for up cycle
            if forecast_price <= current_price:
                forecast_price = current_price + (current_price * 0.01)  # At least 1% above current
            ...
            # Calculate the amplitude using Fibonacci scaling
            # Use the previous minimum to determine amplitude
            ...
            # Ensure forecast price is below current price for down cycle
            if forecast_price >= current_price:
                forecast_price = current_price - (current_price * 0.01)  # At least 1% below current
