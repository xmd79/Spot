...
def mtf_harmonic_target_full(fast_data, inter_data, major_data, biggest_data,
    ...
    # Compute mean fib_best per layer
...
def process_group(tf_list):
    ...
    for tf in tf_list:
        ...
        # Fibonacci Logic: Map 0.000 and 1.000 based on recent extrema (reversals)
        if cycle == "UP":
            fib_0 = abs_low  # Recent argmin (low) as 0.000
            fib_1 = abs_high  # Incoming argmax (high) as 1.000
            direction = 1
        else:
            fib_0 = abs_high  # Recent argmax (high) as 0.000
            fib_1 = abs_low  # Incoming argmin (low) as 1.000
            direction = -1
        ...
        # Build forecast for fibo best target: Closest fib level to fft_mean as incoming fib target
        fib_levels_values = list(fib_levels.values())
        distances = [abs(v - fft_mean) for v in fib_levels_values]
        closest_idx = np.argmin(distances)
        fib_best = fib_levels_values[closest_idx]
...
while not stop_event.is_set():
    try:
        ...
        # ==========================================
        harmonic_target, harmonic_details, harmonic_fib_target, harmonic_fib_details = mtf_harmonic_target_full(fast_data, inter_data, major_data, biggest_data)
        ...
        for tf_list, data in all_data_map:
            for i, tf in enumerate(tf_list):
                ...
                # Fibonacci Output
                fib_levels = data['fib_levels'][i]
                fib_best = data['fib_bests'][i]
