...
# Updated configurable conditions - Now 8 conditions (added close < sma200 for 15s)
CONFIG = {
    "conditions": {
        "fft_forecast_price_15s": True,  # FFT Forecast Price (15s)
        "close_below_sma200_15s": True,  # Close Below SMA200 (15s) - New condition
        "momentum_positive_1m": True,  # Momentum Positive (1m)
        "volume_bullish_dominance_1m": True,  # Volume Bullish Dominance (1m)
        "fft_forecast_up_1m": True,  # FFT Forecast Up (1m) - Added back
        "fft_forecast_up_3m": True,  # FFT Forecast Up (3m) - Added back
        "fft_forecast_up_5m": True,  # FFT Forecast Up (5m)
        "argmin_more_recent_5m": True,  # Argmin More Recent (5m)
    },
...
# ------------------ New Function for 15s Data Generation ------------------
...
def generate_15s_data_from_1m(df_1m):
    ...
    for idx, row in df_1m.iterrows():
        ...
        for i in range(4):
            ...
            timestamp = row['timestamp'] + timedelta(seconds=15 * i)
...
def analyze_thresholds(client, symbol, timeframe='1m', lookback=1200):
    """
...
def analyze_rsi_condition(client, symbol, lookback=1200, timeframe='1m'):
...
def analyze_momentum_condition(client, symbol, lookback=1200, timeframe='1m', period=10):
...
def analyze_fft_cycle(client, symbol, timeframe='1m', lookback=1200):
    """
    ...
    try:
        ...
        # NEW: Get reference price from 1m timeframe for consistency
        reference_price = None
        if timeframe != '1m':
            # Get 1m data for reference
            ref_klines = safe_api_call(client.get_klines, symbol=symbol, interval='1m', limit=lookback)
            if ref_klines and len(ref_klines) >= 100:
                ref_df = pd.DataFrame(ref_klines, columns=[
                    'timestamp','open','high','low','close','volume','close_time',
                    'quote_asset_volume','number_of_trades','taker_buy_base_asset_volume',
                    'taker_buy_quote_asset_volume','ignore'])
                
                # Convert timestamp to datetime in GMT+2 timezone
                ref_df['timestamp'] = pd.to_datetime(ref_df['timestamp'], unit='ms').dt.tz_localize('UTC').dt.tz_convert(LOCAL_TIMEZONE)
                
                # Clean OHLC data
                ref_df = clean_ohlc_data(ref_df)
                
                # Use the last close price from 1m as reference
                reference_price = ref_df['close'].values.astype(float)[-1]
        
        # If we couldn't get reference price, use current price
        if reference_price is None:
            reference_price = current_price
        
        # NEW: Adjust forecast target to be consistent with reference price
        # This ensures all timeframes have a consistent directional bias
        if reference_price is not None:
            # Calculate price difference from reference
            price_diff_from_ref = current_price - reference_price
            
            # Adjust forecast based on reference price difference
            # If current price is above reference, ensure forecast is higher
            if price_diff_from_ref > 0:
                # Current price is above reference, forecast should be higher
                if forecast_target <= current_price:
                    forecast_target = current_price * 1.005  # Small upward adjustment
            # If current price is below reference, forecast should be lower
            elif price_diff_from_ref < 0:
                # Current price is below reference, forecast should be lower
                if forecast_target >= current_price:
                    forecast_target = current_price * 0.995  # Small downward adjustment
            # If prices are similar, keep forecast as is
            else:
...
# ------------------ New Function for FFT Forecast Price (15s) ------------------

def analyze_fft_forecast_price_15s(client, symbol, lookback=1200):
    """
    Analyze FFT forecast price for 15-second timeframe derived from 1-minute data.
    This function replaces the "Up Cycle Confirmed (1m)" condition.
    """
    try:
        # Get 1m data and convert to 15s
        klines = safe_api_call(client.get_klines, symbol=symbol, interval='1m', limit=lookback//4)
        if not klines or len(klines) < 100:
            return {"error": "Insufficient data for 15s FFT forecast"}
        ...
        # Generate 15s data from 1m data
        df_15s = generate_15s_data_from_1m(df)
        ...
        # Use improved FFT for forecasting
        forecast_prices = improved_fft_forecast(close_prices, forecast_periods=16)  # 4 minutes ahead (16 * 15s)
        forecast_target = forecast_prices[-1]
...
# ------------------ New Function for SMA200 Condition (15s) ------------------

def analyze_sma200_condition_15s(client, symbol, lookback=1200):
    """
...
def perform_single_iteration_analysis(client):
    ...
    print("="*80)
    print(f"ENHANCED BTCUSDC TRADING BOT - 8 TRIGGERS")
    ...
    thresholds_5m = analyze_thresholds(client, SYMBOL, timeframe='5m', lookback=1200)
    
    # Fetch volume data for 1m timeframe
    volume_1m = analyze_volume_dominance(client, SYMBOL, timeframe='1m', lookback=1200)
    
    # Fetch FFT data for 1m, 3m, and 5m timeframes
    fft_1m = analyze_fft_cycle(client, SYMBOL, timeframe='1m', lookback=1200)
    fft_3m = analyze_fft_cycle(client, SYMBOL, timeframe='3m', lookback=1200)
    fft_5m = analyze_fft_cycle(client, SYMBOL, timeframe='5m', lookback=1200)
    
    # Fetch FFT forecast price for 15s timeframe
    fft_15s = analyze_fft_forecast_price_15s(client, SYMBOL, lookback=1200)
    
    # Fetch SMA200 condition for 15s timeframe
    sma200_15s = analyze_sma200_condition_15s(client, SYMBOL, lookback=1200)
    
    # Step 2: Analyze all specified conditions
    print("\n2. Analyzing all 8 specified trading conditions...")
    
    conditions_met = 0
    total_conditions = 8  # Total number of conditions
    ...
    if 'error' not in fft_15s:
        fft_15s_met = fft_15s['forecast_up']
        condition_results['FFT Forecast Price (15s)'] = fft_15s_met
        
        print(f"\nCurrent Price: {fft_15s['current_price']:.2f}")
        print(f"Forecast Target: {fft_15s['forecast_target']:.2f}")
        print(f"Forecast Difference: {fft_15s['forecast_diff_pct']:.4f}%")
        print(f"Forecast Up: {fft_15s_met}")
        print(f"Condition Met: {fft_15s_met}")
        
        if fft_15s_met:
            conditions_met += 1
            print("\nTRUE - FFT Forecast Price (15s) condition MET")
        else:
            print("\nFALSE - FFT Forecast Price (15s) condition NOT met")
    else:
        condition_results['FFT Forecast Price (15s)'] = False
        print(f"\nError analyzing FFT forecast price (15s): {fft_15s['error']}")
        print("\nFALSE - FFT Forecast Price (15s) condition NOT met")
    
    # Condition 2: Close Below SMA200 (15s) - New condition
    print("\n--- Condition 2: Close Below SMA200 (15s) ---")
    sma200_15s_met = False
    if 'error' not in sma200_15s:
        sma200_15s_met = sma200_15s['close_below_sma200']
        condition_results['Close Below SMA200 (15s)'] = sma200_15s_met
        
        print(f"\nCurrent Close: {sma200_15s['current_close']:.2f}")
        print(f"Current SMA200: {sma200_15s['current_sma200']:.2f}")
        print(f"Difference: {sma200_15s['difference']:.4f} ({sma200_15s['difference_pct']:.4f}%)")
        print(f"Close Below SMA200: {sma200_15s_met}")
        print(f"Condition Met: {sma200_15s_met}")
        
        if sma200_15s_met:
            conditions_met += 1
            print("\nTRUE - Close Below SMA200 (15s) condition MET")
        else:
            print("\nFALSE - Close Below SMA200 (15s) condition NOT met")
    else:
        condition_results['Close Below SMA200 (15s)'] = False
        print(f"\nError analyzing SMA200 condition (15s): {sma200_15s['error']}")
        print("\nFALSE - Close Below SMA200 (15s) condition NOT met")
    
    # Condition 3: Momentum Positive (1m)
    print("\n--- Condition 3: Momentum Positive (1m) ---")
    momentum_1m_positive, momentum_1m_value, momentum_1m_details = analyze_momentum_condition(client, SYMBOL, 1200, '1m')
    ...
    # Condition 4: Volume Bullish Dominance (1m)
    print("\n--- Condition 4: Volume Bullish Dominance (1m) ---")
    volume_1m_met = False
    ...
    # Condition 5: FFT Forecast Up (1m) - Added back
    ...
    # Condition 6: FFT Forecast Up (3m) - Added back
    ...
    # Condition 8: Argmin More Recent (5m)
    print("\n--- Condition 8: Argmin More Recent (5m) ---")
    argmin_5m_met = False
    if 'error' not in thresholds_5m:
        argmin_5m_met = thresholds_5m['argmin_more_recent']
        condition_results['Argmin More Recent (5m)'] = argmin_5m_met
        
        print(f"\nMin Close Index: {thresholds_5m['min_close_idx']}")
        print(f"Max Close Index: {thresholds_5m['max_close_idx']}")
        print(f"Argmin More Recent: {argmin_5m_met}")
        print(f"Condition Met: {argmin_5m_met}")
        
        if argmin_5m_met:
            conditions_met += 1
            print("\nTRUE - Argmin More Recent (5m) condition MET")
        else:
            print("\nFALSE - Argmin More Recent (5m) condition NOT met")
    else:
...
def main():
    ...
    print("=== ENHANCED BTCUSDC TRADING BOT - 8 TRIGGERS ===")
    print("Press Ctrl+C to stop monitoring.")
    print("\nEach iteration will:")
    print("1. Check for active trade and resume if necessary")
    print("2. Fetch fresh data for all timeframes (15s, 1m, 3m, 5m)")
    print("3. Analyze all 8 specified trading conditions:")
    print("   - FFT Forecast Price (15s)")
    print("   - Close Below SMA200 (15s)")  # New condition
    ...
    print("   - Argmin More Recent (5m)")
    print("4. Execute trade if ALL 8 conditions are met")
    ...
    print("\nEnhanced Features:")
    print("- Now 8 selected triggers (added Close Below SMA200 for 15s)")
    ...
    print("- NEW: 15-second timeframe data generated from 1-minute data")
    print("- NEW: FFT Forecast Price (15s) condition")
    print("- NEW: Close Below SMA200 (15s) condition")
    print("- UPDATED: Lookback period increased from 500 to 1200 for all timeframes")
    print("- FIXED: FFT forecasts now consistent across timeframes using 1m as reference")
    print("="*60)
