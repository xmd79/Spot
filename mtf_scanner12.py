...
while True:
    ...
    # ── selection: rank by composite score (CMO × geometry × HT_SINE) ──
    if len(sel_pairs) > 1:
        ...
        print(f'  Tertiary:   HT_SINE dip score (cycle trough depth, 0-100)')
        ...
        for i, p in enumerate(sel_pairs):
            ...
            ht_dip_v = sel_detail[p].get('ht_sine_dip_score', 0.0)
            # lower CMO = better; higher geo = better; higher ht_dip = better
            scored.append((p, cmo_v, geo_v, ht_dip_v))

        # rank: CMO primary (most negative), then geo, then ht_dip
