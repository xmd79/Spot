...
# CONFIGURATION UPDATED: Added Hurst conditions
CONFIG = {
    "conditions": {
        ...
        "stoch_rsi_precise_30m": True,
        "hurst_trending_1m": True,        # NEW
        "hurst_trending_15sec": True,     # NEW
    },
    "min_conditions_met": 14  # Updated to 14 active conditions
}
...
# =========================================================
# NEW FUNCTION: STOCHASTIC + RSI PRECISE (30m TF)
# =========================================================

def analyze_stoch_rsi_precise_30m(candles):
    """
    ...
    try:
        if not candles or len(candles) < 20:  # Need enough data for rolling windows
            return {"condition_met": False, "error": "Insufficient data for 30m analysis"}
        ...
        if len(smooth_k) < 2 or len(stoch_d) < 2:
            return {"condition_met": False, "error": "Insufficient data length for crossover check"}
        ...
        print(f"Error in analyze_stoch_rsi_precise_30m: {e}")
...
# =========================================================
# UPDATED FUNCTION: MTF RSI & STOCHASTIC BACKUP
# =========================================================

def analyze_mtf_rsi_stoch_backup(candles_1m, candles_15s, lookback=200):
    """
    Multi-Timeframe Backup Check.
    Updated: Analyzes RSI (15s) and Stoch (1m).
    - If Most Recent is OVERSOLD -> Condition True (Buy Bias)
    - If Most Recent is OVERBOUGHT -> Condition False (Sell Bias)
    ...
    # 1. RSI 15s Analysis
    rsi_result = {"state": "NEUTRAL", "condition_met": False, "current_val": 0}
    try:
        ...
        # Analyze last lookback candles
        recent_rsi = rsi.tail(lookback).values
        state, is_oversold = get_last_reversal_state(recent_rsi, oversold_limit=30, overbought_limit=70)
        
        rsi_result["state"] = state
        rsi_result["condition_met"] = is_oversold
        rsi_result["current_val"] = rsi.iloc[-1] if not rsi.empty else 0
        
    except Exception as e:
        print(f"Error analyzing RSI 15s: {e}")

    # 2. Stoch 1m Analysis
    stoch_result = {"state": "NEUTRAL", "condition_met": False, "current_val": 0}
    try:
        # Use 1m candles
        ...
        stoch_k = 100 * (close_1m - lowest_low) / (highest_high - lowest_low)
        stoch_d = stoch_k.rolling(3).mean() # Smoothing
        
        # Analyze last lookback candles based on %K
        recent_k = stoch_k.tail(lookback).values
        state, is_oversold = get_last_reversal_state(recent_k, oversold_limit=20, overbought_limit=80)
        
        stoch_result["state"] = state
        stoch_result["condition_met"] = is_oversold
        stoch_result["current_val"] = stoch_k.iloc[-1] if not stoch_k.empty else 0
        
    except Exception as e:
        print(f"Error analyzing Stoch 1m: {e}")
    ...
    return {
        "rsi_15s": rsi_result,
        "stoch_1m": stoch_result,
        "condition_met": final_condition_met
    }
...
try:
    while True:
        ...
        conditions_status = {
            ...
            "stoch_rsi_precise_30m": False,
            "hurst_trending_1m": False,        # NEW
            "hurst_trending_15sec": False,     # NEW
        }
        ...
        print("\n--- Condition 9: MTF RSI (15s) & Stochastic (1m) Backup ---")
        mtf_backup_result = analyze_mtf_rsi_stoch_backup(candles_1m, candles_15s)
        
        rsi_state = mtf_backup_result['rsi_15s']['state']
        rsi_cond = mtf_backup_result['rsi_15s']['condition_met']
        rsi_val = mtf_backup_result['rsi_15s']['current_val']
        
        stoch_state = mtf_backup_result['stoch_1m']['state']
        stoch_cond = mtf_backup_result['stoch_1m']['condition_met']
        stoch_val = mtf_backup_result['stoch_1m']['current_val']
        
        final_cond = mtf_backup_result['condition_met']
        
        print(f"RSI (15s) Most Recent Found: {rsi_state} (Condition: {rsi_cond}) - Val: {rsi_val:.2f}")
        print(f"Stoch (1m) Most Recent Found: {stoch_state} (Condition: {stoch_cond}) - Val: {stoch_val:.2f}")
        ...
        # Condition 12: Stochastic + RSI Precise (30m)
        print("\n--- Condition 12: Stochastic + RSI Precise (30m) ---")
        stoch_rsi_precise_result = analyze_stoch_rsi_precise_30m(candles_30m)
        
        if 'error' not in stoch_rsi_precise_result:
            conditions_status["stoch_rsi_precise_30m"] = stoch_rsi_precise_result['condition_met']
            print(f"RSI (30m): {stoch_rsi_precise_result['rsi']:.2f}")
            print(f"Stoch %K (30m): {stoch_rsi_precise_result['stoch_k']:.2f}")
            print(f"Stoch %D (30m): {stoch_rsi_precise_result['stoch_d']:.2f}")
            print(f"Bull Cross: {stoch_rsi_precise_result['bull_cross']}")
            print(f"%D < Entry(61.8): {stoch_rsi_precise_result['d_below_entry']}")
            print(f"Condition Met: {stoch_rsi_precise_result['condition_met']}")
        else:
            print(f"Error analyzing Stoch/RSI Precise (30m): {stoch_rsi_precise_result['error']}")
            print(f"Condition Met: {conditions_status['stoch_rsi_precise_30m']}")

        # Condition 13: Hurst Trending (1m)
        print("\n--- Condition 13: Hurst Cycle Analysis (1m) ---")
        ...
        # Condition 14: Hurst Trending (15s)
        print("\n--- Condition 14: Hurst Cycle Analysis (15s) ---")
        # Use simulated 15s candles
        if candles_15s:
