...
def _channel_pass(klines):
    ...
    best_fit_line2 = best_fit_line1 * 1.01   # upper band (unused in gate, kept for reference)
    best_fit_line3 = best_fit_line1 * 0.99   # lower band — must close below this
...
#     → price rejected at the lowest low → confirmed
#       reversal rejection point, not at a local high
#
#  CMO(14) is computed here ONLY for ranking purposes
#  when multiple dips pass. It is NOT a pass/fail gate.
#  The deepest (most negative) raw CMO = best ranked dip.
# ─────────────────────────────────────────────

EXTREMA_LOOKBACK = 500   # 1m candles for extrema + volume check

def check_dip_conditions(pair):
    """
    Returns (passed, detail_dict) where detail_dict contains:
      bull_pct   — bull volume % of total
      bear_pct   — bear volume % of total
      argmin_idx — index of the lowest close in last 500 bars
      argmax_idx — index of the highest close in last 500 bars
      raw_cmo    — CMO(14) on last 500 1m closes (ranking only, not a gate)
      price      — last close price
      cond_vol   — bool: bull% > bear%
      cond_ext   — bool: argmin_idx > argmax_idx (minima is most recent)
    Returns (False, {}) on data error.
    """
    try:
    ...
    argmin_idx = int(np.argmin(close))   # index of lowest low
    argmax_idx = int(np.argmax(close))   # index of highest high
    # argmin MORE RECENT than argmax → price sitting at its
    # lowest point in the window → rejection/reversal low
...
def momentum(pair, sel_pairs, sel_cmo, sel_detail, lock):
    ...
    with lock:
        ...
        if passed:
            ...
            # store raw_cmo for ranking (most negative = deepest oversold = best)
...
while True:
    ...
    sel_pairs  = []
    sel_cmo    = []
    sel_detail = {}   # {symbol: detail_dict from check_dip_conditions}
    ...
    # ── full diagnostic: show every candidate with condition breakdown ──
    print(f'\n  [1m dip diagnostic]  {len(fp3)} candidates'
    ...
    # ── selection: if >1 dip passes, rank by raw CMO (most negative = deepest) ──
    if len(sel_pairs) > 1:
        lbls = [label_map.get(p, p) for p in sel_pairs]
        print(f'  {len(sel_pairs)} mtf dips found: {lbls}')
        print(f'  Ranking by raw CMO — most negative = deepest oversold = best pick')
    ...
    # price already fetched inside check_dip_conditions — use it directly
