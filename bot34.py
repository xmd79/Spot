...
→ Implements major reversal detection using argmin/argmax on 15s TF and 1m TF
...
→ Targets 1.618033988749894% profit (golden ratio) for fast scalps
→ Uses entire USDC balance for trades
→ NO STOP LOSS - Relies on fast imminent spikes/pumps
→ FIXED: Take profit target at 1.618% (golden ratio) for optimal gains
→ FIXED: Improved buy/sell functions to use entire balance
→ FIXED: Proper dust conversion to ensure no BTC remains
→ UPDATED: Using limit orders for take profit at exact target price
→ UPDATED: All float values use 25 decimal places for highest accuracy
→ UPDATED: Properly checks for existing BTC positions and orders
→ UPDATED: Added major reversal detection in both 1m and 15s timeframes
→ UPDATED: Added target reach prediction functionality
"""
...
# Set decimal precision to 25 for highest accuracy
getcontext().prec = 25
...
TP_TARGET_PERCENT = 1.618033988749894  # Golden ratio (1.618%) for optimal gains
TOTAL_FEE_PERCENT = 0.22  # Total fee percentage (0.1% for buy + 0.1% for sell + 0.02% buffer)
MIN_TRADE_AMOUNT = 10
MIN_BTC_AMOUNT = 0.00001  # Minimum BTC amount to consider a position

# Dust Conversion Configuration
MIN_DUST_CONVERSION_AMOUNT = 0.000001
...
trade_active = False
trade_info = {}
take_profit_order_id = None  # Track the take profit order ID
...
def receive_price_webhook():
    ...
    try:
        ...
        if 'price' in data:
            ...
            return jsonify({'status': 'success', 'price': f"{price:.25f}"})
            ...
            if asset == 'BTC' and MIN_DUST_CONVERSION_AMOUNT <= total_balance <= MAX_DUST_CONVERSION_AMOUNT:
                try:
                    ...
                    if usd_value < MIN_TRADE_AMOUNT:
                        print(f"Found BTC dust: {asset} - {total_balance:.25f} (≈${usd_value:.25f})")
            ...
            try:
                ...
                if result.get('success', False):
                    ...
                    for item in result.get('transferResult', []):
                        print(f"  {item['fromAsset']}: {item['amount']:.25f} → USDC")
                    return True
                else:
...
def check_major_reversal_argmin_argmax_condition(client, symbol):
    """
    Implementation using argmin/argmax to identify major reversal points.
    This looks at the last 1200 values in both timeframes to identify the most significant
    dip (minimum) and top (maximum) points, and checks if the most recent one is a dip.
    """
    try:
        # Get 1-minute data
        klines_1m = client.get_klines(symbol=symbol, interval='1m', limit=1200)
        if not klines_1m or len(klines_1m) < 100:
            return False, {"error": "Insufficient 1-minute data"}
        ...
        # Ensure we have at least 1200 values in 15-second timeframe
        if len(df_15sec) < 1200:
            return False, {"error": "Insufficient 15-second data"}
        
        # Get the last 1200 values from each timeframe
        df_1m_last1200 = df_1m.tail(1200)
        df_15sec_last1200 = df_15sec.tail(1200)
        
        # Find major reversal points in 1-minute timeframe using argmin/argmax
        close_prices_1m = df_1m_last1200['close'].values
        
        # Find the index of the minimum price (dip)
        min_idx_1m = np.argmin(close_prices_1m)
        min_price_1m = close_prices_1m[min_idx_1m]
        min_timestamp_1m = df_1m_last1200.iloc[min_idx_1m]['timestamp']
        
        # Find the index of the maximum price (top)
        max_idx_1m = np.argmax(close_prices_1m)
        max_price_1m = close_prices_1m[max_idx_1m]
        max_timestamp_1m = df_1m_last1200.iloc[max_idx_1m]['timestamp']
        
        # Determine which is more recent
        is_dip_recent_1m = min_timestamp_1m > max_timestamp_1m
        
        # Find major reversal points in 15-second timeframe using argmin/argmax
        close_prices_15sec = df_15sec_last1200['close'].values
        
        # Find the index of the minimum price (dip)
        min_idx_15sec = np.argmin(close_prices_15sec)
        min_price_15sec = close_prices_15sec[min_idx_15sec]
        min_timestamp_15sec = df_15sec_last1200.iloc[min_idx_15sec]['timestamp']
        
        # Find the index of the maximum price (top)
        max_idx_15sec = np.argmax(close_prices_15sec)
        max_price_15sec = close_prices_15sec[max_idx_15sec]
        max_timestamp_15sec = df_15sec_last1200.iloc[max_idx_15sec]['timestamp']
        
        # Determine which is more recent
        is_dip_recent_15sec = min_timestamp_15sec > max_timestamp_15sec
        
        # Condition is met if both timeframes show a recent dip as the most recent major reversal
        condition_met = is_dip_recent_1m and is_dip_recent_15sec
        
        details = {
            "is_dip_recent_1m": is_dip_recent_1m,
            "is_dip_recent_15sec": is_dip_recent_15sec,
            "min_price_1m": float(min_price_1m),
            "max_price_1m": float(max_price_1m),
            "min_timestamp_1m": min_timestamp_1m,
            "max_timestamp_1m": max_timestamp_1m,
            "min_price_15sec": float(min_price_15sec),
            "max_price_15sec": float(max_price_15sec),
            "min_timestamp_15sec": min_timestamp_15sec,
            "max_timestamp_15sec": max_timestamp_15sec,
        ...
        print(f"check_major_reversal_argmin_argmax_condition error: {e}")
...
def calculate_momentum_trend(candles, timeframe, period=MOMENTUM_PERIOD, lookback=MOMENTUM_LOOKBACK):
    """
    Calculate momentum and determine if it's positive (> 0).
    
    Parameters:
    - candles: List of candle data
    - timeframe: Timeframe string (e.g., "1m", "5m")
    - period: Period for momentum calculation
    - lookback: Number of recent momentum values to check
    
    Returns:
    - momentum_values: Array of momentum values
    - momentum_positive: Boolean indicating if momentum is positive (> 0)
    - momentum_negative: Boolean indicating if momentum is negative (< 0)
    """
    if len(candles) < period + lookback:
        logging.warning(f"Insufficient data ({len(candles)}) for momentum analysis in {timeframe}")
        print(f"Insufficient data ({len(candles)}) for momentum analysis in {timeframe}")
        return np.zeros(lookback), False, True, False, False
    ...
    if len(momentum) >= lookback:
        ...
        momentum_negative = current_momentum < 0
        
        # For this condition, we only care if momentum > 0
        momentum_increasing = False
        momentum_decreasing = False
        
        logging.info(f"{timeframe} - Momentum: {current_momentum:.25f}, Positive: {momentum_positive}, Negative: {momentum_negative}")
        print(f"{timeframe} - Current Momentum: {current_momentum:.25f}")
        print(f"{timeframe} - Momentum > 0: {momentum_positive}")
        print(f"{timeframe} - Recent Momentum Values: {[round(m, 25) for m in recent_momentum]}")
        
        return recent_momentum, momentum_positive, momentum_negative, momentum_increasing, momentum_decreasing
    else:
        logging.warning(f"Insufficient momentum values ({len(momentum)}) for analysis in {timeframe}")
        print(f"Insufficient momentum values ({len(momentum)}) for analysis in {timeframe}")
        return np.zeros(lookback), False, True, False, False
...
def analyze_instant_target_condition(client, symbol, timeframe='1m', lookback=500):
    ...
    try:
        ...
        # This ensures the take profit target at 1.618% is contained
        target_percentages = {
...
def predict_target_reach(client, symbol, entry_price, target_price, timeframe='1m', lookback=100):
    """
    Predict if the price will reach the target price based on momentum, trend, and other indicators.
    Returns a probability score (0-1) and details about the prediction.
    """
    try:
        # Get market data
        klines = client.get_klines(symbol=symbol, interval=timeframe, limit=lookback)
        ...
        price_momentum = float(df['price_momentum'].iloc[-1])
        volume_momentum = float(df['volume_momentum'].iloc[-1])
        price_acceleration = float(df['price_acceleration'].iloc[-1])
        current_rsi = float(df[f'RSI_{RSI_PERIOD}'].iloc[-1])
        
        # Calculate distance to target
        distance_to_target = target_price - current_price
        distance_pct = (distance_to_target / current_price) * 100
        
        # Calculate factors for prediction
        factors = {
            "momentum_factor": 0.0,
            "volume_factor": 0.0,
        ...
        # Momentum factor (0-1): Higher momentum increases probability
        if price_momentum > 0:
            factors["momentum_factor"] = min(1.0, price_momentum * 100)
        
        # Volume factor (0-1): Higher volume momentum increases probability
        if volume_momentum > 0:
            factors["volume_factor"] = min(1.0, volume_momentum * 10)
        
        # Acceleration factor (0-1): Positive acceleration increases probability
        if price_acceleration > 0:
            factors["acceleration_factor"] = min(1.0, price_acceleration * 1000)
        
        # RSI factor (0-1): RSI between 30-70 increases probability
        if 30 <= current_rsi <= 70:
            factors["rsi_factor"] = 1.0 - abs(current_rsi - 50) / 20
        elif current_rsi < 30:
            factors["rsi_factor"] = 0.5  # Oversold but might bounce
        else:
            factors["rsi_factor"] = 0.2  # Overbought, less likely to go up
        
        # Distance factor (0-1): Closer to target increases probability
        if distance_pct > 0:
            factors["distance_factor"] = min(1.0, 1.0 / (distance_pct / 0.5))
        
        # Calculate overall probability (weighted average of factors)
        weights = {
        ...
        # Additional adjustments based on recent price action
        ...
        if bullish_candles > bearish_candles:
            probability *= 1.1  # Increase probability if recent trend is bullish
        elif bearish_candles > bullish_candles:
            probability *= 0.9  # Decrease probability if recent trend is bearish
        ...
        print(f"predict_target_reach error: {e}")
        return 0.0, {"error": str(e)}

# ------------------ Trade Execution Functions ------------------

def execute_buy_order(client, symbol, usdc_amount):
    """Execute a market buy order using the entire USDC balance with robust LOT_SIZE filter handling."""
    try:
        ...
        # FIXED: Calculate exact amount accounting for fees (assuming 0.1% fee)
        fee_rate = 0.001  # 0.1% fee
        trade_amount = usdc_amount / (1 + fee_rate)
        quantity = trade_amount / current_price
        
        # --- ROBUST LOT_SIZE FILTER HANDLING ---
        try:
            ...
            if lot_size_filter:
                ...
                max_qty = Decimal(lot_size_filter.get('maxQty', float('inf')))
                step_size = Decimal(lot_size_filter.get('stepSize', 0))

                # Convert quantity to Decimal for calculations
                quantity_decimal = Decimal(str(quantity))

                # 1. Check against minQty
                if quantity_decimal < min_qty:
                    print(f"ERROR: Calculated quantity {quantity_decimal:.25f} is below the minimum allowed {min_qty:.25f}")
                    return {'success': False, 'error': f'Quantity {quantity_decimal:.25f} is below minQty {min_qty:.25f}'}

                # 2. Check against maxQty and cap if necessary
                if quantity_decimal > max_qty:
                    print(f"WARNING: Quantity {quantity_decimal:.25f} is above the maximum allowed {max_qty:.25f}. Capping at max_qty.")
                    quantity_decimal = max_qty

                # 3. Adjust for stepSize (round down to the nearest step)
                # The formula is: floor(quantity / stepSize) * stepSize
                if step_size > 0:
                    quantity_decimal = (quantity_decimal // step_size) * step_size
                
                # Final check after rounding down
                if quantity_decimal < min_qty:
                    print(f"ERROR: Quantity {quantity_decimal:.25f} after rounding is below minQty {min_qty:.25f}. Cannot execute trade.")
                    return {'success': False, 'error': f'Adjusted quantity {quantity_decimal:.25f} is below minQty {min_qty:.25f}'}

                quantity = float(quantity_decimal)
                print(f"SUCCESS: Adjusted quantity to {quantity:.25f} to comply with LOT_SIZE filter.")
                
                # Check if the order meets MIN_NOTIONAL requirement
                if min_notional_filter:
                    min_notional = Decimal(min_notional_filter.get('minNotional', 10))
                    order_value = Decimal(str(quantity)) * Decimal(str(current_price))
                    if order_value < min_notional:
                        print(f"ERROR: Order value {order_value:.25f} is below minimum notional {min_notional:.25f}")
                        return {'success': False, 'error': f'Order value {order_value:.25f} is below MIN_NOTIONAL {min_notional:.25f}'}

            else:
                print("WARNING: LOT_SIZE filter not found. Proceeding without quantity adjustment.")
                quantity = round(quantity, 25) # Fallback precision

        except Exception as filter_error:
            print(f"ERROR applying lot size filter: {filter_error}. Proceeding without adjustment.")
            quantity = round(quantity, 25) # Fallback precision
        
        # Calculate the actual cost including fees
        actual_cost = quantity * current_price * (1 + fee_rate)
        ...
        return {
            ...
            'price': current_price,
            'cost': actual_cost,
            'balance_used_pct': (actual_cost / usdc_amount) * 100,
...
def execute_sell_order(client, symbol, target_price=None):
    """
    FIXED: Execute a limit sell order for the entire BTC balance at the target price to ensure 1.618% profit after fees.
    """
    try:
        # Get current price
        current_price = get_current_price_from_webhook()
        ...
        if target_price and current_price < target_price:
            return {
                'success': False,
                'error': f"Current price ({current_price:.25f}) is below target price ({target_price:.25f})"
            }

        # Step 1: Get the entire available BTC balance
        btc_balance = get_account_balance(client, 'BTC')
        
        if btc_balance <= 0:
            return {'success': False, 'error': 'No BTC balance to sell.'}
        
        print(f"Found BTC balance: {btc_balance:.25f}")

        # Step 2: Get symbol info for LOT_SIZE filter
        try:
            symbol_info = client.get_symbol_info(symbol)
            lot_size_filter = next((f for f in symbol_info['filters'] if f['filterType'] == 'LOT_SIZE'), None)
            price_filter = next((f for f in symbol_info['filters'] if f['filterType'] == 'PRICE_FILTER'), None)
        except Exception as e:
            print(f"Could not get symbol info: {e}. Proceeding without LOT_SIZE adjustment.")
            lot_size_filter = None
            price_filter = None

        # Step 3: Calculate maximum sellable quantity
        quantity_to_sell = Decimal(str(btc_balance))
        ...
        if lot_size_filter:
            ...
            # Adjust for stepSize (round down to the nearest step)
            if step_size > 0:
                quantity_to_sell = (quantity_to_sell // step_size) * step_size

            # Final check after rounding down
            if quantity_to_sell < min_qty:
                print(f"Calculated quantity {quantity_to_sell:.25f} is below minQty {min_qty:.25f}.")
                # In this case, we can't sell, so we try to convert it all to dust
                print("Attempting to convert entire balance to dust.")
                if convert_btc_dust_to_usdc(client):
                    return {'success': True, 'message': 'Entire balance converted to USDC as dust.'}
                else:
                    return {'success': False, 'error': f'Balance {btc_balance:.25f} is too small to sell and dust conversion failed.'}

        # Step 4: Calculate the target price for 1.618% profit after fees
        # We need to account for the 0.1% selling fee, so we need to set a higher price
        # Target price = entry_price * (1 + 1.618% + 0.1% fee) = entry_price * 1.01718
        if target_price is None:
            # If no target price provided, use current price
            target_price = current_price
        
        # Adjust price according to PRICE_FILTER if needed
        if price_filter:
            tick_size = Decimal(price_filter.get('tickSize', 0))
            if tick_size > 0:
                # Round up to the nearest tick to ensure the order executes
                target_price_decimal = Decimal(str(target_price))
                target_price_decimal = ((target_price_decimal / tick_size).quantize(Decimal('1'), rounding=ROUND_UP) * tick_size)
                target_price = float(target_price_decimal)
                print(f"Adjusted target price to {target_price:.25f} to comply with PRICE_FILTER")

        # Step 5: Execute the limit sell order
        print(f"Executing limit sell order for {float(quantity_to_sell):.25f} BTC at price {target_price:.25f}...")
        order = client.order_limit_sell(
            symbol=symbol,
            quantity=float(quantity_to_sell),
            price=f"{target_price:.25f}"
        )
        ...
        print(f"Expected USDC Received: {float(quantity_to_sell) * target_price:.25f}")

        return {
            'success': True,
            'order_id': order['orderId'],
            'symbol': symbol,
            'quantity_sold': float(order['origQty']),
            'price': target_price,
            'timestamp': datetime.now(),
            'order': order
        }
...
def place_take_profit_order(client, symbol, entry_price, btc_quantity):
    """
    Place a limit sell order for take profit at the target price (1.618% profit after fees).
    """
    try:
        # Calculate the target price for 1.618% profit after fees
        # We need to account for the 0.1% selling fee, so we need to set a higher price
        # Target price = entry_price * (1 + 1.618% + 0.1% fee) = entry_price * 1.01718
        target_price = entry_price * (1 + (TP_TARGET_PERCENT + 0.1) / 100)
        
        # Get symbol info for PRICE_FILTER
        try:
            symbol_info = client.get_symbol_info(symbol)
            price_filter = next((f for f in symbol_info['filters'] if f['filterType'] == 'PRICE_FILTER'), None)
        except Exception as e:
            print(f"Could not get symbol info: {e}. Proceeding without PRICE_FILTER adjustment.")
            price_filter = None
        ...
        # Get BTC balance to ensure we have enough
        btc_balance = get_account_balance(client, 'BTC')
        
        # Use the minimum of available balance and the quantity we want to sell
        sell_quantity = min(btc_balance, btc_quantity)
        ...
        # Adjust quantity according to LOT_SIZE if needed
        if lot_size_filter:
            min_qty = Decimal(lot_size_filter.get('minQty', 0))
            max_qty = Decimal(lot_size_filter.get('maxQty', float('inf')))
            step_size = Decimal(lot_size_filter.get('stepSize', 0))
            
            quantity_decimal = Decimal(str(sell_quantity))
            ...
            # Final check after rounding down
            if quantity_decimal < min_qty:
                print(f"Calculated quantity {quantity_decimal:.25f} is below minQty {min_qty:.25f}.")
                return {'success': False, 'error': f'Quantity {quantity_decimal:.25f} is below minQty {min_qty:.25f}'}
            
            sell_quantity = float(quantity_decimal)
        
        # Place the limit sell order
        print(f"Placing take profit limit sell order for {sell_quantity:.25f} BTC at price {target_price:.25f}...")
        order = client.order_limit_sell(
            symbol=symbol,
        ...
        print(f"Quantity: {order['origQty']}")
        print(f"Price: {order['price']}")
        print(f"Expected USDC Received: {sell_quantity * target_price:.25f}")
...
def check_take_profit_order(client):
    """
    Check if the take profit order has been filled.
    """
    ...
    try:
        # Check order status
        order = client.get_order(symbol=SYMBOL, orderId=take_profit_order_id)
        
        if order['status'] == 'FILLED':
            print(f"TAKE PROFIT ORDER FILLED! Order ID: {take_profit_order_id}")
            print(f"Quantity: {order['executedQty']}")
            print(f"Price: {order['price']}")
            print(f"USDC Received: {order['cummulativeQuoteQty']}")
            
            # Reset the take profit order ID
            take_profit_order_id = None
            
            return True
        elif order['status'] in ['CANCELED', 'REJECTED', 'EXPIRED']:
            print(f"TAKE PROFIT ORDER NOT FILLED! Status: {order['status']}")
        ...
        print(f"Error checking take profit order: {e}")
        return False
    except Exception as e:
        print(f"Unexpected error checking take profit order: {e}")
        return False

def cancel_take_profit_order(client):
    """
    Cancel the take profit order if it exists.
    """
    global take_profit_order_id
    ...
    try:
        # Cancel the order
        result = client.cancel_order(symbol=SYMBOL, orderId=take_profit_order_id)
        
        print(f"TAKE PROFIT ORDER CANCELED! Order ID: {take_profit_order_id}")
        ...
        print(f"Error canceling take profit order: {e}")
        return False
    except Exception as e:
        print(f"Unexpected error canceling take profit order: {e}")
        return False
...
def check_existing_orders(client):
    """Check for existing orders and return sell orders if any."""
    try:
...
def get_trade_history(client, limit=10):
    """Get recent trade history to determine entry price."""
    try:
        ...
        if buy_trades:
            ...
            buy_trades.sort(key=lambda x: x['time'], reverse=True)
            return buy_trades
        return []
    except Exception as e:
...
def check_trade_status(client):
    """Check if profit target is reached for active trade (NO STOP LOSS)."""
    global trade_active, trade_info, take_profit_order_id
    ...
    try:
        ...
        tp_price = entry_price * (1 + (TP_TARGET_PERCENT + 0.1) / 100)  # 1.618% + 0.1% fee
        ...
        if take_profit_order_id is not None:
            # Check if the take profit order has been filled
            if check_take_profit_order(client):
                print(f"Trade closed successfully at take profit price.")
                trade_active = False
                trade_info = {}
                return True
        else:
            # If no take profit order exists, place one
            btc_balance = get_account_balance(client, 'BTC')
            if btc_balance > 0:
                tp_result = place_take_profit_order(client, SYMBOL, entry_price, btc_balance)
                if tp_result['success']:
                    take_profit_order_id = tp_result['order_id']
                    trade_info['take_profit_order_id'] = take_profit_order_id
                    trade_info['take_profit_price'] = tp_price
                else:
                    print(f"ERROR PLACING TAKE PROFIT ORDER: {tp_result['error']}")
        
        # Check for profit target (1.35%)
        if current_price >= target_price:
            print(f"\nPROFIT TARGET REACHED! Canceling take profit order and selling at market price.")
            
            # Cancel the take profit order
            cancel_take_profit_order(client)
            
            # Execute market sell order
            sell_result = execute_sell_order(client, SYMBOL)
            
            if sell_result['success']:
                print(f"Trade closed successfully at market price.")
                trade_active = False
                trade_info = {}
                return True
            else:
...
def display_trade_status():
    """Display current trade status if active (NO STOP LOSS)."""
    global trade_active, trade_info, take_profit_order_id
    ...
    print(f"{'Price Difference:':<20}{trade_info['price_diff']:+.25f} ({trade_info['price_diff_pct']:+.2f}%)")
    print(f"{'Time Elapsed:':<20}{trade_info['time_elapsed']}")
    print(f"{'TP Price (1.618%):':<20}{trade_info['tp_price']:.25f}")
    print(f"{'Distance to TP:':<20}{trade_info['tp_diff']:.25f} ({trade_info['tp_diff_pct']:.2f}%)")
    print(f"{'Target Price (1.35%):':<20}{trade_info['target_price']:.25f}")
    print(f"{'Distance to Target:':<20}{trade_info['target_diff']:.25f} ({trade_info['target_diff_pct']:.2f}%)")
    print(f"{'Quantity:':<20}{trade_info['quantity']:.25f}")
    if 'balance_used_pct' in trade_info:
        print(f"{'Balance Used:':<20}{trade_info['balance_used_pct']:.2f}%")
    if take_profit_order_id is not None:
        print(f"{'Take Profit Order ID:':<20}{take_profit_order_id}")
    
    # Add target prediction if available
    if 'target_prediction' in trade_info:
        prediction = trade_info['target_prediction']
        print(f"{'Target Reach Probability:':<20}{prediction['probability']:.2%}")
...
def perform_single_iteration_analysis(client):
    ...
    # Check account balances at the beginning
    ...
    sell_orders = check_existing_orders(client)
    print(f"Found {len(sell_orders)} existing sell orders")
    
    # Get trade history
    print("\n2. Checking trade history...")
    buy_trades = get_trade_history(client, limit=5)
    print(f"Found {len(buy_trades)} recent buy trades")
    
    # Determine if we're in an in-trade or out-of-trade scenario
    in_trade_scenario = (btc_balance > MIN_BTC_AMOUNT) or (len(sell_orders) > 0)
    
    if in_trade_scenario:
        print("\n>>> IN-TRADE SCENARIO DETECTED - BTC BALANCE OR SELL ORDERS FOUND <<<")
        
        # If trade is not marked as active but we have BTC or sell orders, set it as active
        if not trade_active:
            print("Trade not marked as active but BTC balance or sell orders found. Activating trade monitoring...")
            
            # Get current price
            current_price = get_current_price(client, SYMBOL)
            if current_price is None:
                print("Error: Could not get current price. Cannot activate trade monitoring.")
                return
            ...
            # Try to get entry price from trade history
            if buy_trades:
                # Use the most recent buy trade
                entry_price = float(buy_trades[0]['price'])
                entry_time = datetime.fromtimestamp(buy_trades[0]['time'] / 1000)
                print(f"Found entry price from trade history: {entry_price:.25f} at {entry_time}")
            else:
                # If no trade history, estimate entry price (we don't know the exact entry price)
                # In a real implementation, you might want to track this more accurately
                entry_price = current_price * 0.99  # Assume we entered at 1% lower price
                entry_time = datetime.now() - timedelta(minutes=30)  # Assume we entered 30 minutes ago
                print(f"Estimated entry price: {entry_price:.25f} at {entry_time}")
            ...
            trade_info['tp_price'] = entry_price * (1 + (TP_TARGET_PERCENT + 0.1) / 100)  # 1.618% + 0.1% fee
            ...
            trade_info['tp_diff'] = trade_info['tp_price'] - current_price
            trade_info['tp_diff_pct'] = (trade_info['tp_diff'] / current_price) * 100
            
            # Check if there's already a sell order
            if sell_orders:
                # Use the first sell order as the take profit order
                take_profit_order_id = sell_orders[0]['orderId']
                trade_info['take_profit_order_id'] = take_profit_order_id
                print(f"Found existing sell order with ID: {take_profit_order_id}")
            else:
        ...
        print("\n>>> OUT-OF-TRADE SCENARIO - NO BTC BALANCE OR SELL ORDERS FOUND <<<")
        
        # If trade is marked as active but we have no BTC or sell orders, reset it
        if trade_active:
            print("Trade marked as active but no BTC balance or sell orders found. Deactivating trade monitoring...")
            trade_active = False
            trade_info = {}
            take_profit_order_id = None
    
    # Step 1: Check and convert BTC dust (only when no active trade)
    print("\n4. Checking for BTC dust...")
    ...
    print("\n6. Performing technical analysis...")
    
    conditions_met = 0
    total_conditions = 7  # Including the new major reversal condition
    condition_details = {}
    ...
    print(f"\nCurrent Price: {target_details.get('current_price', 0):.25f}")
    ...
    for target_name, target_info in target_details.get('targets', {}).items():
        ...
        print(f"{display_name}: {target_info['price']:.25f} {direction_symbol} "
    ...
    # Print details for this condition
    print(f"\nBullish Volume: {volume_details.get('bullish_volume', 0):.25f} ({volume_details.get('bullish_pct', 0):.2f}%)")
    print(f"Bearish Volume: {volume_details.get('bearish_volume', 0):.25f} ({volume_details.get('bearish_pct', 0):.2f}%)")
    print(f"Total Volume: {volume_details.get('total_volume', 0):.25f}")
    ...
    print(f"\nCurrent Momentum: {current_momentum:.25f}")
    print(f"Momentum > 0: {momentum_positive}")
    print(f"Momentum < 0: {momentum_negative}")
    ...
    # Print details for this condition
    print(f"\nCurrent Price: {current_price:.25f}")
    print(f"FFT Forecast Price: {float(fft_forecast):.25f}")
    print(f"Forecast Difference: {float(fft_forecast) - current_price:.25f} ({((float(fft_forecast) / current_price - 1) * 100) if current_price > 0 else 0:.2f}%)")
    ...
    # NEW Condition 7: Major Reversal Point (Dip) in Both Timeframes
    print("\n--- Condition 7: Major Reversal Point (Dip) in Both Timeframes ---")
    major_reversal_met, reversal_details = check_major_reversal_argmin_argmax_condition(client, SYMBOL)
    condition_details['major_reversal'] = {
        'met': major_reversal_met,
        'details': reversal_details
    }
    condition_results['Major Reversal Point (Dip) in Both Timeframes'] = major_reversal_met
    
    # Print details for this condition
    print(f"\n1-minute timeframe: {'Dip' if reversal_details.get('is_dip_recent_1m', False) else 'Top'}")
    print(f"15-second timeframe: {'Dip' if reversal_details.get('is_dip_recent_15sec', False) else 'Top'}")
    print(f"Condition Met: {major_reversal_met}")
    
    if major_reversal_met:
        conditions_met += 1
        print("\nTRUE - Major Reversal Point (Dip) condition MET")
    else:
        print("\nFALSE - Major Reversal Point (Dip) condition NOT met")
    ...
    if conditions_met == total_conditions:
        ...
        if buy_result['success']:
            ...
            print(f"Cost: {buy_result['cost']:.25f} USDC ({buy_result['balance_used_pct']:.2f}% of balance)")
            ...
            trade_info = {
                ...
                'balance_used_pct': buy_result.get('balance_used_pct', 0)
            }
            ...
            if current_price:
                ...
                trade_info['tp_price'] = buy_result['price'] * (1 + (TP_TARGET_PERCENT + 0.1) / 100)  # 1.618% + 0.1% fee
                trade_info['target_diff'] = trade_info['target_price'] - current_price
                trade_info['target_diff_pct'] = (trade_info['target_diff'] / current_price) * 100
                trade_info['tp_diff'] = trade_info['tp_price'] - current_price
                trade_info['tp_diff_pct'] = (trade_info['tp_diff'] / current_price) * 100
            ...
            # Place a take profit order
            tp_result = place_take_profit_order(client, SYMBOL, buy_result['price'], buy_result['quantity'])
            if tp_result['success']:
                take_profit_order_id = tp_result['order_id']
                trade_info['take_profit_order_id'] = take_profit_order_id
            else:
            ...
            # Predict if target will be reached
            target_price = buy_result['price'] * (1 + (TP_TARGET_PERCENT + 0.1) / 100)
            probability, prediction_details = predict_target_reach(
                client, SYMBOL, buy_result['price'], target_price
            )
            
            print(f"\nTARGET REACH PREDICTION:")
            print(f"Probability of reaching target: {probability:.2%}")
            print(f"Current price: {prediction_details.get('current_price', 0):.25f}")
            print(f"Target price: {prediction_details.get('target_price', 0):.25f}")
            print(f"Distance to target: {prediction_details.get('distance_pct', 0):.2f}%")
            
            # Add prediction details to trade_info
            trade_info['target_prediction'] = {
                'probability': probability,
...
def main():
    ...
    print("\nEach iteration will:")
    print("1. Check account balances to determine in-trade or out-of-trade scenario")
    print("2. Check for existing orders and trade history")
    print("3. If in-trade scenario (BTC balance or sell orders found), monitor for take profit")
    print("4. If out-of-trade scenario, perform market analysis")
    print("5. Execute trade if ALL conditions met")
    print("6. Use entire USDC balance for entry")
    print("7. Place a limit order for take profit at 1.618% (including fees)")
    print("8. Monitor for profit target every 5 seconds (NO STOP LOSS)")
    ...
    print("- FFT Up Cycle")
    print("- Major Reversal Point (Dip) in Both Timeframes (NEW)")
    print("\nFIXED: Take profit target at 1.618% (golden ratio) including fees")
    print("FIXED: Using limit orders for take profit at exact target price")
    print("FIXED: All float values use 25 decimal places for highest accuracy")
    print("FIXED: Properly checks for existing BTC positions and orders")
    print("NEW: Added major reversal detection in both 1m and 15s timeframes")
    print("NEW: Added target reach prediction functionality")
    print("\nNO STOP LOSS - Bot relies on fast imminent spikes/pumps")
    print("="*60)
