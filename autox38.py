...
# NEW IMPORTS FOR HARMONIC LOGIC
...
PROFIT_TARGET_PERCENT = 1.5   # 1.5% net profit goal
TOTAL_FEE_PERCENT = 0.22      # 0.22% total fee percentage
MIN_TRADE_AMOUNT = 10
...
# =========================================================
# 🧠 SECTION: HARMONIC NONLINEAR SYSTEM MODULE (UPDATED FORECAST LOGIC)
# =========================================================
...
def dominant_cycle(price, fs=1.0):
    try:
        price = np.array(price, dtype=np.float64)
        if len(price) < 10: return 100, 0.01 # Default fallback
        
        price = price - np.mean(price)
        yf = np.abs(fft(price))
        xf = fftfreq(len(price), d=1/fs)

        pos = xf > 0
        xf, yf = xf[pos], yf[pos]

        if len(yf) == 0: return 100, 0.01

        idx = np.argmax(yf)
        if xf[idx] == 0: return 100, 0.01
        return 1 / xf[idx], xf[idx]
    except:
        return 100, 0.01

def harmonic_phases(length, cycle, harmonics=range(2,10)):
    t = np.arange(length)
    base = 2 * np.pi * (t % cycle) / cycle
    return {h: (base * h) % (2*np.pi) for h in harmonics}

def entropy_weight(x):
    p = np.abs(x) / (np.sum(np.abs(x)) + 1e-9)
    entropy = -np.sum(p * np.log(p + 1e-9))
    return 1 / (1 + entropy)

def harmonic_energy(phases):
    energy = np.zeros(len(next(iter(phases.values()))))
    for h, theta in phases.items():
        e = np.sin(theta) * np.cos(theta)
        w = entropy_weight(e)
        energy += w * e
    
    max_e = np.max(np.abs(energy))
    if max_e == 0: return energy
    return energy / max_e

def fear_greed(x):
    x = np.clip(x, -0.999, 0.999)
    return np.arcsinh(x) - np.arctanh(x)

def market_regime(energy):
    try:
        vol = np.std(np.diff(energy))
        slope = np.mean(np.gradient(energy))
...
def mtf_coherence(prices_dict):
    try:
        phases = []
        for tf, price in prices_dict.items():
            cycle, _ = dominant_cycle(price)
            phase = 2*np.pi*((len(price) - 1) % cycle)/cycle
            phases.append(np.sin(phase))
        return np.mean(phases)
    except:
        return 0

def harmonic_engine(price, mtf_prices=None):
    price = np.array(price)
    if len(price) < 50: return None 

    try:
        cycle, freq = dominant_cycle(price)
        phases = harmonic_phases(len(price), cycle)

        energy = harmonic_energy(phases)
        fg = fear_greed(energy)

        kf = KalmanFilter1D()
        smooth_energy = np.array([kf.update(e) for e in energy])

        curvature = np.gradient(np.gradient(smooth_energy))
        reversal_prob = np.exp(-np.abs(curvature))

        slope = np.gradient(smooth_energy)
        trend = int(np.sign(slope[-1]))

        regime = market_regime(smooth_energy)

        mtf = mtf_coherence(mtf_prices) if mtf_prices else 0

        long_signal = (
            trend > 0 and
            reversal_prob[-1] > 0.6 and
            fg[-1] > 0 and
            mtf > 0
        )
        ...
        # =========================================================
        # 📈 UPDATED FORECAST LOGIC: Force Directional Alignment
        # =========================================================
        # Ensure 5m has highest range, 15s has smallest (naturally via std_dev)
        # But force the forecast to be in the direction of the Trend.
        
        vol_std = np.std(price)
        energy_magnitude = abs(smooth_energy[-1])
        projected_move = energy_magnitude * vol_std
        
        current_price = price[-1]

        if trend > 0:
            # Up Cycle: Forecast MUST be higher than current
            forecast = current_price + projected_move
        elif trend < 0:
            # Down Cycle: Forecast MUST be lower than current
            forecast = current_price - projected_move
        else:
        ...
        return {
            ...
            "regime": regime,
            "energy": float(smooth_energy[-1]),
            "fear_greed": float(fg[-1]),
            "reversal_probability": float(reversal_prob[-1])
        }
    except Exception as e:
        print(f"Error in harmonic engine: {e}")
        return None

def analyze_harmonic_forecast(candles_15s, candles_1m, candles_3m, candles_5m):
    """
    Runs the harmonic engine on 15s, 1m, 3m, and 5m timeframes.
    Returns a dictionary with trend, forecast, and direction for each TF.
    """
    mtf_prices = {}
    raw_candles = {
        '15s': candles_15s,
    ...
    for tf, c_list in raw_candles.items():
        mtf_prices[tf] = np.array([c['close'] for c in c_list], dtype=np.float64)

    results = {}

    for tf in ['15s', '1m', '3m', '5m']:
        if len(mtf_prices[tf]) < 100:
            results[tf] = {"status": "INSUFFICIENT_DATA"}
            continue

        engine_res = harmonic_engine(mtf_prices[tf], mtf_prices=mtf_prices)
        
        if engine_res:
            current_price = mtf_prices[tf][-1]
            forecast = engine_res['forecast_price']
            trend = engine_res['trend']
            
            direction_str = "▲ UP" if trend > 0 else "▼ DOWN"
            if trend == 0: direction_str = "- NEUTRAL"
            ...
            results[tf] = {
                ...
                "regime": engine_res['regime'],
                "signal": "LONG" if engine_res['long'] else ("SHORT" if engine_res['short'] else "NEUTRAL"),
                "forecast_desc": f"Price expected to move {direction_str} towards {forecast:.2f}"
            }
        else:
            results[tf] = {"status": "ERROR"}

    return results


# =========================================================
# SECTION: ORIGINAL UTILITIES & INDICATORS
# =========================================================
...
def get_dip_confirmation(closes, lookback=1200):
    try:
        ...
        if n < 5: return False, -1, -1, 0.0, 0.0

        clean_data = recent_data[~np.isnan(recent_data)]
        if len(clean_data) == 0: return False, -1, -1, 0.0, 0.0
        ...
        for i in range(len(np_closes) - 1, -1, -1):
            ...
            if not found_max and abs(val - max_val) < 1e-8:
                abs_max_idx = i
                found_max = True
            if found_min and found_max: break

        if abs_min_idx == -1 or abs_max_idx == -1: return False, -1, -1, 0.0, 0.0
...
def check_stoch_oversold_vs_overbought_1min(candles_1m):
    try:
        if not candles_1m or len(candles_1m) < 15: return False, "Insufficient data", -1, -1, 0.0
        ...
        for i in range(len(slowk) - 1, -1, -1):
            val = slowk[i]
            if np.isnan(val): continue
            if last_oversold_idx == -1 and val <= oversold_limit: last_oversold_idx = i
            if last_overbought_idx == -1 and val >= overbought_limit: last_overbought_idx = i
            if last_oversold_idx != -1 and last_overbought_idx != -1: break

        condition_met = last_oversold_idx > last_overbought_idx
        description = "Most recent extreme was OVERSOLD (Bullish)" if condition_met else "Most recent extreme was OVERBOUGHT (Bearish)"
        return condition_met, description, last_oversold_idx, last_overbought_idx, slowk[-1]
    except Exception as e:
        return False, str(e), -1, -1, 0.0

def check_rsi_oversold_vs_overbought_15sec(candles_15s):
    try:
        if not candles_15s or len(candles_15s) < 20: return False, "Insufficient data", -1, -1, 0.0
        ...
        condition_met = last_oversold_idx > last_overbought_idx
        description = "Most recent extreme was OVERSOLD (Bullish)" if condition_met else "Most recent extreme was OVERBOUGHT (Bearish)"
        return condition_met, description, last_oversold_idx, last_overbought_idx, rsi_values[-1]
    except Exception as e:
        return False, str(e), -1, -1, 0.0

def analyze_stoch_rsi_precise_strategy_15sec(candles_15s):
    try:
        if not candles_15s or len(candles_15s) < 20: return {"error": "Insufficient 15s data", "condition_met": False}
        ...
        print(f"RSI %D (MA 3):         {current_rsi_d:.2f}")
        ...
        return {
            "timeframe": "15s", "condition_met": condition_met, "current_rsi": current_rsi,
            "current_rsi_d": current_rsi_d, "current_stoch_k": current_stoch_k,
            "current_stoch_d": current_stoch_d, "dip_confirmed": is_dip_15s,
            "rsi_trigger": rsi_condition_met, "stoch_k_trigger": stoch_k_condition_met
        }
    except Exception as e:
...
def analyze_stoch_rsi_precise_strategy_1min(candles_1m):
    try:
        if not candles_1m or len(candles_1m) < 20: return {"error": "Insufficient 1m data", "condition_met": False}
        ...
        return {
            "timeframe": "1m", "condition_met": condition_met, "current_rsi": current_rsi,
            "current_rsi_d": current_rsi_d, "current_stoch_k": current_stoch_k,
            "current_stoch_d": current_stoch_d, "dip_confirmed": is_dip_1m,
            "rsi_trigger": rsi_condition_met, "stoch_k_trigger": stoch_k_condition_met
        }
    except Exception as e:
        return {"error": str(e), "condition_met": False}
...
def get_candles(symbol, timeframe, limit=1200, retries=5, delay=5, endTime=None):
    for attempt in range(retries):
        try:
            ...
            params = {'symbol': symbol, 'interval': timeframe, 'limit': actual_limit}
...
def get_current_price(retries=5, delay=5):
    for attempt in range(retries):
        try:
            ...
            if price > Decimal('0'): return price
...
def get_last_buy_trade():
    try:
        ...
        for trade in reversed(trades):
            if trade['isBuyer']:
                return {"price": Decimal(str(trade['price'])), "qty": Decimal(str(trade['qty'])), "time": trade['time']}
    except BinanceAPIException as e:
...
def buy_asset():
    try:
        ...
        if current_price <= Decimal('0'): return None, None, None, None
        usdc_balance = get_balance('USDC')
        if usdc_balance <= Decimal('0'): return None, None, None, None
        ...
        if adjusted_quantity < min_trade_size: return None, None, None, None
...
def sell_asset(asset_balance):
    try:
        current_price = get_current_price()
        if current_price <= Decimal('0'): return False
        ...
        if sell_quantity < min_trade_size: return False
...
def check_exit_condition(initial_investment, asset_balance, entry_price):
    if initial_investment <= Decimal('0.0') or asset_balance <= Decimal('0.0') or entry_price <= Decimal('0.0'): return False
    current_price = get_current_price()
    if current_price <= Decimal('0.0'): return False
...
def analyze_volume_sentiment(candles):
    try:
        if not candles: return False, 0.0, 0.0, {"error": "No candles provided"}
        ...
        for candle in candles:
            ...
            if close > open_p: total_bullish_vol += vol
            elif close < open_p: total_bearish_vol += vol

        total_vol = total_bullish_vol + total_bearish_vol
        if total_vol == 0: return False, 0.0, 0.0, {"error": "Total volume is zero"}
...
def analyze_thresholds_15sec(candles_15s):
    try:
        if not candles_15s: return {"error": "No 15s data provided", "condition_met": False}
        ...
        if len(close_prices) < 2: return {"error": "Insufficient data for 15s analysis", "condition_met": False}
        ...
        return {
            "timeframe": "15s", "condition_met": condition_met, "current_price": current_price,
            "min_value": min_value, "max_value": max_value, "midpoint": midpoint,
...
def analyze_sine_wave_oscillator_15sec(candles_15s):
    try:
        ...
        if len(recent_candles) < 300: return {"error": "Insufficient 15s data", "condition_met": False}
        ...
        if len(minima_indices) < 2 or len(maxima_indices) < 2: return {"error": "Not enough extrema", "condition_met": False}
        ...
        if len(valid_fft) == 0: return {"error": "No valid frequency components", "condition_met": False}
        
        dominant_freq_idx = np.argmax(valid_fft)
        dominant_freq = valid_freq[dominant_freq_idx]
        dominant_period = 1 / np.abs(dominant_freq)
        
        min_threshold, max_threshold, avg_mtf, momentum_signal, range_price = calculate_thresholds(close_prices)
        ...
        if last_min_idx > last_max_idx:
            ...
            if forecast_price <= current_price: forecast_price = current_price + (current_price * 0.001)
            ...
            for i in range(1, len(all_extrema)):
                if (all_extrema[i] in minima_indices and all_extrema[i-1] in minima_indices) or \
                   (all_extrema[i] in maxima_indices and all_extrema[i-1] in maxima_indices): continue
        ...
        if cycle_type == "UP": cycle_position = (current_idx - last_min_idx) / wave_period if wave_period > 0 else 0
        else: cycle_position = (current_idx - last_max_idx) / wave_period if wave_period > 0 else 0
        
        return {
            "condition_met": condition_met, "cycle_type": cycle_type, "current_price": current_price,
            "forecast_price": forecast_price, "next_extremum_type": next_extremum_type,
            "last_min_idx": last_min_idx, "last_max_idx": last_max_idx,
            "last_min_value": close_prices[last_min_idx], "last_max_value": close_prices[last_max_idx],
            "cycle_position": cycle_position, "wave_period": wave_period, "fib_amplitude": fib_amplitude,
            "dominant_period": dominant_period, "min_threshold": min_threshold, "max_threshold": max_threshold,
            "midpoint_threshold": midpoint_threshold, "momentum_signal": momentum_signal,
            "threshold_signal": "UP_STAGE_1" if current_price < midpoint_threshold else "UP_STAGE_2",
            "avg_period_between_extrema": avg_period_between_extrema,
            "description": f"Current cycle is {cycle_type}, expecting a new {next_extremum_type} at price {forecast_price:.2f}."
        }
    except Exception as e:
...
def analyze_stoch_conditions(candles_1m, candles_15s, lookback=200):
    def get_most_recent_state(values, oversold_limit, overbought_limit):
        for val in reversed(values):
            if val <= oversold_limit: return "OVERSOLD", True
            if val >= overbought_limit: return "OVERBOUGHT", False 
...
def enhanced_aroon(high, low, close, period=14):
    try:
        if len(high) < period or len(low) < period: return None, None, None, None, None, None
        ...
        if recent_high is None or recent_low is None: return None, None, None, None, None, None
...
def analyze_multiple_timeframe_extrema(candles_1m, candles_3m=None, candles_5m=None):
    ...
    try:
        ...
        if aroon_1m[0] is not None:
            ...
            timeframe_extrema['1m'] = {'recent_high': recent_high, 'recent_low': recent_low, 'recent_high_idx': aroon_1m[4], 'recent_low_idx': aroon_1m[5], 'aroon_up': aroon_1m[0], 'aroon_down': aroon_1m[1]}
        ...
        if candles_3m and len(candles_3m) >= 14:
            ...
            if aroon_3m[0] is not None:
                ...
                timeframe_extrema['3m'] = {'recent_high': recent_high, 'recent_low': recent_low, 'recent_high_idx': aroon_3m[4], 'recent_low_idx': aroon_3m[5], 'aroon_up': aroon_3m[0], 'aroon_down': aroon_3m[1]}
        ...
        if candles_5m and len(candles_5m) >= 14:
            ...
            if aroon_5m[0] is not None:
                ...
                timeframe_extrema['5m'] = {'recent_high': recent_high, 'recent_low': recent_low, 'recent_high_idx': aroon_5m[4], 'recent_low_idx': aroon_5m[5], 'aroon_up': aroon_5m[0], 'aroon_down': aroon_5m[1]}
...
def calculate_momentum(candles, period=10):
    try:
        if not candles or len(candles) < period + 1: return None, 0.0, {"error": "Insufficient data"}
        ...
        details = {"timeframe": candles[0]["timeframe"], "current_momentum": current_momentum, "momentum_positive": momentum_positive, "period": period}
...
def generate_15s_data_from_1m(df_1m):
    ...
    for idx, row in df_1m.iterrows():
        ...
        if high_price == low_price:
            for i in range(4):
                ...
                if i == 0: opens.append(open_price); closes.append(open_price + (close_price - open_price) * 0.25)
                elif i == 1: opens.append(open_price + (close_price - open_price) * 0.25); closes.append(open_price + (close_price - open_price) * 0.5)
                elif i == 2: opens.append(open_price + (close_price - open_price) * 0.5); closes.append(open_price + (close_price - open_price) * 0.75)
                else: opens.append(open_price + (close_price - open_price) * 0.75); closes.append(close_price)
                highs.append(high_price)
                lows.append(low_price)
                volumes.append(volume / 4)
        else:
            o1 = open_price; c1 = high_price
            ...
            o3 = c2; c3 = (open_price + close_price) / 2
    ...
    df_15s = pd.DataFrame({'timestamp': timestamps, 'open': opens, 'high': highs, 'low': lows, 'close': closes, 'volume': volumes})
    return df_15s

def analyze_momentum_15sec(candles_1m):
    try:
        if not candles_1m: return {"error": "No 1m data provided"}
        df_1m = pd.DataFrame(candles_1m)
        ...
        if df_15s is None or df_15s.empty: return {"error": "Failed to generate 15s data"}
        
        candles_15s = []
        for idx, row in df_15s.iterrows():
            candles_15s.append({"time": int(row['timestamp'].timestamp()), "open": float(row['open']), "high": float(row['high']), "low": float(row['low']), "close": float(row['close']), "volume": float(row['volume']), "timeframe": "15s"})
        
        momentum_positive, momentum_value, momentum_details = calculate_momentum(candles_15s, period=10)
        return {
            "timeframe": "15s", "momentum_positive": momentum_positive, "current_momentum": momentum_value,
...
def calculate_linear_regression_channel(candles, period=500, dev_multiplier=2.0):
    try:
        if not candles or len(candles) < period: return {"error": "Insufficient data for linear regression channel"}
        ...
        above_upper_indices = []
        for i in range(n):
            if close_prices[i] < lower_channel[i]: below_lower_indices.append(i)
            if close_prices[i] > upper_channel[i]: above_upper_indices.append(i)
        ...
        up_cycle = False
        if most_recent_below_lower and most_recent_above_upper: up_cycle = most_recent_below_lower_index > most_recent_above_upper_index
        elif most_recent_below_lower and not most_recent_above_upper: up_cycle = True
        elif not most_recent_below_lower and most_recent_above_upper: up_cycle = False
        return {
            "current_price": current_price, "current_upper": current_upper, "current_lower": current_lower,
            "above_upper": above_upper, "below_lower": below_lower, "most_recent_below_lower": most_recent_below_lower,
            "most_recent_above_upper": most_recent_above_upper, "most_recent_below_lower_index": most_recent_below_lower_index,
            "most_recent_above_upper_index": most_recent_above_upper_index, "up_cycle": up_cycle, "slope": slope,
            "intercept": intercept, "std_dev": std_dev, "period": period, "dev_multiplier": dev_multiplier,
...
def analyze_linear_regression_channel_break(candles):
    try:
        if not candles: return {"error": "No data provided"}
        lrc_result = calculate_linear_regression_channel(candles, period=500, dev_multiplier=2.0)
        if "error" in lrc_result: return lrc_result
        ...
        return {
            "condition_met": condition_met, "current_price": lrc_result["current_price"],
            "upper_channel": lrc_result["current_upper"], "lower_channel": lrc_result["current_lower"],
            "above_upper": lrc_result["above_upper"], "below_lower": lrc_result["below_lower"],
            "most_recent_below_lower": is_most_recent_below_lower, "most_recent_above_upper": is_most_recent_above_upper,
            "most_recent_below_lower_index": most_recent_below_lower_index, "most_recent_above_upper_index": most_recent_above_upper_index,
            "up_cycle": condition_met, "description": description,
            "below_lower_indices": below_lower_indices[-10:] if len(below_lower_indices) > 0 else [],
            "above_upper_indices": above_upper_indices[-10:] if len(above_upper_indices) > 0 else []
        }
    except Exception as e:
...
def analyze_aroon_only_signal(candles, aroon_period=14):
    try:
        if not candles or len(candles) < aroon_period + 1: return {"error": "Insufficient data", "condition_met": False, "signal": "DOWN"}
        close = [c["close"] for c in candles]
        high = [c["high"] for c in candles]
        low = [c["low"] for c in candles]
        current_price = close[-1]
        aroon_result = enhanced_aroon(high, low, close, aroon_period)
        if aroon_result[0] is None: return {"error": "Failed to calculate Aroon", "condition_met": False, "signal": "DOWN"}
        aroon_up, aroon_down, _, _, recent_high_idx, recent_low_idx = aroon_result
        if aroon_up > aroon_down: signal = "UP"; condition_met = True
        else: signal = "DOWN"; condition_met = False
        ...
        return {
            "signal": signal, "condition_met": condition_met,
            "signal_reason": f"Aroon Up ({aroon_up:.2f}) is {'greater than' if condition_met else 'less than'} Aroon Down ({aroon_down:.2f})",
            "aroon_up": round(aroon_up, 2), "aroon_down": round(aroon_down, 2),
            "current_price": round(current_price, 2), "recent_high": round(recent_high, 2) if recent_high is not None else None,
...
try:
    while True:
        ...
        print(f"Candle counts - 1m: {len(candles_1m)}, 3m: {len(candles_3m)}, 5m: {len(candles_5m)}")
        ...
        # =========================================================
        # GENERATE 15s DATA (Required for Harmonic Analysis)
        # =========================================================
        ...
        # =========================================================
        # RUN HARMONIC FORECAST ANALYSIS
        # =========================================================
        print("\n" + "="*80)
        print("🧠 HARMONIC NONLINEAR SYSTEM ANALYSIS")
        print("="*80)
        
        harmonic_results = analyze_harmonic_forecast(candles_15s, candles_1m, candles_3m, candles_5m)
        
        # Track directions for MTF Consensus
        direction_counts = {"▲ UP": 0, "▼ DOWN": 0, "- NEUTRAL": 0}
        
        for tf in ['5m', '3m', '1m', '15s']:
            res = harmonic_results.get(tf)
            if res and res['status'] == 'ACTIVE':
                ...
                print(f"Forecast:   {res['forecast_price']:.2f}")
                print(f"Logic:      {res['forecast_desc']}")
                print("-" * 40)
                direction_counts[res['direction']] += 1
            else:
                print(f"--- TF: {tf} --- DATA INSUFFICIENT ---")

        # =========================================================
        # HARMONIC CONSENSUS LOGIC
        # =========================================================
        print("\n" + "="*80)
        print("📊 HARMONIC MTF CONSENSUS")
        print("="*80)
        
        # Check if 5m and 1m align UP (Bullish Trigger Criteria)
        harmonic_5m_res = harmonic_results.get('5m')
        harmonic_1m_res = harmonic_results.get('1m')
        harmonic_15s_res = harmonic_results.get('15s')
        
        harmonic_strict_up_condition = False
        harmonic_strict_down_condition = False
        harmonic_consensus = "MIXED"

        if harmonic_5m_res and harmonic_1m_res:
            # Check UP
            if harmonic_5m_res['direction'] == "▲ UP" and harmonic_1m_res['direction'] == "▲ UP":
                # Prefer strict alignment of 5m and 1m. 15s is noise filter.
                if harmonic_15s_res['direction'] == "▲ UP" or harmonic_15s_res['direction'] == "- NEUTRAL":
                    harmonic_strict_up_condition = True
                    harmonic_consensus = "STRONG BULLISH ALIGNMENT (5m+1m+15s)"
            
            # Check DOWN
            if harmonic_5m_res['direction'] == "▼ DOWN" and harmonic_1m_res['direction'] == "▼ DOWN":
                if harmonic_15s_res['direction'] == "▼ DOWN" or harmonic_15s_res['direction'] == "- NEUTRAL":
                    harmonic_strict_down_condition = True
                    harmonic_consensus = "STRONG BEARISH ALIGNMENT (5m+1m+15s)"

        print(f"5m Direction: {harmonic_5m_res['direction'] if harmonic_5m_res else 'N/A'}")
        print(f"1m Direction: {harmonic_1m_res['direction'] if harmonic_1m_res else 'N/A'}")
        print(f"15s Direction: {harmonic_15s_res['direction'] if harmonic_15s_res else 'N/A'}")
        print("-" * 40)
        print(f"CONSENSUS: {harmonic_consensus}")
        print(f"Strict Condition Met (UP): {harmonic_strict_up_condition}")
        print(f"Strict Condition Met (DOWN): {harmonic_strict_down_condition}")
        print("="*80)


        # Initialize all condition results
        conditions_status = {
            "momentum_positive_1m": False, "momentum_positive_15sec": False,
            "linear_regression_channel_break": False, "aroon_only_signal": True,
            "thresholds_15sec": False, "volume_bullish_1min": False,
            "stoch_rsi_precise_15sec": False, "stoch_rsi_precise_1min": False,
            "dip_confirmation_15sec": False, "dip_confirmation_1min": False,
            "stoch_oversold_vs_overbought_1min": False, "rsi_oversold_vs_overbought_15sec": False,
        }
        ...
        # =========================================================
        # UPDATED LOGIC: STRICT CONDITIONS + HARMONIC MTF ALIGNMENT
        # =========================================================
        
        strict_volume_bullish_1m = conditions_status["volume_bullish_1min"]
        strict_dip_conf_1m = conditions_status["dip_confirmation_1min"]
        
        # NEW STRICT CONDITION: Harmonic Alignment
        # Requires 5m, 1m, and 15s to be generally aligned UP (or Neutral/UP)
        strict_harmonic_bullish = harmonic_strict_up_condition
        
        count_condition_met = true_conditions_count >= min_required
        
        # Final Signal: Count met + Volume + Dip + HARMONIC MTF ALIGNMENT
        signal_triggered = count_condition_met and strict_volume_bullish_1m and strict_dip_conf_1m and strict_harmonic_bullish
        
        print("\n" + "="*80)
        print("FINAL TRIGGER LOGIC CHECK")
        print("="*80)
        print(f"[1/4] Count Requirement ({true_conditions_count} >= {min_required}): {'PASS' if count_condition_met else 'FAIL'}")
        print(f"[2/4] Strict Volume Bullish 1m: {'PASS' if strict_volume_bullish_1m else 'FAIL'}")
        print(f"[3/4] Strict Dip Confirmation 1m: {'PASS' if strict_dip_conf_1m else 'FAIL'}")
        print(f"[4/4] Strict Harmonic 5m Alignment: {'PASS' if strict_harmonic_bullish else 'FAIL'}")
        print("-" * 80)
        ...
        if signal_triggered:
            signal_data = {
                ...
                "signal": "BUY",
                "conditions_met": f"{true_conditions_count}/{len(active_conditions_results)} (Strict: Vol, Dip, Harmonic)"
            }
        ...
        if position_open:
            ...
            if signal_triggered:
                ...
                if usdc_balance > Decimal('0'):
                    ...
                    print(f"Strict Conditions: Volume 1m ({strict_volume_bullish_1m}), Dip 1m ({strict_dip_conf_1m}), Harmonic 5m Alignment ({strict_harmonic_bullish})")
                ...
                if not strict_harmonic_bullish:
                    print(f"STRICT CONDITION FAILED: Harmonic 5m Alignment is False.")
