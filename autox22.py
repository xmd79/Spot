...
# CHANGE 1: Removed old conditions and added new sine wave conditions.
CONFIG = {
    "conditions": {
        ...
        "sine_wave_1m": True,          # NEW
        "sine_wave_15s": True,          # NEW
    },
    "min_conditions_met": 5  # Updated to 5 active conditions
}
...
def enhanced_aroon(high, low, close, period=14):
    try:
        ...
        # Use numpy for faster argmin/argmax
        ...
        actual_high_idx = len(high_prices) - lookback_period + recent_high_idx
        actual_low_idx = len(low_prices) - lookback_period + recent_low_idx
        
        periods_since_high = len(close_prices) - 1 - actual_high_idx
        periods_since_low = len(close_prices) - 1 - actual_low_idx
        
        aroon_up = ((period - periods_since_high) / period) * 100
        aroon_down = ((period - periods_since_low) / period) * 100
        
        return aroon_up, aroon_down, periods_since_high, periods_since_low, actual_high_idx, actual_low_idx
...
# =========================================================
# NEW SINE WAVE ANALYSIS FUNCTION
# =========================================================
def analyze_sine_wave_condition(candles, timeframe_name):
    """
    Scales current close price to a sine wave and determines if it's closer to min or max.
    Condition is met if the current sine value is closer to the minimum.
    """
    try:
        if not candles or len(candles) < 50: # HT_SINE needs a decent amount of data
            return {"error": f"Insufficient data for Sine Wave analysis on {timeframe_name}", "condition_met": False}
        ...
        # Replace NaN values with the last valid value or 0 if all are NaN
        last_valid_sine = sine_wave[~np.isnan(sine_wave)][-1] if any(~np.isnan(sine_wave)) else 0
        sine_wave = np.nan_to_num(sine_wave, nan=last_valid_sine)

        # The user's code has sine_wave = -sine_wave. This inverts the wave.
        sine_wave = -sine_wave
        
        current_sine = sine_wave[-1]

        # Calculate min and max of the sine wave over the period
        sine_wave_min = np.min(sine_wave)
        sine_wave_max = np.max(sine_wave)
        hilo_range = sine_wave_max - sine_wave_min

        if hilo_range == 0:
            return {"error": f"Sine wave range is zero for {timeframe_name}", "condition_met": False}

        # Calculate % distances of the CURRENT SINE VALUE
        dist_to_min_percent = ((current_sine - sine_wave_min) / hilo_range) * 100
        dist_to_max_percent = ((sine_wave_max - current_sine) / hilo_range) * 100
        
        # Condition is met if the current sine value is closer to the minimum
        condition_met = dist_to_min_percent < dist_to_max_percent
        ...
        return {
            ...
            "sine_min": sine_wave_min,
            "sine_max": sine_wave_max,
            "dist_to_min_percent": dist_to_min_percent,
            "dist_to_max_percent": dist_to_max_percent,
            "description": f"Current Sine is closer to {'MIN' if condition_met else 'MAX'} (Dist to Min: {dist_to_min_percent:.2f}%, Dist to Max: {dist_to_max_percent:.2f}%)"
        }

    except Exception as e:
        print(f"Error in analyze_sine_wave_condition for {timeframe_name}: {e}")
        return {"error": str(e), "condition_met": False}
...
try:
    while True:
        ...
        conditions_status = {
            ...
            "sine_wave_1m": False,          # NEW
            "sine_wave_15s": False,          # NEW
        }
        ...
        # Condition 6: Sine Wave Analysis (1m) - NEW
        print("\n--- Condition 6: Sine Wave Analysis (1m) ---")
        sine_1m_result = analyze_sine_wave_condition(candles_1m, "1m")
        if 'error' not in sine_1m_result:
            conditions_status["sine_wave_1m"] = sine_1m_result['condition_met']
            print(f"Timeframe: {sine_1m_result['timeframe']}")
            print(f"Current Close: {sine_1m_result['current_close']:.2f}")
            print(f"Current Sine: {sine_1m_result['current_sine']:.4f}")
            print(f"Sine Min: {sine_1m_result['sine_min']:.4f}, Sine Max: {sine_1m_result['sine_max']:.4f}")
            print(f"Distance to Min: {sine_1m_result['dist_to_min_percent']:.2f}%")
            print(f"Distance to Max: {sine_1m_result['dist_to_max_percent']:.2f}%")
            print(f"Description: {sine_1m_result['description']}")
            print(f"Condition Met: {conditions_status['sine_wave_1m']}")
        else:
            print(f"Error analyzing sine wave (1m): {sine_1m_result['error']}")
            print(f"Condition Met: {conditions_status['sine_wave_1m']}")

        # Condition 7: Sine Wave Analysis (15s) - NEW
        print("\n--- Condition 7: Sine Wave Analysis (15s) ---")
        # We need to generate 15s candles first
        df_1m_for_15s = pd.DataFrame(candles_1m)
        df_1m_for_15s['timestamp'] = pd.to_datetime(df_1m_for_15s['time'], unit='s').dt.tz_localize('UTC').dt.tz_convert(LOCAL_TIMEZONE)
        df_15s = generate_15s_data_from_1m(df_1m_for_15s)
        if df_15s is not None and not df_15s.empty:
            candles_15s = []
            for idx, row in df_15s.iterrows():
                candles_15s.append({
                    "time": int(row['timestamp'].timestamp()),
            ...
            sine_15s_result = analyze_sine_wave_condition(candles_15s, "15s")
            if 'error' not in sine_15s_result:
                conditions_status["sine_wave_15s"] = sine_15s_result['condition_met']
                print(f"Timeframe: {sine_15s_result['timeframe']}")
                print(f"Current Close: {sine_15s_result['current_close']:.2f}")
                print(f"Current Sine: {sine_15s_result['current_sine']:.4f}")
                print(f"Sine Min: {sine_15s_result['sine_min']:.4f}, Sine Max: {sine_15s_result['sine_max']:.4f}")
                print(f"Distance to Min: {sine_15s_result['dist_to_min_percent']:.2f}%")
                print(f"Distance to Max: {sine_15s_result['dist_to_max_percent']:.2f}%")
                print(f"Description: {sine_15s_result['description']}")
                print(f"Condition Met: {conditions_status['sine_wave_15s']}")
            else:
                print(f"Error analyzing sine wave (15s): {sine_15s_result['error']}")
                print(f"Condition Met: {conditions_status['sine_wave_15s']}")
        else:
            print("Could not generate 15s data for sine wave analysis.")
            print(f"Condition Met: {conditions_status['sine_wave_15s']}")
