...
# Updated configurable conditions - now with 8 conditions
CONFIG = {
    "conditions": {
        ...
        "ema_crossover_orderflow": True,
        "spectral_phase_reversal": True,
    },
    "min_conditions_met": 8  # Updated to require all 8 conditions
}
...
cycle_intensity = 0  # New variable for cycle intensity (0-100%)
condition_met = False  # Track if condition is met
...
# =========================================================
# SPECTRAL ANALYSIS FUNCTIONS (NEW - ROBUST VERSION)
# =========================================================

def spectral_features(prices):
    """
    Extract spectral features from price data using FFT and TA-LIB.
    This version is robust against NaN and zero values.
    """
    try:
        prices = np.array(prices, dtype=np.float64)
        
        # 1. Input Validation: Handle zero or invalid prices
        if len(prices) < 10 or np.all(prices == 0):
            # Not enough data or all prices are zero
            return None, None, None, 0.0

        # Replace any zero or non-finite prices with the last valid price
        # This prevents division by zero in returns calculation
        ...
        # 2. Volatility adaptive FFT window
        returns = np.diff(prices) / prices[:-1]
        # Handle potential division by zero if prices are constant
        if np.all(np.isnan(returns)):
            vol = 0.0
        else:
            vol = np.nanstd(returns) # Use nanstd to be safe
        
        window = int(np.clip(300 / (vol * 100 + 1e-6), 200, min(1200, len(prices))))
        prices_window = prices[-window:]

        fft_vals = np.fft.fft(prices_window)
        mag = np.abs(fft_vals)

        # Keep top 10 frequencies
        idx = np.argsort(mag)[-10:]
        f_fft = np.zeros_like(fft_vals)
        f_fft[idx] = fft_vals[idx]

        filt = np.fft.ifft(f_fft).real
        
        # 3. Robust Phase Calculation using TA-LIB
        try:
            sine, leadsine = talib.HT_SINE(filt)
            # Check if HT_SINE returned valid data
            if np.all(np.isnan(sine)):
                # Fallback if HT_SINE fails completely - use a simple approach
                # Create a synthetic signal based on the rate of change
                analytic_signal = np.zeros_like(filt)
                for i in range(1, len(filt)):
                    analytic_signal[i] = filt[i] - filt[i-1]
                print("Warning: talib.HT_SINE returned all NaNs, using synthetic signal.")
            else:
                # Replace any remaining NaNs in the sine wave with 0
                sine = np.nan_to_num(sine)
                analytic_signal = sine
        except Exception as e:
            print(f"Error in talib.HT_SINE, using fallback: {e}")
            # Create a synthetic signal based on the rate of change
            analytic_signal = np.zeros_like(filt)
            for i in range(1, len(filt)):
                analytic_signal[i] = filt[i] - filt[i-1]
        ...
        phase = np.angle(analytic_signal + 1j * filt)
        amp = np.abs(analytic_signal)

        # Final check to ensure outputs are clean
        phase = np.nan_to_num(phase)
        amp = np.nan_to_num(amp)
        amp_var = np.nan_to_num(np.var(amp))
        vol = np.nan_to_num(vol)

        return phase, amp, amp_var, vol
    except Exception as e:
        print(f"Error in spectral_features: {e}")
        return None, None, None, 0.0

def phase_metrics(phase):
    """
    Calculate phase metrics for reversal detection.
    This version is robust against NaN and invalid inputs.
    """
    try:
        # 1. Input Validation
        if phase is None or not isinstance(phase, np.ndarray) or len(phase) < 3:
            return 0, 0, 0
        
        # 2. Handle NaNs in the input phase array
        if np.any(np.isnan(phase)):
            # Replace NaNs with the last valid value to maintain continuity
            phase = np.nan_to_num(phase)
        ...
        # 3. Calculate metrics with clipping to prevent extreme values
        velocity = np.clip(abs(p - p1), 0, 2 * np.pi)
        acceleration = np.clip(abs((p - p1) - (p1 - p2)), 0, 2 * np.pi)

        # distance to reversal extrema
        dist = min(abs(np.pi - abs(p)), abs(abs(p)))
        # Ensure denominator is not zero
        phase_score = (1 - dist / np.pi) if np.pi > 0 else 0

        return phase_score, velocity, acceleration
    except Exception as e:
        print(f"Error in phase_metrics: {e}")
        return 0, 0, 0

def reversal_probability(phase_score, vel, acc, amp, amp_var):
    """
    Calculate probability of reversal based on phase metrics.
    This version is robust against NaN and invalid inputs.
    """
    try:
        # 1. Input Validation: Replace NaNs with 0
        phase_score = 0 if np.isnan(phase_score) else phase_score
        vel = 0 if np.isnan(vel) else vel
        acc = 0 if np.isnan(acc) else acc
        amp = 0 if np.isnan(amp) else amp
        amp_var = 0 if np.isnan(amp_var) else amp_var

        # 2. Calculate score components
        vel_score = 1 - np.tanh(vel * 6)
        acc_score = np.tanh(acc * 4)
        
        # Prevent division by zero
        denominator = amp + amp_var + 1e-8
        amp_norm = amp / denominator if denominator > 0 else 0

        prob = phase_score * vel_score * acc_score * amp_norm
        return np.clip(prob, 0, 1)
    except Exception as e:
        print(f"Error in reversal_probability: {e}")
        return 0

def phase_locking(phases):
    """
    Calculate phase locking across multiple timeframes.
    This version is robust against empty or invalid inputs.
    """
    try:
        ...
        for i in range(len(keys)):
            for j in range(i + 1, len(keys)):
                # Ensure phase values are valid numbers before calculating difference
                phase1 = phases[keys[i]]
                phase2 = phases[keys[j]]
                if np.isfinite(phase1) and np.isfinite(phase2):
                    diffs.append(abs(phase1 - phase2))
        
        if not diffs:
            return 0
            
        lock = 1 - np.mean(diffs) / np.pi
        return np.clip(lock, 0, 1)
    except Exception as e:
        print(f"Error in phase_locking: {e}")
        return 0

def analyze_spectral_phase_reversal(candles_1m):
    """
    Analyze spectral phase reversal using only 1m timeframe data.
    This version includes comprehensive spectral analysis with detailed metrics.
    """
    global condition_met, cycle_intensity
    
    try:
        if not candles_1m or len(candles_1m) < 200:
            return {"error": "Insufficient data for spectral analysis", "condition_met": False}
        
        # Extract close prices - ONLY USING 1m TIMEFRAME
        close_1m = [c["close"] for c in candles_1m]
        
        # Analyze 1m timeframe
        phase_1m, amp_1m, amp_var_1m, vol_1m = spectral_features(close_1m)
        ...
        if phase_1m is not None and len(phase_1m) > 0:
            phase_score_1m, vel_1m, acc_1m = phase_metrics(phase_1m)
            prob_1m = reversal_probability(phase_score_1m, vel_1m, acc_1m, 
                                          amp_1m[-1] if amp_1m is not None and len(amp_1m) > 0 else 0, 
                                          amp_var_1m if amp_var_1m is not None else 0)
            
            # Ensure probability is a valid number before storing
            prob_1m = 0 if np.isnan(prob_1m) else prob_1m
        
        # Store 1m timeframe data
        tf_data = {
            "1m": {
                "close": close_1m[-1],
                "phase_score": phase_score_1m,
                "vel": vel_1m,
                "acc": acc_1m,
                "prob": prob_1m,
                "vol": vol_1m if vol_1m is not None else 0
            }
        }
        
        # Store phase only if it's a valid number
        last_phase = phase_1m[-1] if len(phase_1m) > 0 else 0
        phases = {"1m": last_phase} if np.isfinite(last_phase) else {"1m": 0}
        
        # Calculate cycle intensity (0-100%)
        # Phase cycles from 0 to 2π, where 0 and 2π are the same point
        # We'll map this to 0-100% for the cycle intensity
        phase_normalized = (last_phase % (2 * np.pi)) / (2 * np.pi)
        cycle_intensity = phase_normalized * 100
        
        # Determine cycle direction (up or down)
        prev_phase = phase_1m[-2] if len(phase_1m) > 1 else 0
        phase_change = last_phase - prev_phase
        cycle_direction = "UP" if phase_change > 0 else "DOWN"
        
        # Calculate percentage until reversal
        # If cycle is UP (0-50%), reversal is at 100%
        # If cycle is DOWN (50-100%), reversal is at 0%
        if cycle_direction == "UP":
            pct_until_reversal = (100 - cycle_intensity)
        else:
            pct_until_reversal = cycle_intensity
        
        # Forecast reversal price based on cycle position and volatility
        current_price = close_1m[-1]
        expected_move = vol_1m * 2 if vol_1m is not None else 0.0  # realistic intraday expansion
        
        # Calculate potential reversal price
        if cycle_direction == "UP":
            # In UP cycle, reversal is at the top
            reversal_price = current_price * (1 + (pct_until_reversal / 100) * expected_move)
        else:
            # In DOWN cycle, reversal is at the bottom
            reversal_price = current_price * (1 - (pct_until_reversal / 100) * expected_move)
        
        # Enhanced spectral analysis
        prices = np.array(close_1m, dtype=np.float64)
        
        # FFT analysis for frequency, power, phase
        window = min(300, len(prices))
        prices_window = prices[-window:]
        
        # Detrend the data
        detrended = prices_window - np.mean(prices_window)
        ...
        if np.any(positive_freq_idx):
            dominant_freq_idx = np.argmax(power_spectrum[positive_freq_idx])
            dominant_freq = fft_freq[positive_freq_idx][dominant_freq_idx]
            dominant_power = power_spectrum[positive_freq_idx][dominant_freq_idx]
        else:
            dominant_freq = 0
            dominant_power = 0
        
        # Calculate phase angles at different degrees
        phase_angle_deg = np.degrees(last_phase) % 360
        
        # Calculate angles for quadrants
        quadrant_angles = {
            "N": 0,      # North (0°)
            "NE": 45,    # Northeast (45°)
            "E": 90,     # East (90°)
        ...
        # Calculate distance to each quadrant
        quadrant_distances = {}
        for name, angle in quadrant_angles.items():
            # Calculate the shortest angular distance
            distance = min((phase_angle_deg - angle) % 360, (angle - phase_angle_deg) % 360)
            quadrant_distances[name] = distance
        
        # Calculate 45-degree and 90-degree angle ratios
        angle_45 = 45  # degrees
        angle_90 = 90  # degrees
        
        # Calculate angular momentum (phase change rate)
        if len(phase_1m) > 2:
            angular_momentum = phase_1m[-1] - phase_1m[-3]  # Phase change over 2 periods
        else:
            angular_momentum = 0
        
        # Calculate resonance, coherence, interference, symmetry
        # Resonance: How aligned the current phase is with the dominant frequency
        resonance = np.cos(last_phase - 2 * np.pi * dominant_freq)
        
        # Coherence: Measure of phase consistency across frequencies
        phase_coherence = np.std(np.angle(fft_vals)) / np.pi  # Normalized to [0,1]
        
        # Interference: Measure of destructive/constructive patterns
        # Calculate autocorrelation to find interference patterns
        autocorr = np.correlate(detrended, detrended, mode='full')
        center = len(autocorr) // 2
        interference = autocorr[center+1] / autocorr[center] if autocorr[center] != 0 else 0
        
        # Symmetry: Measure of how symmetric the power spectrum is
        half_len = len(power_spectrum) // 2
        symmetry = 1 - np.sum(np.abs(power_spectrum[:half_len] - power_spectrum[half_len:2*half_len])) / np.sum(power_spectrum)
        ...
        spectral_centroid = np.sum(fft_freq[:half_len] * power_spectrum[:half_len]) / np.sum(power_spectrum[:half_len]) if np.sum(power_spectrum[:half_len]) > 0 else 0
        
        # Spectral bandwidth (standard deviation around centroid)
        spectral_bandwidth = np.sqrt(np.sum(((fft_freq[:half_len] - spectral_centroid) ** 2) * power_spectrum[:half_len]) / np.sum(power_spectrum[:half_len])) if np.sum(power_spectrum[:half_len]) > 0 else 0
        
        # Spectral skewness (asymmetry of the spectrum)
        spectral_skewness = np.sum(((fft_freq[:half_len] - spectral_centroid) ** 3) * power_spectrum[:half_len]) / (spectral_bandwidth ** 3 * np.sum(power_spectrum[:half_len])) if spectral_bandwidth > 0 and np.sum(power_spectrum[:half_len]) > 0 else 0
        
        # Spectral kurtosis (tailedness of the spectrum)
        spectral_kurtosis = np.sum(((fft_freq[:half_len] - spectral_centroid) ** 4) * power_spectrum[:half_len]) / (spectral_bandwidth ** 4 * np.sum(power_spectrum[:half_len])) if spectral_bandwidth > 0 and np.sum(power_spectrum[:half_len]) > 0 else 0
        
        # Calculate spectral flatness (ratio of geometric to arithmetic mean)
        geometric_mean = np.exp(np.mean(np.log(power_spectrum[:half_len] + 1e-10)))
        arithmetic_mean = np.mean(power_spectrum[:half_len])
        spectral_flatness = geometric_mean / arithmetic_mean if arithmetic_mean > 0 else 0
        
        # Calculate spectral rolloff (frequency below which a certain percentage of energy is contained)
        cumsum_power = np.cumsum(power_spectrum[:half_len])
        total_power = cumsum_power[-1]
        rolloff_threshold = 0.85 * total_power  # 85% of energy
        spectral_rolloff = fft_freq[:half_len][np.where(cumsum_power >= rolloff_threshold)[0][0]] if np.any(cumsum_power >= rolloff_threshold) else fft_freq[:half_len][-1]
        
        # Calculate spectral flux (rate of change of the spectrum)
        if len(prices) > window:
            prev_window = prices[-2*window:-window]
            prev_detrended = prev_window - np.mean(prev_window)
            prev_fft = np.fft.fft(prev_detrended)
            prev_power = np.abs(prev_fft) ** 2
            spectral_flux = np.sum((power_spectrum[:half_len] - prev_power[:half_len]) ** 2)
        else:
            spectral_flux = 0
        
        # Determine market state
        state = (
            "PHASE LOCKED REVERSAL"
            if prob_1m > 0.65 else
            "ENERGY COMPRESSION"
            if prob_1m > 0.45 else
            "TREND CONTINUATION"
        )
        
        # Check if expected move meets our requirements
        required_move = 0.006  # 0.6% gross move needed (0.35% target + 0.25% fees)
        
        # Improved reversal detection logic
        # Check for dip reversal (bottom of cycle with high probability)
        dip_reversal = (cycle_intensity < 20 or cycle_intensity > 80) and prob_1m > 0.65
        
        # UPDATED LOGIC: Set condition_met based on cycle direction
        # If cycle direction is UP, condition should be true
        if cycle_direction == "UP" and not condition_met:
            condition_met = True
            print(f"CYCLE DIRECTION IS UP - Condition changed from False to True")
        elif cycle_direction == "DOWN" and condition_met:
            condition_met = False
            print(f"CYCLE DIRECTION IS DOWN - Condition changed from True to False")
        ...
        if expected_move < required_move:
            ...
            verdict = "GO LONG (SPOT)"
        elif prob_1m > 0.45:
            verdict = "WAIT (BUILDING)"
        else:
            verdict = "NO TRADE"
        ...
        print(f"Cycle Intensity: {cycle_intensity:.1f}%")
        print(f"Percentage Until Reversal: {pct_until_reversal:.1f}%")
        print(f"Forecast Reversal Price: {reversal_price:.2f}")
        print(f"Expected Move: {expected_move*100:.4f}%")
        print(f"Required Move: {required_move*100:.4f}%")
        print(f"Phase (radians): {last_phase:.4f}")
        print(f"Phase (degrees): {phase_angle_deg:.1f}°")
        print(f"Phase Score: {phase_score_1m:.3f}")
        print(f"Phase Velocity: {vel_1m:.3f}")
        print(f"Phase Acceleration: {acc_1m:.3f}")
        print(f"Angular Momentum: {angular_momentum:.4f}")
        print(f"Reversal Probability: {prob_1m:.3f}")
        
        print("\nFREQUENCY ANALYSIS")
        print("-" * 40)
        print(f"Dominant Frequency: {dominant_freq:.6f}")
        print(f"Dominant Power: {dominant_power:.2f}")
        print(f"Spectral Centroid: {spectral_centroid:.6f}")
        print(f"Spectral Bandwidth: {spectral_bandwidth:.6f}")
        print(f"Spectral Skewness: {spectral_skewness:.4f}")
        print(f"Spectral Kurtosis: {spectral_kurtosis:.4f}")
        print(f"Spectral Flatness: {spectral_flatness:.4f}")
        print(f"Spectral Rolloff: {spectral_rolloff:.6f}")
        print(f"Spectral Flux: {spectral_flux:.2f}")
        print(f"Spectral Entropy: {spectral_entropy:.4f}")
        
        print("\nQUADRANT ANALYSIS")
        print("-" * 40)
        for name, distance in quadrant_distances.items():
            print(f"Distance to {name} ({quadrant_angles[name]}°): {distance:.1f}°")
        
        print("\nSPECIAL ANGLES")
        print("-" * 40)
        print(f"45° Angle Ratio: {np.cos(np.radians(45 - phase_angle_deg)):.3f}")
        print(f"90° Angle Ratio: {np.cos(np.radians(90 - phase_angle_deg)):.3f}")
        
        print("\nSPECTRAL METRICS")
        print("-" * 40)
        print(f"Resonance: {resonance:.3f}")
        print(f"Coherence: {phase_coherence:.3f}")
        ...
        print(f"Dip Reversal: {dip_reversal}")
        ...
        return {
            ...
            "fused_prob": prob_1m,
            "lock_score": 0.5,  # Default for single timeframe
            "state": state,
            "verdict": verdict,
            "tf_data": tf_data,
            "cycle_intensity": cycle_intensity,
            "cycle_direction": cycle_direction,
            "pct_until_reversal": pct_until_reversal,
            ...
            "quadrant_distances": quadrant_distances,
            "resonance": resonance,
            "coherence": phase_coherence,
            "interference": interference,
            "symmetry": symmetry,
            "description": f"Spectral analysis indicates {state} with {prob_1m:.3f} reversal probability and cycle intensity of {cycle_intensity:.1f}%"
        }
        
    except Exception as e:
        print(f"Error in analyze_spectral_phase_reversal: {e}")
        import traceback
        traceback.print_exc()
...
# =========================================================
# EMA CROSSOVER WITH ORDER FLOW ANALYSIS FUNCTIONS (UNCHANGED)
# =========================================================
...
# =========================================================
# ENHANCED TECHNICAL ANALYSIS FUNCTIONS (UNCHANGED)
# =========================================================
...
def analyze_multiple_timeframe_extrema(candles_1m):
    """
...
# =========================================================
# FIXED FFT + AROON + ML REVERSAL ANALYSIS (UNCHANGED)
# =========================================================
...
try:
    while True:
        ...
        # Fetch candles for 1m timeframe only
        ...
        conditions_status = {
            ...
            "ema_crossover_orderflow": False,
            "spectral_phase_reversal": False,
        }
        ...
        # Condition 8: Spectral Phase Reversal (NEW - UPDATED)
        print("\n--- Condition 8: Spectral Phase Reversal (UPDATED) ---")
        spectral_result = analyze_spectral_phase_reversal(candles_1m)
        if 'error' not in spectral_result:
            conditions_status["spectral_phase_reversal"] = spectral_result['condition_met']
            print(f"Current Price: {spectral_result['current_price']:.2f}")
            print(f"Expected Move: {spectral_result['expected_move']*100:.4f}%")
            print(f"Required Move: {spectral_result['required_move']*100:.4f}%")
            print(f"Fused Reversal Probability: {spectral_result['fused_prob']:.3f}")
            print(f"Phase Locking Score: {spectral_result['lock_score']:.3f}")
            print(f"Market State: {spectral_result['state']}")
            print(f"Trade Verdict: {spectral_result['verdict']}")
            print(f"Cycle Intensity: {spectral_result['cycle_intensity']:.1f}%")
            print(f"Dip Reversal: {spectral_result['dip_reversal']}")
            print(f"Description: {spectral_result['description']}")
            
            # Print timeframe data if available
            if 'tf_data' in spectral_result:
                print("\nTimeframe Data:")
                for tf, data in spectral_result['tf_data'].items():
                    print(f"  {tf}: Close={data['close']:.2f}, PhaseScore={data['phase_score']:.3f}, "
                          f"Vel={data['vel']:.3f}, Acc={data['acc']:.3f}, Prob={data['prob']:.3f}")
            
            print(f"Condition Met: {conditions_status['spectral_phase_reversal']}")
        else:
            print(f"Error analyzing spectral phase reversal: {spectral_result['error']}")
            print(f"Condition Met: {conditions_status['spectral_phase_reversal']}")
