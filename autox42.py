...
def save_signal_to_file(signal_type, **kwargs):
    ...
    try:
        ...
        # New CSV format to support detailed trade data
        header = "TIMESTAMP,SYMBOL,SIGNAL,PRICE,PROFIT,TIME_HELD,DETAILS\n"
        
        # If file doesn't exist, create with header
        file_exists = os.path.exists("signals.txt")
        if not file_exists:
...
try:
    while True:
        ...
        # Initialize candle_map to None to fix NameError when In Trade
        candle_map = None
        ...
        # ==========================================
            ...
            if not candles_1m:
                print("Error: '1m' candles not fetched. Skipping this iteration.")
                time.sleep(5)
                continue
            ...
            # [Condition analysis blocks - identical to previous script]
            ...
            # LOGIC: STRICT CONDITIONS + COUNT THRESHOLD
            ...
            strict_checks_list = [
                ...
                ("Volume Bullish (1m)", conditions_status["volume_bullish_1min"]), # ADDED HERE
            ]
            ...
            if signal_triggered:
                if usdc_balance > Decimal('0'):
                    ...
                    print(f"Buying with entire USDC balance: {usdc_balance:.25f} at price {current_price:.25f}")
                    ...
                    if entry_price is not None and quantity_bought is not None and cost is not None:
                        ...
                        print(f"Quantity: {quantity_bought:.25f} BTC")
                        print(f"Cost: {cost:.25f} USDC")
                        print(f"Entry Datetime: {entry_datetime.strftime('%Y-%m-%d %H:%M:%S')}")
                        
                        # NOTE: The loop will restart. 
                        # Balances will be checked, BTC > USDC will be detected.
                        # Next iteration will go to "SCENARIO A: IN TRADE".
                    else:
                        print("Error placing buy order.")
                else:
                    print("Signal triggered but no USDC balance available.")
            else:
        ...
        # FIX: Only delete candle_map if it exists (i.e., if we were Out of Trade)
