...
→ Enhanced dust management for both USDC and BTC
"""
...
# ------------------ Enhanced USDC Dust Conversion ------------------

def convert_usdc_to_btc(client):
    """
    Convert all available USDC to BTC using market order.
    This is used when the bot is in a trade and wants to convert USDC dust to BTC.
    """
    try:
        print("Checking for USDC to convert...")
        usdc_balance = get_account_balance(client, 'USDC')
        
        # Check if USDC balance is meaningful
        if usdc_balance <= MIN_TRADE_AMOUNT:
            print(f"USDC balance is below minimum trade amount: {usdc_balance:.2f}")
            return True
        ...
        quantity = usdc_balance / current_price * 0.99  # 1% buffer for fees
...
# ------------------ Trade Status Detection ------------------

def is_in_trade(client):
    """
    Determine if the bot is currently in a trade.
    Returns True if the bot has a significant BTC balance, False otherwise.
    """
    try:
        btc_balance = get_account_balance(client, 'BTC')
        usdc_balance = get_account_balance(client, 'USDC')
        
        # If BTC balance is significant and USDC balance is below minimum for a trade
        # then the bot is considered to be in a trade
        if btc_balance > MIN_BTC_THRESHOLD and usdc_balance < MIN_TRADE_AMOUNT:
            return True
        else:
            return False
    except Exception as e:
        print(f"Error determining trade status: {e}")
        return False
...
def perform_single_iteration_analysis(client):
    ...
    # Step 1: Enhanced Dust Management
    print("\n1. Performing enhanced dust management...")
    ...
    print(f"Current BTC Balance: {btc_balance:.25f}")
    
    # Determine if the bot is in a trade
    in_trade = is_in_trade(client)
    print(f"Bot Status: {'IN TRADE' if in_trade else 'OUT OF TRADE'}")
    
    if in_trade:
        # Bot is in a trade: primary asset is BTC
        # Check for USDC dust and convert to BTC
        if usdc_balance > 0 and usdc_balance < MIN_TRADE_AMOUNT:
            print(f"\nFound {usdc_balance:.2f} USDC dust while in trade. Converting to BTC...")
            usdc_conversion_success = convert_usdc_to_btc(client)
            if not usdc_conversion_success:
                print("USDC to BTC conversion failed.")
            # Update balance after conversion
            usdc_balance = get_account_balance(client, 'USDC')
            btc_balance = get_account_balance(client, 'BTC')
            print(f"New USDC Balance: {usdc_balance:.2f}")
            print(f"New BTC Balance: {btc_balance:.25f}")
    else:
        # Bot is out of trade: primary asset is USDC
        # Check for BTC dust and convert to USDC
        if btc_balance > MIN_BTC_THRESHOLD:
            print(f"\nFound {btc_balance:.25f} BTC dust while out of trade. Converting to USDC...")
            btc_conversion_success = convert_btc_to_usdc(client)
            if not btc_conversion_success:
                print("BTC to USDC conversion failed.")
            # Update balance after conversion
            usdc_balance = get_account_balance(client, 'USDC')
            btc_balance = get_account_balance(client, 'BTC')
            print(f"New USDC Balance: {usdc_balance:.2f}")
    ...
    # Step 2: Check for sufficient USDC balance for a new trade
    print("\n2. Checking for sufficient USDC balance...")
    if usdc_balance < MIN_TRADE_AMOUNT:
...
def main():
    ...
    print("\nEach iteration will:")
    print("1. Perform enhanced dust management for both USDC and BTC")
    ...
    print("- Enhanced dust management for both USDC and BTC")
    print("="*60)
