...
CONFIG = {
    "conditions": {
        ...
        "double_bottom_pattern_15s": True,  # Replaced linear_regression_forecast_15s
        ...
        "double_bottom_pattern_1m": True,  # New condition for 1m timeframe
    },
...
def analyze_double_bottom_pattern(candles, timeframe="15s"):
    ...
    try:
        ...
        if periods_since_min < periods_since_max:
            # Look for a second low that's more recent than the first low but equal or above it
            second_low_idx = None
            second_low_value = None
            
            # Search for a second low after the first low
            for i in range(actual_min_idx + 1, len(close_prices)):
                if recent_lows[i - (len(close_prices) - lookback)] >= min_value:
                    second_low_idx = i
                    second_low_value = recent_lows[i - (len(close_prices) - lookback)]
                    break  # Found the first qualifying second low
            
            # Check if we found a valid second low
            if second_low_idx is not None and second_low_value is not None:
                # Calculate the percentage difference between the two lows
                low_diff_pct = ((second_low_value - min_value) / min_value) * 100 if min_value > 0 else 0
                
                double_bottom = True
                condition_met = True
                pattern_description = f"Double bottom pattern detected. First low: {min_value:.2f} at index {actual_min_idx}, Second low: {second_low_value:.2f} at index {second_low_idx}, Difference: {low_diff_pct:.2f}%"
        
        # Check if max is more recent than min (potential double top)
        elif periods_since_max < periods_since_min:
            # Look for a second high that's more recent than the first high but equal or below it
            second_high_idx = None
            second_high_value = None
            
            # Search for a second high after the first high
            for i in range(actual_max_idx + 1, len(close_prices)):
                if recent_highs[i - (len(close_prices) - lookback)] <= max_value:
                    second_high_idx = i
                    second_high_value = recent_highs[i - (len(close_prices) - lookback)]
                    break  # Found the first qualifying second high
            
            # Check if we found a valid second high
            if second_high_idx is not None and second_high_value is not None:
                # Calculate the percentage difference between the two highs
                high_diff_pct = ((max_value - second_high_value) / max_value) * 100 if max_value > 0 else 0
                
                double_top = True
                condition_met = False  # Always false for double top
                pattern_description = f"Double top pattern detected. First high: {max_value:.2f} at index {actual_max_idx}, Second high: {second_high_value:.2f} at index {second_high_idx}, Difference: {high_diff_pct:.2f}%"
...
try:
    while True:
        ...
        conditions_status = {
            ...
            "double_bottom_pattern_15s": False,  # Replaced linear_regression_forecast_15s
            ...
            "double_bottom_pattern_1m": False,  # New condition for 1m timeframe
        }
        ...
        # Condition 3: Double Bottom Pattern (15s) - Replaced linear_regression_forecast_15s
        ...
        # Condition 8: Double Bottom Pattern (1m) - New condition for 1m timeframe
