...
# Time estimation
MIN_TIME_TO_TARGET = 300  # Minimum 5 minutes
MAX_TIME_TO_TARGET = 86400 * 7  # Maximum 7 days
...
def detect_golden_ratio_patterns(prices, timestamps):
    ...
    Returns potential support/resistance levels based on golden ratio with percentage positions.
    """
    try:
        if len(prices) < 20:
            return None
        ...
        # Find the most recent minimum (dip) and maximum (peak) indices
        min_idx = np.argmin(prices)
        max_idx = np.argmax(prices)
        
        # Calculate percentage positions from min to max (0% to 100%)
        min_price = prices[min_idx]
        max_price = prices[max_idx]
        
        # Ensure min_price is actually the minimum and max_price is the maximum
        if min_price > max_price:
            min_price, max_price = max_price, min_price
            min_idx, max_idx = max_idx, min_idx
        
        # Calculate current position percentage in the range
        if max_price != min_price:
            current_position_pct = ((current_price - min_price) / (max_price - min_price)) * 100
        else:
            current_position_pct = 50.0  # Default to middle if no range
        
        # Golden ratio levels with enhanced calculation
        ...
        # Add percentage positions for each golden ratio level - FIXED: Create a copy to avoid dict size change during iteration
        fib_levels_copy = fib_levels.copy()
        for level, price in fib_levels_copy.items():
            if max_price != min_price:
                position_pct = ((price - min_price) / (max_price - min_price)) * 100
                # Ensure percentage is within 0-100 range
                position_pct = max(0, min(100, position_pct))
                fib_levels[f"{level}_pct"] = position_pct
        
        # Add current position information
        fib_levels['current_position_pct'] = current_position_pct
        fib_levels['min_idx'] = min_idx
        fib_levels['max_idx'] = max_idx
        
        # Add additional levels to ensure full 0-100% coverage
        fib_levels['min_price'] = min_price
        fib_levels['max_price'] = max_price
        fib_levels['argmin_position'] = 0.0
        fib_levels['argmax_position'] = 100.0
...
def detect_golden_triangle(prices, timestamps):
    """
    Enhanced golden triangle detection with more sensitive criteria for upward patterns.
    ...
    try:
            ...
            for j in range(i+3, n-3):  # Reduce minimum distance
                for k in range(j+3, n):  # Reduce minimum distance
                    ...
                    if golden_matches > 0:
                        ...
                        # Prioritize upward triangles (where the last point is higher)
                        if p3[1] > p2[1] and p2[1] > p1[1]:  # Clear upward trend
                            score *= 1.5  # Boost score for clear upward triangles
                        elif p3[1] > p2[1]:  # Last point higher than middle
                            score *= 1.2  # Moderate boost for upward potential
        ...
        # If no triangle found, default to upward if recent price action is bullish
        recent_change = (prices[-1] - prices[-10]) / prices[-10] if len(prices) >= 10 else 0
        if recent_change > 0.01:  # 1% positive change in last 10 periods
            return "upward", 0.3  # Default upward with moderate confidence
...
def calculate_time_to_target(current_price, target_price, cycle_period, current_phase, octagonal_phase, volume_spike_data, timeframe='1h', price_history=None):
    ...
    try:
        ...
        seconds_per_point = timeframe_seconds.get(timeframe, 3600)  # Default to 1h
...
def analyze_sinuosidal_pattern(prices, timestamps, timeframe='1h'):
    """
    Enhanced sinusoidal pattern analysis with next extremum prediction and price conversion.
    Returns amplitude, frequency, phase, and next expected peak/trough with price target.
    """
    try:
        ...
        # Convert sine value to price target
        # Calculate the price at the next extremum
        current_price = prices[-1]
        
        # Calculate the price range (difference between recent high and low)
        ...
        # Calculate the price target based on the extremum type
        if next_extremum_type == "peak":
            # For a peak, target is current price plus amplitude * price_range
            price_target = current_price + (amplitude * price_range * 2)  # Double amplitude for full range
        else:
            # For a trough, target is current price minus amplitude * price_range
            price_target = current_price - (amplitude * price_range * 2)  # Double amplitude for full range
            
        # Add the price target to the next extremum info
        next_extremum = (next_extremum_time, next_extremum_type, price_target)
            
        return amplitude, dominant_freq, phase, next_extremum, period
    except Exception as e:
...
def run_arima(series):
    ...
    try:
        ...
        if len(s) < 50:
            return None
        # Create a proper index for the series to avoid the warning
...
def get_mtf_thresholds(client, symbol):
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {executor.submit(process_timeframe, tf): tf for tf in DETAILED_1M_TIMEFRAMES}
...
def perform_final_analysis(client, symbol):
    ...
    sin_amplitude, sin_freq, sin_phase, next_extremum, sin_period = analyze_sinuosidal_pattern(df['close'].values, timestamps, '1h')
    ...
    # ENHANCED CONSENSUS CALCULATION - FIXED
    # For assets identified as having upward potential, ensure consensus target is above current price
    consensus_numer = 0.0
    consensus_denom = 0.0
    upward_bias = 0.0
    
    # Check if we have strong upward indicators
    has_upward_phase = octagonal_phase in MIN_UPWARD_PHASES if octagonal_phase is not None else False
    has_upward_triangle = triangle_direction == "upward"
    has_upward_indicators = has_upward_phase and has_upward_triangle
    
    # Add upward bias if indicators suggest upward movement
    if has_upward_indicators:
        upward_bias = 0.15  # 15% upward bias
    ...
    for name, target in model_targets.items():
        ...
        # Apply upward bias to models that predict below current price
        adjusted_target = float(target)
        if has_upward_indicators and adjusted_target < current_price:
            # Boost targets below current price to align with upward indicators
            price_diff = current_price - adjusted_target
            adjusted_target = current_price + (price_diff * (1.0 - upward_bias))
            
        weight = conf * atr_penalty
        consensus_numer += weight * adjusted_target
        consensus_denom += weight

    consensus_target = float(consensus_numer / consensus_denom) if consensus_denom > 0 else current_price
    
    # Final check: ensure consensus target is above current price for upward indicators
    if has_upward_indicators and consensus_target <= current_price:
        # Apply minimum upward movement based on ATR
        atr_latest = float(df[f'ATR_{ATR_PERIOD}'].iloc[-1]) if f'ATR_{ATR_PERIOD}' in df.columns else 0.0
        consensus_target = current_price + (atr_latest * 1.5)  # 1.5x ATR above current price
    ...
    if next_extremum:
        extremum_time, extremum_type, price_target = next_extremum
        print(f"Next {extremum_type}: {extremum_time:.2f} (price target: {price_target:.8f})")
    if golden_levels:
        print("Golden Ratio Levels:")
        for level, price in golden_levels.items():
            if not level.endswith('_pct') and level != 'current_position_pct' and level not in ['min_idx', 'max_idx', 'min_price', 'max_price', 'argmin_position', 'argmax_position']:
                print(f"  {level}: {price:.8f}")
                # Add percentage position if available
                pct_key = f"{level}_pct"
                if pct_key in golden_levels:
                    print(f"    Position: {golden_levels[pct_key]:.2f}% in range")
        print(f"Current Position: {golden_levels.get('current_position_pct', 0):.2f}% in range")
        print(f"Range: {golden_levels.get('min_price', 0):.8f} (0%) to {golden_levels.get('max_price', 0):.8f} (100%)")
