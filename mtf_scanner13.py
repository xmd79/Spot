...
def _rich_tf_dip_check(pair, interval, tf_label, limit=600):
    ...
    try:
        ...
        _sc = 0.0
        if result['channel_pass']:         _sc += 30.0
        if result['argmin_gt_argmax']:     _sc += 20.0
        _sc += min(20.0, result['ht_sine_dip_score'] * 0.20)
        if result['fft_at_trough']:        _sc += 15.0
        if _bp > 50.0:                     _sc += 10.0
        if result['vol_absorption'] > 4.0: _sc += 5.0
        result['composite_score'] = round(_sc, 1)

        # ── 8. Pass decision ──────────────────────────────────────────
        #   MANDATORY : channel_pass
        ...
        result['passed']    = result['channel_pass'] and result['n_confirm'] >= 2
...
# ─────────────────────────────────────────────
#
#  ★ SUGGESTED SELECTION PATH (replaces CMO-only ranking):
#
#   MTF_SPIKE_SCORE = (
#     30 %  CMO depth         — deeper oversold = more compressed = bigger bounce
#     25 %  HT_SINE consensus — avg cycle trough score across ALL filter TFs + 1m
#     20 %  TF argmin agree   — fraction of TFs where deepest wick > highest wick
#     15 %  volume/absorption — avg bullish volume ratio + smart-money absorption
#     10 %  φ·e·π geometry    — structural quality of the dip (curvature etc.)
#   ) × normalisation → 0..100
#
#   Bonuses : +8 LeadSine crossed above Sine on any STF  (Ehlers trigger)
#              +5 sine < −0.9 (extreme trough) on ≥2 TFs
#   Penalty : −15 fake dip detected (negative curvature)
#
#  This single number captures TIMING (HT_SINE), DIRECTION (argmin),
#  STRUCTURE (φ·e·π), FUEL (volume), and DEPTH (CMO) simultaneously.
# ─────────────────────────────────────────────
...
def _compute_mtf_spike_score(pair, tf_cache, sel_detail):
    ...
    _cmo = _d.get('raw_cmo')
    if _cmo is not None:
        _score += min(30.0, max(0.0, -_cmo * 0.30))

    # ── 25 pts: HT_SINE dip consensus across all TFs ──────────
    _ht_w  = {'2h': 0.50, '30m': 0.65, '15m': 0.75, '5m': 0.85, '3m': 0.90, '1m': 1.00}
    _wsum  = 0.0; _wval = 0.0
    for _tk, _rd in _cache.items():
        _w    = _ht_w.get(_tk, 0.60)
        _wval += _rd.get('ht_sine_dip_score', 0.0) * _w
        _wsum += _w
    # 1m from sel_detail
    ...
    if _wsum > 0:
        _score += min(25.0, (_wval / _wsum) * 0.25)

    # ── 20 pts: TF argmin>argmax agreement ratio ───────────────
    _n_am  = sum(1 for _rd in _cache.values() if _rd.get('argmin_gt_argmax'))
    _n_am += (1 if _d.get('cond_ext') else 0)   # 1m argmin > argmax
    _tot   = len(_cache) + 1
    _score += (_n_am / max(_tot, 1)) * 20.0

    # ── 15 pts: volume / absorption average ───────────────────
    _vol_sc = []
    for _rd in _cache.values():
        ...
        _vs  = (min(1.0, max(0.0, (_bp - 45.0) / 10.0)) * 0.60
               + min(1.0, _ab / 8.0) * 0.40) * 15.0
        _vol_sc.append(max(0.0, _vs))
    # 1m real order flow
    _dr  = _d.get('delta_ratio',      0.0)
    _ab1 = _d.get('absorption_score', 0.0)
    _vol_sc.append(max(0.0,
        (min(1.0, max(0.0, _dr)) * 0.50 + min(1.0, _ab1 / 8.0) * 0.50) * 15.0))
    if _vol_sc:
        _score += float(np.mean(_vol_sc))

    # ── 10 pts: φ·e·π geometry score ─────────────────────────
    _score += _d.get('geometry_score', 0.0) * 0.10
...
def print_rich_filter_table(pairs, label_map, tf_cache, tf_key, stage_label):
    ...
    Shows: channel pass, argmin>argmax, HT_SINE dip score, FFT trough,
           volume bull%, CMO, composite score, bars-to-top, entry signals.
    """
    if not pairs:
    ...
    _hdr = (f'  │  {"#":>3}  {"Ticker":<10}  {"Price":>12}  '
            f'{"Chan":>4}  {"AM>AX":>5}  '
            f'{"SineDip":>7}  {"FFTTrg":>6}  {"Bull%":>5}  {"Absorb":>6}  '
    ...
    for _i, _p in enumerate(_ranked, 1):
        ...
        _err  = _rd.get('error')
        if _err:                      _sigs.append(f'[!{_err[:15]}]')
        ...
        _conf_s = f'({_nc}/4 sigs)'
        print(f'  │  {_i:>3}  {_lbl:<10}  {_pr_s:>12}  '
              f'{_cp:>4}  {_am:>5}  '
              f'{_ds:>7.0f}  {_ft:>6}  {_bp:>5.1f}  {_ab:>6.2f}  '
...
def print_mtf_signals_summary(fp4, label_map, tf_cache, sel_detail, spike_scores):
    ...
    Called after 1m gate (sel_detail populated) before final winner pick.
    """
    if not fp4:
        return
    ...
    print(f'  {"═" * _WL}')
    print(f'  ★ NEW SELECTION PATH (replaces CMO-only ranking):')
    print(f'    MTF_SPIKE = CMO(30%) + HT_SINE_avg(25%) + ArgMin_agree(20%)')
    print(f'               + Vol_avg(15%) + Geo(10%)')
    print(f'    Bonus: +8 LeadSine cross  |  +5 sine<-0.9 on ≥2 TFs')
    print(f'    Penalty: -15 fake dip (curvature↓)')
    print()

    # header row
    _tf_hdr = '  '.join(f'[{_t}]Sine AM Ft' for _t in _TFS)
    print(f'  {"Ticker":<10}  {"MTF_Spk":>7}  {_tf_hdr}  CMO    Geo')
    print('  ' + '─' * (_WL + 35))

    for _p in sorted(fp4, key=lambda _x: spike_scores.get(_x, 0.0), reverse=True):
        _lbl = label_map.get(_p, _p.replace('USDC', ''))
        ...
        for _tf in _TFS:
            ...
            _sn_s = f'{_sn:+.2f}' if _sn is not None else '  — '
            _parts.append(f'{_sn_s} {_am} {_ftt}')
        _row = '  '.join(_parts)
        _mark = '★' if _p == sorted(fp4, key=lambda _x: spike_scores.get(_x, 0.0), reverse=True)[0] else ' '
        print(f'  {_mark}{_lbl:<9}  {_msc:>7.1f}  {_row}  {_cmo_s:>5}  {_geo:>5.0f}')
