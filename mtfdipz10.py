...
def engineer_ml_features(c: np.ndarray, h: np.ndarray, l: np.ndarray, v: np.ndarray, o: np.ndarray) -> Tuple[np.ndarray, List[str]]:
    ...
    for p in [20]:
        ...
        bb_width = np.where(mid > 0, (upper - lower) / mid, 0); bb_pos = np.where((upper - lower) > 0, (c - lower) / (upper - lower), 0.5)
    ...
    for p in [10, 20, 50]:
        v_sma = ta.SMA(v, timeperiod=p); features[f'vol_ratio_{p}'] = np.where(v_sma > 0, v / v_sma, 1.0)
    vol_sma_short = ta.SMA(v, timeperiod=5); vol_sma_long = ta.SMA(v, timeperiod=20)
    features['vol_trend'] = np.where(vol_sma_long > 0, vol_sma_short / vol_sma_long, 1.0)
    ...
    for p in [20, 50]:
        ...
        rng = rolling_high - rolling_low; features[f'range_pos_{p}'] = np.where(rng > 0, (c - rolling_low) / rng, 0.5); features[f'range_size_{p}'] = np.where(c > 0, rng / c * 100, 0)
    for p in [5, 10, 20]:
