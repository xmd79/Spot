...
TIMEFRAMES = ['1m', '3m', '5m']  # Added 3m and 5m timeframes
...
# Updated configurable conditions - Now includes new threshold and volume conditions
CONFIG = {
    "conditions": {
        "rsi_oversold_most_recent": True,
        "rsi_oversold_most_recent_15s": True,  # RSI Oversold Most Recent (15s)
        "rsi_oversold_most_recent_3m": True,  # RSI Oversold Most Recent (3m)
        ...
        "argmin_more_recent_3m": True,  # Argmin more recent than argmax for 3m
        "argmin_more_recent_5m": True,  # Argmin more recent than argmax for 5m
        # Original SMA200 conditions (kept)
        ...
        # New conditions replacing SMA200 close conditions
        "close_below_middle_threshold_1m": True,  # Current close below middle threshold (1m)
        "close_below_middle_threshold_15s": True,  # Current close below middle threshold (15s)
        "close_below_middle_threshold_3m": True,  # Current close below middle threshold (3m)
        "close_below_middle_threshold_5m": True,  # Current close below middle threshold (5m)
        "volume_bullish_dominance_1m": True,  # Volume bullish > bearish (1m)
        "volume_bullish_dominance_15s": True,  # Volume bullish > bearish (15s)
        "volume_bullish_dominance_3m": True,  # Volume bullish > bearish (3m)
        "volume_bullish_dominance_5m": True,  # Volume bullish > bearish (5m)
    },
    "min_conditions_met": 24  # Updated to reflect all new conditions
}
...
def analyze_pythagorean_harmonics(client, symbol, timeframe='1m', lookback=500):
    ...
    try:
        ...
        if timeframe == '1m':
            ...
            klines_1m = safe_api_call(client.get_klines, symbol=symbol, interval='1m', limit=min_1m_candles)
...
def create_15sec_timeframe(df_1m):
    ...
    df_1m_clean = clean_ohlc_data(df_1m.copy())
...
def analyze_volume_dominance(client, symbol, timeframe='1m', lookback=100):
    ...
    try:
        ...
        if timeframe == '1m':
            ...
            if not klines_1m or len(klines_1m) < 50:
                return {"error": "Insufficient data for 15s volume analysis"}
...
def perform_single_iteration_analysis(client):
    ...
    print("="*80)
    print(f"ENHANCED BTCUSDC TRADING BOT - ALL SPECIFIED TRIGGERS")
    ...
    # Step 3: Analyze thresholds for 3-minute timeframe
    print("\n3. Analyzing thresholds for 3-minute timeframe...")
    thresholds_3m = analyze_thresholds(client, SYMBOL, timeframe='3m', lookback=500)
    
    if 'error' not in thresholds_3m:
        print(f"\n--- Threshold Analysis: 3-minute Timeframe ---")
        print(f"Min Threshold: {thresholds_3m['min_threshold']:.25f}")
        print(f"Max Threshold: {thresholds_3m['max_threshold']:.25f}")
        print(f"Middle Threshold: {thresholds_3m['middle_threshold']:.25f}")
        print(f"Current Close: {thresholds_3m['current_close']:.25f}")
        print(f"Close Below Middle: {thresholds_3m['close_below_middle']}")
        print(f"Argmin More Recent: {thresholds_3m['argmin_more_recent']}")
    else:
        print(f"Error analyzing 3m thresholds: {thresholds_3m['error']}")
    
    # Step 4: Analyze thresholds for 5-minute timeframe
    print("\n4. Analyzing thresholds for 5-minute timeframe...")
    thresholds_5m = analyze_thresholds(client, SYMBOL, timeframe='5m', lookback=500)
    
    if 'error' not in thresholds_5m:
        print(f"\n--- Threshold Analysis: 5-minute Timeframe ---")
        print(f"Min Threshold: {thresholds_5m['min_threshold']:.25f}")
        print(f"Max Threshold: {thresholds_5m['max_threshold']:.25f}")
        print(f"Middle Threshold: {thresholds_5m['middle_threshold']:.25f}")
        print(f"Current Close: {thresholds_5m['current_close']:.25f}")
        print(f"Close Below Middle: {thresholds_5m['close_below_middle']}")
        print(f"Argmin More Recent: {thresholds_5m['argmin_more_recent']}")
    else:
        print(f"Error analyzing 5m thresholds: {thresholds_5m['error']}")
    ...
    # Step 7: Analyze all specified conditions
    print("\n7. Analyzing all specified trading conditions...")
    
    conditions_met = 0
    total_conditions = 24  # Total number of conditions (updated to 24)
    ...
    # Condition 3: RSI Oversold Most Recent (3m)
    print("\n--- Condition 3: RSI Oversold Most Recent (3m) ---")
    rsi_3m_oversold, rsi_3m_overbought, rsi_3m_value, rsi_3m_details = analyze_rsi_condition(client, SYMBOL, 500, '3m')
    rsi_3m_met = rsi_3m_oversold and not rsi_3m_overbought
    condition_results['RSI Oversold Most Recent (3m)'] = rsi_3m_met
    
    # Print details for this condition
    print(f"\nCurrent RSI: {rsi_3m_value:.25f}")
    print(f"Oversold Most Recent: {rsi_3m_oversold}")
    print(f"Overbought Most Recent: {rsi_3m_overbought}")
    print(f"Condition Met: {rsi_3m_met}")
    
    if rsi_3m_met:
        conditions_met += 1
        print("\nTRUE - RSI Oversold Most Recent (3m) condition MET")
    else:
        print("\nFALSE - RSI Oversold Most Recent (3m) condition NOT met")
    
    # Condition 4: RSI Oversold Most Recent (5m)
    print("\n--- Condition 4: RSI Oversold Most Recent (5m) ---")
    ...
    # Condition 6: FFT Forecast Up (15s)
    print("\n--- Condition 6: FFT Forecast Up (15s) ---")
    ...
    # Condition 7: FFT Forecast Up (3m)
    print("\n--- Condition 7: FFT Forecast Up (3m) ---")
    ...
    if 'error' not in fft_3m:
        ...
        # Print details for this condition
        print(f"\nCurrent Price: {fft_3m['current_price']:.25f}")
        print(f"Forecast Target: {fft_3m['forecast_target']:.25f}")
        print(f"Forecast Difference: {fft_3m['forecast_diff_pct']:.25f}%")
    ...
    # Condition 8: FFT Forecast Up (5m)
    print("\n--- Condition 8: FFT Forecast Up (5m) ---")
    ...
    if 'error' not in fft_5m:
        ...
        # Print details for this condition
        print(f"\nCurrent Price: {fft_5m['current_price']:.25f}")
        print(f"Forecast Target: {fft_5m['forecast_target']:.25f}")
        print(f"Forecast Difference: {fft_5m['forecast_diff_pct']:.25f}%")
    ...
    # Condition 9: Pythagorean Harmonics Up (1m)
    print("\n--- Condition 9: Pythagorean Harmonics Up (1m) ---")
    ...
    # Condition 10: Pythagorean Harmonics Up (15s)
    print("\n--- Condition 10: Pythagorean Harmonics Up (15s) ---")
    ...
    # Condition 13: Argmin More Recent than Argmax (15s)
    print("\n--- Condition 13: Argmin More Recent than Argmax (15s) ---")
    ...
    # Condition 14: Argmin More Recent than Argmax (1m)
    print("\n--- Condition 14: Argmin More Recent than Argmax (1m) ---")
    ...
    # Condition 15: Argmin More Recent than Argmax (3m)
    print("\n--- Condition 15: Argmin More Recent than Argmax (3m) ---")
    argmin_more_recent_3m_met = False
    if 'error' not in thresholds_3m:
        argmin_more_recent_3m_met = thresholds_3m['argmin_more_recent']
        condition_results['Argmin More Recent (3m)'] = argmin_more_recent_3m_met
        
        # Print details for this condition
        print(f"\nMin Close Index: {thresholds_3m['min_close_idx']}")
        print(f"Max Close Index: {thresholds_3m['max_close_idx']}")
        print(f"Argmin More Recent: {argmin_more_recent_3m_met}")
        print(f"Condition Met: {argmin_more_recent_3m_met}")
        
        if argmin_more_recent_3m_met:
            conditions_met += 1
            print("\nTRUE - Argmin More Recent than Argmax (3m) condition MET")
        else:
            print("\nFALSE - Argmin More Recent than Argmax (3m) condition NOT met")
    else:
        condition_results['Argmin More Recent (3m)'] = False
        print(f"\nError analyzing thresholds: {thresholds_3m['error']}")
        print("\nFALSE - Argmin More Recent than Argmax (3m) condition NOT met")
    
    # Condition 16: Argmin More Recent than Argmax (5m)
    print("\n--- Condition 16: Argmin More Recent than Argmax (5m) ---")
    argmin_more_recent_5m_met = False
    if 'error' not in thresholds_5m:
        argmin_more_recent_5m_met = thresholds_5m['argmin_more_recent']
        condition_results['Argmin More Recent (5m)'] = argmin_more_recent_5m_met
        
        # Print details for this condition
        print(f"\nMin Close Index: {thresholds_5m['min_close_idx']}")
        print(f"Max Close Index: {thresholds_5m['max_close_idx']}")
        print(f"Argmin More Recent: {argmin_more_recent_5m_met}")
        print(f"Condition Met: {argmin_more_recent_5m_met}")
        
        if argmin_more_recent_5m_met:
            conditions_met += 1
            print("\nTRUE - Argmin More Recent than Argmax (5m) condition MET")
        else:
            print("\nFALSE - Argmin More Recent than Argmax (5m) condition NOT met")
    else:
        condition_results['Argmin More Recent (5m)'] = False
        print(f"\nError analyzing thresholds: {thresholds_5m['error']}")
        print("\nFALSE - Argmin More Recent than Argmax (5m) condition NOT met")
    
    # Condition 17: Lowest SMA200 More Recent than Highest SMA200 (1m)
    print("\n--- Condition 17: Lowest SMA200 More Recent than Highest SMA200 (1m) ---")
    ...
    # Condition 18: Close Below Middle Threshold (1m)
    print("\n--- Condition 18: Close Below Middle Threshold (1m) ---")
    close_below_middle_1m_met = False
    if 'error' not in thresholds_1m:
        close_below_middle_1m_met = thresholds_1m['close_below_middle']
        condition_results['Close Below Middle Threshold (1m)'] = close_below_middle_1m_met
        
        # Print details for this condition
        print(f"\nMin Threshold: {thresholds_1m['min_threshold']:.25f}")
        print(f"Max Threshold: {thresholds_1m['max_threshold']:.25f}")
        print(f"Middle Threshold: {thresholds_1m['middle_threshold']:.25f}")
        print(f"Current Close: {thresholds_1m['current_close']:.25f}")
        print(f"Close Below Middle: {close_below_middle_1m_met}")
        print(f"Condition Met: {close_below_middle_1m_met}")
        
        if close_below_middle_1m_met:
            conditions_met += 1
            print("\nTRUE - Close Below Middle Threshold (1m) condition MET")
        else:
            print("\nFALSE - Close Below Middle Threshold (1m) condition NOT met")
    else:
        condition_results['Close Below Middle Threshold (1m)'] = False
        print(f"\nError analyzing thresholds: {thresholds_1m['error']}")
        print("\nFALSE - Close Below Middle Threshold (1m) condition NOT met")
    
    # Condition 19: Close Below Middle Threshold (3m)
    print("\n--- Condition 19: Close Below Middle Threshold (3m) ---")
    close_below_middle_3m_met = False
    if 'error' not in thresholds_3m:
        close_below_middle_3m_met = thresholds_3m['close_below_middle']
        condition_results['Close Below Middle Threshold (3m)'] = close_below_middle_3m_met
        
        # Print details for this condition
        print(f"\nMin Threshold: {thresholds_3m['min_threshold']:.25f}")
        print(f"Max Threshold: {thresholds_3m['max_threshold']:.25f}")
        print(f"Middle Threshold: {thresholds_3m['middle_threshold']:.25f}")
        print(f"Current Close: {thresholds_3m['current_close']:.25f}")
        print(f"Close Below Middle: {close_below_middle_3m_met}")
        print(f"Condition Met: {close_below_middle_3m_met}")
        
        if close_below_middle_3m_met:
            conditions_met += 1
            print("\nTRUE - Close Below Middle Threshold (3m) condition MET")
        else:
            print("\nFALSE - Close Below Middle Threshold (3m) condition NOT met")
    else:
        condition_results['Close Below Middle Threshold (3m)'] = False
        print(f"\nError analyzing thresholds: {thresholds_3m['error']}")
        print("\nFALSE - Close Below Middle Threshold (3m) condition NOT met")
    
    # Condition 20: Close Below Middle Threshold (5m)
    print("\n--- Condition 20: Close Below Middle Threshold (5m) ---")
    close_below_middle_5m_met = False
    if 'error' not in thresholds_5m:
        close_below_middle_5m_met = thresholds_5m['close_below_middle']
        condition_results['Close Below Middle Threshold (5m)'] = close_below_middle_5m_met
        
        # Print details for this condition
        print(f"\nMin Threshold: {thresholds_5m['min_threshold']:.25f}")
        print(f"Max Threshold: {thresholds_5m['max_threshold']:.25f}")
        print(f"Middle Threshold: {thresholds_5m['middle_threshold']:.25f}")
        print(f"Current Close: {thresholds_5m['current_close']:.25f}")
        print(f"Close Below Middle: {close_below_middle_5m_met}")
        print(f"Condition Met: {close_below_middle_5m_met}")
        
        if close_below_middle_5m_met:
            conditions_met += 1
            print("\nTRUE - Close Below Middle Threshold (5m) condition MET")
        else:
            print("\nFALSE - Close Below Middle Threshold (5m) condition NOT met")
    else:
        condition_results['Close Below Middle Threshold (5m)'] = False
        print(f"\nError analyzing thresholds: {thresholds_5m['error']}")
        print("\nFALSE - Close Below Middle Threshold (5m) condition NOT met")
    
    # Condition 21: Lowest SMA200 More Recent than Highest SMA200 (15s)
    print("\n--- Condition 21: Lowest SMA200 More Recent than Highest SMA200 (15s) ---")
    ...
    # Condition 22: Close Below Middle Threshold (15s)
    print("\n--- Condition 22: Close Below Middle Threshold (15s) ---")
    close_below_middle_15s_met = False
    if 'error' not in thresholds_15s:
        close_below_middle_15s_met = thresholds_15s['close_below_middle']
        condition_results['Close Below Middle Threshold (15s)'] = close_below_middle_15s_met
        
        # Print details for this condition
        print(f"\nMin Threshold: {thresholds_15s['min_threshold']:.25f}")
        print(f"Max Threshold: {thresholds_15s['max_threshold']:.25f}")
        print(f"Middle Threshold: {thresholds_15s['middle_threshold']:.25f}")
        print(f"Current Close: {thresholds_15s['current_close']:.25f}")
        print(f"Close Below Middle: {close_below_middle_15s_met}")
        print(f"Condition Met: {close_below_middle_15s_met}")
        
        if close_below_middle_15s_met:
            conditions_met += 1
            print("\nTRUE - Close Below Middle Threshold (15s) condition MET")
        else:
            print("\nFALSE - Close Below Middle Threshold (15s) condition NOT met")
    else:
        condition_results['Close Below Middle Threshold (15s)'] = False
        print(f"\nError analyzing thresholds: {thresholds_15s['error']}")
        print("\nFALSE - Close Below Middle Threshold (15s) condition NOT met")
    
    # Condition 23: Volume Bullish Dominance (1m)
    print("\n--- Condition 23: Volume Bullish Dominance (1m) ---")
    volume_1m = analyze_volume_dominance(client, SYMBOL, timeframe='1m', lookback=100)
    volume_bullish_dominance_1m_met = False
    if 'error' not in volume_1m:
        volume_bullish_dominance_1m_met = volume_1m['bullish_dominance']
        condition_results['Volume Bullish Dominance (1m)'] = volume_bullish_dominance_1m_met
        
        # Print details for this condition
        print(f"\nBullish Volume %: {volume_1m['bullish_pct']:.25f}%")
        print(f"Bearish Volume %: {volume_1m['bearish_pct']:.25f}%")
        print(f"Dominance: {volume_1m['dominance']}")
        print(f"Bullish Dominance: {volume_bullish_dominance_1m_met}")
        print(f"Condition Met: {volume_bullish_dominance_1m_met}")
        
        if volume_bullish_dominance_1m_met:
            conditions_met += 1
    ...
    # Condition 24: Volume Bullish Dominance (15s)
    print("\n--- Condition 24: Volume Bullish Dominance (15s) ---")
    volume_15s = analyze_volume_dominance(client, SYMBOL, timeframe='15s', lookback=100)
    volume_bullish_dominance_15s_met = False
    if 'error' not in volume_15s:
        volume_bullish_dominance_15s_met = volume_15s['bullish_dominance']
        condition_results['Volume Bullish Dominance (15s)'] = volume_bullish_dominance_15s_met
        
        # Print details for this condition
        print(f"\nBullish Volume %: {volume_15s['bullish_pct']:.25f}%")
        print(f"Bearish Volume %: {volume_15s['bearish_pct']:.25f}%")
        print(f"Dominance: {volume_15s['dominance']}")
        print(f"Bullish Dominance: {volume_bullish_dominance_15s_met}")
        print(f"Condition Met: {volume_bullish_dominance_15s_met}")
        
        if volume_bullish_dominance_15s_met:
            conditions_met += 1
            print("\nTRUE - Volume Bullish Dominance (15s) condition MET")
        else:
            print("\nFALSE - Volume Bullish Dominance (15s) condition NOT met")
    else:
        condition_results['Volume Bullish Dominance (15s)'] = False
        print(f"\nError analyzing volume: {volume_15s['error']}")
        print("\nFALSE - Volume Bullish Dominance (15s) condition NOT met")
    
    # Condition 25: Volume Bullish Dominance (3m)
    print("\n--- Condition 25: Volume Bullish Dominance (3m) ---")
    volume_3m = analyze_volume_dominance(client, SYMBOL, timeframe='3m', lookback=100)
    volume_bullish_dominance_3m_met = False
    if 'error' not in volume_3m:
        volume_bullish_dominance_3m_met = volume_3m['bullish_dominance']
        condition_results['Volume Bullish Dominance (3m)'] = volume_bullish_dominance_3m_met
        
        # Print details for this condition
        print(f"\nBullish Volume %: {volume_3m['bullish_pct']:.25f}%")
        print(f"Bearish Volume %: {volume_3m['bearish_pct']:.25f}%")
        print(f"Dominance: {volume_3m['dominance']}")
        print(f"Bullish Dominance: {volume_bullish_dominance_3m_met}")
        print(f"Condition Met: {volume_bullish_dominance_3m_met}")
        
        if volume_bullish_dominance_3m_met:
            conditions_met += 1
            print("\nTRUE - Volume Bullish Dominance (3m) condition MET")
        else:
            print("\nFALSE - Volume Bullish Dominance (3m) condition NOT met")
    else:
        condition_results['Volume Bullish Dominance (3m)'] = False
        print(f"\nError analyzing volume: {volume_3m['error']}")
        print("\nFALSE - Volume Bullish Dominance (3m) condition NOT met")
    
    # Condition 26: Volume Bullish Dominance (5m)
    print("\n--- Condition 26: Volume Bullish Dominance (5m) ---")
    volume_5m = analyze_volume_dominance(client, SYMBOL, timeframe='5m', lookback=100)
    volume_bullish_dominance_5m_met = False
    if 'error' not in volume_5m:
        volume_bullish_dominance_5m_met = volume_5m['bullish_dominance']
        condition_results['Volume Bullish Dominance (5m)'] = volume_bullish_dominance_5m_met
        
        # Print details for this condition
        print(f"\nBullish Volume %: {volume_5m['bullish_pct']:.25f}%")
        print(f"Bearish Volume %: {volume_5m['bearish_pct']:.25f}%")
        print(f"Dominance: {volume_5m['dominance']}")
        print(f"Bullish Dominance: {volume_bullish_dominance_5m_met}")
        print(f"Condition Met: {volume_bullish_dominance_5m_met}")
        
        if volume_bullish_dominance_5m_met:
            conditions_met += 1
            print("\nTRUE - Volume Bullish Dominance (5m) condition MET")
        else:
            print("\nFALSE - Volume Bullish Dominance (5m) condition NOT met")
    else:
        condition_results['Volume Bullish Dominance (5m)'] = False
        print(f"\nError analyzing volume: {volume_5m['error']}")
        print("\nFALSE - Volume Bullish Dominance (5m) condition NOT met")
...
def main():
    ...
    print("=== ENHANCED BTCUSDC TRADING BOT - ALL SPECIFIED TRIGGERS ===")
    ...
    print("3. Analyze thresholds for all timeframes (15s, 1m, 3m, 5m)")
    print("4. Analyze Pythagorean Harmonics for both 1m and 15s timeframes")
    print("5. Analyze all specified trading conditions:")
    print("   - RSI Oversold Most Recent (1m, 15s, 3m, 5m)")
    print("   - FFT Forecast Up (1m, 15s, 3m, 5m)")
    print("   - Pythagorean Harmonics Up (1m, 15s)")
    print("   - Momentum > 0 (1m, 15s)")
    print("   - Argmin More Recent than Argmax (15s, 1m, 3m, 5m)")
    print("   - Lowest SMA200 More Recent than Highest SMA200 (1m, 15s)")
    print("   - Close Below Middle Threshold (1m, 15s, 3m, 5m)")
    print("   - Volume Bullish Dominance (1m, 15s, 3m, 5m)")
    print("6. Execute trade if ALL conditions are met")
    print("7. Use 100% of USDC balance for entry")
    print("8. Monitor for profit target every 5 seconds")
    print("9. Use 100% of BTC balance for exit")
    print("10. Clean up for next iteration")
    print("\nEnhanced Features:")
    print("- All specified triggers with multiple timeframes")
