...
PROFIT_TARGET_PERCENT = 0.65  # 0.65% profit target
...
# Updated configurable conditions - Now with distance percentage conditions
CONFIG = {
    "conditions": {
        # Original conditions (with some removed)
        ...
        # New Distance Percentage conditions
        "dist_to_min_1m": True,  # Distance to min threshold (1m) ≤ 15%
        "dist_to_min_3m": True,  # Distance to min threshold (3m) ≤ 10%
        "dist_to_min_5m": True,  # Distance to min threshold (5m) ≤ 5%
        
        # New SMA Crossover condition
        "sma12_below_sma27_15s": True, # Close < SMA12 < SMA27 (15s)
    },
    "min_conditions_met": 14  # ALL 14 conditions must be met to trigger a trade
}
...
def check_exit_condition(initial_investment, asset_balance, entry_price):
    ...
    target_value = initial_investment * Decimal('1.0065')  # 0.65% profit target
...
def analyze_sma_crossover_15s(candles_1m):
    """
    Analyze if current close price is below SMA12 and SMA12 is below SMA27 for 15-second timeframe.
    """
    try:
        if not candles_1m:
            return {"error": "No 1m data provided"}
        ...
        # Calculate SMA12 and SMA27
        if len(close_prices) < 27:
            return {"error": "Not enough data for SMA27 calculation"}
        
        sma12 = talib.SMA(close_prices, timeperiod=12)
        sma27 = talib.SMA(close_prices, timeperiod=27)
        
        current_sma12 = sma12[-1]
        current_sma27 = sma27[-1]
        current_close = close_prices[-1]
        
        # Check if current close < SMA12 < SMA27
        condition_met = current_close < current_sma12 and current_sma12 < current_sma27
        
        return {
            "timeframe": "15s",
            "current_close": current_close,
            "current_sma12": current_sma12,
            "current_sma27": current_sma27,
            "condition_met": condition_met,
            "close_sma12_diff": current_close - current_sma12,
            "sma12_sma27_diff": current_sma12 - current_sma27
        }
        
    except Exception as e:
        print(f"Error analyzing SMA crossover condition (15s): {e}")
        return {"error": str(e)}
...
try:
    while True:
        ...
        conditions_status = {
            ...
            # New Distance Percentage conditions
            "dist_to_min_1m": False,  # Distance to min threshold (1m) ≤ 15%
            "dist_to_min_3m": False,  # Distance to min threshold (3m) ≤ 10%
            "dist_to_min_5m": False,  # Distance to min threshold (5m) ≤ 5%
            
            # New SMA Crossover condition
            "sma12_below_sma27_15s": False, # Close < SMA12 < SMA27 (15s)
        }
        ...
        for timeframe in ['1m', '3m', '5m']:
            if timeframe in candle_map and candle_map[timeframe]:
                ...
                # Calculate distance to min threshold and set condition
                if min_threshold_tf is not None and max_threshold_tf is not None and (max_threshold_tf - min_threshold_tf) > 0:
                    dist_to_min = ((current_close - min_threshold_tf) / (max_threshold_tf - min_threshold_tf)) * Decimal('100')
                    
                    # Set distance percentage conditions based on timeframe
                    if timeframe == '1m':
                        conditions_status["dist_to_min_1m"] = dist_to_min <= Decimal('15')
                    elif timeframe == '3m':
                        conditions_status["dist_to_min_3m"] = dist_to_min <= Decimal('10')
                    elif timeframe == '5m':
                        conditions_status["dist_to_min_5m"] = dist_to_min <= Decimal('5')
                    
                    print(f"Distance to Min Threshold: {dist_to_min:.2f}%")
                else:
                    print("Could not calculate distance to thresholds (division by zero or missing data)")
        ...
        # Condition 5: FFT Forecast Price (15s)
        print("\n--- Condition 5: FFT Forecast Price (15s) ---")
        ...
        # Condition 6: Close Below SMA200 (15s)
        print("\n--- Condition 6: Close Below SMA200 (15s) ---")
        ...
        # Condition 7: Momentum Positive (1m)
        print("\n--- Condition 7: Momentum Positive (1m) ---")
        ...
        # Condition 8: FFT Forecast Up (1m)
        print("\n--- Condition 8: FFT Forecast Up (1m) ---")
        ...
        # Condition 9: FFT Forecast Up (3m)
        print("\n--- Condition 9: FFT Forecast Up (3m) ---")
        ...
        # Condition 10: FFT Forecast Up (5m)
        print("\n--- Condition 10: FFT Forecast Up (5m) ---")
        ...
        # Condition 11: Distance to Min Threshold (1m) ≤ 15%
        print("\n--- Condition 11: Distance to Min Threshold (1m) ≤ 15% ---")
        if '1m' in candle_map and candle_map['1m']:
            closes = [candle['close'] for candle in candle_map['1m']]
            min_threshold_tf, max_threshold_tf, _, _, _, _, _ = calculate_thresholds(closes, period=14, minimum_percentage=2, maximum_percentage=2)
            if min_threshold_tf is not None and max_threshold_tf is not None and (max_threshold_tf - min_threshold_tf) > 0:
                ...
                dist_to_min = ((current_close - min_threshold_tf) / (max_threshold_tf - min_threshold_tf)) * Decimal('100')
                print(f"Current Close: {current_close:.25f}")
                print(f"Min Threshold: {min_threshold_tf:.25f}")
                print(f"Max Threshold: {max_threshold_tf:.25f}")
                print(f"Distance to Min Threshold: {dist_to_min:.2f}%")
                print(f"Distance to Min ≤ 15%: {dist_to_min <= Decimal('15')}")
                print(f"Condition Met: {conditions_status['dist_to_min_1m']}")
            else:
                print("Could not calculate thresholds for 1m timeframe.")
                print(f"Condition Met: {conditions_status['dist_to_min_1m']}")
        else:
            print("No 1m data available for distance calculation.")
            print(f"Condition Met: {conditions_status['dist_to_min_1m']}")
        
        # Condition 12: Distance to Min Threshold (3m) ≤ 10%
        print("\n--- Condition 12: Distance to Min Threshold (3m) ≤ 10% ---")
        if '3m' in candle_map and candle_map['3m']:
            closes = [candle['close'] for candle in candle_map['3m']]
            min_threshold_tf, max_threshold_tf, _, _, _, _, _ = calculate_thresholds(closes, period=14, minimum_percentage=2, maximum_percentage=2)
            if min_threshold_tf is not None and max_threshold_tf is not None and (max_threshold_tf - min_threshold_tf) > 0:
                ...
                print(f"Max Threshold: {max_threshold_tf:.25f}")
                print(f"Distance to Min Threshold: {dist_to_min:.2f}%")
                print(f"Distance to Min ≤ 10%: {dist_to_min <= Decimal('10')}")
                print(f"Condition Met: {conditions_status['dist_to_min_3m']}")
            else:
                print("Could not calculate thresholds for 3m timeframe.")
                print(f"Condition Met: {conditions_status['dist_to_min_3m']}")
        else:
            print("No 3m data available for distance calculation.")
            print(f"Condition Met: {conditions_status['dist_to_min_3m']}")
        
        # Condition 13: Distance to Min Threshold (5m) ≤ 5%
        print("\n--- Condition 13: Distance to Min Threshold (5m) ≤ 5% ---")
        if '5m' in candle_map and candle_map['5m']:
            closes = [candle['close'] for candle in candle_map['5m']]
            min_threshold_tf, max_threshold_tf, _, _, _, _, _ = calculate_thresholds(closes, period=14, minimum_percentage=2, maximum_percentage=2)
            if min_threshold_tf is not None and max_threshold_tf is not None and (max_threshold_tf - min_threshold_tf) > 0:
                ...
                print(f"Max Threshold: {max_threshold_tf:.25f}")
                print(f"Distance to Min Threshold: {dist_to_min:.2f}%")
                print(f"Distance to Min ≤ 5%: {dist_to_min <= Decimal('5')}")
                print(f"Condition Met: {conditions_status['dist_to_min_5m']}")
            else:
                print("Could not calculate thresholds for 5m timeframe.")
                print(f"Condition Met: {conditions_status['dist_to_min_5m']}")
        else:
            print("No 5m data available for distance calculation.")
            print(f"Condition Met: {conditions_status['dist_to_min_5m']}")
            
        # Condition 14: Close < SMA12 < SMA27 (15s)
        print("\n--- Condition 14: Close < SMA12 < SMA27 (15s) ---")
        sma_crossover_15s_result = analyze_sma_crossover_15s(candles_1m)
        if 'error' not in sma_crossover_15s_result:
            conditions_status["sma12_below_sma27_15s"] = sma_crossover_15s_result['condition_met']
            print(f"Current Close: {sma_crossover_15s_result['current_close']:.2f}")
            print(f"Current SMA12: {sma_crossover_15s_result['current_sma12']:.2f}")
            print(f"Current SMA27: {sma_crossover_15s_result['current_sma27']:.2f}")
            print(f"Close < SMA12: {sma_crossover_15s_result['current_close'] < sma_crossover_15s_result['current_sma12']}")
            print(f"SMA12 < SMA27: {sma_crossover_15s_result['current_sma12'] < sma_crossover_15s_result['current_sma27']}")
            print(f"Condition Met (Close < SMA12 < SMA27): {conditions_status['sma12_below_sma27_15s']}")
        else:
            print(f"Error analyzing SMA crossover condition (15s): {sma_crossover_15s_result['error']}")
            print(f"Condition Met: {conditions_status['sma12_below_sma27_15s']}")
        ...
        if position_open:
            ...
            if entry_price > Decimal('0') and target_price > entry_price:
                ...
                # Entry to Target percentage change (should be 0.65%)
            ...
            if check_exit_condition(initial_investment, asset_balance, entry_price):
                print("Target profit of 0.65% reached or exceeded. Initiating exit...")
