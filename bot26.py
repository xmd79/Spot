#!/usr/bin/env python3
"""
ENHANCED BTCUSDC TRADING BOT - MOMENTUM ANALYSIS
→ Uses momentum analysis for trade signals
→ Analyzes last 500 values to calculate current momentum
...
MOMENTUM_PERIOD = 12
MOMENTUM_LOOKBACK = 500  # Number of values to analyze for momentum
...
def analyze_momentum_condition(client, symbol, timeframe='1m', lookback=500):
    ...
    MOMENTUM ANALYSIS
    → Analyzes last 500 values to calculate current momentum
    → Signals for UP if momentum > 0
    → Provides momentum value and direction
    """
    try:
        klines = client.get_klines(symbol=symbol, interval=timeframe, limit=lookback)
        ...
        # Signal for UP if momentum > 0
        signal_for_up = current_momentum > 0
        
        # Calculate momentum percentage relative to price
        current_price = close.iloc[-1]
        momentum_pct = (current_momentum / current_price) * 100 if current_price > 0 else 0
        ...
        return signal_for_up, current_momentum, momentum_pct, details
...
def perform_single_iteration_analysis(client):
    ...
    print("="*80)
    print(f"BTCUSDC TRADING BOT - MOMENTUM ANALYSIS")
    ...
    # Condition 4: Momentum Analysis (replaces Nuclear Pump Engine and Volatility Rules)
    print("\n--- Condition 4: Momentum Analysis ---")
    momentum_met, current_momentum, momentum_pct, mom_details = analyze_momentum_condition(client, SYMBOL, '1m', 500)
    ...
    # Print details for this condition - with detailed momentum information
    print(f"\n--- Condition 4: Momentum Analysis ---")
    print(f"┌─ Momentum Analysis (Period: {MOMENTUM_PERIOD})")
    print(f"│  → Current Momentum: {mom_details['current_momentum']}")
    print(f"│  → Momentum Percentage: {mom_details['momentum_pct']:.2f}%")
    print(f"├─ Momentum Direction")
    print(f"│  → Momentum > 0: {'TRUE' if momentum_met else 'FALSE'}")
    print(f"└─ FINAL MOMENTUM SIGNAL: {'UP SIGNAL' if momentum_met else 'NO SIGNAL'}")

    if momentum_met:
        conditions_met += 1
        print("\nMOMENTUM CONFIRMED — MOMENTUM > 0 — UP SIGNAL DETECTED")
    else:
        print("\nNo momentum signal — waiting for clearer pattern")
...
def main():
    ...
    print("=== BTCUSDC TRADING BOT - MOMENTUM ANALYSIS ===")
    ...
    print("\nMomentum Analysis:")
    print("- Analyzes last 500 values to calculate current momentum")
    print("- Signals UP if momentum > 0")
    print("- Provides momentum value and direction")
    print("="*60)
