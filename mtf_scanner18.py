...
# ── Order-Flow Imbalance (OFI) spike engine ────────────────────────────────
#
#  Spike Probability Index:   S = (Vbuy − Vsell) / L × (1/t)
#    Vbuy, Vsell  = rolling aggressive buy / sell volume approximation
#    L            = liquidity proxy  (price_range / volume — thin = high)
#    t            = time compression factor  (volume density per bar)
#
#  Regime classification:
#    micro   → buy/sell ratio  2:1 –  4:1   (fast wick)
#    strong  → ratio           4:1 – 10:1   (clean breakout)
#    para    → ratio          10:1+          (parabolic / short-squeeze)
#
#  Pre-spike signature (fires BEFORE the spike):
#    vol in last _OFI_ACCEL_BARS ≥ _OFI_VOL_SURGE_MULT × rolling avg
#    AND vol is still accelerating (each bar > previous)
# ──────────────────────────────────────────────────────────────────────────
_OFI_WINDOW_SHORT   = 10     # fast imbalance lookback (bars)
_OFI_WINDOW_LONG    = 30     # slow imbalance lookback (bars)
_OFI_MICRO_THRESH   = 2.0    # buy:sell ratio for micro-spike
_OFI_STRONG_THRESH  = 4.0    # buy:sell ratio for strong spike
_OFI_PARA_THRESH    = 10.0   # buy:sell ratio for parabolic spike
_OFI_VOL_SURGE_MULT = 3.0    # vol surge multiplier for pre-spike flag
_OFI_ACCEL_BARS     = 3      # consecutive bars checked for acceleration
...
# ═══════════════════════════════════════════════════════════════════════════════
#  PHYSICS ENERGY ENGINE  — v18 MODULE
#  (True Momentum · Kinetic Energy · Curvature Reversal · Fast Spike Detector
#   MTF Resonance Cascade · Backup Selector · Energy Divergence Monitor)
# ═══════════════════════════════════════════════════════════════════════════════
...
try:
    ...
    from sklearn.pipeline     import Pipeline
    _PHYS_ML_AVAILABLE = True
except ImportError:
    _PHYS_ML_AVAILABLE = False

# ─────────────────────────────────────────────────────────────────────────────
#  PHYSICS ENGINE CONSTANTS
# ─────────────────────────────────────────────────────────────────────────────

_PHYS_MIN_BARS          = 30    # minimum bars for any physics calculation
_PHYS_VEL_SMOOTH        = 3     # EMA periods for velocity smoothing
_PHYS_ENERGY_WINDOW     = 20    # rolling window for E(t) statistics
_PHYS_CURV_WINDOW       = 5     # bars for d²E/dt² estimation
_PHYS_RESONANCE_THOLD   = 0.60  # resonance threshold for cascade flag
_PHYS_DIV_BARS          = 10    # lookback bars for price/energy divergence
_PHYS_FAST_BARS         = 5     # ultra-fast lookback for spike detection
_PHYS_BACKUP_SCORE_THR  = 35.0  # MTFSpike score below which backup is triggered
_PHYS_SPIKE_CONFIRM     = 3     # minimum conditions for spike_imminent flag

# Phase zone labels  (mapped to circuit quadrants)
_PHYS_PHASE_ZONES = {
    'trough':       (  0.0,  45.0),   # 0°–45°   = accelerate zone   (BUY)
    'rising_fast':  ( 45.0,  90.0),   # 45°–90°  = max velocity       (SPIKE)
    'mid_up':       ( 90.0, 135.0),   # 90°–135° = decelerate rising
    'peak':         (135.0, 180.0),   # 135°–180° = topping out       (SELL prep)
    'falling_fast': (180.0, 225.0),   # 180°–225° = reversal confirmed
    'mid_down':     (225.0, 270.0),   # 225°–270° = declining
    'bottom':       (270.0, 315.0),   # 270°–315° = exhaustion        (DIP entry)
    'recovery':     (315.0, 360.0),   # 315°–360° = pre-bounce coil   (BUY setup)
}

_PHYS_SPIKE_ZONES  = {'trough', 'recovery', 'rising_fast'}
_PHYS_REVERSAL_ZONES = {'peak', 'falling_fast'}


# ═════════════════════════════════════════════════════════════════════════════
#  MODULE 1 — TRUE PHYSICS MOMENTUM ENGINE
#  p(t) = m(t) × v(t)   |   E(t) = ½ m(t) v(t)²   |   d²E/dt²
# ═════════════════════════════════════════════════════════════════════════════

def _phys_velocity(close_arr, smooth=_PHYS_VEL_SMOOTH):
    """
    v(t) = dP/dt  ≈  log(P[t] / P[t-1])  (log return as continuous velocity)

    EMA-smoothed to remove noise. Returns array same length as close_arr.
    """
    ...
    # raw log-return velocity
    v[1:] = np.log(np.maximum(cl[1:], 1e-20) / np.maximum(cl[:-1], 1e-20))
...
def _phys_mass(vol_arr, window=_PHYS_ENERGY_WINDOW):
    """
    m(t) = normalised volume   (rolling window mean normalisation)

    Returns array in [0, ∞), median ≈ 1.0 during typical activity.
    Spikes in volume → m >> 1 → amplify energy.
    """
    ...
    for i in range(w, n):
        mu   = np.mean(vl[i - w: i]) + 1e-20
        m[i] = vl[i] / mu

    # EMA stabilise
    alpha = 2.0 / (3 + 1)
    for i in range(1, n):
        m[i] = alpha * m[i] + (1.0 - alpha) * m[i - 1]

    return np.maximum(m, 0.0)


def _phys_true_momentum(close_arr, vol_arr):
    """
    Physics-true momentum:  p(t) = m(t) × v(t)

    In standard TA:    M(t) = P(t) − P(t-n)   ← displacement, not momentum
    In physics:        p(t) = mass × velocity  ← volume-weighted velocity

    Returns dict:
        velocity    — v(t) array (log-return velocity, smoothed)
        mass        — m(t) array (normalised volume)
        momentum    — p(t) = m × v array
        momentum_now — scalar current value
        velocity_now — scalar
        mass_now    — scalar
        direction   — 'bull' | 'bear' | 'neutral'
        impulse_now — d(p)/dt at last bar (rate of momentum change)
    """
    cl = np.asarray(close_arr, dtype=np.float64)
    vl = np.asarray(vol_arr,   dtype=np.float64)
    n  = len(cl)
    ...
    if n < _PHYS_MIN_BARS:
        return {
            'velocity': np.zeros(n), 'mass': np.ones(n),
            'momentum': np.zeros(n), 'momentum_now': 0.0,
            'velocity_now': 0.0, 'mass_now': 1.0,
            'direction': 'neutral', 'impulse_now': 0.0,
    ...
    # impulse = dp/dt (momentum derivative at last bar)
    impulse = float(p[-1] - p[-2]) if n >= 2 else 0.0

    mom_now = float(p[-1])
    direction = 'bull' if mom_now > 0 else ('bear' if mom_now < 0 else 'neutral')

    return {
        'velocity'    : v,
        'mass'        : m,
        'momentum'    : p,
        'momentum_now': round(mom_now,       8),
        'velocity_now': round(float(v[-1]),  8),
        'mass_now'    : round(float(m[-1]),  4),
        'direction'   : direction,
        'impulse_now' : round(impulse,        8),
    }


def _phys_kinetic_energy(close_arr, vol_arr, window=_PHYS_ENERGY_WINDOW):
    """
    Kinetic market energy:  E(t) = ½ × m(t) × v(t)²

    Key insight:
        • Small volume + high velocity  → low E  = fake / unconfirmed move
        • High volume + high velocity   → high E = TRUE spike energy
        • High volume + low velocity    → moderate E = absorption zone

    Returns dict:
        energy          — E(t) array
        energy_roll     — rolling mean of E(t)
        energy_now      — current scalar
        roll_E_now      — rolling mean scalar
        energy_zscore   — (E_now − μ) / σ   (how many σ above mean?)
        energy_pctile   — percentile rank of current energy (0–100)
        energy_accel    — dE/dt   (energy velocity)
        energy_curv     — d²E/dt² (energy curvature — key for reversals)
        curv_now        — current curvature scalar
        is_energy_spike — True if E_now > μ + 2σ AND dE/dt > 0
        is_exhaustion   — True if d²E/dt² < 0 AND E > μ (energy decelerating at top)
    """
    cl = np.asarray(close_arr, dtype=np.float64)
    vl = np.asarray(vol_arr,   dtype=np.float64)
    n  = len(cl)
    ...
    if n < _PHYS_MIN_BARS:
        null = np.zeros(n)
        return {'energy': null, 'energy_roll': null, 'energy_now': 0.0,
                'roll_E_now': 0.0, 'energy_zscore': 0.0, 'energy_pctile': 50.0,
                'energy_accel': null, 'energy_curv': null, 'curv_now': 0.0,
                'is_energy_spike': False, 'is_exhaustion': False}

    v  = _phys_velocity(cl)
    m  = _phys_mass(vl, window=window)
    E  = 0.5 * m * v ** 2          # kinetic energy per bar

    # rolling mean
    w = min(window, n)
    roll_E = np.full(n, np.nan)
    for i in range(w - 1, n):
        roll_E[i] = np.mean(E[i - w + 1: i + 1])
    roll_E = np.nan_to_num(roll_E, nan=0.0)

    # energy acceleration dE/dt
    e_accel = np.zeros(n)
    e_accel[1:] = np.diff(roll_E)

    # energy curvature d²E/dt²  (second derivative)
    e_curv = np.zeros(n)
    if n >= 3:
        e_curv[2:] = np.diff(e_accel[1:])    # diff of diff

    # statistics on last `window` bars
    e_window = E[max(0, n - window):]
    mu    = float(np.mean(e_window)) + 1e-20
    sigma = float(np.std(e_window))  + 1e-20
    e_now = float(E[-1])
    z     = (e_now - mu) / sigma
    pct   = float(np.mean(E[: -1] <= e_now) * 100.0) if n > 1 else 50.0

    # flags
    is_spike = (z > 2.0 and float(e_accel[-1]) > 0.0)
    is_exh   = (float(e_curv[-1]) < 0.0 and e_now > mu)

    return {
        'energy'          : E,
        'energy_roll'     : roll_E,
        'energy_now'      : round(e_now,                8),
        'roll_E_now'      : round(float(roll_E[-1]),    8),
        'energy_zscore'   : round(float(z),             4),
        'energy_pctile'   : round(pct,                  1),
        'energy_accel'    : e_accel,
        'energy_curv'     : e_curv,
        'curv_now'        : round(float(e_curv[-1]),    8),
        'accel_now'       : round(float(e_accel[-1]),   8),
        'is_energy_spike' : is_spike,
        'is_exhaustion'   : is_exh,
        'mu'              : round(mu,                   8),
        'sigma'           : round(sigma,                8),
    }
...
# ═════════════════════════════════════════════════════════════════════════════
#  MODULE 2 — ENERGY CURVATURE REVERSAL DETECTOR
#  Detects reversal nodes:  price still rising BUT energy decelerating
#  This is where "big money exits" before price turns
# ═════════════════════════════════════════════════════════════════════════════

def _phys_curvature_reversal(close_arr, vol_arr, lookback=_PHYS_DIV_BARS):
    """
    Reversal = energy exhaustion node:
        1. Velocity (price) still rising  (v > 0)
        2. Energy decelerating            (d²E/dt² < 0)
        3. Momentum flattening            (dp/dt ≈ 0 or negative)
        4. Cycle near peak                (fft reconstructed ≈ top)

    Returns dict:
        divergence_score   — 0–100  (100 = perfect reversal setup)
        reversal_imminent  — True when 3+ conditions align
        price_rising       — bool (velocity > 0)
        energy_declining   — bool (curvature < 0 at high energy)
        momentum_flat      — bool (impulse near zero or negative)
        energy_exhaustion  — bool (E > μ AND d²E < 0)
        bars_to_reversal   — estimated bars (via energy decay rate)
        reversal_strength  — 'weak' | 'moderate' | 'strong' | 'extreme'
    """
    cl = np.asarray(close_arr, dtype=np.float64)
    vl = np.asarray(vol_arr,   dtype=np.float64)
    n  = len(cl)
    ...
    null = {
        'divergence_score': 0.0, 'reversal_imminent': False,
        'price_rising': False, 'energy_declining': False,
        'momentum_flat': False, 'energy_exhaustion': False,
        'bars_to_reversal': None, 'reversal_strength': 'unknown',
    }

    if n < _PHYS_MIN_BARS + lookback:
        return null

    try:
        # ── compute all layers ────────────────────────────────────────
        mom_r = _phys_true_momentum(cl, vl)
        eng_r = _phys_kinetic_energy(cl, vl)

        v_now  = float(mom_r['velocity_now'])
        p_now  = float(mom_r['momentum_now'])
        imp    = float(mom_r['impulse_now'])
        curv   = float(eng_r['curv_now'])
        e_now  = float(eng_r['energy_now'])
        mu_e   = float(eng_r['mu'])
        z_e    = float(eng_r['energy_zscore'])

        # ── conditions ───────────────────────────────────────────────
        # C1: Price still rising (velocity positive)
        cond_price_rising = v_now > 0.0

        # C2: Energy curve turning negative (curvature < 0 at elevated energy)
        cond_energy_decl = (curv < 0.0 and e_now > mu_e * 0.8)

        # C3: Momentum flattening (impulse near zero or reversing)
        sigma_p = float(np.std(mom_r['momentum'][-lookback:])) + 1e-20
        cond_mom_flat = abs(imp) < sigma_p * 0.3 or imp < 0.0

        # C4: Energy exhaustion (high z-score + curvature < 0)
        cond_exhaustion = (z_e > 1.5 and curv < 0.0)

        # ── price/energy divergence over lookback ─────────────────────
        # Price going up while energy going down = divergence
        price_slope = float(np.polyfit(np.arange(lookback),
                                       cl[-lookback:], 1)[0]) if n >= lookback else 0.0
        energy_slope= float(np.polyfit(np.arange(lookback),
                                       eng_r['energy'][-lookback:], 1)[0]) if n >= lookback else 0.0
        cond_diverge = (price_slope > 0 and energy_slope < 0)

        n_conds = sum([cond_price_rising, cond_energy_decl,
                       cond_mom_flat, cond_exhaustion, cond_diverge])

        reversal_imminent = n_conds >= 3

        # ── score 0–100 ───────────────────────────────────────────────
        score = 0.0
        if cond_price_rising:   score += 15.0    # price side
        if cond_energy_decl:    score += 30.0    # energy side (key signal)
        if cond_mom_flat:       score += 20.0    # momentum side
        if cond_exhaustion:     score += 25.0    # z-score confirmation
        if cond_diverge:        score += 10.0    # structural divergence

        # ── bars to reversal estimate ──────────────────────────────────
        # If curvature is negative and energy is above mean, estimate how
        # many bars until energy drops below mean at current decay rate
        bars_to_rev = None
        if cond_energy_decl and curv < 0.0:
            # linear extrapolation: E(t) = E_now + accel*t + ½*curv*t²
            accel = float(eng_r['accel_now'])
            # solve for t: E + accel*t + ½*curv*t² = mu_e
            delta_e = e_now - mu_e
            if abs(curv) > 1e-20 and delta_e > 0:
                discriminant = accel ** 2 + 4 * 0.5 * abs(curv) * delta_e
                if discriminant >= 0:
                    t_rev = (-accel + np.sqrt(discriminant)) / abs(curv)
                    bars_to_rev = max(0, min(int(round(t_rev)), 100))
        ...
        return {
            ...
            'price_rising'       : cond_price_rising,
            'energy_declining'   : cond_energy_decl,
            'momentum_flat'      : cond_mom_flat,
            'energy_exhaustion'  : cond_exhaustion,
            'energy_diverging'   : cond_diverge,
            'price_slope'        : round(price_slope,        8),
            'energy_slope'       : round(energy_slope,       8),
            'velocity_now'       : round(v_now,              8),
            'momentum_now'       : round(p_now,              8),
            'energy_curv_now'    : round(curv,               8),
            'energy_zscore'      : round(z_e,                4),
            'n_conditions'       : n_conds,
            'bars_to_reversal'   : bars_to_rev,
            'reversal_strength'  : strength,
        }

    except Exception as ex:
        null['error'] = f'{type(ex).__name__}: {ex}'
        return null


# ═════════════════════════════════════════════════════════════════════════════
#  MODULE 3 — CYCLE PHASE CLASSIFIER
#  Maps current market position to sin(ωt+φ) domain
#  Phase zones: 0°=trough  90°=mid_up  180°=peak  270°=mid_down
# ═════════════════════════════════════════════════════════════════════════════

def _phys_cycle_phase(close_arr, dominant_period=None):
    """
    Compute sinusoidal cycle phase angle and zone for current bar.

    Uses FFT to find dominant period, then computes instantaneous phase
    via analytic signal (Hilbert transform).

    Returns dict:
        phase_deg        — current phase in degrees 0–360
        phase_zone       — zone name from _PHYS_PHASE_ZONES
        in_spike_zone    — True if phase in {trough, recovery, rising_fast}
        in_reversal_zone — True if phase in {peak, falling_fast}
        dominant_period  — dominant cycle period in bars
        omega            — angular frequency  2π / T
        amplitude        — cycle amplitude (price units)
        phase_velocity   — dφ/dt (phase speed — faster = shorter to reversal)
        bars_to_trough   — estimated bars until next trough (0° crossing)
        bars_to_peak     — estimated bars until next peak (180° crossing)
        oscillator_now   — reconstructed oscillator value at current bar
        cycle_strength   — 0–1 (how dominant the main cycle is vs noise)
    """
    cl = np.asarray(close_arr, dtype=np.float64)
    n  = len(cl)

    null = {
        'phase_deg': 0.0, 'phase_zone': 'unknown',
        'in_spike_zone': False, 'in_reversal_zone': False,
        'dominant_period': dominant_period or 20,
        'omega': 0.0, 'amplitude': 0.0,
        'phase_velocity': 0.0,
        'bars_to_trough': None, 'bars_to_peak': None,
        'oscillator_now': 0.0, 'cycle_strength': 0.0,
    }
    ...
    try:
        ...
        p   = np.polyfit(t, cl, 1)
        dtr = cl - np.poly1d(p)(t)

        # ── FFT to get dominant period ────────────────────────────────
        fft_v = np.fft.rfft(dtr)
        freqs = np.fft.rfftfreq(n)
        power = np.abs(fft_v) ** 2
        power[0] = 0.0

        valid = (freqs > 0) & (freqs <= 0.25)
        if not np.any(valid):
            return null

        pw_v = power.copy(); pw_v[~valid] = 0.0
        dom  = int(np.argmax(pw_v))
        T    = max(4.0, round(1.0 / freqs[dom])) if freqs[dom] > 0 else 20.0

        if dominant_period and dominant_period >= 4:
            T = float(dominant_period)

        omega = float(2.0 * np.pi / T)

        # ── reconstruct dominant oscillator ──────────────────────────
        cs = np.zeros_like(fft_v)
        cs[dom] = fft_v[dom]
        osc = np.fft.irfft(cs, n=n)
        amp = float(np.max(osc) - np.min(osc)) / 2.0
        ...
        if n % 2 == 0:
            ...
            h[0] = 1; h[1: (n + 1) // 2] = 2
        analytic = np.fft.ifft(np.fft.fft(dtr) * h)
        phase_rad = np.angle(analytic)   # −π to +π

        # wrap to [0, 2π] and convert to degrees
        phase_now_rad = float(phase_rad[-1]) % (2.0 * np.pi)
        phase_deg     = float(np.degrees(phase_now_rad))

        # ── phase velocity dφ/dt ──────────────────────────────────────
        if n >= 2:
            dp = float(phase_rad[-1] - phase_rad[-2])
            dp = float(np.angle(np.exp(1j * dp)))   # wrap to [-π, π]
            phase_vel = float(np.degrees(dp))
        else:
            phase_vel = 0.0

        # ── zone classification ───────────────────────────────────────
        zone = 'unknown'
        for z_name, (lo, hi) in _PHYS_PHASE_ZONES.items():
            if lo <= phase_deg < hi:
                zone = z_name
                break

        in_spike   = zone in _PHYS_SPIKE_ZONES
        in_reversal= zone in _PHYS_REVERSAL_ZONES

        # ── bars to next trough / peak ─────────────────────────────────
        # phase advances at ~omega rad/bar
        # bars to 0° (trough): (2π − phase_now_rad) / omega
        # bars to π (peak):    (π − phase_now_rad) / omega  [mod 2π]
        if abs(omega) > 1e-6:
            # trough target = 2π → 0° (next cycle start)
            to_trough_rad = (2.0 * np.pi - phase_now_rad) % (2.0 * np.pi)
            bars_to_trough = int(round(to_trough_rad / omega))
            # peak target = π (180°)
            to_peak_rad = (np.pi - phase_now_rad) % (2.0 * np.pi)
            bars_to_peak = int(round(to_peak_rad / omega))
        else:
            bars_to_trough = None
            bars_to_peak   = None

        # ── cycle strength (dominant power / total power) ─────────────
        dom_pow   = float(power[dom])
        total_pow = float(np.sum(power[valid])) + 1e-20
        strength  = min(1.0, dom_pow / total_pow)

        return {
            'phase_deg'       : round(phase_deg,   2),
            'phase_zone'      : zone,
            'in_spike_zone'   : in_spike,
            'in_reversal_zone': in_reversal,
            'dominant_period' : int(round(T)),
            'omega'           : round(omega,       6),
            'amplitude'       : round(amp,         8),
            'phase_velocity'  : round(phase_vel,   4),
            'bars_to_trough'  : bars_to_trough,
            'bars_to_peak'    : bars_to_peak,
            'oscillator_now'  : round(float(osc[-1]), 8),
...
# ═════════════════════════════════════════════════════════════════════════════
#  MODULE 4 — VELOCITY SHOCK DETECTOR
#  Detects sudden v(t) spikes BEFORE price confirms the move
#  v(t) shock = velocity zscore > 2.5 AND volume expanding = breakout warning
# ═════════════════════════════════════════════════════════════════════════════

def _phys_velocity_shock(close_arr, vol_arr, lookback=30):
    """
    Velocity shock = sudden v(t) jump beyond statistical norm.

    Pre-spike fingerprint:
        • v(t) z-score > 2.5   (velocity way above its mean)
        • m(t) rising           (volume accelerating, not declining)
        • p(t) = m×v positive   (both sides of momentum aligned)

    Returns dict:
        shock_detected     — bool (velocity spike fire)
        velocity_zscore    — current v(t) z-score
        velocity_now       — current v(t) value
        shock_direction    — 'bull' | 'bear' | 'none'
        shock_magnitude    — normalised magnitude 0–1
        vol_confirming     — bool (volume rising with velocity)
        pre_spike_prob     — 0–1 (probability spike incoming in next 1–5 bars)
        consecutive_shocks — count of consecutive shock bars (urgency indicator)
    """
    cl = np.asarray(close_arr, dtype=np.float64)
    vl = np.asarray(vol_arr,   dtype=np.float64)
    n  = len(cl)
    ...
    null = {
        'shock_detected': False, 'velocity_zscore': 0.0,
        'velocity_now': 0.0, 'shock_direction': 'none',
        'shock_magnitude': 0.0, 'vol_confirming': False,
        'pre_spike_prob': 0.0, 'consecutive_shocks': 0,
    }

    if n < max(_PHYS_MIN_BARS, lookback + 5):
        return null
    ...
    try:
        ...
        v_window = v[-lookback:]
        mu_v     = float(np.mean(v_window))
        sig_v    = float(np.std(v_window)) + 1e-20
        v_now    = float(v[-1])
        z_v      = (v_now - mu_v) / sig_v

        # volume confirming (last 3-bar average > prior 10-bar average)
        vol_short = float(np.mean(vl[-3:])) + 1e-20
        vol_long  = float(np.mean(vl[-13: -3])) + 1e-20
        vol_conf  = vol_short > vol_long

        shock = abs(z_v) > 2.0
        direction = 'bull' if (shock and v_now > 0) else ('bear' if (shock and v_now < 0) else 'none')

        # consecutive shocks (last N bars with |z| > 1.5)
        n_consec = 0
        for i in range(1, min(6, n)):
            z_i = (float(v[-(i)]) - mu_v) / sig_v
            if abs(z_i) > 1.5:
                n_consec += 1
            else:
                break

        # pre-spike probability (sigmoid of z combined with vol confirmation)
        mag = min(1.0, abs(z_v) / 4.0)
        vol_factor = 1.2 if vol_conf else 0.8
        pre_spike_prob = float(1.0 / (1.0 + np.exp(-(abs(z_v) - 2.0) * vol_factor)))

        return {
            'shock_detected'     : shock,
            'velocity_zscore'    : round(z_v,             4),
            'velocity_now'       : round(v_now,           8),
            'shock_direction'    : direction,
            'shock_magnitude'    : round(mag,             4),
            'vol_confirming'     : vol_conf,
            'pre_spike_prob'     : round(pre_spike_prob,  4),
            'consecutive_shocks' : n_consec,
        }
...
# ═════════════════════════════════════════════════════════════════════════════
#  MODULE 5 — FAST SPIKE INCOMING DETECTOR
#  Ultra-fast (last 5-bar) fingerprint fusion:
#    OFI (order flow) + Energy (physics) + Phase (cycle) + Velocity shock
#  All must align at the SAME TIME for spike_incoming = True
# ═════════════════════════════════════════════════════════════════════════════

def compute_fast_spike_detector(pair, close_arr, high_arr, low_arr, vol_arr,
                                 phase_result=None, ofi_result=None):
    """
    Fast spike incoming detector — designed to fire 1–5 bars BEFORE the move.

    Fuses four independent layers:
        Layer A: Physics — E(t) expanding + v(t) shock
        Layer B: Phase   — cycle in spike zone (0°–90°)
        Layer C: OFI     — buy imbalance accelerating
        Layer D: Micro   — last 3 bars all positive with volume surge

    All 4 layers must align for spike_incoming = True (highest conviction).
    3 layers = ALERT level.  2 layers = WATCH level.

    Parameters
    ----------
    pair        : str
    close_arr   : 1-D close price array (newest last)
    high_arr    : 1-D high array
    low_arr     : 1-D low array
    vol_arr     : 1-D volume array
    phase_result: output of _phys_cycle_phase() if already computed
    ofi_result  : output of _order_flow_imbalance_score() if already computed

    Returns
    -------
    dict:
        spike_incoming       — True = spike in next 1–5 bars (all layers)
        spike_alert          — True = 3 layers aligned
        spike_watch          — True = 2 layers aligned
        urgency_score        — 0–100 composite urgency
        layer_A_physics      — bool (energy + velocity)
        layer_B_phase        — bool (cycle phase)
        layer_C_ofi          — bool (order flow)
        layer_D_micro        — bool (micro price/volume)
        bars_to_spike_est    — estimated bars (from phase + energy decay)
        spike_direction      — 'bull' | 'bear' | 'neutral'
        confidence           — 0.0–1.0
        details              — sub-dict with per-layer scalars
    """
    result = {
        'pair'              : pair,
        ...
        'spike_watch'       : False,
        'urgency_score'     : 0.0,
        'layer_A_physics'   : False,
        'layer_B_phase'     : False,
        'layer_C_ofi'       : False,
        'layer_D_micro'     : False,
        'bars_to_spike_est' : None,
    ...
    try:
        # ── Layer A: Physics (energy + velocity) ──────────────────────
        eng_r = _phys_kinetic_energy(cl, vl)
        shk_r = _phys_velocity_shock(cl, vl)
        mom_r = _phys_true_momentum(cl, vl)

        e_now     = eng_r['energy_now']
        mu_e      = eng_r['mu']
        e_accel   = eng_r['accel_now']
        v_shock   = shk_r['shock_detected']
        v_zscore  = shk_r['velocity_zscore']
        vol_conf  = shk_r['vol_confirming']
        mom_dir   = mom_r['direction']

        # Energy expanding AND velocity shock aligned with momentum
        layer_A = (
            e_now > mu_e * 1.2 and     # energy above average
            e_accel > 0.0 and           # energy still growing
            (v_shock or abs(v_zscore) > 1.5) and  # velocity elevated
            vol_conf                    # volume confirming
        )

        # ── Layer B: Phase (cycle position) ──────────────────────────
        if phase_result is None:
            phase_result = _phys_cycle_phase(cl)
        layer_B = phase_result.get('in_spike_zone', False)
        phase_deg = phase_result.get('phase_deg', 180.0)
        bars_to_peak = phase_result.get('bars_to_peak')

        # ── Layer C: OFI (order flow imbalance) ──────────────────────
        if ofi_result is not None:
            ofi_pre    = bool(ofi_result.get('ofi_pre_spike', False))
            ofi_regime = ofi_result.get('ofi_regime', 'none')
            ofi_imbal  = float(ofi_result.get('ofi_imbalance', 0.0))
            layer_C = (ofi_pre or
                       ofi_imbal > 0.3 or
                       ofi_regime in ('strong', 'para'))
        else:
            # compute simplified OFI from close/volume
            # aggressive buy proxy: close > open bars with high volume
            op_arr = np.roll(cl, 1); op_arr[0] = cl[0]
            buy_mask = cl > op_arr
            buy_vol  = float(vl[-10:][buy_mask[-10:]].sum())
            total_v  = float(vl[-10:].sum()) + 1e-20
            imbal    = (buy_vol / total_v) - 0.5
            layer_C  = imbal > 0.15
            ofi_imbal = imbal

        # ── Layer D: Micro (last 3–5 bars all bullish with vol surge) ─
        k = _PHYS_FAST_BARS
        if n >= k + 3:
            last_closes = cl[-k:]
            last_vols   = vl[-k:]
            all_positive = bool(np.all(np.diff(last_closes) > 0))
            vol_surge    = float(np.mean(last_vols)) > float(np.mean(vl[-(k + 10):-k])) * 1.5
            body_sizes   = np.abs(np.diff(last_closes))
            accel_price  = bool(len(body_sizes) >= 2 and body_sizes[-1] > body_sizes[-2])
            layer_D = all_positive and (vol_surge or accel_price)
        else:
            layer_D = False

        # ── Layer count and verdict ────────────────────────────────────
        n_layers = sum([layer_A, layer_B, layer_C, layer_D])
        spike_incoming = n_layers >= _PHYS_SPIKE_CONFIRM + 1   # all 4
        spike_alert    = n_layers >= _PHYS_SPIKE_CONFIRM        # 3 layers
        spike_watch    = n_layers >= 2                           # 2 layers

        # ── Direction ─────────────────────────────────────────────────
        direction = mom_dir if mom_dir != 'neutral' else (
            'bull' if float(v_zscore) > 0 else 'bear')

        # ── Urgency score 0–100 ────────────────────────────────────────
        urgency = 0.0
        urgency += 25.0 if layer_A else min(12.5, abs(v_zscore) * 3.0)
        urgency += 25.0 if layer_B else max(0.0, (90.0 - abs(phase_deg - 0.0)) / 90.0 * 12.5)
        urgency += 25.0 if layer_C else (10.0 if ofi_imbal > 0.05 else 0.0)
        urgency += 25.0 if layer_D else 0.0
        # bonus: consecutive shocks
        urgency += min(10.0, shk_r.get('consecutive_shocks', 0) * 3.0)
        urgency = round(min(100.0, urgency), 1)

        # ── Confidence ────────────────────────────────────────────────
        conf = round(n_layers / 4.0, 2)

        # ── Bars to spike estimate ─────────────────────────────────────
        bars_est = None
        if bars_to_peak is not None and 0 < bars_to_peak <= 20:
            bars_est = bars_to_peak
        elif e_accel > 0:
            # energy growing; estimate bars until E > 2σ
            sigma_e = eng_r['sigma']
            target  = mu_e + 2.0 * sigma_e
            if target > e_now and e_accel > 0:
                bars_est = max(1, int((target - e_now) / e_accel))

        result.update({
            'spike_incoming'    : spike_incoming,
            'spike_alert'       : spike_alert,
            'spike_watch'       : spike_watch,
            'urgency_score'     : urgency,
            'layer_A_physics'   : layer_A,
            'layer_B_phase'     : layer_B,
            'layer_C_ofi'       : layer_C,
            'layer_D_micro'     : layer_D,
            'bars_to_spike_est' : bars_est,
            'spike_direction'   : direction,
            'confidence'        : conf,
            'n_layers'          : n_layers,
            'details': {
                'energy_now'       : eng_r['energy_now'],
                'energy_zscore'    : eng_r['energy_zscore'],
                'energy_accel'     : eng_r['accel_now'],
                'velocity_zscore'  : shk_r['velocity_zscore'],
                'vol_confirming'   : vol_conf,
                'phase_deg'        : phase_result.get('phase_deg', 0.0),
                'phase_zone'       : phase_result.get('phase_zone', 'unknown'),
                'ofi_imbalance'    : round(ofi_imbal, 4),
                'momentum_direction': mom_dir,
                'consecutive_shocks': shk_r.get('consecutive_shocks', 0),
            },
        })
...
# ═════════════════════════════════════════════════════════════════════════════
#  MODULE 6 — MULTI-TF HARMONIC RESONANCE CASCADE
#  When energy + phase align across TFs simultaneously → cascade spike
# ═════════════════════════════════════════════════════════════════════════════

def compute_mtf_resonance_cascade(pair, tf_cache, sel_detail, current_price):
    """
    Cross-TF harmonic resonance analysis.

    Physics insight:
        A market spike is most powerful when MULTIPLE timeframes
        simultaneously enter the spike-zone of their dominant cycle.
        This is the resonance cascade — like tuning forks vibrating together.

    For each TF in tf_cache, compute:
        • energy score (via physics engine)
        • phase zone (is this TF in spike zone?)
        • momentum direction (bull/bear)

    Then compute:
        • resonance_score   — fraction of TFs in spike zone × energy alignment
        • cascade_imminent  — True when ≥ 3 TFs in spike zone + energy expanding
        • dominant_tf       — which TF is leading the resonance
        • coherence         — 0–1 (how aligned all TF phases are)

    Returns
    -------
    dict with resonance_score, cascade_imminent, per_tf, coherence, ...
    """
    result = {
        'pair'              : pair,
        'resonance_score'   : 0.0,
        'cascade_imminent'  : False,
        'dominant_tf'       : None,
        'coherence'         : 0.0,
        'n_tfs_in_spike'    : 0,
        'n_tfs_expanding'   : 0,
        'per_tf'            : {},
        'combined_verdict'  : '🔴 no resonance',
        'error'             : None,
    }

    try:
        _tf_w = {'2h': 0.40, '30m': 0.60, '15m': 0.75, '5m': 0.90,
                 '3m': 0.95, '1m': 1.00, '1m_gate': 1.00}
        ...
        for tf_label, rd in (tf_cache.get(pair) or {}).items():
            w = _tf_w.get(tf_label, 0.5)
            total_w += w

            # Get close + volume arrays from cache if possible, else skip
            cl_arr = rd.get('close_arr')
            vl_arr = rd.get('vol_arr')

            if cl_arr is None or len(cl_arr) < _PHYS_MIN_BARS:
                # try to fetch
                try:
                    klines = trader.client.get_klines(
                        symbol=pair, interval=tf_label.replace('_gate', ''), limit=200)
                    cl_arr = np.array([float(k[4]) for k in klines], dtype=np.float64)
                    vl_arr = np.array([float(k[5]) for k in klines], dtype=np.float64)
                except Exception:
                    continue

            if len(cl_arr) < _PHYS_MIN_BARS:
                continue

            # Compute physics for this TF
            eng  = _phys_kinetic_energy(cl_arr, vl_arr)
            phs  = _phys_cycle_phase(cl_arr, rd.get('dominant_period'))
            shk  = _phys_velocity_shock(cl_arr, vl_arr)

            tf_in_spike  = phs.get('in_spike_zone', False)
            tf_expanding = eng.get('accel_now', 0.0) > 0.0
            tf_e_score   = min(100.0, eng['energy_zscore'] * 25.0 + 50.0)

            if tf_in_spike:   tfs_in_spike  += 1
            if tf_expanding:  tfs_expanding += 1

            phase_angles.append(phs.get('phase_deg', 0.0))
            energy_scores.append(tf_e_score * w)

            res_component = (0.5 * (1.0 if tf_in_spike else 0.0) +
                             0.3 * (1.0 if tf_expanding else 0.0) +
                             0.2 * min(1.0, tf_e_score / 100.0))
            res_wsum += res_component * w

            if tf_e_score > best_e_score:
                best_e_score = tf_e_score
                best_tf = tf_label

            result['per_tf'][tf_label] = {
                'in_spike_zone'  : tf_in_spike,
                'energy_zscore'  : round(eng['energy_zscore'], 4),
                'energy_expanding': tf_expanding,
                'phase_deg'      : round(phs.get('phase_deg', 0.0), 1),
                'phase_zone'     : phs.get('phase_zone', 'unknown'),
                'velocity_shock' : shk.get('shock_detected', False),
                'momentum'       : float(phs.get('amplitude', 0.0)),
            }

        # ── resonance score ───────────────────────────────────────────
        if total_w > 0:
            resonance = res_wsum / total_w
        else:
            resonance = 0.0

        # ── phase coherence (how aligned are the cycle angles?) ───────
        # Low circular variance = high coherence = cascade imminent
        if len(phase_angles) >= 2:
            angles_rad = np.radians(phase_angles)
            R = abs(np.mean(np.exp(1j * angles_rad)))   # circular mean vector
            coherence = float(R)
        else:
            coherence = 0.5

        # ── cascade imminent ───────────────────────────────────────────
        cascade_imminent = (tfs_in_spike >= 3 and
                            tfs_expanding >= 2 and
                            coherence > 0.5)

        # ── verdict label ─────────────────────────────────────────────
        if   resonance >= 0.75 and coherence > 0.7:
            verdict = '🔥 RESONANCE CASCADE — spike across all TFs imminent'
        elif resonance >= 0.60 and coherence > 0.5:
            verdict = '🟢 HIGH RESONANCE — strong multi-TF alignment'
        elif resonance >= 0.40:
            verdict = '🟡 BUILDING RESONANCE — partial TF alignment'
        else:
            verdict = '🔴 LOW RESONANCE — TFs not aligned'
        ...
        result.update({
            ...
            'n_tfs_in_spike'   : tfs_in_spike,
            'n_tfs_expanding'  : tfs_expanding,
...
# ═════════════════════════════════════════════════════════════════════════════
#  MODULE 7 — MTF BACKUP SELECTOR
#  When the primary winner has a low MTFSpike score, automatically
#  evaluate the next best candidates and select the backup.
# ═════════════════════════════════════════════════════════════════════════════

def compute_mtf_backup(sel_pairs, mtf_spike_scores, sel_detail, tf_cache,
                        label_map, primary_pair=None,
                        threshold=_PHYS_BACKUP_SCORE_THR):
    """
    MTF Backup Selector.

    When the primary winner's MTFSpike score is below `threshold`,
    this function:
        1. Ranks all sel_pairs by their MTFSpike score
        2. Computes a backup quality score (energy + phase + OFI)
        3. Returns the backup pair and its rationale

    Parameters
    ----------
    sel_pairs       : list of pairs that passed 1m dip gate
    mtf_spike_scores: {pair: score} from _compute_mtf_spike_score
    sel_detail      : {pair: dict} from momentum() calls
    tf_cache        : _tf_cache global
    label_map       : {pair: label}
    primary_pair    : the currently selected best pair (or None)
    threshold       : MTFSpike score below which backup triggers

    Returns
    -------
    dict:
        backup_triggered  — bool (primary score < threshold)
        primary_score     — primary pair's MTFSpike score
        backup_pair       — str or None (best alternative)
        backup_score      — MTFSpike score of backup
        backup_reason     — human-readable rationale
        all_ranked        — [(pair, label, score)] sorted desc
        backup_physics    — physics check on backup pair (fast)
    """
    result = {
        ...
        'backup_pair'     : None,
        'backup_score'    : 0.0,
        'backup_reason'   : 'primary score sufficient',
        'all_ranked'      : [],
        'backup_physics'  : None,
        'error'           : None,
    }
    ...
    try:
        # ── rank all pairs ─────────────────────────────────────────────
        ranked = sorted(sel_pairs,
                        key=lambda p: mtf_spike_scores.get(p, 0.0),
                        reverse=True)
        result['all_ranked'] = [
            (p, label_map.get(p, p), round(mtf_spike_scores.get(p, 0.0), 1))
            for p in ranked
        ]

        primary = primary_pair or (ranked[0] if ranked else None)
        if primary is None:
            return result

        primary_score = mtf_spike_scores.get(primary, 0.0)
        result['primary_score'] = round(primary_score, 1)
        result['primary_pair']  = primary

        # ── check if backup needed ────────────────────────────────────
        if primary_score >= threshold:
            result['backup_reason'] = f'primary score {primary_score:.1f} ≥ threshold {threshold}'
            return result

        result['backup_triggered'] = True

        # ── find backup: next ranked pair with better physics ─────────
        backup_pair  = None
        backup_score = 0.0
        backup_reason= 'no backup found'

        for p in ranked:
            if p == primary:
                continue
            sc = mtf_spike_scores.get(p, 0.0)
            d  = sel_detail.get(p, {})

            # quick physics check: energy pre-spike + phase in spike zone
            cl_b = d.get('close_arr')
            vl_b = d.get('vol_arr')
            phys_ok = False

            if cl_b is not None and len(cl_b) >= _PHYS_MIN_BARS:
                try:
                    vl_b  = vl_b if vl_b is not None else np.ones(len(cl_b))
                    eng_b = _phys_kinetic_energy(cl_b, vl_b)
                    phs_b = _phys_cycle_phase(cl_b)
                    phys_ok = (eng_b['accel_now'] > 0 and
                               phs_b.get('in_spike_zone', False))
                except Exception:
                    pass

            # Check OFI pre-spike
            ofi_ok = (d.get('ofi_pre_spike', False) or
                      d.get('energy_pre_spike_v17', False))

            # Prefer pairs with physics confirmation
            if sc > backup_score or (sc >= backup_score * 0.9 and (phys_ok or ofi_ok)):
                backup_pair   = p
                backup_score  = sc
                reasons = []
                if phys_ok:  reasons.append('energy+phase confirmed')
                if ofi_ok:   reasons.append('OFI pre-spike')
                if d.get('ht_sine_strong'): reasons.append('HT_SINE extreme dip')
                if d.get('pump_imminent'):  reasons.append('pump imminent')
                backup_reason = (
                    f'MTFSpike={sc:.1f} | ' +
                    (', '.join(reasons) if reasons else 'next best score')
                )

        result['backup_pair']   = backup_pair
        result['backup_score']  = round(backup_score, 1)
        result['backup_reason'] = backup_reason

        # ── run physics analysis on backup ────────────────────────────
        if backup_pair:
            d = sel_detail.get(backup_pair, {})
            cl_b = d.get('close_arr')
            vl_b = d.get('vol_arr')
            if cl_b is not None and len(cl_b) >= _PHYS_MIN_BARS:
                vl_b = vl_b if vl_b is not None else np.ones(len(cl_b))
                try:
                    result['backup_physics'] = {
                        'energy'  : _phys_kinetic_energy(cl_b, vl_b),
                        'phase'   : _phys_cycle_phase(cl_b),
                        'velocity': _phys_velocity_shock(cl_b, vl_b),
                    }
                except Exception:
                    pass
...
# ═════════════════════════════════════════════════════════════════════════════
#  MODULE 8 — ENERGY DIVERGENCE MONITOR
#  Tracks divergence between price direction and energy direction
#  over rolling windows at multiple lookbacks
# ═════════════════════════════════════════════════════════════════════════════

def compute_energy_divergence_monitor(close_arr, vol_arr,
                                       lookbacks=(5, 10, 20)):
    """
    Multi-lookback energy/price divergence analysis.

    For each lookback window, measures whether:
        • Price slope and energy slope are aligned (no divergence)
        • Price up + energy down = bearish divergence (impending reversal)
        • Price down + energy up = bullish divergence (impending bounce)

    Returns dict:
        bullish_divergence  — bool (price↓ + energy↑ → bounce imminent)
        bearish_divergence  — bool (price↑ + energy↓ → reversal imminent)
        divergence_score    — 0–100
        n_timeframes_div    — how many lookbacks show divergence
        strongest_divergence— which lookback shows strongest divergence
        per_lookback        — {bars: {price_slope, energy_slope, type}}
    """
    cl = np.asarray(close_arr, dtype=np.float64)
    vl = np.asarray(vol_arr,   dtype=np.float64)
    n  = len(cl)
    ...
    null = {
        ...
        'divergence_score': 0.0, 'n_timeframes_div': 0,
        'strongest_divergence': None, 'per_lookback': {},
    }

    if n < max(lookbacks) + _PHYS_MIN_BARS:
        return null
    ...
    try:
        ...
        for lb in lookbacks:
            ...
            t = np.arange(lb, dtype=np.float64)
            p_seg = cl[-lb:]
            e_seg = E[-lb:]

            # linear slopes
            p_slope = float(np.polyfit(t, p_seg, 1)[0])
            e_slope = float(np.polyfit(t, e_seg, 1)[0])

            # divergence type
            if p_slope > 0 and e_slope < 0:
                div_type = 'bearish'   # price up, energy down = reversal
                bear_count += 1
            elif p_slope < 0 and e_slope > 0:
                div_type = 'bullish'   # price down, energy up = bounce
                bull_count += 1
            else:
                div_type = 'none'

            # divergence magnitude = |angle between slopes|
            # normalise slopes by their stds
            p_std = float(np.std(p_seg)) + 1e-20
            e_std = float(np.std(e_seg)) + 1e-20
            mag = abs(p_slope / p_std - e_slope / e_std)

            if div_type != 'none' and mag > max_div:
                max_div   = mag
                strongest = lb

            per_lb[lb] = {
                'price_slope'  : round(p_slope, 10),
                'energy_slope' : round(e_slope, 10),
                'divergence'   : div_type,
                'magnitude'    : round(mag, 4),
            }

        n_div       = bull_count + bear_count
        div_score   = min(100.0, (n_div / max(1, len(lookbacks))) * 60.0
                         + min(40.0, max_div * 10.0))

        return {
            'bullish_divergence'   : bull_count >= 2,
            'bearish_divergence'   : bear_count >= 2,
            'divergence_score'     : round(div_score, 1),
            'n_timeframes_div'     : n_div,
...
#  compute_physics_energy_full()  — orchestrates all modules
# ═════════════════════════════════════════════════════════════════════════════

def compute_physics_energy_full(pair, current_price, sel_detail, tf_cache,
                                 close_arr=None, high_arr=None,
                                 low_arr=None,   vol_arr=None,
                                 ofi_result=None):
    """
    Full physics energy analysis for the MTF winner.

    Runs all v18 physics modules and synthesises into a single result dict.

    Parameters
    ----------
    pair          : str
    current_price : float
    sel_detail    : {pair: dict} from main loop
    tf_cache      : _tf_cache global
    close_arr     : optional pre-fetched 1m close array
    high_arr      : optional pre-fetched 1m high array
    low_arr       : optional pre-fetched 1m low array
    vol_arr       : optional pre-fetched 1m volume array
    ofi_result    : optional OFI result from _order_flow_imbalance_score()

    Returns
    -------
    dict with all sub-results and combined physics_score 0–100
    """
    result = {
        'pair'           : pair,
        ...
        'divergence'     : None,
        'physics_score'  : 0.0,
        'physics_verdict': '🟡 insufficient data',
    ...
    try:
        ...
        # ── fetch arrays if not provided ──────────────────────────────
        if close_arr is None or len(close_arr) < _PHYS_MIN_BARS:
            try:
                k1m = trader.client.get_klines(symbol=pair, interval='1m', limit=500)
                close_arr = np.array([float(k[4]) for k in k1m], dtype=np.float64)
                high_arr  = np.array([float(k[2]) for k in k1m], dtype=np.float64)
                low_arr   = np.array([float(k[3]) for k in k1m], dtype=np.float64)
                vol_arr   = np.array([float(k[5]) for k in k1m], dtype=np.float64)
            except Exception as ex:
                result['error'] = f'1m fetch: {ex}'
                return result

        cl = np.asarray(close_arr, dtype=np.float64)
        vl = np.asarray(vol_arr,   dtype=np.float64)
        hi = np.asarray(high_arr,  dtype=np.float64) if high_arr is not None else cl
        lo = np.asarray(low_arr,   dtype=np.float64) if low_arr  is not None else cl

        if len(cl) < _PHYS_MIN_BARS:
            result['error'] = 'insufficient 1m bars'
            return result

        # ── 1. True physics momentum ──────────────────────────────────
        mom_r = _phys_true_momentum(cl, vl)
        result['momentum'] = mom_r

        # ── 2. Kinetic energy ─────────────────────────────────────────
        eng_r = _phys_kinetic_energy(cl, vl)
        result['energy'] = eng_r

        # ── 3. Energy curvature reversal detector ─────────────────────
        curv_r = _phys_curvature_reversal(cl, vl)
        result['curvature_rev'] = curv_r

        # ── 4. Cycle phase classifier ─────────────────────────────────
        phs_r = _phys_cycle_phase(cl, d.get('dominant_period'))
        result['cycle_phase'] = phs_r

        # ── 5. Velocity shock ─────────────────────────────────────────
        shk_r = _phys_velocity_shock(cl, vl)
        result['velocity_shock'] = shk_r

        # ── 6. Fast spike incoming ────────────────────────────────────
        fsp_r = compute_fast_spike_detector(
            pair, cl, hi, lo, vl,
            phase_result=phs_r,
            ofi_result=ofi_result or d.get('ofi_data')
        )
        result['fast_spike'] = fsp_r

        # ── 7. MTF resonance cascade ──────────────────────────────────
        res_r = compute_mtf_resonance_cascade(pair, tf_cache, sel_detail, current_price)
        result['mtf_resonance'] = res_r

        # ── 8. Energy divergence monitor ─────────────────────────────
        div_r = compute_energy_divergence_monitor(cl, vl)
        result['divergence'] = div_r

        # ── 9. Physics composite score 0–100 ──────────────────────────
        # Design: this is a DIP-BUY score, not a SHORT score.
        # High score = strong dip bounce setup confirmed by physics.
        sc = 0.0

        # Energy expanding (max 25 pts)
        e_zscore = eng_r.get('energy_zscore', 0.0)
        e_accel  = eng_r.get('accel_now',     0.0)
        sc += min(15.0, max(0.0, e_zscore * 5.0)) if e_zscore > 0 else 0.0
        sc += 10.0 if e_accel > 0 else 0.0

        # Cycle phase (max 20 pts)
        sc += 20.0 if phs_r.get('in_spike_zone') else 0.0

        # Velocity shock (max 15 pts)
        v_shock = shk_r.get('shock_detected', False)
        v_z     = abs(shk_r.get('velocity_zscore', 0.0))
        sc += 10.0 if v_shock else min(5.0, v_z * 2.0)
        sc += 5.0 if shk_r.get('vol_confirming') else 0.0

        # Fast spike alignment (max 20 pts)
        n_lyr = fsp_r.get('n_layers', 0)
        sc += min(20.0, n_lyr * 5.0)

        # MTF resonance (max 15 pts)
        res_sc = res_r.get('resonance_score', 0.0)
        sc += res_sc * 15.0

        # Reversal risk penalty (−20 pts if reversal imminent)
        if curv_r.get('reversal_imminent'):
            sc -= 20.0

        # Bullish divergence bonus (price↓ + energy↑ = dip setup)
        if div_r.get('bullish_divergence'):
            sc += 5.0

        # Bearish divergence penalty (price↑ + energy↓ = fading)
        if div_r.get('bearish_divergence'):
            sc -= 10.0

        # Dip quality component (energy at trough + cycle in buy zone)
        dip_quality = 0.0
        if phs_r.get('in_spike_zone') and e_accel > 0:
            dip_quality = min(100.0, e_zscore * 20.0 + phs_r.get('cycle_strength', 0.5) * 50.0)
        elif div_r.get('bullish_divergence'):
            dip_quality = min(60.0, div_r.get('divergence_score', 0.0))

        physics_score   = round(min(100.0, max(0.0, sc)), 1)
        result['physics_score'] = physics_score
        result['dip_quality']   = round(dip_quality, 1)

        # ── 10. Top-level flags ───────────────────────────────────────
        result['spike_imminent'] = (
            fsp_r.get('spike_incoming', False) or
            (fsp_r.get('spike_alert', False) and res_r.get('cascade_imminent', False))
        )
        result['reversal_risk'] = curv_r.get('reversal_imminent', False)

        # ── 11. Physics verdict ───────────────────────────────────────
        if   physics_score >= 80 and result['spike_imminent']:
            verdict = '🔥 SPIKE IMMINENT — full physics + resonance confirmed'
        elif physics_score >= 65:
            verdict = '🟢 STRONG SETUP — energy + phase + velocity aligned'
        elif physics_score >= 45:
            verdict = '🟡 BUILDING — energy growing, phase approaching spike zone'
        elif result['reversal_risk']:
            verdict = '⛔ REVERSAL RISK — energy exhaustion detected, avoid long'
        else:
            verdict = '🔴 LOW CONVICTION — physics layers not aligned'

        result['physics_verdict'] = verdict
...
# ═════════════════════════════════════════════════════════════════════════════
#  MODULE 10 — PRINT FUNCTIONS
# ═════════════════════════════════════════════════════════════════════════════

def print_physics_report(phys_result, label_map):
    """
    Print formatted physics energy block for the MTF winner.
    """
    if not phys_result:
        return

    pair  = phys_result.get('pair', '?')
    lbl   = label_map.get(pair, pair.replace('USDC', ''))
    cp    = phys_result.get('current_price', 0.0)
    pf    = lambda x: (f'{x:.6f}' if x and x < 1.0 else f'{x:.4f}') if x else '—'
    w     = 70

    print(f'\n  {"═" * w}')
    print(f'  ⚛  PHYSICS ENERGY ENGINE  v18  ·  {lbl}  ({pair})')
    print(f'  {"═" * w}')
    print(f'  Entry price : {pf(cp)}')
    print()

    # ── True Momentum block ───────────────────────────────────────────
    mom = phys_result.get('momentum') or {}
    if mom and not mom.get('error'):
        v_now  = mom.get('velocity_now', 0.0)
        m_now  = mom.get('mass_now',     1.0)
        p_now  = mom.get('momentum_now', 0.0)
        imp    = mom.get('impulse_now',  0.0)
        dirn   = mom.get('direction',    '—')
        arr    = '▲' if p_now > 0 else '▼'
        print(f'  ┌─ PHYSICS MOMENTUM (p = m×v) {"─" * 38}┐')
        print(f'  │  Velocity  v(t)    : {v_now:+.8f}  (log-return, EMA-smoothed)')
        print(f'  │  Mass      m(t)    : {m_now:.4f}  (normalised volume)')
        print(f'  │  Momentum  p(t)    : {p_now:+.8f}  {arr} {dirn.upper()}')
        print(f'  │  Impulse  dp/dt    : {imp:+.8f}  (rate of momentum change)')
        print(f'  └{"─" * 68}┘')
        print()

    # ── Kinetic Energy block ──────────────────────────────────────────
    eng = phys_result.get('energy') or {}
    if eng:
        e_now  = eng.get('energy_now',    0.0)
        roll_e = eng.get('roll_E_now',    0.0)
        z_e    = eng.get('energy_zscore', 0.0)
        pct_e  = eng.get('energy_pctile', 50.0)
        accel  = eng.get('accel_now',     0.0)
        curv   = eng.get('curv_now',      0.0)
        spk    = eng.get('is_energy_spike',False)
        exh    = eng.get('is_exhaustion',  False)
        accel_arr = '▲' if accel > 0 else '▼'
        curv_arr  = '▲' if curv  > 0 else '▼'
        print(f'  ┌─ KINETIC ENERGY  E(t) = ½·m(t)·v(t)² {"─" * 29}┐')
        print(f'  │  E now            : {e_now:.10f}')
        print(f'  │  E rolling mean   : {roll_e:.10f}')
        print(f'  │  E z-score        : {z_e:+.4f}  (percentile: {pct_e:.0f}%)')
        print(f'  │  dE/dt  (accel)   : {accel:+.10f}  {accel_arr}')
        print(f'  │  d²E/dt² (curv)   : {curv:+.10f}  {curv_arr}')
        if spk:
            print(f'  │  ⚡ ENERGY SPIKE    : confirmed  (z>{z_e:.1f}σ + dE>0)')
        if exh:
            print(f'  │  ⛔ EXHAUSTION NODE : detected   (d²E<0 at elevated E)')
        print(f'  └{"─" * 68}┘')
        print()

    # ── Cycle Phase block ─────────────────────────────────────────────
    phs = phys_result.get('cycle_phase') or {}
    if phs:
        ph_deg  = phs.get('phase_deg',     0.0)
        zone    = phs.get('phase_zone',    '—')
        in_spk  = phs.get('in_spike_zone', False)
        in_rev  = phs.get('in_reversal_zone', False)
        T       = phs.get('dominant_period', '—')
        amp     = phs.get('amplitude',     0.0)
        b_trough= phs.get('bars_to_trough')
        b_peak  = phs.get('bars_to_peak')
        strength= phs.get('cycle_strength', 0.0)
        zone_icon = '✅' if in_spk else ('⛔' if in_rev else '→')
        print(f'  ┌─ SINUSOIDAL CYCLE PHASE  C(t)=sin(ωt+φ) {"─" * 25}┐')
        print(f'  │  Phase angle      : {ph_deg:.1f}°  zone={zone}  {zone_icon}')
        print(f'  │  Dominant period  : {T} bars   amplitude={pf(amp)}')
        print(f'  │  Cycle strength   : {strength:.3f}  (1.0 = pure oscillation)')
        if b_trough is not None:
            print(f'  │  → Next trough    : ~{b_trough} bars  (0° crossing — BUY zone)')
        if b_peak is not None:
            print(f'  │  → Next peak      : ~{b_peak} bars  (180° crossing — SELL zone)')
        if in_spk:
            print(f'  │  🚀 SPIKE ZONE     : price at optimal acceleration phase')
        if in_rev:
            print(f'  │  ⛔ REVERSAL ZONE  : cycle approaching peak — caution')
        print(f'  └{"─" * 68}┘')
        print()

    # ── Velocity Shock block ──────────────────────────────────────────
    shk = phys_result.get('velocity_shock') or {}
    if shk:
        z_v   = shk.get('velocity_zscore', 0.0)
        shock = shk.get('shock_detected',  False)
        dirn  = shk.get('shock_direction', 'none')
        vol_c = shk.get('vol_confirming',  False)
        pre_p = shk.get('pre_spike_prob',  0.0)
        consec= shk.get('consecutive_shocks', 0)
        if shock or abs(z_v) > 1.0:
            shock_icon = '⚡' if shock else '→'
            print(f'  ┌─ VELOCITY SHOCK DETECTOR {"─" * 43}┐')
            print(f'  │  Velocity z-score : {z_v:+.4f}  {shock_icon} {"SHOCK DETECTED" if shock else "elevated"}  dir={dirn}')
            print(f'  │  Volume confirming: {"✔ YES" if vol_c else "✗ NO"}')
            print(f'  │  Pre-spike prob   : {pre_p:.3f}  ({pre_p*100:.0f}%)')
            if consec > 0:
                print(f'  │  Consecutive shocks: {consec} bars  (urgency ↑)')
            print(f'  └{"─" * 68}┘')
            print()

    # ── Fast Spike Incoming block ─────────────────────────────────────
    fsp = phys_result.get('fast_spike') or {}
    if fsp:
        incoming = fsp.get('spike_incoming', False)
        alert    = fsp.get('spike_alert',    False)
        watch    = fsp.get('spike_watch',    False)
        urgency  = fsp.get('urgency_score',  0.0)
        n_lyr    = fsp.get('n_layers',       0)
        bars_est = fsp.get('bars_to_spike_est')
        dirn     = fsp.get('spike_direction', '—')
        conf     = fsp.get('confidence',     0.0)
        lA = '✔' if fsp.get('layer_A_physics') else '✗'
        lB = '✔' if fsp.get('layer_B_phase')   else '✗'
        lC = '✔' if fsp.get('layer_C_ofi')     else '✗'
        lD = '✔' if fsp.get('layer_D_micro')   else '✗'
        det = fsp.get('details', {})

        if incoming:
            header = '🔥 SPIKE INCOMING — ALL 4 LAYERS ALIGNED'
        elif alert:
            header = '🟠 SPIKE ALERT — 3 LAYERS ALIGNED'
        elif watch:
            header = '🟡 SPIKE WATCH — 2 LAYERS ALIGNED'
        else:
            header = '🔴 NO SPIKE SIGNAL'

        print(f'  ┌─ FAST SPIKE INCOMING DETECTOR ⚡ {"─" * 34}┐')
        print(f'  │  Status           : {header}')
        print(f'  │  Urgency score    : {urgency:.0f}/100  layers={n_lyr}/4  dir={dirn}  conf={conf:.2f}')
        print(f'  │  Layer A (Physics): {lA}  E_z={det.get("energy_zscore",0):+.2f}'
              f'  v_z={det.get("velocity_zscore",0):+.2f}  volOK={det.get("vol_confirming","?")}')
        print(f'  │  Layer B (Phase)  : {lB}  phase={det.get("phase_deg",0):.0f}°  zone={det.get("phase_zone","—")}')
        print(f'  │  Layer C (OFI)    : {lC}  imbal={det.get("ofi_imbalance",0):+.3f}')
        print(f'  │  Layer D (Micro)  : {lD}  consecutive_shocks={det.get("consecutive_shocks",0)}')
        if bars_est is not None:
            print(f'  │  ⏱ Est. bars to spike : ~{bars_est} bars')
        print(f'  └{"─" * 68}┘')
        print()

    # ── MTF Resonance Cascade block ───────────────────────────────────
    res = phys_result.get('mtf_resonance') or {}
    if res:
        res_sc = res.get('resonance_score',  0.0)
        coh    = res.get('coherence',        0.0)
        casc   = res.get('cascade_imminent', False)
        n_spk  = res.get('n_tfs_in_spike',   0)
        n_exp  = res.get('n_tfs_expanding',  0)
        verdict= res.get('combined_verdict', '—')
        dom_tf = res.get('dominant_tf', '—')
        print(f'  ┌─ MTF HARMONIC RESONANCE CASCADE {"─" * 35}┐')
        print(f'  │  {verdict}')
        print(f'  │  Resonance score  : {res_sc:.4f}   coherence={coh:.4f}')
        print(f'  │  TFs in spike zone: {n_spk}   TFs expanding: {n_exp}   lead TF: {dom_tf}')
        if casc:
            print(f'  │  ⚡ CASCADE IMMINENT — ≥3 TFs spike zone + energy expanding')
        per_tf = res.get('per_tf', {})
        if per_tf:
            print(f'  │  {"TF":<8}  {"InSpike":>8}  {"E_z":>7}  {"Phase°":>7}  '
                  f'{"Zone":<14}  {"VelShock"}')
            for tf, td in sorted(per_tf.items()):
                iz = '✔' if td.get('in_spike_zone') else '✗'
                ez = td.get('energy_zscore', 0.0)
                ph = td.get('phase_deg',     0.0)
                zn = td.get('phase_zone',    '—')[:13]
                vs = '⚡' if td.get('velocity_shock') else '  '
                print(f'  │  {tf:<8}  {iz:>8}  {ez:>7.2f}  {ph:>7.1f}  {zn:<14}  {vs}')
        print(f'  └{"─" * 68}┘')
        print()

    # ── Energy Divergence block ───────────────────────────────────────
    div = phys_result.get('divergence') or {}
    if div:
        bull_div = div.get('bullish_divergence', False)
        bear_div = div.get('bearish_divergence', False)
        div_sc   = div.get('divergence_score',   0.0)
        n_div    = div.get('n_timeframes_div',    0)
        if bull_div or bear_div or div_sc > 20:
            print(f'  ┌─ ENERGY DIVERGENCE MONITOR {"─" * 40}┐')
            if bull_div:
                print(f'  │  ✅ BULLISH DIVERGENCE: price↓ + energy↑ → bounce imminent')
            if bear_div:
                print(f'  │  ⛔ BEARISH DIVERGENCE: price↑ + energy↓ → reversal risk')
            print(f'  │  Divergence score : {div_sc:.0f}/100  ({n_div} windows showing divergence)')
            per_lb = div.get('per_lookback', {})
            for lb, ld in sorted(per_lb.items()):
                dtype = ld.get('divergence', 'none')
                if dtype != 'none':
                    icon = '↑' if dtype == 'bullish' else '↓'
                    print(f'  │  {lb}b window         : {dtype} {icon}  '
                          f'(p={ld["price_slope"]:+.6f}  E={ld["energy_slope"]:+.6f})')
            print(f'  └{"─" * 68}┘')
            print()

    # ── Reversal Curvature block ──────────────────────────────────────
    curv = phys_result.get('curvature_rev') or {}
    if curv and (curv.get('reversal_imminent') or curv.get('divergence_score', 0) > 30):
        div_sc  = curv.get('divergence_score',  0.0)
        rev_imm = curv.get('reversal_imminent', False)
        strength= curv.get('reversal_strength', 'weak')
        n_c     = curv.get('n_conditions',      0)
        b_rev   = curv.get('bars_to_reversal')
        print(f'  ┌─ ENERGY CURVATURE REVERSAL DETECTOR {"─" * 31}┐')
        if rev_imm:
            print(f'  │  ⛔ REVERSAL IMMINENT ({strength.upper()}) — {n_c}/5 conditions aligned')
        else:
            print(f'  │  → Reversal risk score: {div_sc:.0f}/100  ({strength}) — monitoring')
        c1 = '✔' if curv.get('price_rising')      else '✗'
        c2 = '✔' if curv.get('energy_declining')   else '✗'
        c3 = '✔' if curv.get('momentum_flat')      else '✗'
        c4 = '✔' if curv.get('energy_exhaustion')  else '✗'
        c5 = '✔' if curv.get('energy_diverging')   else '✗'
        print(f'  │  C1 price_rising  : {c1}   C2 energy_declining: {c2}')
        print(f'  │  C3 momentum_flat : {c3}   C4 exhaustion      : {c4}')
        print(f'  │  C5 diverging     : {c5}   E_z={curv.get("energy_zscore",0):+.2f}'
              f'  curv={curv.get("energy_curv_now",0):+.6f}')
        if b_rev is not None:
            print(f'  │  ⏱ Est. bars to reversal : ~{b_rev} bars')
        print(f'  └{"─" * 68}┘')
        print()

    # ── Final physics verdict ─────────────────────────────────────────
    ps  = phys_result.get('physics_score',   0.0)
    pv  = phys_result.get('physics_verdict', '—')
    dq  = phys_result.get('dip_quality',     0.0)
    si  = phys_result.get('spike_imminent',  False)
    rr  = phys_result.get('reversal_risk',   False)

    print(f'  {"═" * w}')
    print(f'  ★  PHYSICS VERDICT  ·  {lbl}')
    print(f'  {"─" * w}')
    print(f'  ▶  PHYSICS SCORE    : {ps:.0f}/100')
    print(f'  ▶  DIP QUALITY      : {dq:.0f}/100  (energy at trough + cycle in buy zone)')
    print(f'  ▶  SPIKE IMMINENT   : {"🔥 YES" if si else "✗ NO"}')
    print(f'  ▶  REVERSAL RISK    : {"⛔ YES" if rr else "✗ NO"}')
    print(f'  ▶  VERDICT          : {pv}')
    print(f'  {"═" * w}\n')


def print_backup_report(backup_result, label_map):
    """Print the MTF backup selection block."""
    if not backup_result or not backup_result.get('backup_triggered'):
        return

    w = 70
    print(f'\n  {"═" * w}')
    print(f'  🔄  MTF BACKUP SELECTOR  — primary score low, seeking backup')
    print(f'  {"─" * w}')

    primary = backup_result.get('primary_pair', '—')
    p_score = backup_result.get('primary_score', 0.0)
    backup  = backup_result.get('backup_pair', '—')
    b_score = backup_result.get('backup_score', 0.0)
    reason  = backup_result.get('backup_reason', '—')

    p_lbl = label_map.get(primary, primary)
    b_lbl = label_map.get(backup,  backup) if backup else '—'

    print(f'  Primary   : {p_lbl}  MTFSpike={p_score:.1f}  ← below threshold')
    print(f'  Backup    : {b_lbl}  MTFSpike={b_score:.1f}  ← {reason}')

    ranked = backup_result.get('all_ranked', [])
    if len(ranked) > 1:
        print(f'\n  Full ranking:')
        for i, (p, lbl, sc) in enumerate(ranked, 1):
            marker = '★' if p == primary else ('→' if p == backup else ' ')
            print(f'    {marker} {i}. {lbl:<12}  MTFSpike={sc:.1f}')

    print(f'  {"═" * w}\n')


# ═════════════════════════════════════════════════════════════════════════════
#  MODULE 11 — _compute_mtf_spike_score PATCH  (v18 physics layer added)
#
#  REPLACE the existing _compute_mtf_spike_score() with this version.
#  This adds a physics layer component (+12 pts max) to the existing v16 score.
#  All other components remain unchanged.
# ═════════════════════════════════════════════════════════════════════════════

def _compute_mtf_spike_score_v18(pair, tf_cache, sel_detail):
    """
    MTF Spike Score v18 — physics layer added to v16 formula.

    New components vs v16:
      +12 pts: Physics layer (energy + phase + velocity shock)
              = 5 pts energy expanding (accel > 0 + z > 0)
              + 4 pts cycle in spike zone
              + 3 pts velocity shock detected

    Bonuses (new):
      +5: fast spike layer_A + layer_B both confirmed
      +3: resonance cascade imminent (multi-TF energy alignment)
      +3: bullish energy divergence (price↓ + energy↑)

    Penalties (new):
      -10: physics reversal risk (energy curvature exhaustion)
      -5:  bearish energy divergence (price↑ + energy↓)

    All v16 components kept unchanged.
    """
    # ─── Run all existing v16 components first ────────────────────────
    _score = 0.0
    ...
    # ── 10 pts: ADX composite ────────────────────────────────────────
    _adx_scores = [_rd.get('adx_score', 0.0) for _rd in _cache.values()]
    _adx_scores.append((_d.get('adx_data') or {}).get('adx_score', 0.0))
    if _adx_scores:
        _score += float(np.mean(_adx_scores)) * 0.10

    # ── 10 pts: Momentum pump composite ──────────────────────────────
    ...
    # ── 6 pts: OFI (v16) ─────────────────────────────────────────────
    _ofi_tw  = {'2h': 0.30, '30m': 0.45, '15m': 0.60, '5m': 0.80,
                '3m': 0.90, '1m': 1.00, '1m_gate': 1.00}
    _ofi_wv  = 0.0; _ofi_ws = 0.0
    for _tk, _rd in _cache.items():
        _w = _ofi_tw.get(_tk, 0.50)
        _ofi_wv += _rd.get('ofi_score', 0.0) * _w
        _ofi_ws += _w
    _ofi_1m_sc = _d.get('ofi_score', 0.0)
    _ofi_wv += _ofi_1m_sc * _ofi_tw.get('1m', 1.0)
    _ofi_ws += _ofi_tw.get('1m', 1.0)
    if _ofi_ws > 0:
        _score += min(6.0, (_ofi_wv / _ofi_ws) * 0.06)

    # ── BONUSES (v16 unchanged) ───────────────────────────────────────
    ...
    # OFI bonuses (v16)
    _ofi_fast_tfs = ['1m', '1m_gate', '3m', '5m']
    _n_presike = sum(1 for _tk, _rd in _cache.items()
                     if _tk in _ofi_fast_tfs and _rd.get('ofi_pre_spike', False))
    if _d.get('ofi_pre_spike', False):
        _n_presike += 1
    if _n_presike >= 1:
        _score += 6.0

    _n_para = sum(1 for _rd in _cache.values() if _rd.get('ofi_regime') == 'para')
    if _d.get('ofi_regime') == 'para':
        _n_para += 1
    if _n_para >= 1:
        _score += 4.0

    _n_strong_ofi = sum(1 for _rd in _cache.values() if _rd.get('ofi_regime') == 'strong')
    if _d.get('ofi_regime') == 'strong':
        _n_strong_ofi += 1
    if _n_strong_ofi >= 2:
        _score += 2.0

    # PENALTIES (v16)
    if _d.get('geometry_detail', {}).get('is_fake_dip'):
        _score -= 15.0
    if _d.get('exhaust_top_ok') or any(_rd.get('exhaust_top_ok') for _rd in _cache.values()):
        _score -= 15.0
    if not _rok_1m and not _omr_1m:
        _score -= 10.0
    adx_1m = (_d.get('adx_data') or {}).get('adx_now')
    if adx_1m is not None and adx_1m < 15.0:
        _score -= 8.0
    _ofi_sell = sum(1 for _rd in _cache.values()
                    if _rd.get('ofi_imbalance', 0.0) < -0.3 and
                    _rd.get('ofi_regime') in ('strong', 'para'))
    if _ofi_sell >= 2:
        _score -= 6.0

    # ── ⚡ NEW v18: PHYSICS LAYER (+12 pts max) ──────────────────────
    # Use the pre-computed energy_score_v17 if available, or run fresh
    _phys_eng_sc  = _d.get('energy_score_v17', 0.0)   # from _rich_tf_dip_check
    _phys_pre17   = _d.get('energy_pre_spike_v17', False)

    # Fast physics check using close/vol from sel_detail
    _cl_fast = _d.get('close_arr')
    _vl_fast = _d.get('vol_arr')
    _phys_energy_expand = False
    _phys_in_spike_zone = False
    _phys_vel_shock     = False
    _phys_rev_risk      = False
    _phys_bull_div      = False
    _phys_bear_div      = False
    _phys_fast_spike_ok = False
    _phys_resonance_ok  = False

    if _cl_fast is not None and len(_cl_fast) >= _PHYS_MIN_BARS:
        _vl_fast = _vl_fast if _vl_fast is not None else np.ones(len(_cl_fast))
        try:
            _cl_arr = np.asarray(_cl_fast, dtype=np.float64)
            _vl_arr = np.asarray(_vl_fast, dtype=np.float64)

            # Energy
            _eng_q  = _phys_kinetic_energy(_cl_arr, _vl_arr)
            _phys_energy_expand = (_eng_q['accel_now'] > 0 and
                                   _eng_q['energy_zscore'] > 0)

            # Phase
            _phs_q = _phys_cycle_phase(_cl_arr, _d.get('dominant_period'))
            _phys_in_spike_zone = _phs_q.get('in_spike_zone', False)

            # Velocity shock
            _shk_q = _phys_velocity_shock(_cl_arr, _vl_arr)
            _phys_vel_shock = _shk_q.get('shock_detected', False)

            # Reversal risk
            _curv_q = _phys_curvature_reversal(_cl_arr, _vl_arr)
            _phys_rev_risk = _curv_q.get('reversal_imminent', False)

            # Divergence
            _div_q = compute_energy_divergence_monitor(_cl_arr, _vl_arr)
            _phys_bull_div = _div_q.get('bullish_divergence', False)
            _phys_bear_div = _div_q.get('bearish_divergence', False)

        except Exception:
            pass

    # Fast spike from cached result
    _phys_fast_spike_ok = _phys_pre17 or _phys_energy_expand

    # Resonance from cache (n_tfs_pre_spike from energy_mtf_resonance)
    # (v17 already sets this; we reuse it)

    # Physics scoring (max 12 pts)
    _phys_pts = 0.0
    if _phys_energy_expand:   _phys_pts += 5.0
    if _phys_in_spike_zone:   _phys_pts += 4.0
    if _phys_vel_shock:        _phys_pts += 3.0
    _score += min(12.0, _phys_pts)

    # v18 Physics bonuses
    if _phys_energy_expand and _phys_in_spike_zone:
        _score += 5.0   # +5: both physics conditions aligned
    if _phys_bull_div:
        _score += 3.0   # +3: bullish energy divergence (dip bounce signal)

    # v18 Physics penalties
    if _phys_rev_risk:
        _score -= 10.0  # -10: energy exhaustion reversal risk
    if _phys_bear_div:
        _score -= 5.0   # -5: bearish energy divergence (fading move)

    return round(min(200.0, max(0.0, float(_score))), 1)


# ═════════════════════════════════════════════════════════════════════════════
#  MAIN LOOP INTEGRATION PATCH
#  Add the following 4 blocks AFTER the energy v17 block in the main loop
#  (~line 8974 in mtf_scanner17.py, after print_energy_spike_report call)
# ═════════════════════════════════════════════════════════════════════════════



# v18 alias: redirect existing call to v18 physics-enhanced version
_compute_mtf_spike_score = _compute_mtf_spike_score_v18


def _main_loop_physics_patch(
        best_symbol, current_price, sel_detail, sel_pairs,
        mtf_spike_scores, tf_cache, label_map,
        close_1m=None, high_1m=None, low_1m=None, vol_1m=None):
    """
    Call this function after print_energy_spike_report() in the main loop.

    Runs all v18 physics blocks and prints reports for:
        ① Physics energy full analysis
        ② Fast spike incoming detector
        ③ MTF resonance cascade
        ④ MTF backup selector
    """
    lbl = label_map.get(best_symbol, best_symbol)

    # ── ① Full physics energy analysis ──────────────────────────────
    print(f'  Running v18 Physics Energy Engine on {lbl}...')
    d = sel_detail.get(best_symbol, {})
    ofi_r = d.get('ofi_data')

    phys_result = compute_physics_energy_full(
        best_symbol, current_price, sel_detail, tf_cache,
        close_arr=close_1m, high_arr=high_1m,
        low_arr=low_1m,     vol_arr=vol_1m,
        ofi_result=ofi_r,
    )
    print_physics_report(phys_result, label_map)

    # ── ② MTF Backup Selector ────────────────────────────────────────
    if len(sel_pairs) > 1:
        print(f'  Running MTF Backup Selector...')
        backup_result = compute_mtf_backup(
            sel_pairs, mtf_spike_scores, sel_detail, tf_cache,
            label_map, primary_pair=best_symbol,
            threshold=_PHYS_BACKUP_SCORE_THR,
        )
        print_backup_report(backup_result, label_map)
    else:
        backup_result = None

    return phys_result, backup_result
...
# ─────────────────────────────────────────────────────────────────────────────
#
#  STEP 1: After FIB_RATIOS block (~line 204), insert this entire file.
#          All functions become available to the rest of the file.
#
#  STEP 2: Replace _compute_mtf_spike_score with _compute_mtf_spike_score_v18.
#          Find: "def _compute_mtf_spike_score(pair, tf_cache, sel_detail):"
#          Replace the ENTIRE function body.
#          Then update the call site (~line 8840):
#              _p: _compute_mtf_spike_score(_p, _tf_cache, sel_detail)
#          → becomes:
#              _p: _compute_mtf_spike_score_v18(_p, _tf_cache, sel_detail)
#
#  STEP 3: In the main while-True loop, AFTER the existing energy v17 block:
#
#          # ── ⚡ Energy · Harmonic · ML Spike Engine  (v17) ──────────────────
#          print(f'  Running Energy·Harmonic·ML spike analysis on {lbl}...')
#          eng_result_v17 = compute_energy_spike_analysis(...)
#          print_energy_spike_report(eng_result_v17, label_map)
#
#          ADD THESE LINES (v18 hook):
#
#          # ── ⚛  Physics Energy Engine  (v18) ─────────────────────────────
#          phys_result_v18, backup_v18 = _main_loop_physics_patch(
#              best_symbol, current_price, sel_detail, sel_pairs,
#              mtf_spike_scores, _tf_cache, label_map,
#              close_1m=close_1m,   # use the k1m arrays already fetched above
#              high_1m=high_1m,
#              low_1m=low_1m,
#              vol_1m=vol_1m,
#          )
#
#  STEP 4: (Optional) For fast spike standalone usage, call:
#
#          fast_spike = compute_fast_spike_detector(
#              pair, close_arr, high_arr, low_arr, vol_arr)
#          if fast_spike['spike_incoming']:
#              print(f'  🔥 FAST SPIKE INCOMING on {pair}!')
#
#  STEP 5: (Optional) Standalone dip quality check before full analysis:
#
#          div_check = compute_energy_divergence_monitor(close_arr, vol_arr)
#          if div_check['bullish_divergence']:
#              print('  ✅ Bullish energy divergence — dip bounce confirmed by physics')
#
# ─────────────────────────────────────────────────────────────────────────────
#  END OF v18 PHYSICS ENERGY ENGINE MODULE
# ─────────────────────────────────────────────────────────────────────────────


# ═══════════════════════════════════════════════════════════════════════════════
#  ENERGY · HARMONIC · PHASE · RESONANCE · ML SPIKE ENGINE  — v17 module
#
#  ADD THIS ENTIRE FILE CONTENTS directly after the existing imports/constants
#  block in mtf_scanner16.py  (around line 204, after FIB_RATIOS definition),
#  and follow the INTEGRATION GUIDE at the bottom of this file for the
#  3 additional hook points needed in:
#    ① _compute_mtf_spike_score()   — add energy score component
#    ② main while-True loop         — call energy report after phi_reversal block
#
#  Theory:
#    signal(t)  = volume-weighted log returns → STRUCTURE
#    E(t)       = signal²  → ENERGY (nonlinear amplification)
#    FFT        → dominant cycles, spectral entropy (compression)
#    harmonics  → inner oscillations between extrema (sin²→2ω doubling)
#    phase      → Hilbert-instantaneous alignment (timing trigger)
#    resonance  → cross-TF energy alignment score
#    ML         → RandomForest + GradientBoosting + Ridge ensemble spike probability
#    forecast   → harmonic reconstruction + energy-weighted FFT projection
#
#  Output: spike_probability 0–1  +  tiered price forecasts (STF/MTF/HTF)
#          + per-TF energy scores  + MTF resonance index
# ═══════════════════════════════════════════════════════════════════════════════
...
# ─── optional imports (same guards as the main file) ─────────────────────────
try:
    from sklearn.ensemble        import RandomForestRegressor, GradientBoostingRegressor
    from sklearn.linear_model    import Ridge
    from sklearn.preprocessing   import StandardScaler
...
#  CONSTANTS
# ─────────────────────────────────────────────────────────────────────────────
_ENG_WINDOW_DEFAULT  = 20     # rolling energy window (bars)
_ENG_HARMONICS       = 8      # number of FFT harmonics to extract
_ENG_ENTROPY_BINS    = 64     # spectral bins for entropy calc
_ENG_MIN_BARS        = 64     # minimum bars for any analysis
_ENG_PHASE_WINDOW    = 30     # phase alignment rolling window
_ENG_ML_TRAIN_RATIO  = 0.80   # 80 % train / 20 % test split
_ENG_RESONANCE_THOLD = 0.55   # resonance threshold for spike zone
_ENG_SPIKE_THOLD     = 0.75   # spike-probability threshold for 🔥 flag

# Timeframe configs for MTF energy pass  (label, interval, weight, horizon_bars)
# horizon_bars = how many bars = ~1 minute horizon on that TF
_ENG_TF_CONFIGS = [
    ('5m',  '5m',   0.90,  12,  '~1h'),
    ('15m', '15m',  0.75,  8,   '~2h'),
    ('30m', '30m',  0.60,  6,   '~3h'),
...
# ─────────────────────────────────────────────────────────────────────────────
#  1. SIGNAL BUILDER  (volume-weighted log returns = hybrid signal)
# ─────────────────────────────────────────────────────────────────────────────

def _build_energy_signal(close_arr, vol_arr, smooth=3):
    """
    Composite signal:  log_return × normalised_volume
    This puts price movement AND participation into the same scalar
    so that energy E = signal² captures TRUE buying pressure magnitude.

    Parameters
    ----------
    close_arr : 1-D array  (newest bar last)
    vol_arr   : 1-D array  same length
    smooth    : int — EMA smoothing on vol normalisation (reduces spike noise)

    Returns
    -------
    signal : 1-D float64 array, len = len(close_arr)
    """
    cl  = np.asarray(close_arr, dtype=np.float64)
    vl  = np.asarray(vol_arr,   dtype=np.float64)
    n   = len(cl)
    ...
    lr  = np.zeros(n)
    lr[1:] = np.log(np.maximum(cl[1:], 1e-20) / np.maximum(cl[:-1], 1e-20))

    # volume normalisation by rolling 20-bar mean
    vm  = np.full(n, 1.0)
    w   = min(20, n)
    for i in range(w, n):
        vm[i] = vl[i] / (np.mean(vl[i - w: i]) + 1e-20)

    # EMA smooth on volume multiplier
    alpha = 2.0 / (smooth + 1)
    for i in range(1, n):
        vm[i] = alpha * vm[i] + (1.0 - alpha) * vm[i - 1]

    return lr * vm


# ─────────────────────────────────────────────────────────────────────────────
#  2. ENERGY ENGINE  (E = signal²  with rolling stats)
# ─────────────────────────────────────────────────────────────────────────────

def _compute_rolling_energy(signal, window=None):
    """
    Compute:
      energy       = signal²
      rolling_E    = rolling mean of energy over `window` bars
      energy_growth = rolling_E.diff()  (momentum of energy)
      norm_growth  = energy_growth / (rolling_E + ε)

    Returns dict with array fields.
    """
    w   = window or _ENG_WINDOW_DEFAULT
    e   = signal ** 2
    n   = len(e)
    ...
    # diff (growth)
    e_growth  = np.full(n, 0.0)
    e_growth[1:] = np.diff(roll_E)

    norm_growth = e_growth / (roll_E + 1e-12)
    norm_growth = np.nan_to_num(norm_growth, nan=0.0)
    roll_E      = np.nan_to_num(roll_E,      nan=0.0)

    return {
        'energy_raw'   : e,
        'energy_roll'  : roll_E,
        'energy_growth': e_growth,
        'norm_growth'  : norm_growth,
        # summary scalars (last bar)
        'energy_now'   : float(e[-1]),
        'roll_E_now'   : float(roll_E[-1]),
        'growth_now'   : float(e_growth[-1]),
        'norm_growth_now': float(norm_growth[-1]),
    }


# ─────────────────────────────────────────────────────────────────────────────
#  3. FFT + HARMONIC DECOMPOSITION
# ─────────────────────────────────────────────────────────────────────────────

def _fft_harmonic_decomposition(signal, n_harmonics=None):
    """
    Full FFT analysis on `signal` (already detrended / log-return form).

    Returns
    -------
    dict:
      dominant_period  — bars per dominant cycle
      dominant_freq    — cycles per bar
      power_spectrum   — |FFT|² array (positive freqs only)
      freqs            — frequency axis (positive only)
      top_harmonics    — list of (freq, period, power, amplitude) sorted by power desc
      harmonic_phases  — dict {freq_idx: phase_radians} for top harmonics
      spectral_entropy — −Σ p·log(p)  (low = compressed → pre-spike)
      harmonic_compression — ratio of top-N power to total power (high = structured)
      reconstructed    — top-N harmonic reconstruction array (same length as signal)
      energy_in_harmonics — fraction of total energy in top harmonics
    """
    nh  = n_harmonics or _ENG_HARMONICS
    ...
    # positive frequency mask (exclude DC + Nyquist artifacts)
    pos = (freqs > 0) & (freqs <= 0.5)

    # spectral entropy
    p_norm = power[pos] / (np.sum(power[pos]) + 1e-20)
    entropy = float(-np.sum(p_norm * np.log(p_norm + 1e-12)))

    # dominant frequency
    pw_pos   = power.copy(); pw_pos[~pos] = 0.0
    dom_idx  = int(np.argmax(pw_pos))
    dom_freq = float(freqs[dom_idx])
    dom_per  = max(4, int(round(1.0 / dom_freq))) if dom_freq > 0 else 20

    # top harmonics (indices of highest power in positive range)
    pos_idxs = np.where(pos)[0]
    if len(pos_idxs) < nh:
        nh = max(1, len(pos_idxs))
    top_idxs = pos_idxs[np.argsort(power[pos_idxs])[-nh:]]

    harmonics = []
    for idx in sorted(top_idxs, key=lambda i: -power[i]):
        f    = float(freqs[idx])
        per  = max(2, int(round(1.0 / f))) if f > 0 else 1
        pw   = float(power[idx])
        amp  = float(2.0 * abs(fft_v[idx]) / n)   # single-sided
        ph   = float(np.angle(fft_v[idx]))
        harmonics.append({'freq': f, 'period': per, 'power': pw,
                          'amplitude': amp, 'phase': ph, 'idx': idx})

    # harmonic compression ratio (top-N / total)
    top_pow  = sum(h['power'] for h in harmonics)
    tot_pow  = float(np.sum(power[pos])) + 1e-20
    compression = top_pow / tot_pow

    # reconstruct from top harmonics
    fft_r = np.zeros(len(fft_v), dtype=complex)
    for h in harmonics:
        fft_r[h['idx']] = fft_v[h['idx']]
    reconstructed = np.fft.irfft(fft_r, n=n)

    # energy in harmonics
    total_energy = float(np.sum(sig ** 2)) + 1e-20
    harm_energy  = float(np.sum(reconstructed ** 2))
    e_in_harm    = min(1.0, harm_energy / total_energy)

    return {
        'dominant_period'     : dom_per,
        'dominant_freq'       : dom_freq,
        'power_spectrum'      : power[pos],
        'freqs'               : freqs[pos],
        'top_harmonics'       : harmonics,
        'spectral_entropy'    : entropy,
        'harmonic_compression': compression,
        'reconstructed'       : reconstructed,
        'energy_in_harmonics' : e_in_harm,
        'n_bars'              : n,
    }


# ─────────────────────────────────────────────────────────────────────────────
#  4. PHASE ALIGNMENT (timing trigger)
#
#  Idea: if all dominant harmonics are aligning in phase (heading to the same
#  direction simultaneously), a spike is near.  We measure this as the std-dev
#  of the instantaneous phase differences.  Low std → tight alignment → pre-spike.
# ─────────────────────────────────────────────────────────────────────────────

def _phase_alignment_score(signal, window=None):
    """
    Compute instantaneous phase alignment quality.

    Method:
      1. Compute analytic signal via FFT (keep only positive freq → IFFT)
      2. Extract instantaneous phase per bar
      3. Rolling std of phase differences over `window` bars

    Returns dict:
      phase_alignment   — 0–1 score (1 = perfectly aligned → spike imminent)
      phase_std         — raw std of phase diffs (lower = better)
      phase_arr         — instantaneous phase array
      phase_diff_arr    — bar-by-bar phase changes
      compress_trend    — True if std_phase_diffs is declining (compression building)
    """
    w   = window or _ENG_PHASE_WINDOW
    sig = np.asarray(signal, dtype=np.float64)
    n   = len(sig)

    if n < _ENG_MIN_BARS:
        return {'phase_alignment': 0.5, 'phase_std': 1.0,
                'phase_arr': np.zeros(n), 'phase_diff_arr': np.zeros(n),
                'compress_trend': False}

    # analytic signal via one-sided FFT
    fft_v = np.fft.fft(sig)
    # zero negative frequencies → analytic
    h = np.zeros(n)
    if n % 2 == 0:
        h[0] = 1; h[n // 2] = 1; h[1: n // 2] = 2
    else:
        h[0] = 1; h[1: (n + 1) // 2] = 2
    analytic = np.fft.ifft(fft_v * h)
    phase    = np.angle(analytic)

    # phase differences
    pd = np.diff(phase)
    # unwrap to avoid -π/+π jumps giving spurious std
    pd = np.angle(np.exp(1j * pd))   # wrap to [-π, π]

    # rolling std of phase diffs
    roll_std  = np.full(n, 1.0)
    for i in range(w, n):
        roll_std[i] = float(np.std(pd[i - w: i]) + 1e-12)

    # current phase alignment score  (lower std → higher alignment)
    cur_std    = float(roll_std[-1])
    alignment  = float(1.0 / (1.0 + cur_std))   # mapped to (0,1)

    # compression trend: std is decreasing over last 10 bars?
    if n >= w + 10:
        compress_trend = bool(roll_std[-1] < roll_std[-10])
    else:
        compress_trend = False

    # pad phase_diff to same length
    pd_full = np.concatenate([[0.0], pd])

    return {
        'phase_alignment'  : round(alignment, 4),
        'phase_std'        : round(cur_std,   4),
        'phase_arr'        : phase,
        'phase_diff_arr'   : pd_full,
        'compress_trend'   : compress_trend,
        'roll_std_arr'     : roll_std,
    }


# ─────────────────────────────────────────────────────────────────────────────
#  5. SINGLE-TF ENERGY SPIKE ANALYSIS  (combines all primitives)
# ─────────────────────────────────────────────────────────────────────────────

def _energy_spike_single_tf(close_arr, vol_arr, window=None):
    """
    Full energy + harmonic + phase analysis for a single timeframe.

    Returns dict:
      energy_*        — from _compute_rolling_energy()
      fft_*           — from _fft_harmonic_decomposition()
      phase_*         — from _phase_alignment_score()
      energy_score    — 0–100 composite for this TF
      pre_spike_flag  — True when all three conditions align
      spike_condition — dict of individual condition booleans
      harmonic_forecast_levels  — list of (bars_ahead, price_level) tuples
    """
    cl  = np.asarray(close_arr, dtype=np.float64)
    vl  = np.asarray(vol_arr,   dtype=np.float64)
    n   = len(cl)
    ...
    result = {
        'n_bars'               : n,
        'energy_score'         : 0.0,
        'pre_spike_flag'       : False,
        'spike_condition'      : {},
        'harmonic_forecast_levels': [],
        'error'                : None,
    }

    if n < _ENG_MIN_BARS:
        result['error'] = f'insufficient bars ({n} < {_ENG_MIN_BARS})'
        return result

    try:
        # 1. Signal
        sig = _build_energy_signal(cl, vl, smooth=3)

        # 2. Energy
        eng = _compute_rolling_energy(sig, window=w)
        result.update({f'energy_{k}': v for k, v in eng.items()})

        # 3. FFT
        fft = _fft_harmonic_decomposition(sig, n_harmonics=_ENG_HARMONICS)
        if fft:
            result.update({f'fft_{k}': v for k, v in fft.items()
                           if k not in ('power_spectrum', 'freqs',
                                        'reconstructed', 'top_harmonics')})
            result['fft_reconstructed'] = fft['reconstructed']
            result['fft_top_harmonics'] = fft['top_harmonics']
            result['fft_power_spectrum'] = fft['power_spectrum']

        # 4. Phase
        pha = _phase_alignment_score(sig, window=_ENG_PHASE_WINDOW)
        result.update({f'phase_{k}': v for k, v in pha.items()
                       if k not in ('phase_arr', 'phase_diff_arr', 'roll_std_arr')})
        result['phase_alignment'] = pha['phase_alignment']
        result['phase_std']       = pha['phase_std']
        result['phase_compress_trend'] = pha['compress_trend']

        # ── Spike condition logic ──────────────────────────────────
        # Condition A: energy growing (norm_growth > 0 and roll_E > window median)
        cond_A = (eng['norm_growth_now'] > 0.0 and
                  eng['roll_E_now'] > np.median(eng['energy_roll'][w:] + 1e-20))

        # Condition B: spectral entropy LOW (harmonics compressed = pre-release)
        ent_now   = fft['spectral_entropy'] if fft else 5.0
        ent_thresh = float(np.log(min(_ENG_ENTROPY_BINS, n // 4 + 1)))  # natural reference
        cond_B    = ent_now < (ent_thresh * 0.70) if fft else False

        # Condition C: phase tightly aligned (std low, trending tighter)
        cond_C    = (pha['phase_alignment'] > 0.40 and pha['compress_trend'])

        # Condition D: harmonic compression high (>50% power in top harmonics)
        cond_D    = (fft['harmonic_compression'] > 0.50) if fft else False

        # Condition E: FFT reconstructed below mean (at trough of dominant cycle)
        if fft and fft['reconstructed'] is not None:
            rec_now  = float(fft['reconstructed'][-1])
            rec_mean = float(np.mean(fft['reconstructed']))
            cond_E   = rec_now < rec_mean   # oscillation at trough → bounce near
        else:
            cond_E = False

        result['spike_condition'] = {
            'energy_growing'       : cond_A,
            'harmonics_compressed' : cond_B,
            'phase_aligned'        : cond_C,
            'spectral_compression' : cond_D,
            'at_harmonic_trough'   : cond_E,
        }
        n_conds = sum([cond_A, cond_B, cond_C, cond_D, cond_E])
        result['pre_spike_flag'] = n_conds >= 3

        # ── Energy score 0–100 ─────────────────────────────────────
        score = 0.0

        # Energy growth momentum (max 30 pts)
        ng = min(1.0, max(0.0, eng['norm_growth_now'] * 10.0))
        score += ng * 30.0

        # Phase alignment (max 25 pts)
        score += pha['phase_alignment'] * 25.0

        # Spectral compression  (max 20 pts)
        comp_score = fft['harmonic_compression'] if fft else 0.0
        score += min(20.0, comp_score * 20.0)

        # Entropy inverse  (max 15 pts)  — lower entropy = higher score
        if fft:
            entropy_score = max(0.0, 1.0 - ent_now / (ent_thresh + 1e-3))
            score += entropy_score * 15.0

        # At harmonic trough bonus (max 10 pts)
        if cond_E:
            score += 10.0

        result['energy_score'] = round(min(100.0, score), 1)

        # ── Harmonic price forecast ────────────────────────────────
        # Project current price forward using dominant + top harmonics
        if fft and len(fft['top_harmonics']) >= 2:
            cp   = float(cl[-1])
            t    = np.arange(n, dtype=np.float64)
            # linear trend (reattach)
            p1   = np.polyfit(np.arange(n), cl, 1)
            trend_slope = float(p1[0])

            forecasts = []
            for horizon in [5, 12, 24, 48]:   # bars ahead
                price_proj = cp + trend_slope * horizon
                # add harmonic contributions
                for h in fft['top_harmonics'][:4]:
                    f_h   = h['freq']
                    amp_h = h['amplitude']
                    ph_h  = h['phase']
                    # energy weight: amplitude × compression
                    t_fwd = float(n - 1 + horizon)
                    wave  = amp_h * np.sin(2.0 * np.pi * f_h * t_fwd + ph_h)
                    price_proj += wave
                forecasts.append((horizon, round(float(price_proj), 8)))

            result['harmonic_forecast_levels'] = forecasts
...
# ─────────────────────────────────────────────────────────────────────────────
#  6. MTF ENERGY RESONANCE ANALYSIS
#  Runs energy analysis across multiple timeframes and measures cross-TF
#  alignment (resonance).  High resonance = spike imminent across scales.
# ─────────────────────────────────────────────────────────────────────────────

def _energy_mtf_resonance(pair, tf_cache, sel_detail):
    """
    Compute per-TF energy scores + overall resonance score
    using already-cached kline data from _tf_cache (no extra API calls).

    Falls back to fetching from Binance if a TF is not in cache.

    Returns dict:
      tf_results       — dict {tf_label: energy_result_dict}
      resonance_score  — 0–1  (cross-TF energy × phase × entropy alignment)
      resonance_label  — '🔴 noise' | '🟡 build-up' | '🟢 pre-spike' | '🔥 IMMINENT'
      weighted_energy_score — 0–100 weighted average energy score
      n_tfs_pre_spike  — count of TFs where pre_spike_flag is True
      mtf_spike_prob   — sigmoid-normalised spike probability 0–1
    """
    tf_results = {}
    e_scores   = []
    phase_aligns = []
    ...
    for tf_label, interval, weight, horizon_bars, horizon_lbl in _ENG_TF_CONFIGS:
        # try to get OHLCV from tf_cache first
        cached = (tf_cache.get(pair) or {}).get(tf_label)
        cl_arr = vl_arr = None

        if cached:
            # We only have summary scalars in cache, not raw arrays.
            # Use the energy_score if already computed, else skip.
            # For full analysis, fall through to API fetch.
            pass

        # fetch from Binance (reuse global `trader` object from main file)
        try:
            klines = trader.client.get_klines(
                symbol=pair, interval=interval, limit=300)
            if len(klines) >= _ENG_MIN_BARS:
                cl_arr = np.array([float(k[4]) for k in klines], dtype=np.float64)
                vl_arr = np.array([float(k[5]) for k in klines], dtype=np.float64)
        except Exception:
            pass

        if cl_arr is None or len(cl_arr) < _ENG_MIN_BARS:
            continue

        r = _energy_spike_single_tf(cl_arr, vl_arr)
        r['tf_label']     = tf_label
        r['weight']       = weight
        r['horizon_bars'] = horizon_bars
        r['horizon_lbl']  = horizon_lbl
        tf_results[tf_label] = r

        if r.get('error'):
            continue

        e_scores.append(r['energy_score'] * weight)
        phase_aligns.append(r.get('phase_alignment', 0.5) * weight)
        entropies.append(r.get('fft_spectral_entropy', 3.0))
        comp = r.get('fft_harmonic_compression', 0.3)
        compressions.append(comp * weight)

    total_w = sum(w for _, _, w, _, _ in _ENG_TF_CONFIGS if w > 0)

    # weighted average energy score
    w_energy = sum(e_scores) / total_w if total_w > 0 else 0.0

    # resonance components
    e_component   = float(np.mean(e_scores))          / 100.0   if e_scores    else 0.5
    ph_component  = float(np.mean(phase_aligns))                 if phase_aligns else 0.5
    ent_ref       = np.log(max(16, _ENG_ENTROPY_BINS * 0.5))
    ent_component = 1.0 / (1.0 + float(np.mean(entropies))) if entropies else 0.5
    comp_component= float(np.mean(compressions))                 if compressions else 0.3

    # resonance = product of normalised components
    resonance = (e_component * 0.35 +
                 ph_component * 0.30 +
                 ent_component * 0.20 +
                 comp_component * 0.15)
    resonance = min(1.0, max(0.0, float(resonance)))

    # count TFs with pre_spike_flag
    n_pre = sum(1 for r in tf_results.values() if r.get('pre_spike_flag', False))

    # sigmoid-normalised spike probability
    raw_input = (resonance - _ENG_RESONANCE_THOLD) * 8.0
    spike_prob = float(1.0 / (1.0 + np.exp(-raw_input)))

    # label
    if spike_prob < 0.30:
        label = '🔴 noise — no structure'
    elif spike_prob < 0.55:
        label = '🟡 build-up — watch energy growth'
    elif spike_prob < 0.75:
        label = '🟢 pre-spike — compression + alignment'
    else:
        label = '🔥 IMMINENT — full resonance'

    return {
        'tf_results'              : tf_results,
        'resonance_score'         : round(resonance, 4),
        'resonance_label'         : label,
        'weighted_energy_score'   : round(w_energy, 1),
        'n_tfs_pre_spike'         : n_pre,
        'mtf_spike_prob'          : round(spike_prob, 4),
        'phase_mean'              : round(float(np.mean(phase_aligns)) if phase_aligns else 0.5, 4),
        'entropy_mean'            : round(float(np.mean(entropies))    if entropies    else 3.0, 4),
        'compression_mean'        : round(float(np.mean(compressions)) if compressions else 0.3, 4),
        'e_scores_by_tf'          : {tf: r['energy_score'] for tf, r in tf_results.items()},
    }


# ─────────────────────────────────────────────────────────────────────────────
#  7. ML SPIKE CLASSIFIER
#
#  Features built from energy + phase + spectral data per TF.
#  Target: next-bar energy growth > threshold (spike label).
#  Ensemble: RandomForest + GradientBoosting + Ridge
# ─────────────────────────────────────────────────────────────────────────────

def _build_energy_ml_features(close_arr, vol_arr, high_arr, low_arr):
    """
    Build feature matrix for ML spike classifier.
    Each row = one bar.  Targets = next-bar normalised energy growth.

    Features (31 total):
      [0]  t_norm             — time index / n
      [1]  energy_raw         — E(t) = signal²
      [2]  energy_roll        — rolling mean energy (20-bar)
      [3]  norm_growth        — energy growth normalised
      [4]  energy_accel       — diff of norm_growth (2nd derivative)
      [5]  phase_align_roll   — rolling phase alignment
      [6]  phase_std_roll     — rolling phase std
      [7]  spectral_ent       — per-bar spectral entropy estimate
      [8]  harmonic_comp      — harmonic compression ratio
      [9]  fft_osc_pos        — 1 if FFT oscillation < mean (at trough)
      [10] log_ret1           — log return lag 1
      [11] log_ret3           — log return lag 3
      [12] log_ret5           — log return lag 5
      [13] vol_norm           — vol / rolling_20_mean_vol
      [14] vol_surge          — 1 if vol > 2× rolling avg
      [15] rsi_norm           — RSI-14 / 100
      [16] atr_norm           — ATR-14 / close
      [17] bb_pos             — Bollinger position (0=lower, 1=upper)
      [18] close_vs_ema20     — (close - ema20) / ema20
      [19] close_vs_ema50     — (close - ema50) / ema50
      [20] hl_range           — (high - low) / close  (bar volatility)
      [21] wick_bull_ratio    — (close-low) / (high-low+ε)
      [22] energy_percentile  — % rank of current energy vs 100-bar window
      [23] vol_energy_corr    — rolling 20-bar correlation(vol, energy)
      [24] dom_period_norm    — FFT dominant period / n
      [25] sq_signal          — raw signal squared (direct energy)
      [26] sq_signal_z        — z-score of sq_signal vs 50-bar window
      [27] phase_cos          — cos(rolling_phase)
      [28] phase_sin          — sin(rolling_phase)
      [29] energy_ratio_hl    — energy / (hl_range² + ε)
      [30] n_spike_conds      — count of 5 spike conditions True
    """
    cl  = np.asarray(close_arr, dtype=np.float64)
    vl  = np.asarray(vol_arr,   dtype=np.float64)
    hi  = np.asarray(high_arr,  dtype=np.float64)
    ...
    if n < _ENG_MIN_BARS + 20:
        return None, None

    sig = _build_energy_signal(cl, vl)
    eng = _compute_rolling_energy(sig, window=20)
    pha = _phase_alignment_score(sig, window=_ENG_PHASE_WINDOW)

    # FFT on rolling 64-bar windows for per-bar spectral features
    ent_ts = np.zeros(n)
    comp_ts = np.zeros(n)
    fft_osc_ts = np.zeros(n)
    dom_per_ts = np.zeros(n)
    fft_w = 64
    for i in range(fft_w, n):
        seg = sig[i - fft_w: i]
        f   = _fft_harmonic_decomposition(seg, n_harmonics=4)
        if f:
            ent_ts[i]     = f['spectral_entropy']
            comp_ts[i]    = f['harmonic_compression']
            rec = f['reconstructed']
            fft_osc_ts[i] = 1.0 if float(rec[-1]) < float(np.mean(rec)) else 0.0
            dom_per_ts[i] = f['dominant_period'] / n

    # rolling phase alignment (window already in pha arrays, use roll_std_arr)
    ph_align_ts = np.zeros(n)
    ph_std_ts   = np.zeros(n)
    for i in range(_ENG_PHASE_WINDOW, n):
        seg_s = sig[i - _ENG_PHASE_WINDOW: i]
        p_out = _phase_alignment_score(seg_s, window=_ENG_PHASE_WINDOW // 2)
        ph_align_ts[i] = p_out['phase_alignment']
        ph_std_ts[i]   = p_out['phase_std']

    # log returns
    lr1 = np.zeros(n); lr3 = np.zeros(n); lr5 = np.zeros(n)
    for i in range(1, n): lr1[i] = np.log(cl[i] / (cl[i-1]+1e-20))
    for i in range(3, n): lr3[i] = np.log(cl[i] / (cl[i-3]+1e-20))
    for i in range(5, n): lr5[i] = np.log(cl[i] / (cl[i-5]+1e-20))

    # volume norm
    vol_roll20 = np.zeros(n)
    for i in range(20, n):
        vol_roll20[i] = np.mean(vl[i-20:i]) + 1e-20
    vol_norm = np.zeros(n)
    vol_norm[20:] = vl[20:] / vol_roll20[20:]
    vol_surge = (vol_norm > 2.0).astype(float)

    # EMAs
    def _ema(a, p):
        e = np.zeros(n); a2 = float(2.0/(p+1))
        e[p] = np.mean(a[:p])
        for i in range(p+1, n): e[i] = a[i]*a2 + e[i-1]*(1-a2)
        return e
    ema20 = _ema(cl, 20); ema50 = _ema(cl, 50)
    c_vs_e20 = (cl - ema20) / (ema20 + 1e-20)
    c_vs_e50 = (cl - ema50) / (ema50 + 1e-20)

    # RSI-14
    try:
        import talib as ta
        rsi = ta.RSI(cl, timeperiod=14) / 100.0
        atr = ta.ATR(hi, lo, cl, timeperiod=14) / (cl + 1e-20)
        rsi = np.nan_to_num(rsi); atr = np.nan_to_num(atr)
    except Exception:
        rsi = np.full(n, 0.5); atr = np.full(n, 0.02)

    # Bollinger
    rm20 = np.zeros(n); rs20 = np.zeros(n)
    for i in range(20, n):
        rm20[i] = np.mean(cl[i-20:i]); rs20[i] = np.std(cl[i-20:i]) + 1e-10
    bb_pos = (cl - (rm20 - 2*rs20)) / (4*rs20 + 1e-20)

    # HL range & wick
    hl_range = (hi - lo) / (cl + 1e-20)
    wick_bull = (cl - lo) / (hi - lo + 1e-20)

    # energy percentile (last 100 bars rolling)
    e_pct = np.zeros(n)
    for i in range(100, n):
        w100 = eng['energy_raw'][i-100:i]
        e_pct[i] = float(np.sum(w100 < eng['energy_raw'][i])) / 100.0

    # rolling corr(vol, energy) 20-bar
    ve_corr = np.zeros(n)
    for i in range(20, n):
        v_seg = vl[i-20:i]; e_seg = eng['energy_raw'][i-20:i]
        if np.std(v_seg) > 1e-12 and np.std(e_seg) > 1e-12:
            ve_corr[i] = float(np.corrcoef(v_seg, e_seg)[0,1])

    # sq_signal z-score 50-bar
    sq_sig   = sig ** 2
    sq_z     = np.zeros(n)
    for i in range(50, n):
        seg = sq_sig[i-50:i]
        sq_z[i] = (sq_sig[i] - np.mean(seg)) / (np.std(seg) + 1e-10)

    # phase cos/sin (instantaneous)
    ph_arr = pha.get('phase_arr', np.zeros(n))
    if len(ph_arr) != n:
        ph_arr = np.zeros(n)
    ph_cos = np.cos(ph_arr); ph_sin = np.sin(ph_arr)

    # energy / hl_range² 
    e_hl_ratio = eng['energy_raw'] / (hl_range**2 + 1e-20)

    # spike condition count per bar (simplified)
    ng_ts   = eng['norm_growth']
    n_conds = ((ng_ts > 0).astype(float) +
               (ent_ts  < 2.5).astype(float) +
               (ph_align_ts > 0.40).astype(float) +
               (comp_ts > 0.50).astype(float) +
               fft_osc_ts)

    # stack features
    X = np.column_stack([
        np.arange(n) / n,           # t_norm
        eng['energy_raw'],
        eng['energy_roll'],
        eng['norm_growth'],
        np.concatenate([[0.0], np.diff(eng['norm_growth'])]),  # accel
        ph_align_ts,
        ph_std_ts,
        ent_ts,
        comp_ts,
    ...
    # Target: next-bar norm_growth  (predict energy momentum direction)
    y = np.roll(eng['norm_growth'], -1)
    y[-1] = 0.0   # last bar has no next-bar target

    return X, y


def _energy_ml_forecast(pair, close_arr, vol_arr, high_arr, low_arr,
                         current_price, horizons_bars=(5, 15, 30, 60)):
    """
    Train ML ensemble on energy features, forecast spike probability
    and price targets at multiple horizons.

    Returns
    -------
    dict:
      spike_prob         — 0–1 probability of significant energy spike
      spike_prob_label   — descriptive label
      model_scores       — {model_name: {'mae', 'r2', 'dir_acc'}}
      indiv_probs        — {model_name: spike_prob}
      ensemble_prob      — ensemble weighted spike probability
      price_forecasts    — [(horizon_bars, price_level, pct_change)]
      feature_importance — top 10 feature importances from RF (if available)
      error              — None or error string
    """
    result = {
        'spike_prob'         : 0.5,
        'spike_prob_label'   : '🟡 uncertain',
        'model_scores'       : {},
        'indiv_probs'        : {},
        'ensemble_prob'      : 0.5,
        'price_forecasts'    : [],
    ...
    try:
        X, y = _build_energy_ml_features(close_arr, vol_arr, high_arr, low_arr)
        if X is None:
            result['error'] = 'insufficient data for ML'
            return result

        n = len(X)
        split = int(n * _ENG_ML_TRAIN_RATIO)
        if split < 40 or n - split < 10:
            result['error'] = 'too few bars for train/test split'
            return result
        ...
        for name, mdl in models.items():
            try:
                ...
                ss_res  = float(np.sum((y_hat - y_te)**2))
                ss_tot  = float(np.sum((y_te - y_te.mean())**2))
                r2      = float(1.0 - ss_res / (ss_tot + 1e-20))
                dir_acc = float(np.mean(np.sign(y_hat) == np.sign(y_te)))

                scores_all[name] = {'mae': mae, 'rmse': rmse,
                                    'r2': r2, 'dir_acc': dir_acc}

                # predict next bar
                x_last = X[-1].reshape(1, -1)
                pred_v = float(mdl.predict(x_last)[0])
                pred_last[name] = pred_v

            except Exception as ex:
                scores_all[name] = {'mae': 999, 'rmse': 999,
                                    'r2': -1, 'dir_acc': 0.5}
                pred_last[name] = 0.0

        result['model_scores'] = scores_all

        # ── Spike probability conversion ──────────────────────────
        # predicted norm_growth → spike prob via sigmoid
        # norm_growth > 0 = energy growing = bullish
        # high positive value = high spike probability
        def _to_prob(ng_val):
            return float(1.0 / (1.0 + np.exp(-ng_val * 15.0)))

        indiv_probs = {nm: _to_prob(v) for nm, v in pred_last.items()}
        result['indiv_probs'] = indiv_probs

        # MAE-weighted ensemble
        inv_mae = {nm: 1.0 / (scores_all[nm]['mae'] + 1e-10)
                   for nm in scores_all}
        tot_inv = sum(inv_mae.values())
        ens_prob = sum(indiv_probs[nm] * inv_mae[nm] / tot_inv
                       for nm in indiv_probs)
        ens_prob  = min(1.0, max(0.0, float(ens_prob)))
        result['ensemble_prob'] = round(ens_prob, 4)
        result['spike_prob']    = round(ens_prob, 4)

        # label
        if   ens_prob < 0.30:  lbl = '🔴 0.0–0.3 noise — no trade'
        elif ens_prob < 0.55:  lbl = '🟡 0.3–0.5 build-up — watch'
        elif ens_prob < 0.75:  lbl = '🟢 0.5–0.7 pre-spike — prepare entry'
        else:                  lbl = '🔥 0.7–1.0 SPIKE IMMINENT — fast profits'
        result['spike_prob_label'] = lbl

        # ── Price forecasts at multiple horizons ──────────────────
        # Use RF model (best for non-linear) if available
        ...
        try:
            ...
            for h_bars in horizons_bars:
                # simple walk-forward: repeat last feature row
                # (energy growth persists over short horizon)
                x_pred = X_fwd.reshape(1, -1)
                ng_pred = float(rf_mdl.predict(x_pred)[0])
                # convert energy growth to price change estimate
                # norm_growth ≈ d(E)/E  ≈ 2 × d(signal)/signal
                # for small changes: price_change ≈ ng_pred × ATR_scale × horizon
                atr_scale = float(np.std(close_arr[-20:]) + 1e-20) / cp
                dp = ng_pred * atr_scale * min(h_bars, 20) * cp * 0.5
                fc_price = round(cp + dp, 8)
                pct      = round((fc_price - cp) / cp * 100.0, 3)
                forecasts.append((h_bars, fc_price, pct))
            result['price_forecasts'] = forecasts
        except Exception:
            pass

        # ── Feature importance ────────────────────────────────────
        FEAT_NAMES = [
            't_norm','energy_raw','energy_roll','norm_growth','energy_accel',
            'phase_align','phase_std','spectral_ent','harmonic_comp',
            'fft_osc_pos','lr1','lr3','lr5','vol_norm','vol_surge',
            'rsi','atr','bb_pos','c_vs_e20','c_vs_e50',
            'hl_range','wick_bull','e_pct','ve_corr','dom_per_norm',
            'sq_sig','sq_z','ph_cos','ph_sin','e_hl_ratio','n_conds',
        ]
        try:
            rf_step = models['RF'].named_steps['m']
            imp = rf_step.feature_importances_
            top10 = sorted(zip(FEAT_NAMES, imp),
                           key=lambda x: -x[1])[:10]
            result['feature_importance'] = [(nm, round(float(v), 4))
                                             for nm, v in top10]
        except Exception:
            pass
...
# ─────────────────────────────────────────────────────────────────────────────
#  8. ENERGY PRICE FORECAST ENGINE
#  Harmonic + trend + energy-weighted projection at STF / MTF / HTF horizons
# ─────────────────────────────────────────────────────────────────────────────

def _energy_price_forecast(close_arr, vol_arr, current_price):
    """
    Project price at STF (~30m), MTF (~2h), HTF (~8h) using:
      1. Harmonic reconstruction (top-4 FFT components)
      2. Linear trend extension
      3. Energy-growth momentum scaling
      4. Fibonacci-aligned target snapping (optional)

    Returns dict:
      stf_forecast  — (price, pct_change, horizon_desc) for ~30m
      mtf_forecast  — for ~2h
      htf_forecast  — for ~8h
      harmonic_band_upper — upper harmonic envelope estimate
      harmonic_band_lower — lower harmonic envelope estimate
    """
    cl  = np.asarray(close_arr, dtype=np.float64)
    vl  = np.asarray(vol_arr,   dtype=np.float64)
    n   = len(cl)
    ...
    result = {
        ...
        'mtf_forecast': None,
        'htf_forecast': None,
        'harmonic_band_upper': None,
        'harmonic_band_lower': None,
    }
    ...
    try:
        sig  = _build_energy_signal(cl, vl)
        eng  = _compute_rolling_energy(sig, window=20)
        fft  = _fft_harmonic_decomposition(sig, n_harmonics=6)

        if fft is None:
            return result

        # Linear trend on raw price
        t = np.arange(n, dtype=np.float64)
        p = np.polyfit(t, cl, 1)
        slope     = float(p[0])
        intercept = float(p[1])

        # energy momentum factor (amplify trend in high-energy states)
        ng = eng['norm_growth_now']
        # clip to ±0.5 to avoid extreme values
        ng_clip = max(-0.5, min(0.5, ng))
        e_factor = 1.0 + ng_clip   # range [0.5, 1.5]

        # horizon mapping (assumes 1m data; adjust if different TF)
        horizons = {
            'stf': (30,   '~30m'),
            'mtf': (120,  '~2h'),
            'htf': (480,  '~8h'),
        ...
        for key, (h_bars, h_desc) in horizons.items():
            t_fwd = float(n - 1 + h_bars)

            # trend component
            trend_fc = slope * t_fwd + intercept

            # harmonic components
            harm_fc = 0.0
            for hm in fft['top_harmonics'][:4]:
                f_h  = hm['freq']
                amp  = hm['amplitude']
                ph   = hm['phase']
                # decay amplitude for longer horizons (energy dissipation)
                decay = np.exp(-f_h * h_bars * 0.05)
                harm_fc += amp * decay * np.sin(2.0 * np.pi * f_h * t_fwd + ph)

            raw_fc = trend_fc + harm_fc * e_factor
            # bound forecast to ±5% of current price for STF, ±15% MTF, ±30% HTF
            bounds = {'stf': 0.05, 'mtf': 0.15, 'htf': 0.30}
            b = bounds[key]
            raw_fc = float(np.clip(raw_fc, cp * (1.0 - b), cp * (1.0 + b)))
            pct_fc = round((raw_fc - cp) / cp * 100.0, 3)

            result[f'{key}_forecast'] = (round(raw_fc, 8), pct_fc, h_desc)

        # Harmonic envelope (amplitude envelope from all harmonics)
        total_amp = sum(hm['amplitude'] for hm in fft['top_harmonics'][:4])
        result['harmonic_band_upper'] = round(cp * (1.0 + total_amp), 8)
        result['harmonic_band_lower'] = round(cp * (1.0 - total_amp), 8)
...
# ─────────────────────────────────────────────────────────────────────────────
#  9. MAIN ENTRY POINT — compute_energy_spike_analysis()
# ─────────────────────────────────────────────────────────────────────────────

def compute_energy_spike_analysis(pair, current_price, sel_detail, tf_cache):
    """
    Full energy/FFT/harmonic/ML/forecast analysis for the MTF winner.

    Parameters
    ----------
    pair          : str    — e.g. 'BTCUSDC'
    current_price : float
    sel_detail    : dict  — from main loop sel_detail[pair]
    tf_cache      : dict  — from _tf_cache[pair]

    Returns
    -------
    dict with all sub-results:
      1m_result      — single-TF energy analysis on 1m data
      mtf_resonance  — cross-TF resonance result
      ml_result      — ML ensemble spike probability
      price_forecast — STF/MTF/HTF harmonic price forecasts
      combined_score — final combined spike score 0–100
      spike_verdict  — human-readable final verdict
    """
    result = {
        '1m_result'       : None,
        'mtf_resonance'   : None,
    ...
    try:
        # ── 1. Get 1m data (reuse from sel_detail if available) ───
        d          = sel_detail.get(pair, {})
        close_1m   = d.get('close_arr')
        high_1m    = d.get('high_arr')
        low_1m     = d.get('low_arr')
        vol_1m     = None  # volume not stored in sel_detail by default

        # Fetch fresh 1m data
        try:
            k1m = trader.client.get_klines(
                symbol=pair, interval='1m', limit=500)
            close_1m = np.array([float(k[4]) for k in k1m], dtype=np.float64)
            high_1m  = np.array([float(k[2]) for k in k1m], dtype=np.float64)
            low_1m   = np.array([float(k[3]) for k in k1m], dtype=np.float64)
            vol_1m   = np.array([float(k[5]) for k in k1m], dtype=np.float64)
        except Exception as ex:
            result['error'] = f'1m fetch failed: {ex}'
            return result

        if len(close_1m) < _ENG_MIN_BARS:
            result['error'] = 'insufficient 1m bars'
            return result

        # ── 2. Single-TF energy analysis on 1m ───────────────────
        r1m = _energy_spike_single_tf(close_1m, vol_1m)
        result['1m_result'] = r1m

        # ── 3. MTF resonance ──────────────────────────────────────
        mtf_res = _energy_mtf_resonance(pair, tf_cache, sel_detail)
        result['mtf_resonance'] = mtf_res

        # ── 4. ML spike probability ───────────────────────────────
        ml_r = _energy_ml_forecast(pair, close_1m, vol_1m, high_1m, low_1m,
                                    current_price,
                                    horizons_bars=(5, 15, 30, 60))
        result['ml_result'] = ml_r

        # ── 5. Harmonic price forecast ────────────────────────────
        pf = _energy_price_forecast(close_1m, vol_1m, current_price)
        result['price_forecast'] = pf

        # ── 6. Combined spike score (0–100) ───────────────────────
        #   40 pts  ML ensemble spike probability
        #   30 pts  MTF resonance score
        #   20 pts  1m energy score
        #   10 pts  pre-spike condition bonuses
        sc = 0.0
        sc += ml_r.get('ensemble_prob',        0.5) * 40.0
        sc += mtf_res.get('resonance_score',   0.5) * 30.0
        sc += r1m.get('energy_score',          0.0) * 0.20
        # bonus: pre-spike flag on 1m
        if r1m.get('pre_spike_flag', False):
            sc += 5.0
        # bonus: multiple TFs pre-spike
        sc += min(5.0, mtf_res.get('n_tfs_pre_spike', 0) * 2.5)

        result['combined_score'] = round(min(100.0, max(0.0, sc)), 1)

        # ── 7. Spike verdict ──────────────────────────────────────
        sp = result['combined_score']
        if   sp >= 80:  verdict = '🔥 SPIKE IMMINENT — full resonance confirmed'
        elif sp >= 65:  verdict = '🟢 PRE-SPIKE ZONE — compression + alignment'
        elif sp >= 45:  verdict = '🟡 BUILD-UP — energy growing, watch carefully'
        else:           verdict = '🔴 NOISE — insufficient structure for spike'
        result['spike_verdict'] = verdict
...
def print_energy_spike_report(eng_result, label_map):
    """
    Print the full Energy / Harmonic / ML spike analysis report.
    Call this AFTER print_phi_reversal_block() in the main while-True loop.

    Usage:
        eng_result = compute_energy_spike_analysis(
            best_symbol, current_price, sel_detail, _tf_cache)
        print_energy_spike_report(eng_result, label_map)
    """
    if not eng_result:
        return

    pair = eng_result.get('pair', '?')
    lbl  = label_map.get(pair, pair.replace('USDC', ''))
    cp   = eng_result.get('current_price') or 0.0
    w    = 68

    def pf(v):
        if v is None: return '—'
        return f'{v:.6f}' if (v is not None and abs(v) < 1) else f'{v:.4f}'

    def pp(v, base=None):
        if v is None: return '—'
        b = base or cp
        pct = (v - b) / (b + 1e-20) * 100.0
        arrow = '▲' if pct >= 0 else '▼'
        return f'{pf(v)}  ({arrow}{pct:+.2f}%)'

    print(f'\n  {"═"*w}')
    print(f'  ⚡  ENERGY · HARMONIC · ML SPIKE ENGINE  v17  ·  {lbl}')
    print(f'  {"═"*w}')
    ...
    # ── 1. Combined verdict ───────────────────────────────────────
    combined = eng_result.get('combined_score', 0.0)
    verdict  = eng_result.get('spike_verdict', '—')
    bar_n    = int(combined / 5)
    comb_bar = '█' * bar_n + '░' * (20 - bar_n)
    print(f'\n  ▶  COMBINED SPIKE SCORE: {combined:.1f}/100  [{comb_bar}]')
    print(f'  ▶  VERDICT : {verdict}')
    print(f'  ▶  ENTRY   : {pf(cp)}')

    # ── 2. MTF Resonance ─────────────────────────────────────────
    mtf = eng_result.get('mtf_resonance') or {}
    print(f'\n  ┌─ ① MTF ENERGY RESONANCE  (cross-timeframe alignment) {"─"*9}┐')
    res = mtf.get('resonance_score', 0.0)
    res_bar = '█' * int(res * 20) + '░' * (20 - int(res * 20))
    print(f'  │  Resonance score   : {res:.4f}  [{res_bar}]')
    print(f'  │  Label             : {mtf.get("resonance_label","—")}')
    print(f'  │  MTF spike prob    : {mtf.get("mtf_spike_prob",0.0):.1%}')
    print(f'  │  TFs pre-spike     : {mtf.get("n_tfs_pre_spike",0)} of {len(mtf.get("tf_results",{}))} analysed')
    print(f'  │  Phase mean        : {mtf.get("phase_mean",0.0):.4f}')
    print(f'  │  Entropy mean      : {mtf.get("entropy_mean",0.0):.3f}  '
          f'(lower = more compressed)')
    print(f'  │  Compression mean  : {mtf.get("compression_mean",0.0):.3f}  '
          f'(higher = more structured)')
    print(f'  │')
    print(f'  │  {"TF":<5}  {"EnergyScore":>11}  {"PreSpike":>8}  '
          f'{"PhaseAlign":>10}  {"Entropy":>7}  {"Comp":>6}  {"Horizon"}')
    print(f'  │  {"─"*5}  {"─"*11}  {"─"*8}  {"─"*10}  {"─"*7}  {"─"*6}  {"─"*8}')
    for tf_label, tr in (mtf.get('tf_results') or {}).items():
        ps  = '🔥 YES' if tr.get('pre_spike_flag') else '  no'
        es  = tr.get('energy_score', 0.0)
        pa  = tr.get('phase_alignment', 0.0)
        ent = tr.get('fft_spectral_entropy', 0.0)
        cmp = tr.get('fft_harmonic_compression', 0.0)
        hl  = tr.get('horizon_lbl', '?')
        e_bar = '█' * int(es / 10) + '░' * (10 - int(es / 10))
        print(f'  │  {tf_label:<5}  {es:>6.1f}/100  {ps:>8}  {pa:>10.4f}  '
              f'{ent:>7.3f}  {cmp:>6.3f}  {hl}  [{e_bar}]')
    print(f'  └{"─"*w}┘')

    # ── 3. 1m Energy analysis ─────────────────────────────────────
    r1m = eng_result.get('1m_result') or {}
    print(f'\n  ┌─ ② 1m ENERGY ANALYSIS  (signal² engine) {"─"*21}┐')
    print(f'  │  Energy score      : {r1m.get("energy_score",0.0):.1f}/100')
    print(f'  │  Pre-spike flag    : {"🔥 YES" if r1m.get("pre_spike_flag") else "  no"}')
    print(f'  │  Energy now        : {r1m.get("energy_energy_now", 0.0):.8f}')
    print(f'  │  Rolling E (20b)   : {r1m.get("energy_roll_E_now", 0.0):.8f}')
    print(f'  │  Norm growth       : {r1m.get("energy_norm_growth_now", 0.0):+.6f}  '
          f'(+= energy accelerating)')
    # FFT
    print(f'  │  Dom period        : {r1m.get("fft_dominant_period", "—")} bars')
    print(f'  │  Spectral entropy  : {r1m.get("fft_spectral_entropy", 0.0):.4f}  '
          f'(low = compressed = pre-spike)')
    print(f'  │  Harmonic compress : {r1m.get("fft_harmonic_compression", 0.0):.4f}  '
          f'(high = structured energy)')
    print(f'  │  Energy in harm    : {r1m.get("fft_energy_in_harmonics", 0.0):.1%}')
    # Phase
    print(f'  │  Phase alignment   : {r1m.get("phase_alignment", 0.0):.4f}  '
          f'(1.0 = perfectly aligned)')
    print(f'  │  Phase std         : {r1m.get("phase_std", 0.0):.4f}  '
          f'(lower = tighter timing)')
    print(f'  │  Compression trend : {"✔ YES (tightening)" if r1m.get("phase_compress_trend") else "  no"}')
    # Spike conditions
    sc = r1m.get('spike_condition', {})
    print(f'  │  ── Spike conditions ──────────────────────────────────────────')
    cond_map = {
        'energy_growing'       : 'A. Energy growing    (norm_growth > 0)',
        'harmonics_compressed' : 'B. Harmonics compressed (entropy < 70% ref)',
        'phase_aligned'        : 'C. Phase aligned     (align > 0.40 + tightening)',
        'spectral_compression' : 'D. Spectral compres  (top-N power > 50%)',
        'at_harmonic_trough'   : 'E. At harmonic trough (FFT osc < mean → bounce)',
    }
    n_cond = 0
    for ck, cl_txt in cond_map.items():
        ok = sc.get(ck, False)
        if ok: n_cond += 1
        print(f'  │    {"✔" if ok else "✗"}  {cl_txt}')
    print(f'  │  → {n_cond}/5 conditions met  '
          f'{"🔥 PRE-SPIKE CONFIRMED" if n_cond >= 3 else ("⚠ partial" if n_cond >= 2 else "✗ no spike setup")}')
    print(f'  └{"─"*w}┘')

    # ── 4. ML Ensemble ────────────────────────────────────────────
    ml = eng_result.get('ml_result') or {}
    print(f'\n  ┌─ ③ ML ENSEMBLE SPIKE PROBABILITY  (RF + GBM + Ridge) {"─"*8}┐')
    sp  = ml.get('ensemble_prob', 0.5)
    sp_bar = '█' * int(sp * 20) + '░' * (20 - int(sp * 20))
    print(f'  │  Ensemble spike prob : {sp:.1%}  [{sp_bar}]')
    print(f'  │  Label               : {ml.get("spike_prob_label","—")}')
    print(f'  │  {"Model":<8}  {"SpikeProb":>10}  {"MAE":>12}  {"R²":>7}  {"DirAcc":>8}')
    print(f'  │  {"─"*8}  {"─"*10}  {"─"*12}  {"─"*7}  {"─"*8}')
    for nm, ip in sorted((ml.get('indiv_probs') or {}).items()):
        sc_dict = (ml.get('model_scores') or {}).get(nm, {})
        mae = sc_dict.get('mae', 0); r2 = sc_dict.get('r2', 0)
        da  = sc_dict.get('dir_acc', 0)
        print(f'  │  {nm:<8}  {ip:>9.1%}  {mae:>12.8f}  {r2:>7.4f}  {da:>7.1%}')
    fi = ml.get('feature_importance', [])
    if fi:
        print(f'  │  ── Top feature importances (RF) ───────────────────────────────')
        for feat_nm, feat_v in fi[:5]:
            bar_f = '█' * int(feat_v * 100) + '░' * (10 - min(10, int(feat_v * 100)))
            print(f'  │    {feat_nm:<20}  {feat_v:.4f}  [{bar_f}]')
    print(f'  └{"─"*w}┘')

    # ── 5. Harmonic price forecasts ───────────────────────────────
    pf_r = eng_result.get('price_forecast') or {}
    print(f'\n  ┌─ ④ ENERGY-HARMONIC PRICE FORECASTS {"─"*25}┐')
    print(f'  │  Entry    : {pf(cp)}  (current 1m close)')
    print(f'  │  Upper harmonic band : {pf(pf_r.get("harmonic_band_upper"))}  '
          f'(amplitude envelope)')
    print(f'  │  Lower harmonic band : {pf(pf_r.get("harmonic_band_lower"))}')
    print(f'  │')
    tier_map = [
        ('stf', 'STF  ~30m  (next minutes →)'),
        ('mtf', 'MTF  ~2h   (next 1–2 hours)'),
        ('htf', 'HTF  ~8h   (next 4–8 hours)'),
    ]
    for key, tier_lbl in tier_map:
        fc = pf_r.get(f'{key}_forecast')
        if fc:
            fc_p, fc_pct, fc_desc = fc
            arrow = '▲' if fc_pct >= 0 else '▼'
            print(f'  │  {tier_lbl:<30} : {pf(fc_p)}'
                  f'  ({arrow}{fc_pct:+.3f}%)')
        else:
            print(f'  │  {tier_lbl:<30} : —')

    # ML-based forecasts
    ml_fc = (ml.get('price_forecasts') or [])
    if ml_fc:
        print(f'  │')
        print(f'  │  ML MODEL FORECASTS (RF energy momentum projection):')
        print(f'  │  {"Horizon":>8}  {"Price":>14}  {"Δ%":>8}')
        print(f'  │  {"─"*8}  {"─"*14}  {"─"*8}')
        for h_b, fc_p, fc_pct in ml_fc:
            arrow = '▲' if fc_pct >= 0 else '▼'
            print(f'  │  {h_b:>5}b  →  {pf(fc_p):>14}  {arrow}{fc_pct:+.3f}%')

    print(f'  └{"─"*w}┘')

    # ── 6. Key takeaways ─────────────────────────────────────────
    print(f'\n  ┌─ ⑤ ENERGY ENGINE SUMMARY & TRADE GUIDANCE {"─"*17}┐')
    print(f'  │')
    print(f'  │  ⚡ Energy²   : price² amplifies dominant cycles → reveals real spikes')
    print(f'  │  🌊 FFT       : structure layer → dominant period '
          f'{r1m.get("fft_dominant_period","?")} bars')
    print(f'  │  🎯 Phase     : timing layer → {"COMPRESSING (spike near)" if r1m.get("phase_compress_trend") else "relaxed"}')
    print(f'  │  🤖 ML        : learned energy → momentum mapping  '
          f'→  {ml.get("ensemble_prob",0.5):.1%} spike prob')
    print(f'  │  📊 Resonance : {mtf.get("resonance_score",0.0):.4f}  '
          f'({mtf.get("resonance_label","—")})')
    print(f'  │')

    # final action guidance
    if combined >= 80:
        print(f'  │  ★ ACTION: 🔥 STRONG SPIKE SETUP — Energy fully loaded,')
        print(f'  │           resonance confirmed across TFs.  FAST ENTRY NOW.')
    elif combined >= 65:
        print(f'  │  ★ ACTION: 🟢 PREPARE ENTRY — Phase compressing,')
        print(f'  │           energy growth confirmed. Enter on next pullback.')
    elif combined >= 45:
        print(f'  │  ★ ACTION: 🟡 WATCH — Energy building.  Wait for')
        print(f'  │           phase_alignment > 0.50 + ≥3 conditions confirmed.')
    else:
        print(f'  │  ★ ACTION: 🔴 STAND ASIDE — Insufficient structure.')
        print(f'  │           Wait for resonance > 0.55 before entry.')
    print(f'  │')
    print(f'  └{"─"*w}┘')
    print(f'  {"═"*w}\n')
...
# ═══════════════════════════════════════════════════════════════════════════════
#  INTEGRATION GUIDE — 3 changes needed in mtf_scanner16.py
# ═══════════════════════════════════════════════════════════════════════════════
#
#  ①  In _compute_mtf_spike_score()  (around line 3148, before return statement)
#     ADD the following block to incorporate the energy score into the composite:
#
#  ── INSERT before  `return round(max(0.0, _score), 1)`  ──────────────────────
#
#     # ── Energy / Harmonic / Phase spike score (v17) ──────────────────────
#     #   Run a lightweight single-TF energy check using 1m sel_detail price arr.
#     #   Weight: 8 pts from energy score,  +6 pre-spike bonus
#     try:
#         _1m_cl = np.asarray(_d.get('close_arr', []), dtype=np.float64)
#         _1m_vl = np.asarray(_d.get('vol_arr',   []), dtype=np.float64)
#         if len(_1m_cl) >= _ENG_MIN_BARS and len(_1m_vl) == len(_1m_cl):
#             _eng_r = _energy_spike_single_tf(_1m_cl, _1m_vl)
#             _score += _eng_r.get('energy_score', 0.0) * 0.08
#             if _eng_r.get('pre_spike_flag', False):
#                 _score += 6.0
#     except Exception:
#         pass
#
#  ─────────────────────────────────────────────────────────────────────────────
#
#  ②  In _rich_tf_dip_check()  (around line 583, in the composite score section)
#     ADD to `_soft_conds` list:
#
#     # energy pre-spike flag (v17)
#     try:
#         _eng_r17 = _energy_spike_single_tf(_cl, _vl)
#         _soft_conds.append(bool(_eng_r17.get('pre_spike_flag', False)))
#         result['energy_score_v17']     = _eng_r17.get('energy_score', 0.0)
#         result['energy_pre_spike_v17'] = _eng_r17.get('pre_spike_flag', False)
#     except Exception:
#         _soft_conds.append(False)
#
#  ─────────────────────────────────────────────────────────────────────────────
#
#  ③  In the main while-True loop  (around line 7505, after print_phi_reversal_block)
#     ADD:
#
#     # ── ⚡ Energy / Harmonic / ML spike engine (v17) ──────────────────────
#     print(f'  Running Energy·Harmonic·ML spike analysis on {lbl}...')
...
def _rich_tf_dip_check(pair, interval, tf_label, limit=600):
    ...
    result = {
        ...
        'exhaust_score': 0.0,
        # Order-Flow Imbalance spike engine (v16)
        'ofi_data': None, 'ofi_score': 0.0,
        'ofi_regime': 'none', 'ofi_pre_spike': False,
        'ofi_imbalance': 0.0, 'ofi_spike_index': 0.0,
        'ofi_vol_pressure': 0.0, 'ofi_liq_proxy': 0.0,
        'ofi_vol_acceleration': 0.0, 'ofi_buy_ratio': 0.0,
        # misc
        'cmo': None, 'price': None,
    ...
    try:
        ...
        # ── 17. Order-Flow Imbalance Spike Engine (v16) ───────────────
        #   Computes the full spike probability index:
        #     S = imbalance × volume_pressure × liquidity_thinness
        #   Classifies regime (none/micro/strong/para) and fires the
        #   pre-spike flag when volume is surging AND accelerating.
        _ofi = _order_flow_imbalance_score(_cl, _hi, _lo, _vl)
        result['ofi_data']            = _ofi
        if _ofi:
            result['ofi_score']           = float(_ofi.get('ofi_score',          0.0))
            result['ofi_regime']          = _ofi.get('ofi_regime',               'none')
            result['ofi_pre_spike']       = bool(_ofi.get('ofi_pre_spike',        False))
            result['ofi_imbalance']       = float(_ofi.get('ofi_imbalance',       0.0))
            result['ofi_spike_index']     = float(_ofi.get('ofi_spike_index',     0.0))
            result['ofi_vol_pressure']    = float(_ofi.get('ofi_vol_pressure',    0.0))
            result['ofi_liq_proxy']       = float(_ofi.get('ofi_liq_proxy',       0.0))
            result['ofi_vol_acceleration']= float(_ofi.get('ofi_vol_acceleration',0.0))
            result['ofi_buy_ratio']       = float(_ofi.get('ofi_buy_ratio',       0.0))
        ...
        # v16 OFI spike engine (max 8 pts)
        _sc += min(8.0, result.get('ofi_score', 0.0) * 0.08)
        # penalties
        ...
        # ── v17: energy pre-spike flag ─────────────────────────────────────
        _eng_pre17 = False
        try:
            _eng_r17 = _energy_spike_single_tf(_cl, _vl)
            _eng_pre17 = bool(_eng_r17.get('pre_spike_flag', False))
            result['energy_score_v17']     = _eng_r17.get('energy_score', 0.0)
            result['energy_pre_spike_v17'] = _eng_pre17
        except Exception:
            result['energy_score_v17']     = 0.0
            result['energy_pre_spike_v17'] = False
        ...
        _soft_conds = [
            ...
            bool(result.get('ofi_pre_spike',   False)),   # v16 pre-spike OFI
            _eng_pre17,                                   # v17 energy pre-spike
        ]
...
#  ORDER-FLOW IMBALANCE SPIKE ENGINE  (v16)
#
#  Theory:  price spikes when aggressive market orders overwhelm available
#  liquidity.  The driver is NOT raw volume but the ratio:
#
#      Imbalance  =  Aggressive Volume  /  Available Liquidity
#
#  We implement the full Spike Probability Index:
#
#      S = [(Vbuy − Vsell) / Vtotal]  ×  [Vtotal / window]  ×  liq_proxy
#
#  Where:
#      Vbuy   = volume on up-candles  (proxy for aggressive buying)
#      Vsell  = volume on down-candles
#      liq_proxy = price_range / volume  (thin book → high impact per unit vol)
#      window  = look-back bars (time compression factor)
#
#  Regime thresholds  (buy:sell ratio):
#      micro   2:1 –  4:1   fast wick, usually retraces
#      strong  4:1 – 10:1   clean breakout + momentum continuation
#      para   10:1+          parabolic / FOMO / short-squeeze
#
#  Pre-spike signature (fires BEFORE the actual spike):
#      - Volume in last _OFI_ACCEL_BARS ≥ _OFI_VOL_SURGE_MULT × rolling avg
#      - AND each of those bars has higher volume than the previous  (acceleration)
#
#  Score 0–100:
#      40 pts  spike_index magnitude (log-scaled)
#      25 pts  regime bonus  (0 none / 10 micro / 20 strong / 25 para)
#      20 pts  pre-spike signature
#      15 pts  volume acceleration strength (normalised)
# ─────────────────────────────────────────────────────────────────────────────

def _order_flow_imbalance_score(close_arr, high_arr, low_arr, vol_arr,
                                 window_short=None, window_long=None):
    """
    Compute the order-flow imbalance spike probability index from OHLCV arrays.

    Parameters
    ----------
    close_arr, high_arr, low_arr, vol_arr : array-like
        Raw OHLCV data (equal-length, newest bar last).
    window_short : int, optional
        Fast imbalance look-back (default: _OFI_WINDOW_SHORT = 10).
    window_long  : int, optional
        Slow imbalance look-back used for normalisation (default: _OFI_WINDOW_LONG = 30).

    Returns
    -------
    dict with keys:
        ofi_imbalance       – net directional ratio in [-1, +1]  (fast window)
        ofi_buy_ratio       – buy_vol / total_vol  (fast window)
        ofi_vol_pressure    – total vol / window  (volume density)
        ofi_liq_proxy       – smoothed price_range / volume  (thinness proxy)
        ofi_spike_index     – S = imbalance × vol_pressure × liq_proxy
        ofi_vol_acceleration– vol change rate (last _OFI_ACCEL_BARS bars)
        ofi_regime          – 'none' | 'micro' | 'strong' | 'para'
        ofi_pre_spike       – True when vol surge + acceleration align
        ofi_score           – composite 0–100
        ofi_buy_vol_sum     – raw buy volume (fast window)
        ofi_sell_vol_sum    – raw sell volume (fast window)
    """
    _ws = window_short or _OFI_WINDOW_SHORT
    _wl = window_long  or _OFI_WINDOW_LONG
    ...
    try:
        ...
        #  Up candle  (close ≥ open proxy: close ≥ prev close) → buy aggressor
        #  Down candle                                          → sell aggressor
        #  We approximate open ≈ previous close which is exact for most feeds.
        _price_chg = np.diff(cl, prepend=cl[0])
        _buy_vl    = np.where(_price_chg >= 0, vl, 0.0)
        _sell_vl   = np.where(_price_chg <  0, vl, 0.0)

        # ── Fast-window rolling sums ──────────────────────────────────
        _buy_sum_f  = float(np.sum(_buy_vl[-_ws:]))
        _sell_sum_f = float(np.sum(_sell_vl[-_ws:]))
        _tot_sum_f  = float(np.sum(vl[-_ws:])) + 1e-20

        # ── Slow-window for normalisation ─────────────────────────────
        _tot_sum_l  = float(np.sum(vl[-_wl:])) + 1e-20

        # ── Imbalance  (net ratio  in [-1, +1]) ───────────────────────
        _imbalance  = (_buy_sum_f - _sell_sum_f) / _tot_sum_f
        _buy_ratio  = _buy_sum_f / _tot_sum_f

        # ── Buy : sell ratio (for regime gating) ──────────────────────
        _bs_ratio   = _buy_sum_f / (max(_sell_sum_f, 1e-12))

        # ── Volume pressure (density = vol per bar) ────────────────────
        _vol_pressure = _tot_sum_f / _ws

        # ── Liquidity proxy (price impact per unit volume) ────────────
        #   High value → thin book → bigger spike per unit vol
        _range_bars = (hi[-_ws:] - lo[-_ws:]) + 1e-12
        _liq_raw    = _range_bars / (vl[-_ws:] + 1e-12)
        _liq_proxy  = float(np.mean(_liq_raw))
        # Normalise: compare against slow-window mean
        _range_slow = (hi[-_wl:] - lo[-_wl:]) + 1e-12
        _liq_slow   = float(np.mean(_range_slow / (vl[-_wl:] + 1e-12)))
        _liq_norm   = _liq_proxy / (max(_liq_slow, 1e-12))  # > 1 = thinner than usual

        # ── Spike Probability Index  S = imbalance × pressure × liq ──
        #   Use log-scaling on pressure so large volumes don't dominate
        _vol_press_log = float(np.log1p(_vol_pressure))
        _spike_index   = abs(_imbalance) * _vol_press_log * _liq_norm

        # ── Volume acceleration ────────────────────────────────────────
        #   Derivative of volume over last _OFI_ACCEL_BARS
        _accel_vols = vl[-(_OFI_ACCEL_BARS + 1):]
        if len(_accel_vols) >= 2:
            _vol_accel = float(np.mean(np.diff(_accel_vols)))
        else:
            _vol_accel = 0.0

        # ── Pre-spike signature ────────────────────────────────────────
        #   Condition 1: recent average volume ≥ _OFI_VOL_SURGE_MULT × baseline
        _baseline_vol   = float(np.mean(vl[-_wl:-_OFI_ACCEL_BARS])) + 1e-12
        _recent_avg_vol = float(np.mean(vl[-_OFI_ACCEL_BARS:]))
        _vol_surge_flag = _recent_avg_vol >= _OFI_VOL_SURGE_MULT * _baseline_vol

        #   Condition 2: volume monotonically accelerating over last N bars
        _accel_window   = vl[-_OFI_ACCEL_BARS:]
        _vol_accel_flag = bool(np.all(np.diff(_accel_window) > 0)) if len(_accel_window) >= 2 else False

        #   Pre-spike = BOTH conditions + positive imbalance (buyers aggressive)
        _pre_spike = bool(_vol_surge_flag and _vol_accel_flag and _imbalance > 0.0)

        # ── Regime classification ──────────────────────────────────────
        if   _bs_ratio >= _OFI_PARA_THRESH   and _imbalance > 0.0:
            _regime = 'para'
        elif _bs_ratio >= _OFI_STRONG_THRESH and _imbalance > 0.0:
            _regime = 'strong'
        elif _bs_ratio >= _OFI_MICRO_THRESH  and _imbalance > 0.0:
            _regime = 'micro'
        else:
            _regime = 'none'

        # ── Composite OFI score 0–100 ──────────────────────────────────
        #   40 pts  spike_index magnitude (log-scaled, capped)
        #   25 pts  regime bonus
        #   20 pts  pre-spike signature
        #   15 pts  volume acceleration (normalised, capped)
        _sc_spike   = min(40.0, float(np.log1p(_spike_index * 10.0)) * 10.0)
        _sc_regime  = {'none': 0.0, 'micro': 10.0, 'strong': 20.0, 'para': 25.0}[_regime]
        _sc_presike = 20.0 if _pre_spike else 0.0
        _sc_accel   = min(15.0, max(0.0, _vol_accel / (max(_baseline_vol, 1.0)) * 15.0))
        _ofi_score  = round(min(100.0, _sc_spike + _sc_regime + _sc_presike + _sc_accel), 1)

        return {
            'ofi_imbalance':        round(_imbalance,    4),
            'ofi_buy_ratio':        round(_buy_ratio,    4),
            'ofi_buy_vol_sum':      round(_buy_sum_f,    2),
            'ofi_sell_vol_sum':     round(_sell_sum_f,   2),
            'ofi_vol_pressure':     round(_vol_pressure, 4),
            'ofi_liq_proxy':        round(_liq_proxy,    8),
            'ofi_liq_norm':         round(_liq_norm,     4),
            'ofi_spike_index':      round(_spike_index,  6),
            'ofi_vol_acceleration': round(_vol_accel,    4),
            'ofi_vol_surge':        _vol_surge_flag,
            'ofi_vol_accel_flag':   _vol_accel_flag,
            'ofi_pre_spike':        _pre_spike,
            'ofi_bs_ratio':         round(_bs_ratio,     3),
            'ofi_regime':           _regime,
            'ofi_score':            _ofi_score,
        }
...
# ─────────────────────────────────────────────
#  MTF SPIKE SCORE  (v16 — OFI engine added)
# ─────────────────────────────────────────────
#
#  ★ COMPOSITE RANKING FACTORS (v16):
#
#   MTF_SPIKE_SCORE = (
...
#      6 %  OFI spike engine    — imbalance × vol_pressure × liq_proxy (NEW v16)
...
#              −6 OFI sell imbalance > 30% on 1m  (v16)
#
#  Principle: volume precedes price. HTF confirmation is a HARD factor.
#             OFI regime fires BEFORE the spike — it is a leading indicator.
# ─────────────────────────────────────────────
...
def _compute_mtf_spike_score(pair, tf_cache, sel_detail):
    ...
    v16 factors:
      ...
      10% Momentum | 8% Force | 7% VolIntensity | 6% OFI (new v16)
      5% CurVol | 5% RSI | 4% Impulse | 4% Geometry
    Bonuses: +8 LeadSine cross | +6 HTFimp | +6 OFI pre-spike
             +5 sine<-0.9 ≥2TFs | +5 exhaust_dip_ok 1m | +4 POC+Book
             +4 OFI para | +3 pump_strong | +2 di_cross 1m | +2 OFI strong ≥2
    Penalties: -15 fake dip | -15 exhaust_top_ok 1m | -10 RSI not confirming
               -8 ADX < 15 (ranging) | -6 OFI sell imbalance (v16)
    """
    _score = 0.0
    _d     = sel_detail.get(pair, {}); _cache = tf_cache.get(pair, {}); _tot = len(_cache) + 1
    ...
    # ── 6 pts: Order-Flow Imbalance spike score (v16) ─────────────
    #   Weighted avg of OFI score across all cached TFs + 1m gate.
    #   Higher weight on faster TFs (1m most predictive for near-term spike).
    _ofi_tw  = {'2h': 0.30, '30m': 0.45, '15m': 0.60, '5m': 0.80,
                '3m': 0.90, '1m': 1.00, '1m_gate': 1.00}
    _ofi_wv  = 0.0; _ofi_ws = 0.0
    for _tk, _rd in _cache.items():
        ...
        _ofi_wv += _rd.get('ofi_score', 0.0) * _w
        _ofi_ws += _w
    # also include 1m sel_detail ofi_score
    _ofi_1m_sc = _d.get('ofi_score', 0.0)
    _ofi_wv += _ofi_1m_sc * _ofi_tw.get('1m', 1.0)
    _ofi_ws += _ofi_tw.get('1m', 1.0)
    if _ofi_ws > 0:
    ...
    # ── OFI BONUSES / PENALTIES (v16) ─────────────────────────────
    # +6: pre-spike signature on any fast TF (1m/3m/5m) — imminent vol explosion
    _ofi_fast_tfs = ['1m', '1m_gate', '3m', '5m']
    _n_presike = sum(1 for _tk, _rd in _cache.items()
                     if _tk in _ofi_fast_tfs and _rd.get('ofi_pre_spike', False))
    if _d.get('ofi_pre_spike', False):
    ...
    # +4: parabolic OFI regime on any TF (10:1+ buy:sell ratio)
    _n_para = sum(1 for _rd in _cache.values() if _rd.get('ofi_regime') == 'para')
    if _d.get('ofi_regime') == 'para':
        _n_para += 1
    if _n_para >= 1:
    ...
    # +2: strong OFI regime on ≥2 TFs (4:1–10:1 sustained pressure)
    _n_strong = sum(1 for _rd in _cache.values() if _rd.get('ofi_regime') == 'strong')
    if _d.get('ofi_regime') == 'strong':
        _n_strong += 1
    if _n_strong >= 2:
        _score += 2.0

    # −6: sell imbalance detected on 1m (aggressive sellers dominating)
    #     ofi_imbalance < −0.30 means 65%+ of recent vol is sell-side
    if _d.get('ofi_imbalance', 0.0) < -0.30:
        _score -= 6.0
    ...
    # ── Energy / Harmonic / Phase spike score (v17) ─────────────────────
    #   Lightweight single-TF energy check from 1m sel_detail price arr.
    #   8 pts energy score + 6 pts pre-spike bonus
    try:
        _1m_cl = np.asarray(_d.get('close_arr', []), dtype=np.float64)
        _1m_vl = np.asarray(_d.get('vol_arr',   []), dtype=np.float64)
        if len(_1m_cl) >= _ENG_MIN_BARS and len(_1m_vl) == len(_1m_cl):
            _eng_r17 = _energy_spike_single_tf(_1m_cl, _1m_vl)
            _score += _eng_r17.get('energy_score', 0.0) * 0.08
            if _eng_r17.get('pre_spike_flag', False):
                _score += 6.0
    except Exception:
        pass
...
def print_rich_filter_table(pairs, label_map, tf_cache, tf_key, stage_label):
    ...
    _hdr = (f'  │  {"#":>3}  {"Ticker":<10}  {"Price":>12}  '
            ...
            f'{"CMO":>6}  {"CompSc":>6}  {"Pd(b)":>5}  {"→Top":>5}  '
            f'{"OFI":>5}  {"Regime":>6}  {"PreSpk":>6}  {"Signals"}')
    _sep = '  │' + '─' * (len(_hdr) - 4) + '│'
    ...
    for _i, _p in enumerate(_ranked, 1):
        ...
        # v16 OFI signals
        if _rd.get('ofi_pre_spike'):  _sigs.append('🔴PreSpk')
        _ofi_r = _rd.get('ofi_regime', 'none')
        if _ofi_r == 'para':          _sigs.append('🌋PARA')
        elif _ofi_r == 'strong':      _sigs.append('💪OFIx')
        # OFI display values
        _ofi_sc  = _rd.get('ofi_score', 0.0)
        _ofi_rg  = _rd.get('ofi_regime', 'none')[:6]
        _ofi_ps  = '✔' if _rd.get('ofi_pre_spike') else '✗'
        ...
        print(f'  │  {_i:>3}  {_lbl:<10}  {_pr_s:>12}  '
              ...
              f'{_ofi_sc:>5.0f}  {_ofi_rg:>6}  {_ofi_ps:>6}  '
              f'{_sig_s}  {_conf_s}  │')
...
def print_mtf_signals_summary(fp4, label_map, tf_cache, sel_detail, spike_scores):
    ...
    Now includes HTF vol, RSI, impulse, OFI, market-profile columns.
    """
    if not fp4:
        return
    ...
    _WL  = 90
    print(f'\n  {"═" * _WL}')
    print(f'  ◈  MTF SIGNAL MATRIX  ({len(fp4)} candidates)  [v16]')
    print(f'  {"═" * _WL}')
    print(f'  ★ SELECTION FACTORS (v16):')
    print(f'    CMO(18%) + HT_SINE(15%) + ArgMin(12%) + HTFvol(12%)')
    print(f'    + ADX(10%) + Momentum(10%) + Force(8%) + VolIntensity(7%)')
    print(f'    + OFI(6%) + CurVol(5%) + RSI(5%) + Impulse(4%) + Geo(4%)')
    print(f'    Bonus: +8 LeadSine | +6 HTFimp | +6 OFI-PreSpk | +5 SineExtreme')
    print(f'           +5 ExhaustDip | +4 POC+Book | +4 OFI-Para | +3 PumpStrong')
    print(f'           +2 DI_Cross | +2 OFI-Strong≥2')
    print(f'    Penalty: -15 fake dip | -15 exhaust_top | -10 RSI fail')
    print(f'             -8 ADX<15 | -6 OFI sell imbalance')
    print()
    _tf_hdr = '  '.join(f'[{_t}]Sn AM HV Px Ex OF' for _t in _TFS)
    print(f'  {"Ticker":<10}  {"MTFSpk":>6}  {_tf_hdr}  CMO  ADX Mmt Frc  Geo RSI1m Imp1m  OFI1m Rg1m')
    print('  ' + '─' * (_WL + 90))
    ...
    for _p in _sorted_fp4:
        ...
        # 1m OFI
        _ofi1_sc  = _d.get('ofi_score', 0.0)
        _ofi1_rg  = (_d.get('ofi_regime') or 'none')[:4]
        _ofi1_ps  = '🔴' if _d.get('ofi_pre_spike') else '  '
        _parts = []
        for _tf in _TFS:
            if _tf == '1m':
                ...
                _of  = f'{_d.get("ofi_score",0.0):>3.0f}' if _d.get('ofi_score') else '  —'
            else:
                ...
                _of  = f'{_rd.get("ofi_score",0.0):>3.0f}' if _rd.get('ofi_score') else '  —'
            _sn_s = f'{_sn:+.1f}' if _sn is not None else '  — '
            _parts.append(f'{_sn_s} {_am} {_hv} {_px} {_ex} {_of}')
        _row  = '  '.join(_parts)
        _mark = '★' if _p == _sorted_fp4[0] else ' '
        print(f'  {_mark}{_lbl:<9}  {_msc:>6.1f}  {_row}  {_cmo_s:>4}  {_adx1_s:>3} {_mmt1}/5 {_frc1}  '
              f'{_geo:>4.0f} {_rsi1_s:>5} {_imp1}  {_ofi1_sc:>5.0f} {_ofi1_rg:>4} {_ofi1_ps}')
...
while True:
    ...
    #    MTF_SPIKE = CMO(18%) + HT_SINE(15%) + ArgMin(12%) + HTFvol(12%)
    #               + ADX(10%) + Momentum(10%) + Force(8%) + VolIntensity(7%)
    #               + OFI(6%) + CurVol(5%) + RSI(5%) + Impulse(4%) + Geo(4%)
    #               + bonuses/penalties
    if len(sel_pairs) > 1:
        lbls = [label_map.get(p, p) for p in sel_pairs]
        print(f'  {len(sel_pairs)} mtf dips found: {lbls}')
        print(f'  Ranking by: ★ MTF Spike Score v16 (OFI-enhanced multi-factor composite)')
        print(f'  Factors   : CMO 18% | HT_SINE 15% | ArgMin 12% | HTFvol 12% | ADX 10%')
        print(f'              Momentum 10% | Force 8% | VolIntensity 7% | OFI 6% | CurVol 5%')
        ...
        # OFI summary for best symbol
        _b_ofi_sc  = best_d.get('ofi_score',    0.0)
        _b_ofi_rg  = best_d.get('ofi_regime',   'none')
        _b_ofi_ps  = best_d.get('ofi_pre_spike', False)
        _b_ofi_im  = best_d.get('ofi_imbalance', 0.0)
        _ofi_warn  = '  🔴 PRE-SPIKE' if _b_ofi_ps else ''
        print(f'  Best → {label_map.get(best_symbol, best_symbol)}'
              ...
              f'  OFI={_b_ofi_sc:.0f}/100 regime={_b_ofi_rg}'
              f'  imbal={_b_ofi_im:+.3f}{_ofi_warn}'
              f'{fake_warn}\n')
    ...
    if current_price:
        ...
        if stf_results or htf_results:
            ...
            # ── ⚛  Physics Energy Engine  (v18) — True Momentum · Kinetic Energy ─────
            #    p(t)=m×v  |  E(t)=½mv²  |  d²E/dt² curvature  |  fast spike detector
            print(f'  Running v18 Physics Energy Engine on {lbl}...')
            try:
                # reuse 1m arrays already fetched above for energy v17
                _phys_cl = close_1m if 'close_1m' in dir() else None
                _phys_hi = high_1m  if 'high_1m'  in dir() else None
                _phys_lo = low_1m   if 'low_1m'   in dir() else None
                _phys_vl = vol_1m   if 'vol_1m'   in dir() else None
                _phys_ofi = sel_detail.get(best_symbol, {}).get('ofi_data')

                phys_result_v18, backup_v18 = _main_loop_physics_patch(
                    best_symbol, current_price, sel_detail, sel_pairs,
                    mtf_spike_scores, _tf_cache, label_map,
                    close_1m=_phys_cl, high_1m=_phys_hi,
                    low_1m=_phys_lo,   vol_1m=_phys_vl,
                )
            except Exception as _phys_ex:
                print(f'  [v18 Physics] error: {_phys_ex}')
