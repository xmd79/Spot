...
# Updated configurable conditions - Removed time_distance_condition
CONFIG = {
    "conditions": {
    ...
    "min_conditions_met": 4  # ALL 4 conditions must be met to trigger a trade
}
...
def calculate_linear_regression_channel(candles, period=500, dev_multiplier=2.0):
    ...
    try:
        ...
        # Check for recent channel crossings
        # We need to find the most recent occurrence of crossing below lower and above upper
        most_recent_below_lower = False
        most_recent_above_upper = False
        
        # Check if we recently crossed below the lower channel
        for i in range(min(10, n-1)):  # Check last 10 periods
            if close_prices[n-2-i] >= lower_channel[n-2-i] and close_prices[n-1-i] < lower_channel[n-1-i]:
                most_recent_below_lower = True
                break
        
        # Check if we recently crossed above the upper channel
        for i in range(min(10, n-1)):  # Check last 10 periods
            if close_prices[n-2-i] <= upper_channel[n-2-i] and close_prices[n-1-i] > upper_channel[n-1-i]:
                most_recent_above_upper = True
                break
        
        # Determine if the most recent crossing was below the lower channel
        # This indicates an upward cycle according to the user's requirement
