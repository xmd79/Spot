...
def get_sr_targets(raw_klines: list, current_price: float) -> Dict:
    ...
    lookbacks = [
        ...
        for lb in [500, 800, 1200] if len(raw_klines) >= lb
    ]
...
def score_15m_candidate(trader, symbol) -> Tuple[str, float, dict]:
    ...
    try:
        ...
        cmo_score      = float(np.clip(-cmo_val / 100.0, 0.0, 1.0))   # deeper oversold → higher
        ...
        score = (
            ...
            freq_align    * 0.15 +
            bull_ratio    * 0.10 +
            (1.0 - pos_in_range) * 0.10   # lower in range = better
        )
...
def format_sr_output(symbol, sr, current_price, cmo_val, vratio,
    ...
    # Rebalanced weights: add freq as 5th component (reduce others slightly)
    enhanced_prob = (
        0.28 * ml_prob +
        0.18 * cluster_prob +
        0.13 * trigger_bonus +
        0.13 * golden_contrib +
        0.18 * sinusoidal_contrib +
        0.10 * freq_contrib
    )
    print(f"\n  ⚡  Enhanced Spike Probability (φ + sinusoidal + wavelet-freq):")
    ...
    print(f"  Expected Move     : Strong reversal spike expected (φ-compression + exhaustion + wavelet)")
    print("=" * W + "\n")
    return v, ns
...
def print_scan_header(scan_count: int):
    ...
    print("║" + " " * W + "║")
    print("║" + "  🔍  CASCADE: 1D→4H→2H→30M→15M→5M(REG)→1M(REG+SINE+WAVELET)".ljust(W) + "║")
    print("║" + " " * W + "║")
    print("╚" + "═" * W + "╝")
...
def main():
    ...
    print("=" * 78)
    print("  🌊  STRICT MTF CASCADE DIP DETECTOR  (WAVELET + FFT + SINE + φ)  🌊")
    print("=" * 78)
    print()
    ...
    while scan_count < max_scans:
        ...
        try:
            ...
            thirty_m_passed = run_tf_filter(trader, two_h_passed, '30m', max_workers=20)
            ...
            if not five_m_passed:
                ...
                if not fallback:
                    print("❌ MTF fallback scoring returned no candidates. Retrying...")
                    time.sleep(5)
                    continue
            ...
            if not results_1m:
                print("❌ No 1m analysis results (Sine Extrema + Up Cycle + Wavelet failed). Retrying...")
                time.sleep(5)
                continue
            ...
            def final_score(r):
                ...
                score  = ml * 0.35 + gs * 0.25 + freq_alignment * 0.15
                if near_bottom:  score += 0.10
                if turning_up:   score += 0.08
                if est_bars < 15: score += 0.07
                score -= r[1] * 0.0015
                return score
