...
def analyze_mtf_rsi_stoch_backup(candles_5m, candles_30m, lookback=200):
    """
    Multi-Timeframe Backup Check.
    Analyzes RSI (5m) and Stoch (30m) to see which extreme event happened MOST RECENTLY.
    ...
    def get_last_reversal_state(values, oversold_limit, overbought_limit):
        ...
        return "NEUTRAL", False # Default if no extremes found in window
    ...
    try:
        ...
        rsi_result["current_val"] = rsi.iloc[-1]
        
    except Exception as e:
        print(f"Error analyzing RSI 5m: {e}")

    # 2. Stoch 30m Analysis
    ...
    try:
        ...
        # Calculate Stoch
        lowest_low = low_30m.rolling(14).min()
        highest_high = high_30m.rolling(14).max()
        stoch_k = 100 * (close_30m - lowest_low) / (highest_high - lowest_low)
        ...
        stoch_result["current_val"] = stoch_k.iloc[-1]
...
try:
    while True:
        ...
        rsi_state = mtf_backup_result['rsi_5m']['state']
        rsi_cond = mtf_backup_result['rsi_5m']['condition_met']
        rsi_val = mtf_backup_result['rsi_5m']['current_val']
        
        stoch_state = mtf_backup_result['stoch_30m']['state']
        stoch_cond = mtf_backup_result['stoch_30m']['condition_met']
        stoch_val = mtf_backup_result['stoch_30m']['current_val']
        
        final_cond = mtf_backup_result['condition_met']
        
        print(f"RSI (5m) Most Recent Found: {rsi_state} (Condition: {rsi_cond}) - Val: {rsi_val:.2f}")
        print(f"Stoch (30m) Most Recent Found: {stoch_state} (Condition: {stoch_cond}) - Val: {stoch_val:.2f}")
