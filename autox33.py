...
# CONFIGURATION UPDATED: Split mtf_rsi_stoch_backup into 4 separate conditions
CONFIG = {
    "conditions": {
        ...
        # "oversold_mtf_rsi_stoch": True, # REMOVED - Split into 4 below
        "oversold_rsi_15sec": True,    # NEW
        "oversold_stoch_15sec": True,   # NEW
        "oversold_rsi_1m": True,       # NEW
        "oversold_stoch_1m": True,      # NEW
        "volume_bullish_1min": True,
        "volume_bullish_15sec": True,
        "stoch_rsi_precise_1m": True,
        "stoch_rsi_precise_15sec": True,
        "hurst_trending_1m": True,
        "hurst_trending_15sec": True,
    },
    "min_conditions_met": 18 # Updated: 15 (previous) - 1 (removed) + 4 (added) = 18
}
...
# =========================================================
# NEW FUNCTIONS: STOCHASTIC + RSI PRECISE (1m & 15s)
# =========================================================

def analyze_stoch_rsi_precise(candles, timeframe_label):
    """
    Implements 'Stochastic + RSI Precise Target Exit Strategy v2' logic.
    Generic function used for 1m and 15s timeframes.
    Returns entry condition (bool) and details.
    """
    try:
        if not candles or len(candles) < 20:  # Need enough data for rolling windows
            return {"condition_met": False, "error": f"Insufficient data for {timeframe_label} analysis"}
        ...
        # === STOCHASTIC ===
        lowest_low = low.rolling(stoch_k_length).min()
        highest_high = high.rolling(stoch_k_length).max()

        stoch_k = 100 * (close - lowest_low) / (highest_high - lowest_low)
        smooth_k = stoch_k.rolling(stoch_k_smoothing).mean()
        stoch_d = smooth_k.rolling(stoch_d_smoothing).mean()
        ...
        # === ENTRY CONDITION ===
        # Check for crossover
        # We need at least 2 rows to check cross [-2] and [-1]
        if len(smooth_k) < 2 or len(stoch_d) < 2:
            return {"condition_met": False, "error": f"Insufficient data length for crossover check ({timeframe_label})"}

        bull_cross = (
            smooth_k.iloc[-2] < stoch_d.iloc[-2]
            and smooth_k.iloc[-1] > stoch_d.iloc[-1]
        )

        entry_condition = bull_cross and (stoch_d.iloc[-1] < stoch_entry_level)

        # Details for logging
        current_rsi = rsi.iloc[-1] if not rsi.empty else 0
        current_k = smooth_k.iloc[-1] if not smooth_k.empty else 0
        current_d = stoch_d.iloc[-1] if not stoch_d.empty else 0

        return {
            "timeframe": timeframe_label,
            "condition_met": entry_condition,
            "rsi": current_rsi,
            "stoch_k": current_k,
            "stoch_d": current_d,
            "bull_cross": bull_cross,
            "d_below_entry": stoch_d.iloc[-1] < stoch_entry_level
        }

    except Exception as e:
        print(f"Error in analyze_stoch_rsi_precise ({timeframe_label}): {e}")
        return {"condition_met": False, "error": str(e)}

# Wrapper functions for clarity in the main loop
def analyze_stoch_rsi_precise_1m(candles_1m):
    return analyze_stoch_rsi_precise(candles_1m, "1m")

def analyze_stoch_rsi_precise_15sec(candles_15s):
    return analyze_stoch_rsi_precise(candles_15s, "15s")


# =========================================================
# UPDATED FUNCTION: HURST CYCLE ANALYSIS (OPTIMIZED)
# =========================================================

def analyze_hurst_cycles(candles, timeframe_label, window_size=200, sampling_rate=20):
    """
    Optimized Hurst Exponent analysis.
    Only calculates H for the most recent window to save CPU and ensure fresh data.
    Condition Met: H > 0.5 (Trending/Persistent).
    """
    try:
        if not candles or len(candles) < window_size:
            return {"condition_met": False, "H": 0.0, "error": "Insufficient data for Hurst analysis"}

        # Extract close prices
        series = np.array([c["close"] for c in candles], dtype=np.float64)
        
        # OPTIMIZATION: Only take the most recent window
        # We don't care about history, only about current state
        window = series[-window_size:]
        
        # Ensure series is positive for 'price' kind
        if np.any(window <= 0):
            window = window - np.min(window) + 1e-10 

        # Validate window data
        if np.std(window) < 1e-10 or np.max(window) == np.min(window):
            return {"condition_met": False, "H": 0.0, "error": "Flat market (Zero Range)"}
        
        if np.any(np.isnan(window)) or np.any(np.isinf(window)):
            return {"condition_met": False, "H": 0.0, "error": "Invalid Data (NaN/Inf)"}

        # Try different 'kind' parameters for robustness
        # Priority: 'random_walk' is often best for prices, then 'price'
        H_calculated = None
        for kind in ['price', 'random_walk', 'change']:
            try:
                H, c, data = compute_Hc(window, kind=kind, simplified=True)
                H_calculated = H
                break # Success, stop trying other kinds
            except Exception:
                continue
        
        if H_calculated is None:
            return {"condition_met": False, "H": 0.0, "error": "Calculation Failed for all kinds"}

        # Logic: H > 0.5 is trending (Persistent). H < 0.5 is mean reverting.
        condition_met = H_calculated > 0.5

        return {
            "condition_met": condition_met,
            "H": H_calculated,
            "timeframe": timeframe_label,
            "description": f"Hurst Exponent: {H_calculated:.4f} ({'Trending' if condition_met else 'Mean Reverting'})"
        }

    except Exception as e:
        print(f"Error in analyze_hurst_cycles: {e}")
        return {"condition_met": False, "H": 0.0, "error": str(e)}

# =========================================================
# UPDATED FUNCTION: OVERSOLD MTF RSI & STOCH
# =========================================================

def analyze_oversold_mtf(candles_15s, candles_1m, lookback=200):
    """
    Checks if the most recent extreme found for RSI and Stoch on both 15s and 1m timeframes is Oversold.
    Logic: Iterate backwards through history. The first extreme (Oversold or Overbought) encountered wins.
    Returns individual statuses for each.
    """
    ...
    results = {
        "rsi_15s": {"state": "NEUTRAL", "condition_met": False, "current_val": 0},
        "stoch_15s": {"state": "NEUTRAL", "condition_met": False, "current_val": 0},
        "rsi_1m": {"state": "NEUTRAL", "condition_met": False, "current_val": 0},
        "stoch_1m": {"state": "NEUTRAL", "condition_met": False, "current_val": 0},
    }

    try:
        # --- 15s Analysis ---
        if candles_15s and len(candles_15s) > 20:
            df_15s = pd.DataFrame(candles_15s)
            
            # RSI 15s
            delta = df_15s["close"].diff()
            gain = delta.clip(lower=0)
            loss = -delta.clip(upper=0)
            ...
            state, is_oversold = get_last_reversal_state(recent_rsi, 30, 70)
            results["rsi_15s"] = {"state": state, "condition_met": is_oversold, "current_val": rsi.iloc[-1] if not rsi.empty else 0}

            # Stoch 15s
            lowest_low = df_15s["low"].rolling(14).min()
            highest_high = df_15s["high"].rolling(14).max()
            stoch_k = 100 * (df_15s["close"] - lowest_low) / (highest_high - lowest_low)
            
            recent_k = stoch_k.tail(lookback).values
            state, is_oversold = get_last_reversal_state(recent_k, 20, 80)
            results["stoch_15s"] = {"state": state, "condition_met": is_oversold, "current_val": stoch_k.iloc[-1] if not stoch_k.empty else 0}
        ...
        if candles_1m and len(candles_1m) > 20:
            ...
            recent_rsi = rsi.tail(lookback).values
            state, is_oversold = get_last_reversal_state(recent_rsi, 30, 70)
            results["rsi_1m"] = {"state": state, "condition_met": is_oversold, "current_val": rsi.iloc[-1] if not rsi.empty else 0}

            # Stoch 1m
            lowest_low = df_1m["low"].rolling(14).min()
            highest_high = df_1m["high"].rolling(14).max()
            stoch_k = 100 * (df_1m["close"] - lowest_low) / (highest_high - lowest_low)

            recent_k = stoch_k.tail(lookback).values
            state, is_oversold = get_last_reversal_state(recent_k, 20, 80)
            results["stoch_1m"] = {"state": state, "condition_met": is_oversold, "current_val": stoch_k.iloc[-1] if not stoch_k.empty else 0}
                                    
        return results

    except Exception as e:
        print(f"Error in analyze_oversold_mtf: {e}")
        return results
...
def analyze_sine_wave_oscillator_5m(candles_5m=None):
    try:
        ...
        if len(fresh_candles) < 500:
            return {"error": f"Insufficient fresh data: only {len(fresh_candles)} candles", "condition_met": False}
        ...
        if last_min_idx > last_max_idx:
            ...
            if forecast_price <= current_price:
                forecast_price = current_price + (current_price * 0.01)
        ...
        print(f"Error in analyze_sine_wave_oscillator_5m: {e}")
...
try:
    while True:
        ...
        conditions_status = {
            ...
            # "oversold_mtf_rsi_stoch": False, # REMOVED
            "oversold_rsi_15sec": False,    # ADDED
            "oversold_stoch_15sec": False,   # ADDED
            "oversold_rsi_1m": False,       # ADDED
            "oversold_stoch_1m": False,      # ADDED
            "volume_bullish_1min": False,
            "volume_bullish_15sec": False,
            "stoch_rsi_precise_1m": False,
            "stoch_rsi_precise_15sec": False,
            "hurst_trending_1m": False,
            "hurst_trending_15sec": False,
        }
        ...
        # Condition 5: Thresholds Analysis (1m)
        print("\n--- Condition 5: Thresholds Analysis (1m) ---")
        ...
        # Condition 6: Thresholds Analysis (3m)
        print("\n--- Condition 6: Thresholds Analysis (3m) ---")
        ...
        # Condition 7: Thresholds Analysis (5m)
        print("\n--- Condition 7: Thresholds Analysis (5m) ---")
        ...
        # Condition 8: Sine Wave Oscillator (5m)
        print("\n--- Condition 8: Sine Wave Oscillator (5m) ---")
        sine_wave_result = analyze_sine_wave_oscillator_5m(candles_5m)
        if 'error' not in sine_wave_result:
            conditions_status["sine_wave_oscillator_5m"] = sine_wave_result['condition_met']
            print(f"Cycle Type: {sine_wave_result['cycle_type']}")
            print(f"Current Price: {sine_wave_result['current_price']:.2f}")
            print(f"Forecast Price: {sine_wave_result['forecast_price']:.2f}")
            print(f"Next Extremum Type: {sine_wave_result['next_extremum_type']}")
            print(f"Last Min Index: {sine_wave_result['last_min_idx']}")
            print(f"Last Max Index: {sine_wave_result['last_max_idx']}")
            print(f"Last Min Value: {sine_wave_result['last_min_value']:.2f}")
            print(f"Last Max Value: {sine_wave_result['last_max_value']:.2f}")
        ...
        # Condition 9: Oversold MTF RSI & Stoch (SPLIT INTO 4)
        print("\n--- Condition 9: Oversold MTF RSI & Stoch (Split Conditions) ---")
        oversold_mtf_result = analyze_oversold_mtf(candles_15s, candles_1m)
        
        # 9a: 15s RSI
        rsi_15s_cond = oversold_mtf_result['rsi_15s']['condition_met']
        rsi_15s_state = oversold_mtf_result['rsi_15s']['state']
        rsi_15s_val = oversold_mtf_result['rsi_15s']['current_val']
        conditions_status["oversold_rsi_15sec"] = rsi_15s_cond
        print(f"15s RSI: {rsi_15s_state} (Cond: {rsi_15s_cond}) - Val: {rsi_15s_val:.2f}")
        
        # 9b: 15s Stoch
        stoch_15s_cond = oversold_mtf_result['stoch_15s']['condition_met']
        stoch_15s_state = oversold_mtf_result['stoch_15s']['state']
        stoch_15s_val = oversold_mtf_result['stoch_15s']['current_val']
        conditions_status["oversold_stoch_15sec"] = stoch_15s_cond
        print(f"15s Stoch: {stoch_15s_state} (Cond: {stoch_15s_cond}) - Val: {stoch_15s_val:.2f}")
        
        # 9c: 1m RSI
        rsi_1m_cond = oversold_mtf_result['rsi_1m']['condition_met']
        rsi_1m_state = oversold_mtf_result['rsi_1m']['state']
        rsi_1m_val = oversold_mtf_result['rsi_1m']['current_val']
        conditions_status["oversold_rsi_1m"] = rsi_1m_cond
        print(f"1m RSI: {rsi_1m_state} (Cond: {rsi_1m_cond}) - Val: {rsi_1m_val:.2f}")
        
        # 9d: 1m Stoch
        stoch_1m_cond = oversold_mtf_result['stoch_1m']['condition_met']
        stoch_1m_state = oversold_mtf_result['stoch_1m']['state']
        stoch_1m_val = oversold_mtf_result['stoch_1m']['current_val']
        conditions_status["oversold_stoch_1m"] = stoch_1m_cond
        print(f"1m Stoch: {stoch_1m_state} (Cond: {stoch_1m_cond}) - Val: {stoch_1m_val:.2f}")
        ...
        # Condition 12: Stochastic + RSI Precise (1m)
        print("\n--- Condition 12: Stochastic + RSI Precise (1m) ---")
        stoch_rsi_precise_1m_result = analyze_stoch_rsi_precise_1m(candles_1m)
        
        if 'error' not in stoch_rsi_precise_1m_result:
            conditions_status["stoch_rsi_precise_1m"] = stoch_rsi_precise_1m_result['condition_met']
            print(f"RSI (1m): {stoch_rsi_precise_1m_result['rsi']:.2f}")
            print(f"Stoch %K (1m): {stoch_rsi_precise_1m_result['stoch_k']:.2f}")
            print(f"Stoch %D (1m): {stoch_rsi_precise_1m_result['stoch_d']:.2f}")
            print(f"Bull Cross: {stoch_rsi_precise_1m_result['bull_cross']}")
            print(f"%D < Entry(61.8): {stoch_rsi_precise_1m_result['d_below_entry']}")
            print(f"Condition Met: {stoch_rsi_precise_1m_result['condition_met']}")
        else:
            print(f"Error analyzing Stoch/RSI Precise (1m): {stoch_rsi_precise_1m_result['error']}")
            print(f"Condition Met: {conditions_status['stoch_rsi_precise_1m']}")

        # Condition 13: Stochastic + RSI Precise (15s)
        print("\n--- Condition 13: Stochastic + RSI Precise (15s) ---")
        if candles_15s:
            stoch_rsi_precise_15s_result = analyze_stoch_rsi_precise_15sec(candles_15s)
            
            if 'error' not in stoch_rsi_precise_15s_result:
                conditions_status["stoch_rsi_precise_15sec"] = stoch_rsi_precise_15s_result['condition_met']
                print(f"RSI (15s): {stoch_rsi_precise_15s_result['rsi']:.2f}")
                print(f"Stoch %K (15s): {stoch_rsi_precise_15s_result['stoch_k']:.2f}")
                print(f"Stoch %D (15s): {stoch_rsi_precise_15s_result['stoch_d']:.2f}")
                print(f"Bull Cross: {stoch_rsi_precise_15s_result['bull_cross']}")
                print(f"%D < Entry(61.8): {stoch_rsi_precise_15s_result['d_below_entry']}")
                print(f"Condition Met: {stoch_rsi_precise_15s_result['condition_met']}")
            else:
                print(f"Error analyzing Stoch/RSI Precise (15s): {stoch_rsi_precise_15s_result['error']}")
                print(f"Condition Met: {conditions_status['stoch_rsi_precise_15sec']}")
        else:
            print("No 15s candles available for Stoch/RSI analysis.")
            print(f"Condition Met: {conditions_status['stoch_rsi_precise_15sec']}")

        # Condition 14: Hurst Trending (1m)
        print("\n--- Condition 14: Hurst Cycle Analysis (1m) ---")
        hurst_1m_result = analyze_hurst_cycles(candles_1m, "1m", window_size=200, sampling_rate=20)
        
        if 'error' not in hurst_1m_result:
            conditions_status["hurst_trending_1m"] = hurst_1m_result['condition_met']
            print(f"Hurst Exponent (H): {hurst_1m_result['H']:.4f}")
            print(f"State: {hurst_1m_result['description']}")
            print(f"Condition Met (H > 0.5): {hurst_1m_result['condition_met']}")
        else:
            print(f"Error analyzing Hurst (1m): {hurst_1m_result.get('error', 'Unknown')}")
            print(f"Condition Met: {conditions_status['hurst_trending_1m']}")

        # Condition 15: Hurst Trending (15s)
        print("\n--- Condition 15: Hurst Cycle Analysis (15s) ---")
        # Use simulated 15s candles
        if candles_15s:
            hurst_15s_result = analyze_hurst_cycles(candles_15s, "15s", window_size=200, sampling_rate=20)
            
            if 'error' not in hurst_15s_result:
                conditions_status["hurst_trending_15sec"] = hurst_15s_result['condition_met']
                print(f"Hurst Exponent (H): {hurst_15s_result['H']:.4f}")
                print(f"State: {hurst_15s_result['description']}")
                print(f"Condition Met (H > 0.5): {hurst_15s_result['condition_met']}")
            else:
                print(f"Error analyzing Hurst (15s): {hurst_15s_result.get('error', 'Unknown')}")
                print(f"Condition Met: {conditions_status['hurst_trending_15sec']}")
        else:
            print("No 15s candles available for Hurst analysis.")
            print(f"Condition Met: {conditions_status['hurst_trending_15sec']}")
