...
BASE_ASSET = TRADE_SYMBOL.replace("USDC", "")
...
# =========================================================
# UPDATED TRADING FUNCTIONS (MAX BALANCE LOGIC + DETAILED LOGS)
# =========================================================
...
def buy_asset():
    ...
    try:
        ...
        pre_usdc_balance = get_balance('USDC')
        pre_btc_balance = get_balance(BASE_ASSET)
        
        print("\n" + "="*60)
        print(">>> PRE-ENTRY BALANCE SNAPSHOT <<<")
        print(f"USDC Balance Available: {pre_usdc_balance:.25f}")
        print(f"BTC Balance Current:     {pre_btc_balance:.25f}")
        print("="*60)
        ...
        # Calculate Max Quantity based on current USDC balance
        # We use floor division to ensure we don't overspend
        raw_quantity = pre_usdc_balance / current_price
        ...
        if estimated_cost < min_notional:
            print(f"Warning: Estimated cost (${estimated_cost:.25f}) is below Minimum Notional (${min_notional}). Adjusting.")
        ...
        # Sanity check against balance (should not happen due to // logic, but good for safety)
        if estimated_cost > pre_usdc_balance:
            print(f"Warning: Adjusted cost exceeds balance. Reducing slightly.")
            adjusted_quantity = ((pre_usdc_balance / current_price) // step_size) * step_size
            adjusted_quantity = adjusted_quantity.quantize(Decimal('0.' + '0' * step_precision))
            estimated_cost = adjusted_quantity * current_price
        ...
        print(f"Estimated Cost:        {estimated_cost:.25f} USDC")
        
        order = client.order_market_buy(symbol=TRADE_SYMBOL, quantity=float(adjusted_quantity))
        
        # --- CALCULATE EXACT ENTRY STATS FROM FILLS ---
        ...
        for fill in order['fills']:
            ...
            # commission is usually deducted separately, but cost is price*qty
            total_spent_usdc += qty * price
        ...
        print(f"USDC Balance Remaining: {post_usdc_balance:.25f} (Dust)")
...
def sell_asset():
    ...
    try:
        ...
        print("\n" + "="*60)
        print(">>> PRE-EXIT BALANCE SNAPSHOT <<<")
        print(f"USDC Balance Current:    {pre_usdc_balance:.25f}")
        print(f"BTC Balance To Sell:     {pre_btc_balance:.25f}")
        print("="*60)
        ...
        # Floor quantity to step size (leaves max dust possible within limits)
        ...
        print(f"Quantity to Sell:       {sell_quantity:.25f} BTC")
...
def check_exit_condition(initial_investment, asset_balance, entry_price):
    ...
    # Target Price = (Target USDC Value) / (BTC Quantity Held)
    ...
    while True:
        ...
        if position_open:
            ...
            if entry_price <= Decimal('0.0') or initial_investment <= Decimal('0.0'):
                ...
                if recovered_price > Decimal('0.0'):
                    ...
                    # Recovery approximates investment
        ...
        if signal_triggered:
            ...
            if position_open:
                ...
                print(f"!!! NOTE: Entry Logic Disabled. No trade executed. No signal written to file. !!!")
        ...
        # ==========================================
        # SCENARIO C: ENTRY LOGIC (ENABLED)
        # ==========================================
        if signal_triggered and not position_open:
            ...
            if ep is not None and edt is not None:
                entry_price = ep
                entry_datetime = edt
                # Update initial_investment with the ACTUAL cost from fills
                initial_investment = cost 
                
                asset_balance = get_balance(BASE_ASSET)
                
                # Recalculate target price based on ACTUAL cost
                total_required_gross_percent = PROFIT_TARGET_PERCENT + TOTAL_FEE_PERCENT
                multiplier = Decimal(str(1.0 + total_required_gross_percent / 100))
                target_value = initial_investment * multiplier
                target_price = target_value / asset_balance
        ...
        final_btc = get_balance(BASE_ASSET)
