...
def calculate_gann_octave_progression(close_prices, period=14):
    ...
    try:
        ...
        # 3. Determine the Primary Forecast Target
        if swing_direction == "upward":
            if current_price > r1: forecast_target = r2
            elif current_price > r2: forecast_target = r3
            elif current_price > r3: forecast_target = r4
            else: forecast_target = r1
        else: # downward swing
            if current_price < s1: forecast_target = s2
            elif current_price < s2: forecast_target = s3
            elif current_price < s3: forecast_target = s4
            else: forecast_target = s1
        ...
        # 5. REVISED Final Forecast Confirmation
        # The forecast is the primary signal. The cycle phase acts as a confirmation filter.
        forecast_is_upward = forecast_target > current_price
        condition_met = False
        
        if forecast_is_upward:
            # For an upward forecast, the condition is met if the swing is upward
            # and the cycle is not strongly indicating a reversal.
            if swing_direction == "upward" and cycle_phase != "DOWNWARD_OCTAVE":
                condition_met = True
        else: # downward forecast
            # For a downward forecast, the condition is met if the swing is downward
            # and the cycle is not strongly indicating a reversal.
            if swing_direction == "downward" and cycle_phase != "UPWARD_OCTAVE":
                condition_met = True
...
def analyze_gann_octave_progression(candles):
    ...
    try:
        ...
        if gann_data['condition_met']:
            description += "Forecast is confirmed by cycle analysis."
        else:
            description += "Forecast is not confirmed by cycle analysis."
...
try:
    while True:
        ...
        gann_octave_result = analyze_gann_octave_progression(candles_1m)
