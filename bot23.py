#!/usr/bin/env python3
"""
ENHANCED BTCUSDC TRADING BOT - NUCLEAR PUMP SNIPER
...
def analyze_momentum_condition(client, symbol, timeframe='1m', lookback=600):
    """
    GOD-TIER PUMP FILTER 2025
    → Momentum > 0
    → Momentum accelerating (mom > mom[1])
    → Short-term momentum crushing medium-term
    → MASSIVE VOLUME SPIKE (current volume > 1.8× 20-period avg + rising)
    """
    try:
        klines = client.get_klines(symbol=symbol, interval=timeframe, limit=lookback)
        ...
        # === VOLUME SPIKE FILTER — THIS IS THE KILLER === 
        vol_sma = volume.rolling(20).mean()
        vol_ratio = volume / vol_sma

        # Current volume must be exploding AND still rising
        volume_spike = vol_ratio.iloc[-1] > VOLUME_SPIKE_THRESHOLD           # 85%+ above 20MA = real buyers stepping in
        volume_rising = volume.iloc[-1] > volume.iloc[-2]  # volume still climbing this candle

        # Optional: even stricter — volume in top 5 of last 50 candles
        volume_top_strength = volume.iloc[-1] > volume.iloc[-50:].quantile(0.9)

        # === FINAL ULTRA-STRICT PUMP CONDITION ===
        ...
        condition_met = (momentum_positive
                         ...
                         and volume_spike
                         and volume_rising
                         and volume_top_strength)   # removes 90% of fakeouts
        ...
        details = {
            ...
            "vol_spike_185x": volume_spike,
            "vol_still_rising": volume_rising,
            "vol_top_10percent_last50": volume_top_strength,
...
def perform_single_iteration_analysis(client):
    ...
    print("="*80)
    print(f"BTCUSDC TRADING BOT - NUCLEAR PUMP SNIPER")
    ...
    # Condition 5: NUCLEAR PUMP ENGINE (Momentum + Volume Explosion)
    print("\n--- Condition 5: NUCLEAR PUMP ENGINE (Momentum + Volume Explosion) ---")
    ...
    # Print details for this condition - with detailed momentum and volume information
    print(f"\n--- Condition 5: NUCLEAR PUMP ENGINE (Momentum + Volume Explosion) ---")
    ...
    print(f"├─ Volume Spike")
    print(f"│  → Volume ratio (x20MA) : {mom_details['vol_ratio_x']:.2f}x")
    print(f"│  → Spike >1.85x         : {'YES' if mom_details['vol_spike_185x'] else 'NO':<3}")
    print(f"│  → Volume still rising  : {'YES' if mom_details['vol_still_rising'] else 'NO':<3}")
    print(f"│  → Top 10% last 50 bars : {'YES' if mom_details['vol_top_10percent_last50'] else 'NO':<3}")
    print(f"└─ FINAL PUMP SIGNAL      : {'GO GO GO' if momentum_met else 'Waiting...':<12}")

    if momentum_met:
        conditions_met += 1
        print("\nNUCLEAR PUMP CONFIRMED — VOLUME + ACCELERATING MOMENTUM — ENTERING NOW")
    else:
...
def main():
    ...
    print("=== BTCUSDC TRADING BOT - NUCLEAR PUMP SNIPER ===")
