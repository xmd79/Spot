...
def display_trade_status():
    ...
    # Clear screen and show trade monitor
...
def perform_single_iteration_analysis(client):
    ...
    if trade_active:
        ...
        if trade_closed:
            ...
            # When in trade, only wait for the next iteration without clearing the screen
            time.sleep(MIN_ITERATION_INTERVAL)
            return  # Exit the function early
    
    # --- This entire block only runs when NO trade is active ---
...
def main():
    ...
    print("- When in-trade, only shows trade monitor status")
    print("="*60)
    ...
    while not stop_event.is_set():
        ...
        # Different waiting behavior based on trade status
        if trade_active:
            # When in trade, the function handles its own waiting, so just a small sleep here
            time.sleep(1) # Small sleep to prevent tight loop if check_trade_status was fast
        else:
            # When not in trade, show the countdown
