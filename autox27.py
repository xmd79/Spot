...
# --- GANN & OCTAVE CONSTANTS ---
PHI = (1 + math.sqrt(5)) / 2  # Golden Ratio, approximately 1.618
SQRT_2 = math.sqrt(2)        # Gann's Square of 2, for 45-degree angles
OCTAVE_RATIO = 2              # Fundamental octave ratio (frequency doubling)
...
CONFIG = {
    "conditions": {
        ...
        "thresholds_3m": True,
        "thresholds_5m": True,
        "gann_octave_progression": True,  # Renamed and updated condition
    },
...
# =========================================================
# GANN-STYLE OCTAVE PROGRESSION ANALYSIS FUNCTION
# =========================================================
...
def calculate_gann_octave_progression(close_prices, period=14):
    """
    Calculates price targets based on Gann's geometric progression and the Law of Octaves.
    This replaces the fixed percentage model with a dynamic, Golden Ratio-based forecast.
    """
    try:
        close_prices = np.array(close_prices)
        
        # 1. Identify the Cardinal Pivot Point (CPP) - the most significant recent high or low
        lookback = min(period * 5, len(close_prices)) 
        recent_prices = close_prices[-lookback:]
        
        min_val_idx = np.argmin(recent_prices)
        max_val_idx = np.argmax(recent_prices)
        
        if max_val_idx > min_val_idx: # Last swing was up
            cpp_price = recent_prices[min_val_idx]
            cpp_idx = len(close_prices) - lookback + min_val_idx
            cpp_type = "low"
            swing_direction = "upward"
        else: # Last swing was down
            cpp_price = recent_prices[max_val_idx]
            cpp_idx = len(close_prices) - lookback + max_val_idx
            cpp_type = "high"
            swing_direction = "downward"

        current_price = close_prices[-1]
        hilo_range = np.nanmax(close_prices) - np.nanmin(close_prices)
        
        # 2. Calculate the Gann Progression Levels
        base_unit = hilo_range / 8 
        
        # Resistance Levels (for upward swing)
        r1 = cpp_price + base_unit * SQRT_2
        r2 = cpp_price + base_unit * PHI
        r3 = cpp_price + base_unit * 2 
        r4 = cpp_price + base_unit * PHI**2 
        
        # Support Levels (for downward swing)
        s1 = cpp_price - base_unit * SQRT_2
        s2 = cpp_price - base_unit * PHI
        s3 = cpp_price - base_unit * 2 
        s4 = cpp_price - base_unit * PHI**2 

        # 3. FIXED: Determine the Primary Forecast Target
        # For upward swing, target should always be above current price
        # For downward swing, target should always be below current price
        if swing_direction == "upward":
            # Find the first resistance level above the current price
            if current_price < r1: forecast_target = r1
            elif current_price < r2: forecast_target = r2
            elif current_price < r3: forecast_target = r3
            elif current_price < r4: forecast_target = r4
            else: forecast_target = current_price * 1.01  # Small upward target if all levels are exceeded
        else: # downward swing
            # Find the first support level below the current price
            if current_price > s1: forecast_target = s1
            elif current_price > s2: forecast_target = s2
            elif current_price > s3: forecast_target = s3
            elif current_price > s4: forecast_target = s4
            else: forecast_target = current_price * 0.99  # Small downward target if all levels are exceeded
            
        # 4. Analyze Cycle Strength and Phase
        momentum = talib.MOM(close_prices, timeperiod=period)
        current_momentum = momentum[-1]
        
        phase_shear_value = phase_shear(close_prices)
        
        # Determine cycle phase based on Gann's concept of time and price balance
        if phase_shear_value > 0.5: 
            cycle_phase = "MI_FA_TRANSITION"
            cycle_strength = min(phase_shear_value * 20, 100)
        elif swing_direction == "upward" and current_momentum > 0:
            cycle_phase = "UPWARD_OCTAVE"
            cycle_strength = min(abs(current_momentum) / hilo_range * 100, 100)
        elif swing_direction == "downward" and current_momentum < 0:
            cycle_phase = "DOWNWARD_OCTAVE"
            cycle_strength = min(abs(current_momentum) / hilo_range * 100, 100)
        else:
            cycle_phase = "STABLE"
            cycle_strength = 0
            
        # 5. IMPROVED: Final Forecast Confirmation for SPOT TRADING
        # For spot trading, we want either:
        # 1. Confirmed upward trend (swing_direction == "upward" and cycle_phase == "UPWARD_OCTAVE")
        # 2. Potential reversal from downward to upward (swing_direction == "downward" and cycle_phase == "UPWARD_OCTAVE")
        condition_met = False
        
        if (swing_direction == "upward" and cycle_phase == "UPWARD_OCTAVE") or \
           (swing_direction == "downward" and cycle_phase == "UPWARD_OCTAVE"):
            condition_met = True
        ...
        return {
            ...
            "base_unit": base_unit,
            "resistance_levels": {"r1": r1, "r2": r2, "r3": r3, "r4": r4},
            "support_levels": {"s1": s1, "s2": s2, "s3": s3, "s4": s4},
        ...
        print(f"Error in calculate_gann_octave_progression: {e}")
        import traceback
        traceback.print_exc()
        return {"error": str(e), "condition_met": False}

def analyze_gann_octave_progression(candles):
    """
    Analyzes the current market cycle using Gann's Octave Progression.
    """
    try:
        if not candles or len(candles) < 100:
            return {"error": "Insufficient data for Gann Octave analysis", "condition_met": False}
        
        close_prices = np.array([candle["close"] for candle in candles], dtype=np.float64)
        
        # Calculate the Gann progression
        gann_data = calculate_gann_octave_progression(close_prices, period=14)
        
        if "error" in gann_data:
            return gann_data

        # Create a comprehensive description
        description = (
            f"Current Cycle Phase: {gann_data['cycle_phase']} with strength {gann_data['cycle_strength']:.2f}. "
            f"Primary swing is {gann_data['swing_direction']} from Cardinal Pivot Point (CPP) "
            f"({gann_data['cpp_type']}) at {gann_data['cpp_price']:.2f}. "
            f"Forecast Target: {gann_data['forecast_target']:.2f}. "
        )
        
        # Update description based on the new logic
        if gann_data['condition_met']:
            if gann_data['swing_direction'] == "upward":
                description += "Confirmed upward trend with UPWARD_OCTAVE cycle phase (good for spot trading)."
            else:  # downward swing with UPWARD_OCTAVE cycle phase
                description += "Potential reversal from downward trend with UPWARD_OCTAVE cycle phase (good entry point)."
        else:
            description += "Not a favorable entry condition for spot trading."
            
        return {
            "condition_met": gann_data['condition_met'],
            "description": description,
            **gann_data # Include all data from the calculation
        }

    except Exception as e:
        print(f"Error in analyze_gann_octave_progression: {e}")
        import traceback
        traceback.print_exc()
        return {"error": str(e), "condition_met": False}
...
try:
    while True:
        ...
        candles_5m = candle_map.get('5m', [])  # Get 5m candles for Gann analysis
        ...
        conditions_status = {
            ...
            "thresholds_3m": False,
            "thresholds_5m": False,
            "gann_octave_progression": False,  # New condition for Gann Octave Progression
        }
        ...
        # Condition 9: Gann Octave Progression Analysis (NEW)
        print("\n--- Condition 9: Gann Octave Progression Analysis ---")
        # Changed to use 5m candles instead of 1m
        gann_octave_result = analyze_gann_octave_progression(candles_5m)
        if 'error' not in gann_octave_result:
            conditions_status["gann_octave_progression"] = gann_octave_result['condition_met']
            print(f"Timeframe: 5m")
            print(f"Current Price: {gann_octave_result['current_price']:.2f}")
            print(f"Cardinal Pivot Point (CPP): {gann_octave_result['cpp_type']} at {gann_octave_result['cpp_price']:.2f}")
            print(f"Swing Direction: {gann_octave_result['swing_direction']}")
            print(f"Forecast Target: {gann_octave_result['forecast_target']:.2f}")
            print(f"Cycle Phase: {gann_octave_result['cycle_phase']}")
            print(f"Cycle Strength: {gann_octave_result['cycle_strength']:.2f}")
            print(f"Resistance Levels: R1={gann_octave_result['resistance_levels']['r1']:.2f}, R2={gann_octave_result['resistance_levels']['r2']:.2f}, R3={gann_octave_result['resistance_levels']['r3']:.2f}, R4={gann_octave_result['resistance_levels']['r4']:.2f}")
            print(f"Support Levels: S1={gann_octave_result['support_levels']['s1']:.2f}, S2={gann_octave_result['support_levels']['s2']:.2f}, S3={gann_octave_result['support_levels']['s3']:.2f}, S4={gann_octave_result['support_levels']['s4']:.2f}")
            print(f"Description: {gann_octave_result['description']}")
            print(f"Condition Met: {conditions_status['gann_octave_progression']}")
        else:
            print(f"Error analyzing Gann Octave Progression: {gann_octave_result['error']}")
            print(f"Condition Met: {conditions_status['gann_octave_progression']}")
