...
CONFIG = {
    "conditions": {
        ...
        "stoch_rsi_precise": True,  # NEW CONDITION ADDED
    },
...
# =========================================================
# NEW FUNCTION: STOCHASTIC RSI PRECISE CONDITION
# =========================================================

def stochastic_rsi_precise_condition(
    candles,
    stoch_k_length=14,
    stoch_k_smoothing=1,
    ...
    """
    Python equivalent of Pine Script Stochastic RSI logic.
    Returns (entry_signal, exit_signal)
    """
    try:
        # Convert candle list to DataFrame
        df = pd.DataFrame(candles)
        if df.empty or len(df) < stoch_k_length + rsi_length:
            return False, False
        ...
        # Need at least 2 values for crossover check
        ...
        # D must be below entry level
        entry_condition = bull_cross and stoch_d.iloc[-1] < stoch_entry_level

        # === EXIT CONDITIONS ===
        rsi_low = target_rsi * (1 - precision_rsi / 100)
        rsi_high = target_rsi * (1 + precision_rsi / 100)

        k_low = target_k * (1 - precision_k / 100)
        k_high = target_k * (1 + precision_k / 100)

        d_low = target_d * (1 - precision_d / 100)
        d_high = target_d * (1 + precision_d / 100)

        # Check if all indicators are in the target high range (Overbought)
        rsi_in_range = rsi_low <= rsi.iloc[-1] <= rsi_high
        k_in_range = k_low <= smooth_k.iloc[-1] <= k_high
        d_in_range = d_low <= stoch_d.iloc[-1] <= d_high

        indicator_exit = rsi_in_range and k_in_range and d_in_range

        # === PROFIT TARGET EXIT ===
        profit_exit = False
        if use_profit_target and entry_price is not None:
            profit_exit = close.iloc[-1] >= entry_price * (1 + profit_target_pct / 100)

        exit_condition = indicator_exit or profit_exit

        return entry_condition, exit_condition
    except Exception as e:
        print(f"Error in stochastic_rsi_precise_condition: {e}")
        return False, False
...
try:
    while True:
        ...
        conditions_status = {
            ...
            "stoch_rsi_precise": False, # NEW CONDITION ADDED
        }
        ...
        # Condition 9: Stochastic RSI Precise (1m) - NEW
        print("\n--- Condition 9: Stochastic RSI Precise (1m) ---")
        stoch_entry_sig, stoch_exit_sig = stochastic_rsi_precise_condition(
            candles=candles_1m,
            use_profit_target=False, # Financial profit handled by main logic, this focuses on technical exit
            profit_target_pct=PROFIT_TARGET_PERCENT,
            entry_price=entry_price if position_open else None
        )
        conditions_status["stoch_rsi_precise"] = stoch_entry_sig
        print(f"Entry Signal (K cross D): {stoch_entry_sig}")
        print(f"Exit Signal (Precision Overbought): {stoch_exit_sig}")
        print(f"Condition Met: {conditions_status['stoch_rsi_precise']}")
        ...
        if position_open:
            ...
            # UPDATED EXIT LOGIC: Check main profit OR Stoch RSI Precision exit
            is_profit_exit = check_exit_condition(initial_investment, asset_balance, entry_price)
            
            if is_profit_exit or stoch_exit_sig:
                exit_reason = ""
                if is_profit_exit:
                    exit_reason = "Profit Target Reached"
                elif stoch_exit_sig:
                    exit_reason = "Stochastic/RSI Precision Exit (Overbought)"
                
                print(f"{exit_reason}. Initiating exit...")
