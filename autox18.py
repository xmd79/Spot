...
PROFIT_TARGET_PERCENT = 0.25  # Changed from 0.75% to 0.25% net profit target
...
def check_exit_condition(initial_investment, asset_balance, entry_price):
    ...
    # To achieve 0.25% net profit after 0.22% fees, we need 0.25% + 0.22% = 0.47% gross profit
    target_value = initial_investment * Decimal('1.0047')  # 0.47% gross profit for 0.25% net profit
...
def analyze_fft_prediction_15sec(candles_1m):
    """
    Analyzes FFT prediction based on 15-second data generated from 1-minute candles.
    """
    try:
        if not candles_1m:
            return {"error": "No 1m data provided to generate 15s data"}
        ...
        df_15s = generate_15s_data_from_1m(df_1m)
        if df_15s is None or df_15s.empty:
            return {"error": "Failed to generate 15s data for FFT prediction"}
        
        # Convert the 15s DataFrame back to a list of candle dictionaries
        candles_15s = []
        for idx, row in df_15s.iterrows():
            candles_15s.append({
                "time": int(row['timestamp'].timestamp()),
        ...
        # Perform FFT prediction on the generated 15s candles
        fft_result = predict_prices_with_fft(candles_15s, num_predictions=5, filter_ratio=0.8)
        ...
        return {
            ...
            "description": "FFT (on 15s data) predicts ultra short-term upward price movement (next ~15s)"
        }
        
    except Exception as e:
        print(f"Error analyzing FFT prediction (15s): {e}")
        return {"error": str(e)}
...
try:
    while True:
        ...
        # Condition 4: FFT Prediction (15s) - ANALYSIS ONLY
        print("\n--- Condition 4: FFT Prediction (15s) [ANALYSIS ONLY] ---")
        fft_prediction_result = analyze_fft_prediction_15sec(candles_1m)
        if 'error' not in fft_prediction_result:
            conditions_status["fft_prediction_15sec"] = fft_prediction_result['condition_met']
            ...
            print(f"Condition Met: {conditions_status['fft_prediction_15sec']} [IGNORED FOR TRIGGER]")
        else:
            print(f"Error analyzing FFT prediction: {fft_prediction_result['error']}")
            print(f"Condition Met: {conditions_status['fft_prediction_15sec']} [IGNORED FOR TRIGGER]")
            
        # Condition 5: Aroon-Only Signal
        print("\n--- Condition 5: Aroon-Only Signal ---")
        ...
        # Condition 6: Volume Bias Condition
        print("\n--- Condition 6: Volume Bias Condition ---")
        ...
        # Condition 7: Thresholds Analysis (1m)
        print("\n--- Condition 7: Thresholds Analysis (1m) ---")
        ...
        # Condition 8: Thresholds Analysis (3m)
        print("\n--- Condition 8: Thresholds Analysis (3m) ---")
        ...
        # Condition 9: Thresholds Analysis (5m)
        print("\n--- Condition 9: Thresholds Analysis (5m) ---")
        ...
        # Condition 10: SMA Cascade Condition (UPDATED) - ANALYSIS ONLY
        print("\n--- Condition 10: SMA Cascade Condition (UPDATED) [ANALYSIS ONLY] ---")
        ...
        if 'error' not in sma_cascade_result:
            ...
            print(f"Condition Met: {conditions_status['sma_cascade_condition']} [IGNORED FOR TRIGGER]")
        else:
            print(f"Error analyzing SMA cascade: {sma_cascade_result['error']}")
            print(f"Condition Met: {conditions_status['sma_cascade_condition']} [IGNORED FOR TRIGGER]")
        ...
        if position_open:
            ...
            target_value = initial_investment * Decimal('1.0047')
            ...
            print(f"Price for 0.25% Net Profit Target (after fees): {target_price:.25f}")
            ...
            if check_exit_condition(initial_investment, asset_balance, entry_price):
                print("Target net profit of 0.25% (after fees) reached or exceeded. Initiating exit...")
