...
# Updated configurable conditions - Now 24 conditions as specified
CONFIG = {
    "conditions": {
        "rsi_oversold_most_recent_1m": True,  # RSI Oversold Most Recent (1m)
        "rsi_oversold_most_recent_3m": True,  # RSI Oversold Most Recent (3m)
        "rsi_oversold_most_recent_5m": True,  # RSI Oversold Most Recent (5m)
        "momentum_positive_1m": True,  # Momentum > 0 (1m) - Changed from negative to positive
        "momentum_positive_3m": True,  # Momentum > 0 (3m) - NEW
        "momentum_positive_5m": True,  # Momentum > 0 (5m) - NEW
        "fft_forecast_up_1m": True,  # FFT Forecast Up (1m)
        "fft_forecast_up_3m": True,  # FFT Forecast Up (3m) - NEW
        "fft_forecast_up_5m": True,  # FFT Forecast Up (5m) - NEW
        "sma200_lowest_more_recent_1m": True,  # Lowest SMA200 more recent than highest SMA200 (1m)
        "sma200_lowest_more_recent_3m": True,  # Lowest SMA200 more recent than highest SMA200 (3m) - NEW
        "sma200_lowest_more_recent_5m": True,  # Lowest SMA200 more recent than highest SMA200 (5m) - NEW
        ...
        "volume_bullish_dominance_3m": True,  # Volume bullish > bearish (3m) - NEW
        "volume_bullish_dominance_5m": True,  # Volume bullish > bearish (5m) - NEW
        ...
        "argmin_more_recent_5m": True,  # Argmin more recent than argmax for 5m - NEW
        "up_cycle_confirmed_1m": True,  # Up cycle confirmed for 1m - NEW
        "up_cycle_confirmed_3m": True,  # Up cycle confirmed for 3m - NEW
        "up_cycle_confirmed_5m": True,  # Up cycle confirmed for 5m - NEW
    },
    "min_conditions_met": 24  # ALL 24 conditions must be met to trigger a trade
}
...
def perform_single_iteration_analysis(client):
    ...
    print("="*80)
    print(f"ENHANCED BTCUSDC TRADING BOT - 24 TRIGGERS")
    ...
    # Fetch SMA200 data for all timeframes
    sma200_1m = analyze_sma200_extrema(client, SYMBOL, timeframe='1m', lookback=500)
    sma200_3m = analyze_sma200_extrema(client, SYMBOL, timeframe='3m', lookback=500)
    sma200_5m = analyze_sma200_extrema(client, SYMBOL, timeframe='5m', lookback=500)
    ...
    # Fetch FFT data for all timeframes
    ...
    # Fetch local extrema data for all timeframes (NEW)
    local_extrema_1m = analyze_local_extrema(client, SYMBOL, timeframe='1m', lookback=500)
    local_extrema_3m = analyze_local_extrema(client, SYMBOL, timeframe='3m', lookback=500)
    local_extrema_5m = analyze_local_extrema(client, SYMBOL, timeframe='5m', lookback=500)
    
    # Step 2: Analyze all specified conditions
    print("\n2. Analyzing all 24 specified trading conditions...")
    
    conditions_met = 0
    total_conditions = 24  # Total number of conditions
    ...
    # Condition 2: RSI Oversold Most Recent (3m)
    print("\n--- Condition 2: RSI Oversold Most Recent (3m) ---")
    ...
    # Condition 3: RSI Oversold Most Recent (5m)
    print("\n--- Condition 3: RSI Oversold Most Recent (5m) ---")
    ...
    # Condition 5: Momentum Positive (3m) - NEW
    print("\n--- Condition 5: Momentum Positive (3m) ---")
    momentum_3m_positive, momentum_3m_value, momentum_3m_details = analyze_momentum_condition(client, SYMBOL, 500, '3m')
    condition_results['Momentum Positive (3m)'] = momentum_3m_positive
    
    # Print details for this condition
    print(f"\nCurrent Momentum: {momentum_3m_value:.4f}")
    print(f"Momentum Positive: {momentum_3m_positive}")
    print(f"Condition Met: {momentum_3m_positive}")
    
    if momentum_3m_positive:
        conditions_met += 1
        print("\nTRUE - Momentum Positive (3m) condition MET")
    else:
        print("\nFALSE - Momentum Positive (3m) condition NOT met")
    
    # Condition 6: Momentum Positive (5m) - NEW
    print("\n--- Condition 6: Momentum Positive (5m) ---")
    momentum_5m_positive, momentum_5m_value, momentum_5m_details = analyze_momentum_condition(client, SYMBOL, 500, '5m')
    condition_results['Momentum Positive (5m)'] = momentum_5m_positive
    
    # Print details for this condition
    print(f"\nCurrent Momentum: {momentum_5m_value:.4f}")
    print(f"Momentum Positive: {momentum_5m_positive}")
    print(f"Condition Met: {momentum_5m_positive}")
    
    if momentum_5m_positive:
        conditions_met += 1
        print("\nTRUE - Momentum Positive (5m) condition MET")
    else:
        print("\nFALSE - Momentum Positive (5m) condition NOT met")
    
    # Condition 7: FFT Forecast Up (1m)
    print("\n--- Condition 7: FFT Forecast Up (1m) ---")
    ...
    # Condition 8: FFT Forecast Up (3m) - NEW
    print("\n--- Condition 8: FFT Forecast Up (3m) ---")
    ...
    # Condition 9: FFT Forecast Up (5m) - NEW
    print("\n--- Condition 9: FFT Forecast Up (5m) ---")
    ...
    # Conditions 10-12: SMA200 Lowest More Recent (1m, 3m, 5m)
    for i, (tf, sma_data) in enumerate([('1m', sma200_1m), ('3m', sma200_3m), ('5m', sma200_5m)], start=10):
        print(f"\n--- Condition {i}: SMA200 Lowest More Recent ({tf}) ---")
        sma_met = False
        if 'error' not in sma_data:
            sma_met = sma_data['lowest_more_recent']
            condition_results[f'SMA200 Lowest More Recent ({tf})'] = sma_met
            
            print(f"\nLowest SMA200: {sma_data['lowest_sma200']:.2f}")
            print(f"Highest SMA200: {sma_data['highest_sma200']:.2f}")
            print(f"Lowest SMA200 Price: {sma_data['lowest_sma200_price']:.2f}")
            print(f"Highest SMA200 Price: {sma_data['highest_sma200_price']:.2f}")
            print(f"Lowest SMA200 More Recent: {sma_met}")
            print(f"Condition Met: {sma_met}")
            
            if sma_met:
                conditions_met += 1
                print(f"\nTRUE - SMA200 Lowest More Recent ({tf}) condition MET")
            else:
                print(f"\nFALSE - SMA200 Lowest More Recent ({tf}) condition NOT met")
        else:
            condition_results[f'SMA200 Lowest More Recent ({tf})'] = False
            print(f"\nError analyzing SMA200: {sma_data['error']}")
            print(f"\nFALSE - SMA200 Lowest More Recent ({tf}) condition NOT met")
    
    # Conditions 13-15: Close Below Middle Threshold (1m, 3m, 5m)
    for i, (tf, thresholds_data) in enumerate([('1m', thresholds_1m), ('3m', thresholds_3m), ('5m', thresholds_5m)], start=13):
        print(f"\n--- Condition {i}: Close Below Middle Threshold ({tf}) ---")
        close_met = False
        if 'error' not in thresholds_data:
            close_met = thresholds_data['close_below_middle']
            condition_results[f'Close Below Middle Threshold ({tf})'] = close_met
            
            print(f"\nMin Threshold: {thresholds_data['min_threshold']:.2f}")
            print(f"Max Threshold: {thresholds_data['max_threshold']:.2f}")
            print(f"Middle Threshold: {thresholds_data['middle_threshold']:.2f}")
            print(f"Current Close: {thresholds_data['current_close']:.2f}")
            print(f"Close Below Middle: {close_met}")
            print(f"Condition Met: {close_met}")
            
            if close_met:
                conditions_met += 1
                print(f"\nTRUE - Close Below Middle Threshold ({tf}) condition MET")
            else:
                print(f"\nFALSE - Close Below Middle Threshold ({tf}) condition NOT met")
        else:
            condition_results[f'Close Below Middle Threshold ({tf})'] = False
            print(f"\nError analyzing thresholds: {thresholds_data['error']}")
            print(f"\nFALSE - Close Below Middle Threshold ({tf}) condition NOT met")
    
    # Condition 16: Volume Bullish Dominance (1m)
    print("\n--- Condition 16: Volume Bullish Dominance (1m) ---")
    volume_1m_met = False
    ...
    # Condition 17: Volume Bullish Dominance (3m) - NEW
    print("\n--- Condition 17: Volume Bullish Dominance (3m) ---")
    volume_3m_met = False
    if 'error' not in volume_3m:
        volume_3m_met = volume_3m['bullish_dominance']
        condition_results['Volume Bullish Dominance (3m)'] = volume_3m_met
        
        print(f"\nBullish Volume %: {volume_3m['bullish_pct']:.2f}%")
        print(f"Bearish Volume %: {volume_3m['bearish_pct']:.2f}%")
        print(f"Dominance: {volume_3m['dominance']}")
        print(f"Bullish Dominance: {volume_3m_met}")
        print(f"Condition Met: {volume_3m_met}")
    ...
    # Condition 18: Volume Bullish Dominance (5m) - NEW
    print("\n--- Condition 18: Volume Bullish Dominance (5m) ---")
    volume_5m_met = False
    if 'error' not in volume_5m:
        volume_5m_met = volume_5m['bullish_dominance']
        condition_results['Volume Bullish Dominance (5m)'] = volume_5m_met
        
        print(f"\nBullish Volume %: {volume_5m['bullish_pct']:.2f}%")
        print(f"Bearish Volume %: {volume_5m['bearish_pct']:.2f}%")
        print(f"Dominance: {volume_5m['dominance']}")
        print(f"Bullish Dominance: {volume_5m_met}")
        print(f"Condition Met: {volume_5m_met}")
    ...
    # Conditions 19-21: Argmin More Recent (1m, 3m, 5m)
    for i, (tf, thresholds_data) in enumerate([('1m', thresholds_1m), ('3m', thresholds_3m), ('5m', thresholds_5m)], start=19):
        print(f"\n--- Condition {i}: Argmin More Recent ({tf}) ---")
        argmin_met = False
        if 'error' not in thresholds_data:
            argmin_met = thresholds_data['argmin_more_recent']
            condition_results[f'Argmin More Recent ({tf})'] = argmin_met
            
            print(f"\nMin Close Index: {thresholds_data['min_close_idx']}")
            print(f"Max Close Index: {thresholds_data['max_close_idx']}")
            print(f"Argmin More Recent: {argmin_met}")
            print(f"Condition Met: {argmin_met}")
            
            if argmin_met:
                conditions_met += 1
                print(f"\nTRUE - Argmin More Recent ({tf}) condition MET")
            else:
                print(f"\nFALSE - Argmin More Recent ({tf}) condition NOT met")
        else:
            condition_results[f'Argmin More Recent ({tf})'] = False
            print(f"\nError analyzing thresholds: {thresholds_data['error']}")
            print(f"\nFALSE - Argmin More Recent ({tf}) condition NOT met")
    
    # Conditions 22-24: Up Cycle Confirmed (1m, 3m, 5m) - NEW - FIXED
    for i, (tf, extrema_data) in enumerate([('1m', local_extrema_1m), ('3m', local_extrema_3m), ('5m', local_extrema_5m)], start=22):
        print(f"\n--- Condition {i}: Up Cycle Confirmed ({tf}) ---")
        up_cycle_met = False
        if 'error' not in extrema_data:
            up_cycle_met = extrema_data['up_cycle_confirmed']
            condition_results[f'Up Cycle Confirmed ({tf})'] = up_cycle_met
            
            print(f"\nMost Recent Extrema: {extrema_data['most_recent_extrema']}")
            print(f"Argmin Value: {extrema_data['argmin']['value']:.2f}")
            print(f"Argmax Value: {extrema_data['argmax']['value']:.2f}")
            print(f"Hi-Lo Range: {extrema_data['hilo_range']:.2f}")
            print(f"Local Dip Value: {extrema_data['local_dip']['value']:.2f}")
            print(f"Local Top Value: {extrema_data['local_top']['value']:.2f}")
            print(f"Distance: {extrema_data['distance']['value']:.2f} ({extrema_data['distance']['description']})")
            print(f"Distance Meets Hi-Lo Range: {extrema_data['distance_meets_hilo_range']}")
            print(f"Argmin More Recent: {extrema_data['argmin_more_recent']}")
            print(f"Up Cycle Confirmed: {up_cycle_met}")
            print(f"Condition Met: {up_cycle_met}")
            
            if up_cycle_met:
                conditions_met += 1
                print(f"\nTRUE - Up Cycle Confirmed ({tf}) condition MET")
            else:
                print(f"\nFALSE - Up Cycle Confirmed ({tf}) condition NOT met")
        else:
            condition_results[f'Up Cycle Confirmed ({tf})'] = False
            print(f"\nError analyzing local extrema: {extrema_data['error']}")
            print(f"\nFALSE - Up Cycle Confirmed ({tf}) condition NOT met")
...
def main():
    ...
    print("=== ENHANCED BTCUSDC TRADING BOT - 24 TRIGGERS ===")
    ...
    print("3. Fetch fresh data for all timeframes (1m, 3m, 5m)")
    print("4. Analyze all 24 specified trading conditions:")
    print("   - RSI Oversold (1m, 3m, 5m)")
    print("   - Momentum Positive (1m, 3m, 5m)")
    print("   - FFT Forecast Up (1m, 3m, 5m)")
    print("   - SMA200 Lowest More Recent (1m, 3m, 5m)")
    print("   - Close Below Middle Threshold (1m, 3m, 5m)")
    print("   - Volume Bullish Dominance (1m, 3m, 5m)")
    print("   - Argmin More Recent (1m, 3m, 5m)")
    print("   - Up Cycle Confirmed (1m, 3m, 5m) - FIXED")
    print("5. Execute trade if ALL 24 conditions are met")
    ...
    print("\nEnhanced Features:")
    print("- Now 24 selected triggers (added Up Cycle Confirmed 1m/3m/5m)")
