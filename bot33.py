...
→ FIXED: Lowered TP target to 0.35% for faster gains
...
TP_TARGET_PERCENT = 0.35  # FIXED: Changed from 0.75% to 0.35% for faster gains
...
def analyze_instant_target_condition(client, symbol, timeframe='1m', lookback=500):
    ...
    try:
        ...
        # This ensures the take profit target at 0.35% is contained
        target_percentages = {
...
def execute_sell_order(client, symbol, target_price=None):
    """
    FIXED: Execute a limit sell order for the entire BTC balance at the target price to ensure 0.35% profit after fees.
    """
    try:
        # Get current price
        current_price = get_current_price_from_webhook()
        ...
        # Step 4: Calculate the target price for 0.35% profit after fees
...
def place_take_profit_order(client, symbol, entry_price, btc_quantity):
    """
    Place a limit sell order for take profit at the target price (0.35% profit after fees).
    """
    try:
        # Calculate the target price for 0.35% profit after fees
        # We need to account for the 0.1% selling fee, so we need to set a higher price
        # Target price = entry_price * (1 + 0.35% + 0.1% fee) = entry_price * 1.0045
...
def check_trade_status(client):
    ...
    try:
        ...
        tp_price = entry_price * (1 + (TP_TARGET_PERCENT + 0.1) / 100)  # 0.35% + 0.1% fee
...
def display_trade_status():
    ...
    print(f"{'TP Price (0.35%):':<20}{trade_info['tp_price']:.25f}")
...
def perform_single_iteration_analysis(client):
    ...
    if in_trade_scenario:
        ...
        if not trade_active:
            ...
            trade_info['tp_price'] = entry_price * (1 + (TP_TARGET_PERCENT + 0.1) / 100)  # 0.35% + 0.1% fee
    ...
    conditions_met = 0
    total_conditions = 6  # FIXED: Changed from 7 to 6 to match actual conditions
    condition_details = {}
    ...
    if conditions_met == total_conditions:
        ...
        if buy_result['success']:
            ...
            if current_price:
                ...
                trade_info['tp_price'] = buy_result['price'] * (1 + (TP_TARGET_PERCENT + 0.1) / 100)  # 0.35% + 0.1% fee
                trade_info['target_diff'] = trade_info['target_price'] - current_price
                trade_info['target_diff_pct'] = (trade_info['target_diff'] / current_price) * 100
                trade_info['tp_diff'] = trade_info['tp_price'] - current_price
                trade_info['tp_diff_pct'] = (trade_info['tp_diff'] / current_price) * 100
...
def main():
    ...
    print("7. Place a limit order for take profit at 0.35% (including fees)")
    ...
    print("- FFT Up Cycle")
    print("\nFIXED: Take profit target at 0.35% (including fees)")
