                ...
                for i in range(dip_idx, top_idx, max(1, segment_length // 10)):
                    if i + 10 < len(data_clean):
                        ...
                        if total_segment_power > 0:
                            dominance = (pos_power - neg_power) / total_segment_power
                            frequencies.append(dominance)
...
def analyze_ht_sine_fft_condition(client, symbol, lookback=500, timeframe='1m'):
    """
    FIXED HT_SINE FFT analysis with proper cycle direction alignment.
    Implements argmin/argmax analysis, cycle stages, and frequency dominance.
    """
    try:
        ...
        if df_ht is None or 'HT_SINE' not in df_ht.columns:
            return False, {"error": "Failed to calculate HT_SINE"}
        
        # Get HT_SINE values
        ht_sine_values = df_ht['HT_SINE'].values.astype(float)
        
        # Find argmin and argmax for HT_SINE
        ht_sine_argmin = np.argmin(ht_sine_values)
        ht_sine_argmax = np.argmax(ht_sine_values)
        
        # FIXED: Determine which extreme is more recent for HT_SINE
        # There's no neutral cycle - it's either up (dip more recent) or down (top more recent)
        ht_sine_dip_more_recent = ht_sine_argmin > ht_sine_argmax
        ht_sine_cycle_direction = "up" if ht_sine_dip_more_recent else "down"
        
        # FIXED: Make price cycle direction consistent with HT_SINE cycle direction
        # The cycle direction is determined by the most recent argmin/argmax occurrence
        price_cycle_direction = ht_sine_cycle_direction
        
        # Get current price and HT_SINE
        current_price = df['close'].iloc[-1]
        current_ht_sine = ht_sine_values[-1]
        
        # Analyze frequency gradient from dip to top for HT_SINE
        ht_sine_freq_analysis = analyze_frequency_gradient(ht_sine_values, ht_sine_argmin, ht_sine_argmax)
        
        # Analyze HT_SINE FFT forecast
        ht_sine_forecast = improved_fft_forecast(ht_sine_values, forecast_periods=4)
        forecast_target = ht_sine_forecast[-1]
        
        # FIXED: Determine if HT_SINE is rising or falling based on cycle direction
        # For up cycles, we expect HT_SINE to be rising (from dip to top)
        # For down cycles, we expect HT_SINE to be falling (from top to dip)
        recent_ht_sine = ht_sine_values[-5:]
        ht_sine_diff = recent_ht_sine[-1] - recent_ht_sine[0] if len(recent_ht_sine) > 0 else 0
        
        # FIXED: For up cycles, we want HT_SINE to be rising
        # For down cycles, we want HT_SINE to be falling
        if ht_sine_cycle_direction == "up":
            ht_sine_rising = ht_sine_diff > 0
        else:  # down cycle
            ht_sine_rising = ht_sine_diff < 0
        
        # FIXED: Calculate cycle stage and progress properly
        # The issue was in how we calculated current_position
        cycle_stage = None
        cycle_progress_pct = 0
        
        if ht_sine_cycle_direction == "up":
            # Up cycle: from dip (A) to top (D)
            cycle_length = ht_sine_argmax - ht_sine_argmin
            if cycle_length > 0:
                # FIXED: Calculate current position correctly
                # Current position is how far we've come from the dip
                current_position = len(ht_sine_values) - 1 - ht_sine_argmin
                # Ensure position is within cycle bounds
                current_position = min(current_position, cycle_length)
                cycle_progress_pct = (current_position / cycle_length) * 100
                
                # FIXED: Determine stage based on progress - never None or neutral
                if cycle_progress_pct <= 25:
                    cycle_stage = "A"  # Beginning of up cycle
                elif cycle_progress_pct <= 50:
                    cycle_stage = "B"  # First quarter
                elif cycle_progress_pct <= 75:
                    cycle_stage = "C"  # Third quarter
                else:
                    cycle_stage = "D"  # Approaching top
            else:
                # Default to stage A if cycle length is 0 or negative
                cycle_stage = "A"
        else:
            # Down cycle: from top (D) to dip (A)
            cycle_length = ht_sine_argmin - ht_sine_argmax
            if cycle_length > 0:
                # FIXED: Calculate current position correctly
                # Current position is how far we've come from the top
                current_position = len(ht_sine_values) - 1 - ht_sine_argmax
                # Ensure position is within cycle bounds
                current_position = min(current_position, cycle_length)
                cycle_progress_pct = (current_position / cycle_length) * 100
                ...
                # FIXED: Determine stage based on progress - never None or neutral
                if cycle_progress_pct <= 25:
                    cycle_stage = "D"  # Beginning of down cycle
                elif cycle_progress_pct <= 50:
                    cycle_stage = "C"  # First quarter
                elif cycle_progress_pct <= 75:
                    cycle_stage = "B"  # Third quarter
                else:
                    cycle_stage = "A"  # Approaching dip
            else:
                # Default to stage D if cycle length is 0 or negative
                cycle_stage = "D"
        
        # Calculate distance of current close to min and max of current cycle
        if ht_sine_cycle_direction == "up":
            cycle_min = ht_sine_values[ht_sine_argmin]
            cycle_max = ht_sine_values[ht_sine_argmax]
            distance_to_min = abs(current_ht_sine - cycle_min)
            distance_to_max = abs(cycle_max - current_ht_sine)
            cycle_range = cycle_max - cycle_min
            if cycle_range > 0:
                position_in_cycle = ((current_ht_sine - cycle_min) / cycle_range) * 100
            else:
                position_in_cycle = 0
        else:
            cycle_min = ht_sine_values[ht_sine_argmin]
            cycle_max = ht_sine_values[ht_sine_argmax]
            distance_to_min = abs(cycle_min - current_ht_sine)
            distance_to_max = abs(current_ht_sine - cycle_max)
            cycle_range = cycle_max - cycle_min
            if cycle_range > 0:
                position_in_cycle = ((cycle_max - current_ht_sine) / cycle_range) * 100
            else:
                position_in_cycle = 0
        
        # Calculate min, middle, and max thresholds
        if cycle_range > 0:
            min_threshold = cycle_min + (cycle_range * 0.1)
            middle_threshold = cycle_min + (cycle_range * 0.5)
            max_threshold = cycle_max - (cycle_range * 0.1)
        else:
            min_threshold = current_ht_sine
            middle_threshold = current_ht_sine
            max_threshold = current_ht_sine
        
        # Forecast incoming argmax/argmin price value
        forecast_price = None
        if ht_sine_dip_more_recent:  # Dip is most recent, forecast argmax
            # Find the price at the next expected argmax
            # Use linear regression to estimate the price at the next argmax
            if ht_sine_cycle_direction == "up" and len(df) > 10:
                # Get the price at the last argmax
                last_argmax_price = df['high'].iloc[ht_sine_argmax]
                
                # Estimate the next argmax price based on current trend
                # Simple projection: current price + (current price - last argmin price)
                last_argmin_price = df['low'].iloc[ht_sine_argmin]
                price_range = last_argmax_price - last_argmin_price
                
                if price_range > 0:
                    # Estimate how much of the cycle we've completed
                    ht_sine_position = (current_ht_sine - ht_sine_values[ht_sine_argmin]) / (ht_sine_values[ht_sine_argmax] - ht_sine_values[ht_sine_argmin])
                    if 0 <= ht_sine_position <= 1:
                        forecast_price = current_price + (price_range * (1 - ht_sine_position))
        else:  # Top is most recent, forecast argmin
            # Find the price at the next expected argmin
            # Use linear regression to estimate the price at the next argmin
            if ht_sine_cycle_direction == "down" and len(df) > 10:
                # Get the price at the last argmin
                last_argmin_price = df['low'].iloc[ht_sine_argmin]
                
                # Estimate the next argmin price based on current trend
                # Simple projection: current price - (last argmax price - current price)
                last_argmax_price = df['high'].iloc[ht_sine_argmax]
                price_range = last_argmin_price - last_argmax_price
                
                if price_range > 0:
                    # Estimate how much of the cycle we've completed
                    ht_sine_position = (ht_sine_values[ht_sine_argmax] - current_ht_sine) / (ht_sine_values[ht_sine_argmax] - ht_sine_values[ht_sine_argmin])
                    if 0 <= ht_sine_position <= 1:
                        forecast_price = current_price - (price_range * (1 - ht_sine_position))
        
        # Determine predominant frequency
        predominant_frequency = "negative" if ht_sine_freq_analysis['negative_dominance_pct'] > ht_sine_freq_analysis['positive_dominance_pct'] else "positive"
        
        # FIXED LOGIC: Determine condition based on cycle direction and FFT analysis
        if ht_sine_cycle_direction == "up":
            # For up cycles (dip to top):
            # 1. HT_SINE should be rising (positive trend)
            # 2. Forecast should be above current HT_SINE value
            # 3. Gradient should be positive (increasing positive frequency dominance)
            # 4. Negative frequency should dominate (from dip to top)
            condition_met = (ht_sine_rising and 
                           forecast_target > current_ht_sine and 
                           ht_sine_freq_analysis['gradient'] > 0 and
                           predominant_frequency == "negative")
        else:  # down cycle
            # For down cycles (top to dip):
            # 1. HT_SINE should be falling (negative trend)
            # 2. Forecast should be below current HT_SINE value
            # 3. Gradient should be negative (decreasing positive frequency dominance)
            # 4. Positive frequency should dominate (from top to dip)
            condition_met = (ht_sine_rising and 
                           forecast_target < current_ht_sine and 
                           ht_sine_freq_analysis['gradient'] < 0 and
                           predominant_frequency == "positive")
        
        # Prepare results
        results = {
            "timeframe": timeframe,
            "ht_sine_cycle_direction": ht_sine_cycle_direction,
            "price_cycle_direction": price_cycle_direction,  # Now consistent with HT_SINE
            "current_ht_sine": current_ht_sine,
            "ht_sine_forecast": forecast_target,
            "ht_sine_diff": ht_sine_diff,
            "ht_sine_rising": ht_sine_rising,
            "positive_dominance": ht_sine_freq_analysis['positive_dominance_pct'],
            "negative_dominance": ht_sine_freq_analysis['negative_dominance_pct'],
            "frequency_gradient": ht_sine_freq_analysis['gradient'],
            "predominant_frequency": predominant_frequency,
            "condition_met": condition_met,
            "ht_sine_dip_more_recent": ht_sine_dip_more_recent,
            "ht_sine_argmin": ht_sine_argmin,
            "ht_sine_argmax": ht_sine_argmax,
            "cycle_stage": cycle_stage,  # Never None or neutral now
        ...
        print(f"Error analyzing HT_SINE FFT condition: {e}")
...
def perform_single_iteration_analysis(client):
    """Perform single iteration analysis with all conditions including HT_SINE FFT analysis."""
    ...
    conditions_met = 0
    total_conditions = 10  # Total number of conditions (updated)
    ...
    # Condition 9: HT_SINE FFT (1m) - FIXED
    print("\n--- Condition 9: HT_SINE FFT Analysis (1m) ---")
    ht_sine_fft_1m_met, ht_sine_fft_1m_details = analyze_ht_sine_fft_condition(client, SYMBOL, 500, '1m')
    condition_results['HT_SINE FFT (1m)'] = ht_sine_fft_1m_met
    
    # Print details for this condition
    print(f"\nHT_SINE Cycle Direction: {ht_sine_fft_1m_details.get('ht_sine_cycle_direction', 'unknown')}")
    print(f"Price Cycle Direction: {ht_sine_fft_1m_details.get('price_cycle_direction', 'unknown')}")
    print(f"Current HT_SINE: {ht_sine_fft_1m_details.get('current_ht_sine', 0):.25f}")
    print(f"HT_SINE Forecast: {ht_sine_fft_1m_details.get('ht_sine_forecast', 0):.25f}")
    print(f"HT_SINE Rising: {ht_sine_fft_1m_details.get('ht_sine_rising', False)}")
    print(f"HT_SINE Diff: {ht_sine_fft_1m_details.get('ht_sine_diff', 0):.25f}")
    print(f"Positive Dominance: {ht_sine_fft_1m_details.get('positive_dominance', 0):.25f}%")
    print(f"Negative Dominance: {ht_sine_fft_1m_details.get('negative_dominance', 0):.25f}%")
    print(f"Frequency Gradient: {ht_sine_fft_1m_details.get('frequency_gradient', 0):.25f}")
    print(f"Predominant Frequency: {ht_sine_fft_1m_details.get('predominant_frequency', 'unknown')}")
    print(f"Cycle Stage: {ht_sine_fft_1m_details.get('cycle_stage', 'unknown')}")
    print(f"Cycle Progress: {ht_sine_fft_1m_details.get('cycle_progress_pct', 0):.25f}%")
    print(f"Position in Cycle: {ht_sine_fft_1m_details.get('position_in_cycle', 0):.25f}%")
    print(f"Distance to Min: {ht_sine_fft_1m_details.get('distance_to_min', 0):.25f}")
    print(f"Distance to Max: {ht_sine_fft_1m_details.get('distance_to_max', 0):.25f}")
    print(f"Min Threshold: {ht_sine_fft_1m_details.get('min_threshold', 0):.25f}")
    print(f"Middle Threshold: {ht_sine_fft_1m_details.get('middle_threshold', 0):.25f}")
    print(f"Max Threshold: {ht_sine_fft_1m_details.get('max_threshold', 0):.25f}")
    if ht_sine_fft_1m_details.get('forecast_price'):
        print(f"Forecast Price: {ht_sine_fft_1m_details.get('forecast_price', 0):.25f}")
    print(f"Condition Met: {ht_sine_fft_1m_met}")
    
    if ht_sine_fft_1m_met:
        conditions_met += 1
        print("\nTRUE - HT_SINE FFT (1m) condition MET")
    else:
        print("\nFALSE - HT_SINE FFT (1m) condition NOT met")
    
    # Condition 10: HT_SINE FFT (15s) - FIXED
    print("\n--- Condition 10: HT_SINE FFT Analysis (15s) ---")
    ht_sine_fft_15s_met, ht_sine_fft_15s_details = analyze_ht_sine_fft_condition(client, SYMBOL, 500, '15s')
    condition_results['HT_SINE FFT (15s)'] = ht_sine_fft_15s_met
    
    # Print details for this condition
    print(f"\nHT_SINE Cycle Direction: {ht_sine_fft_15s_details.get('ht_sine_cycle_direction', 'unknown')}")
    print(f"Price Cycle Direction: {ht_sine_fft_15s_details.get('price_cycle_direction', 'unknown')}")
    print(f"Current HT_SINE: {ht_sine_fft_15s_details.get('current_ht_sine', 0):.25f}")
    print(f"HT_SINE Forecast: {ht_sine_fft_15s_details.get('ht_sine_forecast', 0):.25f}")
    print(f"HT_SINE Rising: {ht_sine_fft_15s_details.get('ht_sine_rising', False)}")
    print(f"HT_SINE Diff: {ht_sine_fft_15s_details.get('ht_sine_diff', 0):.25f}")
    print(f"Positive Dominance: {ht_sine_fft_15s_details.get('positive_dominance', 0):.25f}%")
    print(f"Negative Dominance: {ht_sine_fft_15s_details.get('negative_dominance', 0):.25f}%")
    print(f"Frequency Gradient: {ht_sine_fft_15s_details.get('frequency_gradient', 0):.25f}")
    print(f"Predominant Frequency: {ht_sine_fft_15s_details.get('predominant_frequency', 'unknown')}")
    print(f"Cycle Stage: {ht_sine_fft_15s_details.get('cycle_stage', 'unknown')}")
    print(f"Cycle Progress: {ht_sine_fft_15s_details.get('cycle_progress_pct', 0):.25f}%")
    print(f"Position in Cycle: {ht_sine_fft_15s_details.get('position_in_cycle', 0):.25f}%")
    print(f"Distance to Min: {ht_sine_fft_15s_details.get('distance_to_min', 0):.25f}")
    print(f"Distance to Max: {ht_sine_fft_15s_details.get('distance_to_max', 0):.25f}")
    print(f"Min Threshold: {ht_sine_fft_15s_details.get('min_threshold', 0):.25f}")
    print(f"Middle Threshold: {ht_sine_fft_15s_details.get('middle_threshold', 0):.25f}")
    print(f"Max Threshold: {ht_sine_fft_15s_details.get('max_threshold', 0):.25f}")
    if ht_sine_fft_15s_details.get('forecast_price'):
        print(f"Forecast Price: {ht_sine_fft_15s_details.get('forecast_price', 0):.25f}")
    print(f"Condition Met: {ht_sine_fft_15s_met}")
    
    if ht_sine_fft_15s_met:
        conditions_met += 1
        print("\nTRUE - HT_SINE FFT (15s) condition MET")
    else:
        print("\nFALSE - HT_SINE FFT (15s) condition NOT met")
...
def main():
    ...
    print("   - FIXED: HT_SINE FFT Analysis (1m)")
    print("   - FIXED: HT_SINE FFT Analysis (15s)")
    ...
    print("- FIXED: HT_SINE FFT with proper cycle direction alignment")
    print("- FIXED: Argmin/Argmax analysis for both HT_SINE and price")
    print("- FIXED: Cycle stage analysis (A, B, C, D)")
    print("- FIXED: Cycle progress percentage calculation")
    print("- FIXED: Distance to min/max thresholds")
    print("- FIXED: Forecast price calculation based on cycle position")
    print("- FIXED: Predominant frequency analysis")
    print("- FIXED: Proper data cleaning before ANY analysis")
    print("- FIXED: Correct frequency dominance calculation from dip to top")
    print("="*60)
